...
[![Release](https://img.shields.io/github/v/release/Autonomous-Asset-Management-Agents/autonomous_?include_prereleases&label=Release)](https://github.com/Autonomous-Asset-Management-Agents/autonomous_/releases)
...
1. **Download** the installer for your OS:
   * ⬇️ **Windows:** [Download for Windows (autonomous_setup.exe)](https://github.com/Autonomous-Asset-Management-Agents/autonomous_/releases)
   * 🍏 **macOS (Apple Silicon):** [Download for Mac (AAAgents-arm64.dmg)](https://github.com/Autonomous-Asset-Management-Agents/autonomous_/releases) — Apple Silicon (M1–M4) only, ad-hoc-signed Beta (first launch: right-click → Open, or *System Settings → Privacy & Security → Open Anyway*). Intel Macs / from-source: **[macOS Quickstart](./docs/oss/macos-quickstart.md)**
...
* **14-Agent Consensus:** A local board composed of technical indicators, sentiment analysis, LSTMs, and Reinforcement Learning determines the signals.
...
This table shows the **deployment** differences across the **Desktop Edition**, **OSS Self-Hosted Edition**, and the **Enterprise Edition**. The **canonical, code-verified capability matrix** is the generated block at the top of this file; the same generated block is published in [Vision & Editions](./docs/oss/VISION_AND_EDITIONS.md). Where this operational table and the generated matrix differ, the generated matrix wins.
...
| **Extensibility** | Standard 14-Agent Board | Custom Python Plugins (`plugins/`) | Managed Enterprise Agent Pipelines |
...
| Setup | Behavior |
|---|---|
| **Without Alpaca Keys** | **Offline Mode** — The engine starts, all 14 agents vote, but no orders are sent. Perfect for getting to know the software. |
...
| **Alpaca + GEMINI_API_KEY** | **Full Sentiment Mode** — the LLM research reports become available, which the `SpecialistAlphaAgent` votes on. Without a key that one agent falls back to a neutral 0.5 vote; the other ten are unaffected, and `NewsSentimentAgent` scores headlines with a local NLP backend either way. |
