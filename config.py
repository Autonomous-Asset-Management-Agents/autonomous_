...
MIN_ORDER_VALUE_USD = (
    1.0  # Dust filter — reject orders below Alpaca's $1 notional minimum
)
...
# --- #1953 TRD-2: Vol-Targeting / Risk-Parity sizing (DARK — default OFF) ---
# Master switch: ON => calculate_position_size multiplies the plumbed HAR-RV
# forecast_vol into final_risk_scaler as clip(target/fv, lo, hi); OFF (default)
# => byte-identical vol-blind sizing. ACTIVATION GATE (plan #1953 §11, prove-it-
# or-kill-it): only after the walk-forward OOS net-of-cost Sharpe proof (Inc 2)
# + human sign-off — never flip ON on the strength of this dark-ship alone.
# Mirrored in config.py (BORA parity, TFT_DEFLATED_SHARPE_GATE idiom).
VOL_TARGETING_SIZING_ENABLED = (
    os.getenv("VOL_TARGETING_SIZING_ENABLED", "False").lower() == "true"
)
...
SMART_EXIT_MIN_HOLD_DAYS = float(os.getenv("SMART_EXIT_MIN_HOLD_DAYS", "5.0"))
SMART_EXIT_EXIT_RANK_HYSTERESIS = float(
    os.getenv("SMART_EXIT_EXIT_RANK_HYSTERESIS", "3.0")
)
...
# once (it drains gradually over cycles). Default 2; <=0 = unlimited (byte-identical
# rollback). SELL-side sibling of #2546's INTRADAY_TIMING buy-pacing; mirrored in config.py.
ROTATION_MAX_EXITS_PER_CYCLE = int(os.getenv("ROTATION_MAX_EXITS_PER_CYCLE", "2"))
...
STOPOUT_REENTRY_COOLDOWN_MIN = float(
    os.getenv("STOPOUT_REENTRY_COOLDOWN_MIN", "0") or "0"
)
...
# VADER lexicon) instead of the headline-less LLM symbol guess. OFF (default)
# -> byte-identical LLM-/abstain-path. Activation is RTR-0-gated (#1947).
NEWS_SENTIMENT_NLP_ENABLED = (
    os.getenv("NEWS_SENTIMENT_NLP_ENABLED", "False").lower() == "true"
)
...
DESKTOP_FEATURE_SNAPSHOT_ENABLED = (
    os.getenv("DESKTOP_FEATURE_SNAPSHOT_ENABLED", "False").lower() == "true"
)
...
REPORT_CACHE_PERSIST_ENABLED = (
    os.getenv("REPORT_CACHE_PERSIST_ENABLED", "False").lower() == "true"
)
...
ROUND_TABLE_TOP_K_EVAL = int(os.getenv("ROUND_TABLE_TOP_K_EVAL", "20") or "20")
...
EXIT_POLICY_TRAILING_ENABLED = (
    os.getenv("EXIT_POLICY_TRAILING_ENABLED", "False").lower() == "true"
)
...
EXECUTION_BLOCK_TELEMETRY_ENABLED = (
    os.getenv("EXECUTION_BLOCK_TELEMETRY_ENABLED", "False").lower() == "true"
)
...
DECISION_CAPTURE_ENABLED = (
    os.getenv("DECISION_CAPTURE_ENABLED", "False").lower() == "true"
)
