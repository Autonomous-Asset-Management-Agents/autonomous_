...
CONSENSUS_RETURN_HARNESS_PATH = os.getenv("CONSENSUS_RETURN_HARNESS_PATH", "")
...
# #3145: which producer fills data/financials_cache. 'sec' = free SEC EDGAR
# companyfacts (no key, public-domain → bundleable, offline; DEFAULT); 'polygon' =
# the legacy keyed feed. Reader/agents are source-agnostic — this only selects the
# nightly producer. Agents stay weight=0 (dark) regardless until a walk-forward.
FUNDAMENTALS_SOURCE = (os.getenv("FUNDAMENTALS_SOURCE", "sec") or "sec").strip().lower()
# #3146: ValuationAgent falls back from PEG to P/S / P/B (and later P/FFO) where PEG
# is undefined (loss-makers, financials, REITs). Default OFF (dark) — the agent is
# weight=0 anyway; ON only widens which names it can speak on. Byte-identical when OFF.
VALUATION_MULTIMETRIC_ENABLED = (
...
)  # Max je Position; #3154 Env-Naht
# #3154: Conviction-Sizer-Band-Obergrenze, kanonisch (vorher nur getattr-Fallback im Sizer).
MAX_POSITION_PERCENT_SIZING = float(
    os.getenv("MAX_POSITION_PERCENT_SIZING", "0.30") or "0.30"
)
MIN_POSITION_PERCENT = float(
    os.getenv("MIN_POSITION_PERCENT", "0.05") or "0.05"
)  # Min je Trade; #3154 Env-Naht
...
# ADR-R11 (#3135): proportional slot-cash split. The equal (cash/slots) cash cap flattens
# every concurrent BUY of a cycle to the SAME dollar amount, erasing the per-name
# IV/conviction size differentiation produced upstream by #3094 (VIXAware-IV -> forecast_vol
# -> vol-targeting) and the conviction sizer. ON scales each slot by the position's own
# target value; it can only SHRINK vs the equal split (no-leverage axiom preserved). PROMOTED
# Default ON (#3135, owner decision 31.08.): complements #3094 (already ON) — the cash cap
# otherwise re-flattened its differentiation (measured live, real-money loss 31.08.).
# Clamp-inventory verified: differentiation survives to the executed order value. Sim sweep
# FOLLOWS (post-promote). Revert: PROPORTIONAL_SLOT_CASH_ENABLED=false. Basis: MiFID II Art. 17.
PROPORTIONAL_SLOT_CASH_ENABLED = (
...
# #3199: UpsideSkew as a SIZING-asymmetry tilt (distinct from the #3095 DIRECTIONAL
# agent above). The 25Δ RR skew PERCENTILE becomes a bounded multiplicative factor on
# the vol-targeting scaler (risk_manager.skew_size_tilt) — bullish upside skew sizes up,
# crash skew sizes down — mirroring #3094 (IV LEVEL -> size) around the IV SKEW dimension.
# Default OFF / CAP 0.0 => tilt 1.0 => byte-identical (sizer AND MiFID II audit). Mirrored
# in config.py (BORA parity). Promote only after a measured paper/prod A/B (plan #3199 §5).
SKEW_SIZE_TILT_ENABLED = os.getenv("SKEW_SIZE_TILT_ENABLED", "false").lower() == "true"
SKEW_SIZE_TILT_CAP = float(os.getenv("SKEW_SIZE_TILT_CAP", "0.0"))

# #3210: Coverage-Vorsichts-Sizing-Abschlag (Prudenz, KEIN Alpha). lambda des
# abgeschwächten √coverage-Abschlags; 0.0 = aus (byte-identisch, dark-ship). ADR-R17.
COVERAGE_SIZING_STRENGTH = float(os.getenv("COVERAGE_SIZING_STRENGTH", "0.0") or "0.0")
# #3284 (Epic #3086): Clean-weight sizing (Variant 2, PER-SYMBOL). "off" => byte-identical
# conviction path (dark-ship default); "a" => strict 1/N (DeMiguel baseline); "b" => 1/N x
# vol-targeting x active tilts. Conviction leaves the SIZE lever. Mirrored in config.py
# (BORA parity). Promote only after RTR-0 + #3281 TC + human sign-off (MiFID II RTS 6).
CLEAN_WEIGHT_SIZING = os.getenv("CLEAN_WEIGHT_SIZING", "b").strip().lower() or "b"
# #3094 (a): per-name IMPLIED vol -> #1953 forecast_vol sizer; VIXAware leaves the
# direction mean. Default OFF = byte-identical. IV_VRP_DEBIAS_FACTOR (ADR-a01)
# removes the VRP level bias before the daily scaler. CALIBRATED on corpus
# 7cf4c4b03bf0: 0.85 reproduces the HAR-RV mean scaler (0.847) -> exposure-neutral,
# so (a) swaps the vol ESTIMATOR without moving the risk LEVEL.
# PROMOTED to default ON: VIXAware's directionless IV magnitude held 26.5% of the
# directional weight and flipped 20.18% of 150,355 logged decisions across the 0.65
# buy threshold. Volatility now acts on SIZE only. Revert: IMPLIED_VOL_FORECAST_ENABLED=false.
IMPLIED_VOL_FORECAST_ENABLED = (
    os.getenv("IMPLIED_VOL_FORECAST_ENABLED", "true").lower() == "true"
)
IV_VRP_DEBIAS_FACTOR = float(os.getenv("IV_VRP_DEBIAS_FACTOR", "0.85"))
...
)  # Hard stop %; #3154 Env-Naht
TAKE_PROFIT_PCT = float(
    os.getenv("TAKE_PROFIT_PCT", "25.0") or "25.0"
)  # Take profit %; #3154 Env-Naht
...
# #3291 (dark, default OFF = byte-identical; mirrors config.py): current-lot days_held —
# stamped on a witnessed absent→present entry, reset on full exit — instead of
# min(_trade_history), which dated a re-entered name (sell→rebuy within 30d) to a prior lot
# and defeated the rotation min-hold (live churn 2026-09-08). Promote after the #3291 sim A/B.
ROTATION_MIN_HOLD_CURRENT_LOT = (
...
# ROTATION now defaults OFF (owner directive 2026-09-08 — live-churn mitigation; was ON
# 2026-08-03 and force-sold BUY-rated names at a loss, ~85% turnover in 8 days). When re-enabled,
# it is bounded by BOTH ROTATION_MAX_EXITS_PER_CYCLE=1 (≤1 exit/cycle) AND
...
# 'flatten every name to 10%' half of that incident). Re-enable rotation with ROTATION_EXIT_ENABLED=true.
ROTATION_EXIT_ENABLED = os.getenv("ROTATION_EXIT_ENABLED", "False").lower() == "true"
DECONCENTRATION_TRIM_ENABLED = (
    os.getenv("DECONCENTRATION_TRIM_ENABLED", "False").lower() == "true"
)
# #3291 (mirrors config.py): master switch for opportunity DISPLACEMENT (full-book swap that
# sells the weakest holding to fund a new BUY). Default ON = byte-identical; OFF ⇒ full book
# waits for a slot via consensus SELL / risk exit. The last opportunity forced-sell that
# ROTATION_EXIT OFF does not stop (#3291 live churn).
DISPLACEMENT_ENABLED = os.getenv("DISPLACEMENT_ENABLED", "True").lower() == "true"
...
)  # 95% max investiert; #3154 Env-Naht
...
# #3349 (Epic #2963): Earnings-Proximity-Guard — BUY-only entry gate around a symbol's
# earnings report (EDGAR 8-K Item 2.02). Blocks NEW buys POST_DAYS after (exact) /
# PRE_DAYS before (cadence estimate) a report. Default OFF/dark => byte-identical.
# Fail-open on cold cache; fail-closed on a fetch error.
EARNINGS_GUARD_ENABLED = os.getenv("EARNINGS_GUARD_ENABLED", "False").lower() == "true"
EARNINGS_GUARD_POST_DAYS = int(os.getenv("EARNINGS_GUARD_POST_DAYS", "2"))
EARNINGS_GUARD_PRE_DAYS = int(os.getenv("EARNINGS_GUARD_PRE_DAYS", "0"))
...
# at 0.35 as the shipped default (owner decision 2026-09-03): the walk-forward deviations
# are usable without score centering, whose effect was measured negligible. Overridable via
# env. Mirrors config.py (BORA dual-edition parity: same env names, same default).
FUNDAMENTALS_AGENT_WEIGHT = float(
    os.getenv("FUNDAMENTALS_AGENT_WEIGHT", "0.35") or "0.35"
)
VALUATION_AGENT_WEIGHT = float(os.getenv("VALUATION_AGENT_WEIGHT", "0.35") or "0.35")

# #3154 (UXC-1 S1, spiegelt config.py): Gewichts-Naht + per-Agent-Enable-Flags.
NEWS_SENTIMENT_WEIGHT = float(os.getenv("NEWS_SENTIMENT_WEIGHT", "0.35") or "0.35")
# #3250 (spiegelt config.py): TA-Feature-Voter Konsens-Gewichte, DARK-Default 0.0.
TREND_AGENT_WEIGHT = float(os.getenv("TREND_AGENT_WEIGHT", "0.0") or "0.0")
VOLUME_CONFIRM_AGENT_WEIGHT = float(
    os.getenv("VOLUME_CONFIRM_AGENT_WEIGHT", "0.0") or "0.0"
)
# #3275: QualityAgent — Konsens-Stimmgewicht. Dormantes 0.30 (min 0.10, wie
# UpsideSkew): dark über QUALITY_AGENT_ENABLED (Enthaltung ⇒ byte-identisch), aber
# attribuierbar sobald armt. BORA-parallel zu config.py.
QUALITY_AGENT_WEIGHT = float(os.getenv("QUALITY_AGENT_WEIGHT", "0.30") or "0.30")
MOMENTUM_AGENT_ENABLED = os.getenv("MOMENTUM_AGENT_ENABLED", "true").lower() == "true"
...
VIX_RISK_AGENT_ENABLED = os.getenv("VIX_RISK_AGENT_ENABLED", "true").lower() == "true"
...
VALUATION_AGENT_ENABLED = os.getenv("VALUATION_AGENT_ENABLED", "true").lower() == "true"
...
# #3250 (spiegelt config.py): TA-Feature-Voter Master-Gates, Default FALSE (dark).
TREND_AGENT_ENABLED = os.getenv("TREND_AGENT_ENABLED", "false").lower() == "true"
VOLUME_CONFIRM_AGENT_ENABLED = (
    os.getenv("VOLUME_CONFIRM_AGENT_ENABLED", "false").lower() == "true"
)
# #3275 (spiegelt config.py): QualityAgent Master-Gate, Default FALSE (dark).
QUALITY_AGENT_ENABLED = os.getenv("QUALITY_AGENT_ENABLED", "false").lower() == "true"
...
# #3180 Consensus Retention Gate (plan #3189, dark) — BORA mirror of config.py. Suppresses a held
# name's OPINION exit while its live blend-consensus > threshold; hard risk stops never gated.
# Default 0.0 = OFF (byte-identical). Single float so the A/B sweep varies it numerically.
CONSENSUS_RETENTION_THRESHOLD = float(
