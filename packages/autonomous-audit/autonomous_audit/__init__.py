...
def get_anchor(path: Union[str, Path], genesis_hash: str = GENESIS_HASH) -> str:
    """Returns the current chain anchor ``<record_count>:<head_hash>``.
    Does NOT verify the chain's integrity (run :func:`verify_chain` first if needed).
    """
    n = 0
    prev = genesis_hash
    p = Path(path)
    if not p.exists():
        return f"0:{genesis_hash}"
...
def verify_chain(
    ...
    If ``expected_anchor`` is provided (format: ``<count>:<hash>``), the chain is also
    verified against it to detect tail-truncation or rewrites.
    """
    expected_n = None
    expected_hash = None
    if expected_anchor is not None:
        try:
            parts = expected_anchor.split(":", 1)
            expected_n = int(parts[0])
            expected_hash = parts[1]
        except (ValueError, IndexError):
            return False, f"invalid expected_anchor format: {expected_anchor!r}"
    ...
    with open(p, "r", encoding="utf-8") as fh:
        for i, line in enumerate(fh, 1):
            ...
            if expected_n is not None and n == expected_n:
                if prev != expected_hash:
                    return (
                        False,
                        f"anchor mismatch at record {n}: expected {expected_hash}, found {prev}",
                    )
    ...
    if expected_n is not None and n < expected_n:
        return (
            False,
            f"truncation detected: anchor expects record {expected_n}, but log only has {n} records",
        )
