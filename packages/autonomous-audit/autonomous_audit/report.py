...
def _infer_columns(entries: List[dict]) -> Optional[List[str]]:
    """Determine if all records share a uniform schema (keys), returning the columns if so."""
    if not entries:
        return None

    first_keys = {k for k in entries[0].keys() if k not in ("hash", "prev_hash")}
    if not first_keys:
        return None

    for e in entries:
        keys = {k for k in e.keys() if k not in ("hash", "prev_hash")}
        if keys != first_keys:
            return None

    # Preserve order from the first entry
    return [k for k in entries[0].keys() if k not in ("hash", "prev_hash")]
...
def render_html_report(
    path: Union[str, Path],
    title: str = "autonomous_ Decision Audit",
    ...
    inferred_cols = schema_columns
    if inferred_cols == ["auto"]:
        inferred_cols = _infer_columns(entries)

    if inferred_cols:
        headers = (
            "".join(f"<th>{html.escape(c)}</th>" for c in inferred_cols)
            + "<th>Hash</th>"
        )
        for e in entries:
            row_cells = "".join(
                f"<td>{html.escape(str(e.get(c, '')))}</td>" for c in inferred_cols
            )
            short_hash = str(e.get("hash", ""))[:12]
            row_cells += f'<td class="hash">{html.escape(short_hash)}</td>'
            rows.append(f"<tr>{row_cells}</tr>")
        body = (
            "\n".join(rows)
            or f"<tr><td colspan={len(inferred_cols) + 1}>(no records)</td></tr>"
        )
    else:
        headers = "<th>When (UTC)</th><th>Kind</th><th>Subject</th><th>Verdict</th><th>Detail</th><th>Hash</th>"
        for e in entries:
            s = summarize_entry(e)
            rows.append(
