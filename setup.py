...
def _prompt_alpaca_keys(prompt_enabled: bool) -> tuple:
    ...
    try:
        api_key = input("Alpaca API Key (or Enter to skip): " if on_tty else "").strip()
...
def main():
    ...
    alpaca_key, alpaca_secret = _prompt_alpaca_keys(
        prompt_enabled=not args.non_interactive
    )
        ...
        try:
            for line in lines:
                ...
                if rewritten is not line:
                    f.write(rewritten)
                    continue
                rewritten = _rewrite_alpaca_line(
                    line, "ALPACA_SECRET_KEY", alpaca_secret
                )
                if rewritten is not line:
                    f.write(rewritten)
                    continue
            ...
            f.write(
                "# Auto-generated. Do NOT commit this file or share these values.\n"
            )
            f.write(
                "# HMAC-SHA256 key signing requests from the Public API to the Backend Engine.\n"
            )
            ...
            f.write(
                "# Auto-generated. Required by docker-compose.oss.yml (redis-server --requirepass)\n"
            )
            f.write(
                "# and embedded in REDIS_URL by both the backend and the public-api services.\n"
            )
    ...
    if alpaca_key and alpaca_secret:
        ...
        print(
            "[!] Alpaca keys are empty - bot will boot in Offline Mode (no orders).\n"
        )
