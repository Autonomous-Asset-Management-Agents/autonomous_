...
async def lifespan(app: FastAPI):
    ...
    # #2647 follow-up: off-cloud (loopback/desktop/OSS) there is NO proxy in front of the engine, so
    # REQUIRE_SIG defaults OFF and the shared secret is legitimately absent — never fail-fast there.
    # Cloud Run (K_SERVICE) still enforces it. Same resolver the engine's verify_user_id_sig uses.
    from core.auth import resolve_require_sig

    require_sig = resolve_require_sig(os.environ)
...
# #3628: server-side read-only guard. When PUBLIC_READ_ONLY is set (the public
# live-demo deployment), every mutating request is refused (405) before auth or any
# upstream engine call — the server-side half of src/lib/publicMode.ts (which only
# hides controls in the browser). Registered first so the outer CORS/Otel middleware
# still wrap the 405 (CORS headers + span). Method-based, so a new POST route added
# later cannot slip past.
from public_readonly import read_only_guard  # noqa: E402
