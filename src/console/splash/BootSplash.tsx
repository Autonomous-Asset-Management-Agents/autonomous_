...
import { bootStatusLabel, provisionLabel, PROVISION_IDLE, type ProvisionStatus } from "./bootStatus";
...
export function BootSplash({
  onDone,
  hold = false,
  provision = PROVISION_IDLE,
}: {
  onDone: () => void;
  hold?: boolean;
  /** First-launch runtime provisioning (mac, B4) as ConsoleApp tracks it via useProvisionStatus. */
  ...
  // B4: while the shell fetches the runtime the engine cannot start, so a timeout or a stale engine
  // error would be a false "engine didn't start" (live on 0.4.13-rc4: error + Retry after 120 s, and
  // Retry then produced "Engine source not found"). A provisioning failure IS a hard boot error.
  const provisioning = provision.active;
  const provisionFailed = !!provision.error;
  ...
  const { phase, showStatus } = useBootPhase({
    dataReady: operational,
    engineErrored: engineErrored || provisionFailed,
    engineActive,
    provisioning,
  });
  ...
  // output). The raw log stays in Settings → Engine-Log and the error-state tail below. During
  // provisioning the text follows the shell's stage: percent while downloading, "Verifying…" /
  // "Unpacking…" for the steps that report no bytes (no frozen percent for minutes).
  const statusText = provisioning ? provisionLabel(provision) : bootStatusLabel(engine.logs);

  useEffect(() => {
    if (hold || phase !== "ready") return;
  ...
  // `hold` (keychain probe pending, e.g. the first-launch runtime download) disables both reveals:
  // there is nothing to reveal yet, and the shell ends the hold itself.
  useEffect(() => {
    if (hold) return;
    const t = setTimeout(onDone, SAFETY_REVEAL_MS);
    return () => clearTimeout(t);
  }, [hold, onDone, SAFETY_REVEAL_MS]);
        ...
        <div className="aaa-boot__below">
          {errored ? (
            <div className="aaa-boot__err">
              <p className="aaa-boot__msg">{provisionFailed ? "engine runtime download failed" : "engine didn't start"}</p>
              {provisionFailed && <p className="aaa-boot__detail">{provision.error}</p>}
              <div className="aaa-boot__actions">
                {/* Retry keeps the engine.start() contract. The shell has no re-provision IPC yet, so
                    after a download failure the path that re-runs provisioning is quit + reopen. */}
