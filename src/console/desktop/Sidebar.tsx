...
export function Sidebar() {
  ...
  const trading = tradingLabel(strategyRunning, paperTrading === false, marketOpen);

  // ── Sidebar engine control (Layout F) ──────────────────────────────────────
  const { handleKill } = useKillSwitch();
    ...
    <aside className="w-[210px] shrink-0 flex flex-col bg-black/40 border-r border-white/5">
      ...
      <div className="px-4 py-5 space-y-3.5">
        ...
        {isSenior && (
          engineBooting ? (
          ...
          ) : paperTrading === false && systemHalted !== true ? (
            <button
            ...
            <button
              ...
              className="w-full rounded-full px-6 py-2.5 text-[13px] font-bold tracking-wide uppercase text-white bg-[#00c27a] hover:bg-[#11d089] transition-all transform active:scale-[0.97]"
            >
              Live Trading
            </button>
          )
