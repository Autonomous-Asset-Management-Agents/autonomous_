...
export function Sidebar() {
  ...
  const [activeSettingsTab, setActiveSettingsTab] = useState(() => {
    if (typeof window !== "undefined" && window.location.hash) {
      ...
      if (["trading", "trading-agents", "system", "notifications", "about", "legal"].includes(hash)) {
        return hash;
      }
    }
    return "trading";
  ...
  // B11: WHY the loop is idle (from /health startup_blocked_reason) → the actionable hint next to
  // "Idle" while the engine waits for the local LLM. The hint jumps to Settings → System.
  const startupBlockedReason = useStore((s) => s.startupBlockedReason);
  const trading = tradingLabel(
    strategyRunning,
    paperTrading === false,
    marketOpen,
    componentsDegraded,
    ...
    <aside className="w-[210px] shrink-0 flex flex-col bg-black/40 border-r border-white/5">
            ...
            <div key={id} className="space-y-0.5">
              ...
              {isSettings && active && (
                <div className="pl-4 pr-1 py-1 space-y-0.5 border-l border-white/5 ml-4 mt-1">
                  {[
                    {
                      ...
                      subLabel: "Trading",
                      children: [
                        { subId: "trading", subLabel: "General" },
                        { subId: "trading-agents", subLabel: "Agents" },
                      ],
                    },
                  ...
                  ].map(({ subId, subLabel, badge, children }) => {
                    // Variante E (#3156): an entry with children renders as a GROUP
                    // label plus one indented, hash-routed button per child page —
                    // the active child carries the sanctioned live-dot marker.
                    if (children) {
                      const groupActive = children.some(
                        (c) => activeSettingsTab === c.subId,
                      );
                      return (
                        <div key={subId} className="space-y-0.5">
                          <div
                            className={`px-3 pt-1.5 pb-0.5 text-[11.5px] font-semibold ${
                              groupActive ? "text-white/85" : "text-white/55"
                            }`}
                          >
                            {subLabel}
                          </div>
                          {children.map((c) => {
                            const childActive = activeSettingsTab === c.subId;
                            return (
                              <button
                                key={c.subId}
                                onClick={() => {
                                  if (typeof window !== "undefined") {
                                    window.location.hash = c.subId;
                                    window.dispatchEvent(new Event("hashchange"));
                                  }
                                }}
                                className={`w-full flex items-center gap-2 pl-6 pr-3 py-1.5 rounded-md text-[11.5px] transition-all ${
                                  childActive
                              ...
                              >
                                ...
                                <span className="flex-1 text-left">{c.subLabel}</span>
                              </button>
                            );
                          })}
                        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        {badge && (
                          <span aria-hidden="true" className="text-[8px] font-semibold tracking-wider px-1 py-px rounded bg-white/10 text-white/55 ml-1 leading-none">
                            {badge}
                          </span>
                        )}
                      </button>
      ...
      <div className="px-4 py-5 space-y-3.5">
        ...
        {isSenior && (
          engineBooting ? (
            ...
            <button
              ...
              className="w-full rounded-full px-6 py-2.5 text-[13px] font-bold tracking-wide uppercase text-white bg-[#00c27a] hover:bg-[#00d687] transition-all transform active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-60"
            >
              Live Trading
            </button>
          )
        ...
        <div className="space-y-1.5 text-[11px] num text-white/35">
          ...
          {/* B11: the engine waits for the local LLM (Ollama not running / no model pulled): a bare
              "Idle" reads like a fault; say what is missing and where to fix it. */}
          {trading.hint && (
            <button
              type="button"
              onClick={goToSystemSettings}
              className="w-full text-left text-[11px] text-amber-300/80 hover:text-amber-200 leading-relaxed"
              title="The trading loop is waiting for the local AI model. Install it under Settings → System; the loop starts by itself once the model answers."
            >
              {trading.hint}
            </button>
          )}
