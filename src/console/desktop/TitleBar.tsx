...
import { isDesktop, getPlatform, minimizeWindow, toggleMaximizeWindow, closeWindow } from "@/lib/desktopBridge";
...
export function TitleBar() {
  ...
  // #2744/S5: on macOS the shell uses the native window (hiddenInset) with OS traffic lights on the
  // LEFT, so the console hides its own controls and insets the logo clear of them.
  ...
  return (
    ...
    >
      <div className={`flex items-center gap-2.5 pr-4 ${isMac ? "pl-20" : "pl-3.5"}`}>
      ...
      {/* Right: window controls (desktop, non-mac — macOS uses native traffic lights).
          The language switcher moved to Settings → System (its own card, above AI model). */}
      <div className="flex items-center gap-3">
        {desktop && !isMac ? <WindowControls /> : <div className="pr-3" />}
      </div>
    </div>
  );
}
