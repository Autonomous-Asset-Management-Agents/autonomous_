  ...
  async function apply(patch: Partial<Omit<HitlPolicy, "HITL_ENABLED">>) {
    ...
    try {
      ...
      const updated = await updateHitlPolicy({
        // Both 0 = the documented "all-manual" pair; the mode is carried solely by the waiver flag.
  ...
  return (
    <div className="surface p-6">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        {tile("hitl", "Approve every order", "Nothing reaches the broker without you — every order waits in Orders → Awaiting approval. Protective, risk-reducing sells stay exempt.")}
      </div>
      ...
      {/* #2700: the per-trade / per-day amount inputs are GONE.
          Every value-based mode had to convert equity → % → $, and each conversion was a way to
          end up somewhere the operator did not choose — the wizard's 25% collapsed to 0 when the
          live equity was unknown, and a per-trade budget was silently negated by a per-day budget
          of 0 (`hitl_gate.py:379` reads 0 as "no autonomous budget", which is correct). Two modes
          need no arithmetic: the tiles above set HITL_AUTONOMOUS_UNLIMITED and nothing else.
          Re-introducing a threshold needs BOTH limits filled and a real equity base — see #2700. */}
    </div>
  );
}
