    ...
    <div className="space-y-3.5">
      ...
      {!q && sections.length > 0 && (
        <div className="flex gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
          {/* UXC-1 S8 (#3177): the section jump is an action → .btn, not a pill (pills are labels). */}
          {sections.map((s) => (
            <button key={s.id} className="btn btn-compact" onClick={() => jump(s.id)}>
              {s.title}
            </button>
          ))}
        </div>
  ...
  return (
    <span className="flex items-center justify-start gap-2">
      ...
      {r.reportAuditOk != null && (
        ...
        >
          <span className={`inline-block h-[7px] w-[7px] flex-none rounded-full ${r.reportAuditOk ? "bg-[#00c27a]" : "bg-[#ff5a52]"}`} />
        </span>
      )}
      {r.dataStale && (
...
export function Reports() {
  ...
  // #2732 Strand B: a deep-linked symbol whose row we still need to scroll into view once it renders
  // (the reports list may arrive AFTER the click, and the target can be far down the list).
  ...
  // Strand B (#2732): consume a symbol deep-link — open that report expanded AND scroll to it, then
  // clear the target so a later manual collapse (open=null) still wins. Syncing local UI state to an
  // external one-shot store signal is a legitimate effect.
  /* eslint-disable react-hooks/set-state-in-effect -- consume external deep-link signal */
  useEffect(() => {
    if (!reportSymbol) return;
    // Resolve to the ACTUAL report symbol — a caller may pass a base ticker (e.g. Overview strips
    // the exchange suffix: "BRK.B" -> "BRK"), which would otherwise match no row and open nothing.
    const target = resolveDeepLinkSymbol(
      readable.map((r) => r.symbol),
      reportSymbol,
    );
    setOpen(target);
    setTouched(true);
    setQuery(""); // a stale search filter must not hide the deep-linked row
    setPendingScroll(target);
    setReportSymbol(null);
  }, [reportSymbol, setReportSymbol, readable]);

  // Scroll the deep-linked row into view once it is actually in the DOM (handles the reports list
  // loading after the click). Clears itself so manual toggles never trigger a scroll.
  useEffect(() => {
    if (!pendingScroll) return;
    const el = document.getElementById(`report-row-${pendingScroll}`);
    if (el) {
        ...
        <div className="surface overflow-hidden">
          ...
          {filtered.map((r) => {
            ...
            return (
              <div
                key={r.symbol}
                id={`report-row-${r.symbol}`}
                className="scroll-mt-24 border-t border-white/6 first:border-t-0"
              >
                <button
                  type="button"
                ...
                >
                  <span className={`num text-[13px] leading-none transition-transform ${isOpen ? "rotate-90 text-white/80" : "text-white/35"}`}>&rsaquo;</span>
