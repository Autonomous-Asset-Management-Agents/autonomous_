...
import { RoundTableDecisionRow } from "@/console/shared/RoundTableView";
...
function SentimentBar({ score }: { score: number | null }) {
  ...
  const pct = score * 10;
  const color = score >= 6 ? "#30d158" : score <= 4 ? "#ff453a" : "rgba(255,255,255,0.55)";
  return (
...
function MlContributionBlock({ r }: { r: SpecialistReport }) {
  ...
  return (
    <div className="surface p-6 space-y-4 relative overflow-hidden">
      <div className="halo-soft absolute -top-16 -left-16 w-[200px] h-[200px] pointer-events-none opacity-40" />
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 mt-3 flex-wrap">
            ...
            {r.escalate && <span className="pill pill-warn text-[10.5px] font-semibold">ESCALATED</span>}
          </div>
        </div>
        {updatedStr && (
    ...
    <div className="px-8 py-7 space-y-6 max-w-[1100px]">
      <div>
        ...
        <h1 className="text-[26px] font-bold tracking-tight2 text-white/92">
          {roundTable.length} {roundTable.length === 1 ? "decision" : "decisions"}
        </h1>
        <p className="text-white/45 text-[13px] mt-1.5 max-w-xl">
          The latest Round-Table verdict per symbol — agent votes, conviction and any hard vetoes.
        </p>
      </div>

      {roundTable.length === 0 ? (
        <div className="surface px-8 py-14 text-center text-white/35 text-[13px]">
          No Round-Table decisions yet — the engine produces them as it evaluates the universe.
        </div>
      ) : (
        <div className="space-y-3">
          {roundTable.map((d) => (
            <RoundTableDecisionRow key={d.symbol} d={d} />
          ))}
        </div>
      )}
      ...
      <div className="pt-2">
        <div className="eyebrow mb-2">Specialist reports</div>
        <h2 className="text-[20px] font-bold tracking-tight2 text-white/92">
          {specialistReports.length}{" "}
          {specialistReports.length === 1 ? "report" : "reports"}
        </h2>
        <p className="text-white/45 text-[13px] mt-1.5 max-w-xl">
          Per-symbol research + on-device ML read — sentiment, key signals, the TFT
          model's scenario returns and the deterministic decision summary.
        </p>
      </div>
