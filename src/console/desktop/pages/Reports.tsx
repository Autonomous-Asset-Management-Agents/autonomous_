...
import { reportBadgeRecommendation } from "@/console/live/specialist";
...
function SymbolCombo({
  ...
  const results = useMemo(() => {
    ...
    return reports.filter(
      (r) => !s || r.symbol.toLowerCase().includes(s) || (getCompanyName(r.symbol) || "").toLowerCase().includes(s),
    );
  }, [q, reports]);

  const label = (r?: SpecialistReport | null) =>
    r ? `${r.symbol}${getCompanyName(r.symbol) ? " — " + getCompanyName(r.symbol) : ""}` : "";
  const sel = reports.find((r) => r.symbol === selected) ?? null;

  function commit(r?: SpecialistReport) {
    if (!r) return;
    onPick(r.symbol);
    setQ("");
    setOpen(false);
  ...
  return (
    <div className="relative min-w-[280px]">
      <svg className="absolute left-3 top-[10px] opacity-40" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="m21 21-4-4" /></svg>
      <input
        className="w-full rounded-xl border border-white/8 bg-white/[0.04] py-[9px] pl-[34px] pr-3 text-[13px] font-semibold text-white/92 outline-none focus:border-white/25"
        placeholder="Search a company — NVDA, Apple, Nvidia …"
        autoComplete="off"
        value={open ? q : label(sel)}
        onFocus={() => { setQ(""); setActive(-1); setOpen(true); }}
        onChange={(e) => { setQ(e.target.value); setOpen(true); setActive(-1); }}
        onBlur={() => { blurTimer.current = window.setTimeout(() => setOpen(false), 130); }}
        onKeyDown={(e) => {
          if (e.key === "ArrowDown") { e.preventDefault(); setActive((a) => Math.min(a + 1, results.length - 1)); }
          else if (e.key === "ArrowUp") { e.preventDefault(); setActive((a) => Math.max(a - 1, 0)); }
          else if (e.key === "Enter") { e.preventDefault(); commit(results[active >= 0 ? active : 0]); }
          else if (e.key === "Escape") { setOpen(false); }
        }}
      />
      {open && (
        <div
          className="absolute left-0 right-0 top-[calc(100%+6px)] z-30 max-h-[300px] overflow-y-auto rounded-2xl border border-white/10 bg-[rgba(20,20,22,0.98)] p-1.5"
          onMouseDown={() => { if (blurTimer.current) window.clearTimeout(blurTimer.current); }}
        >
          {results.length === 0 ? (
            <div className="px-3 py-4 text-center text-[12px] text-white/35">No company found</div>
          ) : (
            results.map((r, i) => (
              <div
                key={r.symbol}
                className={`flex cursor-pointer items-center justify-between gap-2.5 rounded-[10px] px-2.5 py-2 ${i === active ? "bg-white/[0.06]" : "hover:bg-white/[0.06]"}`}
                onMouseEnter={() => setActive(i)}
                onMouseDown={(e) => { e.preventDefault(); commit(r); }}
              >
                <div>
                  <div className="text-[13px] font-semibold text-white/92">{r.symbol}</div>
                  {getCompanyName(r.symbol) && <div className="text-[11px] text-white/40">{getCompanyName(r.symbol)}</div>}
                </div>
                <RecPill rec={reportBadgeRecommendation(r)} />
              </div>
            ))
          )}
        </div>
...
export function Reports() {
  ...
  // verdict for the selected symbol (shared store; no-op when no decisions exist).
  ...
  // Any specialist with content is selectable (report_markdown when the report
  ...
  const selected = readable.find((r) => r.symbol === sym) ?? readable[0] ?? null;
  ...
  return (
    <div className="px-8 py-7 space-y-5 max-w-[1040px]">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        {readable.length > 0 && (
          <SymbolCombo reports={readable} selected={selected?.symbol ?? null} onPick={setSym} />
        )}
      </div>
      ...
      <div className="surface-flat rounded-xl px-5 py-4 flex items-start gap-3">
        ...
        <div className="text-[13px] text-white/70 leading-relaxed space-y-1.5">
          <p>
            <span className="font-semibold text-white/90">How to read these reports.</span>{" "}
            Each report shows the analysis and recommendation of a single specialized AI assistant. The final trading decisions, however, are made by a collective vote of all its AI assistants. This is why a trade can differ from what you see on a single report.
          </p>
        </div>
      </div>
      ...
      {readable.length === 0 ? (
        ...
        <ReportReader key={selected.symbol} r={selected} />
      ) : null}
    </div>
  );
}
