      ...
      <div>
        ...
        <div className="flex items-baseline gap-6 flex-wrap">
          ...
          <span className="text-[13px] text-white/55">
            Invested <span className="num text-white/92">{money(invested)}</span> · {fmtPct(investedPct, 1).replace("+", "")} of book
          </span>
          <span className="text-[13px] text-white/55">
            Cash <span className="num text-white/92">{cashEUR !== null ? money(cashEUR) : "—"}</span>
          </span>
          ...
          <table className="w-full text-[12.5px]">
            ...
            <tbody>
              {positions.map((p) => {
                ...
                // today) — null when the broker didn't serve change_today.
                const todayUsd =
                ...
                return (
                  <tr key={p.symbol} className="border-t border-white/5 hover:bg-white/[0.025] transition-colors">
                    ...
                    <td className="px-4 py-3.5 text-right num">{fmtQty(p.qty)}</td>
                    <td className="px-4 py-3.5 text-right num text-white/55">{money(p.avgEntry)}</td>
                    <td className="px-4 py-3.5 text-right num text-white/92">
                      {/* #3421: Börsen-App-Stil — live quote prominent, today's move
                          (%/$, coloured) beneath it; just the price when unknown. */}
                      <div>{money(p.last)}</div>
                      {p.changeTodayPct !== null && (
                        <div
                          className={`text-[10px] opacity-90 ${p.changeTodayPct >= 0 ? "text-bull" : "text-bear"}`}
                        >
                    ...
                    <td className="px-4 py-3.5 text-right num text-white/92">{money(p.marketValue)}</td>
