...
import {
  ...
  PAGE_LABELS,
} from "@/console/desktop/consoleCommands";
...
export function Chat() {
  ...
  async function submit() {
    ...
    setInput("");
    // XAI-T9 command-surface (#1338): a recognized navigation command drives the console
    // instead of the engine, when enabled. Flag off (default) -> unchanged behaviour; a real
    // question never matches the interpreter and always falls through to the engine.
    if (isCommandSurfaceEnabled()) {
      const cmd = interpretConsoleCommand(text);
      if (cmd) {
        setDesktopPage(cmd.page);
        addMessage("assistant", `Opening ${PAGE_LABELS[cmd.page]}.`);
        return;
      }
    }
    setSending(true);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-8 py-6 border-b border-white/5">
        ...
        {/* Chat is BETA — set expectations, especially for the local-LLM (Ollama) path. */}
        <p className="text-[11px] text-[#ffcc66]/85 mt-2 max-w-xl">
          <strong>BETA</strong> — chat is still rough, especially with a local model (Ollama):
          replies can be slow or imperfect. It is improving.
        </p>
      </div>
      ...
      <div className="px-8 py-5 border-t border-white/5">
        <div className="flex items-end gap-2">
          ...
          <button
            ...
            className="shrink-0 rounded-xl px-4 py-2.5 text-[13px] font-semibold bg-[#00c27a] text-black disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#00d687] transition-colors"
          >
            Send
          </button>
        </div>
