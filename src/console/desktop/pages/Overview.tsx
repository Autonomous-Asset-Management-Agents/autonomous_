...
import { computeSharpe, benchmarkTodayPct, liveHistoryReady } from "@/console/live/metrics";
...
export function Overview({ cancelOrder = realCancelOrder }: { cancelOrder?: typeof realCancelOrder } = {}) {
  ...
  const benchToday = liveMetricsBuilding ? null : benchmarkTodayPct(benchmarkCurve);
          ...
          <div>
            ...
            <div className="flex items-center gap-6 mt-4 text-[13px] text-white/60">
              ...
              {netDeposits != null && netDeposits !== 0 && (
                <>
                  <span className="hairline-v h-3" />
                  {/* #1957: net external cash in/out, kept separate from performance so a deposit
                      reads as capital added — not as a gain. */}
                  <span>Net deposits <span className="num text-white/92">{money(netDeposits, { sign: true, compact: true })}</span></span>
                </>
              )}
            </div>
          </div>
