...
import { IconChevronRight, IconShield } from "@/console/shared/Icons";
...
export function Overview() {
  ...
  const { killArmed, killMsg, killResetting, handleKill, handleResetKill } = useKillSwitch();
    ...
    <div className="px-8 py-7 space-y-7">
      ...
      <div className="flex items-end justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="pill">Market · {marketPillLabel(marketOpen)}</span>
        </div>
      </div>

      {/* C-D: a fresh install has no data yet — say so, so the blank "—" values below don't read as "broken". */}
      {currentEquity == null && (
        <div className="surface p-4 text-[12px] text-white/55 leading-relaxed">
          <strong className="text-white/80">Setting up.</strong> Your dashboard fills in as the engine
          syncs its first data and starts paper-trading — the blank values below are expected on a fresh
          start, not an error, and can take a moment after launch.
        </div>
      )}

      {/* Total-equity surface */}
      <div className="surface p-7 relative overflow-hidden">
        <div className="halo-soft absolute -top-32 -right-20 w-[480px] h-[480px] pointer-events-none" />
        <div className="grid grid-cols-[1fr_auto] gap-6 items-start relative">
          <div>
            ...
            <div className="flex items-center gap-6 mt-4 text-[12px] text-white/55">
              <span>
                Since inception{" "}
                {totalReturnPct != null ? (
          ...
          <div className="flex flex-col items-end gap-1.5">
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`text-[11px] px-2.5 py-1 rounded-md border transition-colors ${percent ? "border-transparent text-[#00c27a] bg-[#00c27a]/12" : "border-white/5 text-white/55 hover:text-white/92 hover:border-white/15"}`}
              >
                %
              </button>
              {EQUITY_RANGES.map((p) => (
            ...
            <div className="text-[10px] text-white/30 num">Last sync · {ago(lastSyncAt, now)}</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 mt-4 text-[11px] text-white/55">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          ...
          <div key={k.label} className="surface-flat p-4">
            ...
            <div className="text-[10px] text-white/30 mt-1">{k.hint}</div>
          </div>
        ))}
      </div>

      {/* Kill switch — prominent safety control (#1642) */}
      <div
        className="surface p-4 flex items-center gap-4"
        style={{ borderColor: "rgba(255,69,58,0.22)" }}
      >
        <div
          className="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
          style={{ background: "rgba(255,69,58,0.15)", border: "1px solid rgba(255,69,58,0.30)" }}
        >
          <IconShield width={16} height={16} className="text-bear" />
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[13px] font-semibold text-white/92">Kill switch</span>
          {systemHalted == null ? (
            <span className="text-[11px] text-white/45">—</span>
          ) : systemHalted ? (
            <span className="text-[11px] font-semibold text-bear">● GESTOPPT</span>
          ) : (
            <span className="text-[11px] font-semibold text-bull">● AKTIV</span>
          )}
        </div>
        {killMsg && <div className="text-[11px] text-white/55 truncate">{killMsg}</div>}
        <div className="flex gap-2 ml-auto shrink-0">
          <button className="btn btn-bear" onClick={() => void handleKill()}>
      ...
      <div className="grid grid-cols-[1.15fr_1fr] gap-5">
        <div className="surface p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setDesktopPage("decisions")} className="text-[11px] text-white/55 hover:text-white/92 flex items-center gap-1">
          ...
          <div className="space-y-2">
            {roundTable.slice(0, 4).map((d) => (
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="ml-auto pill pill-strong">{fmtPct(d.conviction * 100, 0).replace("+", "")} conviction</span>
                </div>
            ...
            {roundTable.length === 0 && lastKnown.length > 0 && (
              <>
                <div className="text-[10px] text-white/30 px-1 pb-1">
                ...
                {lastKnown.map((v) => (
                  <div key={v.symbol} className="w-full p-3 rounded-lg bg-white/[0.015] border border-white/5 opacity-60">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="ml-auto text-[10px] text-white/40 num">last known · {fmtTime(v.ts)}</span>
                    </div>
                  </div>
                ))}
              </>
            ...
            {roundTable.length === 0 && lastKnown.length === 0 && (
              <div className="text-[12px] text-white/30 p-4 text-center">No decisions yet — the round table runs as the engine ticks.</div>
            )}
          </div>
        </div>
        ...
        <div className="surface p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <button onClick={() => setDesktopPage("positions")} className="text-[11px] text-white/55 hover:text-white/92 flex items-center gap-1">
              View all <IconChevronRight width={10} height={10} />
            </button>
          </div>
            ...
            {positions.slice(0, 5).map((p) => {
              ...
              return (
                <div key={p.symbol} className="flex items-center gap-3 py-2.5 border-b border-white/5 last:border-0">
                  <div className="w-12">
                    <div className="font-bold text-[13px] tracking-tight2 text-white/92">{p.symbol.split(".")[0]}</div>
                  </div>
                  ...
                  <div className={`num text-[11px] w-16 text-right ${p.unrealizedPct >= 0 ? "text-bull" : "text-bear"}`}>{fmtPct(p.unrealizedPct)}</div>
                </div>
              );
            })}
            {positions.length === 0 && <div className="text-[12px] text-white/30 p-4 text-center">No open positions.</div>}
          </div>
        </div>
      </div>
    </div>
