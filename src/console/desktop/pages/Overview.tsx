      ...
      <div className="surface p-7 relative overflow-hidden">
        ...
        <div className="grid grid-cols-[1fr_auto] gap-6 items-start relative">
          ...
          <div className="flex flex-col items-end gap-1.5">
            ...
            <div className="text-[10px] text-white/30 num">Last sync · —</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-5 mt-4 text-[11px] text-white/55">
          <span className="flex items-center gap-2"><span className="w-3 h-[2px] rounded" style={{ background: "#00c27a" }} /> AAAgents</span>
          {benchView.length >= 2 && (
        ...
        <div className="surface p-5">
          ...
          <div className="space-y-0">
            {positions.slice(0, 5).map((p, i) => {
              const sparkData = Array.from({ length: 12 }, (_, k) => p.avgEntry + (p.last - p.avgEntry) * (k / 11) + Math.sin(k + i) * (p.last * 0.01));
              return (
