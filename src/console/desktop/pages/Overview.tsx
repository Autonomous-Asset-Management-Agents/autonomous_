...
export function Overview({ cancelOrder = realCancelOrder }: { cancelOrder?: typeof realCancelOrder } = {}) {
  ...
  const showOrderBook = showOrderBookOnOverview(paperTrading);
  ...
  return (
    <div className="px-8 py-7 space-y-7 max-w-[1100px]">
      ...
      {/* PR-3: LIVE with a known equity but NO accumulated live history yet (the paper→live
          restart shows NO paper carryover — see /benchmark-equity "no_live_history"). This is a
          THIRD, disjoint condition: both banners above require currentEquity==null, this one
          requires a non-null equity, so it never overlaps the reconnect / paper-setup banners.
          Started / Since-inception / Max-drawdown already fall to "—" from the empty curve. */}
      {paperTrading === false && currentEquity != null && equityCurve.length === 0 && (
        <div className="surface-flat rounded-xl px-5 py-4 flex items-start gap-3">
          <IconLightbulb width={16} height={16} className="text-white/70 mt-1 shrink-0" />
          <div className="text-[13.5px] text-white/70 leading-relaxed">
            <p>
              <span className="font-semibold text-white/90">Live history is building.</span> Since-inception and drawdown appear here as live snapshots accumulate — your paper-trading history is intentionally not carried into the live account.
            </p>
            <p className="text-[13px] text-white/45 mt-1">
              Your current live equity is shown below; the curve fills in from your first live trading day.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="grid grid-cols-[1fr_auto] gap-6 items-start relative">
          <div>
            ...
            <div className="flex items-baseline gap-4 flex-wrap">
              ...
              {dayPL != null && (
                <div className={`flex items-baseline gap-3 ${dayPL >= 0 ? "text-bull" : "text-bear"}`}>
                  <span className="num text-[20px] font-semibold">{money(dayPL, { sign: true })}</span>
            ...
            <div className="flex items-center gap-6 mt-4 text-[13px] text-white/60">
              ...
              <span className="hairline-v h-3" />
              <span>Started <span className="num text-white/92">{startEquity != null ? money(startEquity, { compact: true }) : "—"}</span></span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          {
            label: "Daily P / L",
            value: dayPL != null ? money(dayPL, { sign: true }) : "—",
            hint: dayPLPct != null ? `${fmtPct(dayPLPct)} today` : `today · ${iso}`,
      ...
      {/* Order book — live-only (#2190): market orders fill instantly in paper so the book is
          always empty → shown ONLY in live trading (a Senior/Plus feature). Per-order HITL
          Cancel lives on the Orders page. History is NOT shown here. */}
      {showOrderBook && (
              ...
              <tbody>
                {openOrders.slice(0, 6).map((o) => (
                  <tr key={o.id} className="border-t border-white/5 hover:bg-white/[0.025] transition-colors">
                    <td className="px-4 py-3 font-bold tracking-tight2 text-white/92">{o.symbol}</td>
