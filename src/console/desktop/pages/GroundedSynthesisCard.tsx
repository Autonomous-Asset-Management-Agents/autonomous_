...
import { partitionRoundTable } from "@/console/live/roundTable";
  ...
  return (
    <div className="surface px-6 py-5 space-y-5">
      ...
      {decision && decision.senators.length > 0 && (() => {
        // Split the board the SAME way the gate does: Direction (counted in the
        // mean) vs Sizing & Risk (the IV-driven VIXAware sizer + base
        // conditioner/veto-guard — NOT in the vote). #3094: the panel used to
        // re-compute over weight>0 and wrongly re-include the sizer, showing a
        // number that disagreed with the gate ("64% + BUY", live 2026-09-08).
        const { direction, sizingRisk, dormant, wSum, wTot } = partitionRoundTable(
          decision.senators,
          decision.consensusScore,
        );
        // Headline = the AUTHORITATIVE consensus the ENGINE served (never a
        // frontend re-compute). Fall back to the signed-conviction rebasing only
        // when the raw score is absent (older engine).
        const consensusPct = Math.round(
          (typeof decision.consensusScore === "number"
            ? decision.consensusScore
            : (decision.conviction + 1) / 2) * 100,
        );
        ...
        const roleLabel: Record<string, string> = { sizer: "sizer", conditioner: "conditioner", veto_guard: "veto guard" };
        const headerRow = (
        ...
        const agentRow = (s: RoundTableSenator, i: number, counted: boolean) => (
          <div key={i} className={`${grid} items-start border-t border-white/6 py-1 ${counted ? "" : "opacity-70"}`}>
            <span className="text-[12px] text-white/70">
              {s.name}
              {!counted && s.role && roleLabel[s.role] && (
                <span className="ml-1.5 num text-[9px] uppercase tracking-wide text-white/35">{roleLabel[s.role]}</span>
              )}
            </span>
            ...
            <span className="num text-right text-[11.5px] text-white/45">{counted ? s.weight.toFixed(2) : "—"}</span>
        ...
        return (
          <div className="border-t border-white/6 pt-4 space-y-3">
            ...
            {/* Consensus · the result of the weighted DIRECTION vote */}
            <div className="flex items-baseline gap-2.5 flex-wrap">
            ...
            <div className="num text-[11px] text-white/55">
              ...
              <span className="text-white/30"> · full board tally</span>
            </div>
            {confPct != null && (
            ...
            <div className="mt-1 space-y-1">
              ...
              {headerRow}
              {direction.map((s, i) => agentRow(s, i, true))}
              {/* how the consensus is derived — Σ over the DIRECTION set only */}
              <div className="border-t border-white/10 pt-2 num text-[11.5px] leading-relaxed text-white/55">
                ...
                <span className="text-white/85">{consensusPct}%</span>{" "}
                · thresholds: BUY ≥ 65% · SELL ≤ 35% · else HOLD
              </div>
            </div>

            {/* ── SIZING & RISK: NOT in the vote — VIXAware sizer + conditioners ── */}
            {sizingRisk.length > 0 && (
              <div className="mt-2 space-y-1">
                <div className="num text-[10px] uppercase tracking-wide text-white/40">
                  Sizing &amp; risk — not in the vote{" "}
                  <span className="tracking-normal normal-case text-white/30">· informs position size, not direction</span>
                </div>
                {headerRow}
                {sizingRisk.map((s, i) => agentRow(s, i, false))}
              </div>
            )}
            ...
            {dormant.length > 0 && (
              <div className="num text-[10.5px] text-white/35">
                {dormant.length} dormant this cycle (weight 0 → no vote): {dormant.map((s) => s.name).join(", ")}
              </div>
            )}
          </div>
        );
