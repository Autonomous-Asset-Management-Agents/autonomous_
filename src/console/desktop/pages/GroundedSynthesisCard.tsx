...
function Arrow({ dir }: { dir: SpecialistReport["mlDirection"] }) {
  if (dir === "up")
    return <span className="text-bull num text-[12px]">ML ↑</span>;
  if (dir === "down")
    return <span className="text-bear num text-[12px]">ML ↓</span>;
  if (dir === "neutral")
    return <span className="num text-[12px] text-white/45">ML →</span>;
  return null;
}
...
function ModelConfirmChip({ r }: { r: SpecialistReport }) {
  const confPct =
    r.mlConfidence != null ? Math.round(r.mlConfidence * 100) : null;
  ...
  return (
    <div className="text-[11px] text-white/40 leading-snug">
      ...
      <span className="italic">
        ...
        {base != null &&
          ` · base scenario ${base > 0 ? "+" : ""}${base.toFixed(1)}%`}
...
function ProseBlock({
  ...
  const sentences =
    prose && !/^No supported (?:bull|bear|case|thesis)/i.test(prose)
      ? splitSentences(prose)
      : [];
    ...
    <div className="space-y-2">
      ...
      {empty ? (
        <div className="text-[12.5px] italic text-white/35">
          No supported case from this news cycle yet — updated with the next
          research round.
        </div>
      ) : (
          ...
          {(() => {
            ...
            return sentences.map((s, i) => {
              ...
              const cites = citations.filter(
                (c) =>
                  c.claim === s || c.claim.includes(s) || s.includes(c.claim),
              );
...
export function GroundedSynthesisCard({
  ...
  const mlAvailable =
    r.mlDirection === "up" ||
    r.mlDirection === "down" ||
    r.mlDirection === "neutral";
  const mlConfPct =
    r.mlConfidence != null ? Math.round(r.mlConfidence * 100) : null;
  const hasScenario =
    r.mlReturns.bear != null ||
    r.mlReturns.base != null ||
    r.mlReturns.bull != null;
    ...
    <div className="surface px-6 py-5 space-y-5">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-baseline gap-2.5 flex-wrap">
          ...
          {getCompanyName(r.symbol) && (
            <span className="num text-[13px] text-white/40">{r.symbol}</span>
          )}
        ...
        <div className="flex flex-col items-end gap-0.5">
          ...
          {r.reportAuditOk != null && (
            <span
              ...
              title={
                r.reportAuditSummary ??
                "Every displayed figure is checked against its source"
              }
            >
          ...
          {r.dataStale && (
            <span
              className="num text-[10.5px] text-white/50"
              title="A source's filings are stale"
            >
              stale filings
            </span>
          )}
        ...
        <div className="surface-flat rounded-xl px-4 py-3 flex items-start gap-3">
          ...
          <div>
            ...
            <div className="mt-1 text-[11.5px] leading-snug text-white/40">
              An unusual signal tripped the specialist&rsquo;s escalation rule —
              an attention marker only. It is not a buy/sell action and does not
              change the Round-Table decision.
            </div>
          </div>
        </div>
      )}
      ...
      {r.summary && r.summary.trim() && (
        <div className="border-t border-white/6 pt-4">
          <div className="text-[10px] uppercase tracking-wide text-white/35">
            Summary{" "}
            <span className="tracking-normal normal-case text-white/25">
              · additive — the full note follows
            </span>
          </div>
          <div className="mt-1 text-[14.5px] font-semibold leading-snug text-white/90">
            {r.summary}
          </div>
        </div>
      )}
      ...
      <div className="surface-flat rounded-xl px-4 py-3 space-y-2">
        ...
        {mlAvailable ? (
          <div className="space-y-2">
            ...
            {hasScenario && (
              <div className="grid grid-cols-3 gap-2 text-center">
                <div>
                  <div className="text-[10.5px] uppercase tracking-wide text-white/35">
                    Bear
                  </div>
                  <div className="num text-[13px] text-bear">
                    {fmtPct(r.mlReturns.bear)}
                  </div>
                </div>
                <div>
                  <div className="text-[10.5px] uppercase tracking-wide text-white/35">
                  ...
                  <div className="num text-[13px] text-white/80">
                    {fmtPct(r.mlReturns.base)}
                  </div>
                </div>
                <div>
                  <div className="text-[10.5px] uppercase tracking-wide text-white/35">
                  ...
                  <div className="num text-[13px] text-bull">
                    {fmtPct(r.mlReturns.bull)}
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="text-[11.5px] text-white/40 leading-snug">
            No directional model signal for this name — only what survives
            walk-forward validation is shown.
          </div>
        ) : null}
        ...
        {r.priceBandLow != null && r.priceBandHigh != null && (
          <div className="border-t border-white/6 pt-2 space-y-1">
            ...
            <div className="text-[11px] text-white/45 leading-snug">
              ...
              {r.lastClose != null && (
                <> · last close ${r.lastClose.toFixed(2)}</>
              )}
              {r.volBandSigmaPct != null && (
                <> · ±{r.volBandSigmaPct.toFixed(1)}%</>
              )}
            </div>
          </div>
        )}
        ...
        {(r.chg20dPct != null || r.dist52wHighPct != null) && (
          <div className="border-t border-white/6 pt-2 num text-[11.5px] text-white/55 flex gap-4 flex-wrap">
            ...
            {r.dist52wHighPct != null && (
              <span>{r.dist52wHighPct.toFixed(1)}% vs 52-week high</span>
            )}
          </div>
        )}
        {r.priceBandLow == null && (
            ...
            {r.volBandSigmaPct != null ? (
              <>
                <div className="num text-[12px] text-white/70">
                  ±{r.volBandSigmaPct.toFixed(1)}% volatility band (HAR-RV, 20
                  trading days)
                </div>
                <div className="text-[11px] text-white/40 leading-snug">
                  A spread, not a direction call — it sizes risk, it does not
                  pick a side.
                </div>
              </>
            ) : (
              ...
              <>
                ...
                <div className="text-[11px] text-white/40 leading-snug">
                  The HAR-RV volatility band needs a live market-data connection
                  — it returns once the market reopens.
                </div>
              </>
            )}
          </div>
      ...
      <div className="surface-flat rounded-xl px-4 py-3 space-y-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-2">
          {(
            [
              ...
              [
                "Congress trades",
                r.politicalTradesCount != null
                  ? `${r.politicalTradesCount}`
                  : "—",
              ],
              ["8-K filings", `${r.materialEventsCount ?? 0}`],
              [
                "Reddit mentions (24h)",
                r.redditMentions != null ? `${r.redditMentions}` : "—",
              ],
              [
                "Short interest",
                r.shortInterestPct != null
                  ? `${r.shortInterestPct.toFixed(1)}%`
                  : "—",
              ],
              [
                "Price history",
        ...
        <div className="text-[10.5px] text-white/30 leading-snug">
          Every source the system checked for this name this cycle — a 0 means
          checked, nothing found; — means the source could not be checked this
          cycle.
        </div>
      </div>
        ...
        <div className="border-t border-white/6 pt-4 space-y-3">
          ...
          {confPct != null && (
            <div className="text-[11.5px] text-white/40">
              Specialist note confidence: {confPct}% — data quality of this
              report, not the vote result.
            </div>
          )}
          {/* #3354: the SAME shared partitioned breakdown as the Decisions page. */}
          <RoundTableVote decision={decision} />
        </div>
      )}
      ...
      {hasWalkForward && (
        <div className="border-t border-white/6 pt-3 num text-[11px] text-white/40 flex gap-4">
          {r.walkforwardIc != null && (
            <span>IC {r.walkforwardIc.toFixed(3)}</span>
          )}
          {r.walkforwardSharpe != null && (
            <span>Sharpe {r.walkforwardSharpe.toFixed(2)}</span>
          )}
          ...
          <ul className="mt-2 list-disc space-y-1.5 pl-5">
            {r.synthesisDropped.map((d, i) => (
              <li key={i} className="text-[12px] leading-relaxed text-white/45">
                <span className="text-white/70">&ldquo;{d.claim}&rdquo;</span> —{" "}
                {d.reason}
              </li>
            ))}
          </ul>
