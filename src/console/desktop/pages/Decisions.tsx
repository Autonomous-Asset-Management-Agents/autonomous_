...
import {
  RoundTableBar,
  ConvictionMeter,
} from "@/console/shared/RoundTableView";
...
const actionClass = (a: string) =>
  a === "BUY" ? "text-bull" : a === "SELL" ? "text-bear" : "text-white/55";
const sourceLabel = (d: ConsoleRoundTableDecision) =>
  d.source === "hitl" ? "HITL" : "Autonom";
...
function outcomeBadge(
  code?: string | null,
): { label: string; tone: "on" | "off" | "neutral" } | null {
  switch (code) {
    case "executed":
    ...
    case "blocked:market_closed":
      return { label: "Blocked · market closed", tone: "off" };
    case "hitl_held":
...
function DecisionCard({ d }: { d: ConsoleRoundTableDecision }) {
  ...
  // #3354: the collapsed vote bar reflects only the DIRECTIONAL voters (weight>0, not overlays),
  // so it reconciles with the verdict — not the mixed full-board count.
  ...
  return (
    <div className="surface-flat rounded-xl overflow-hidden">
      ...
      >
        <span className="text-white/30 text-[11px] w-3">
          {open ? "▾" : "▸"}
        </span>
        <span className="w-32 min-w-0">
          <span className="block font-bold tracking-tight2 text-white/92 text-[14px] truncate">
            <SymbolLink symbol={d.symbol} />
          </span>
          {name && (
            <span className="block text-[10px] font-normal text-white/40 truncate leading-tight">
              {name}
            </span>
          )}
        </span>
        <span
          className={`text-[12px] num font-semibold w-12 ${actionClass(d.action)}`}
        >
          {d.action}
        </span>
        {outcome && (
        ...
        <span className="pill !text-[9px] !px-1.5 !py-0.5 ml-auto">
          {sourceLabel(d)}
        </span>
        <span className="num text-white/30 text-[10px] w-10 text-right">
          {d.ts}
        </span>
      </button>
      ...
      {open && (
        <div className="px-4 pb-3.5 pt-1 space-y-3 border-t border-white/5">
          ...
          {d.vetoReason && (
            <div className="text-bear text-[11px]">{d.vetoReason}</div>
          )}
          {/* #3354: the SAME partitioned breakdown as the Reports synthesis (shared component). */}
          <RoundTableVote decision={d} />
        </div>
      )}
    </div>
  );
...
export function Decisions() {
  ...
  const shown =
    filter === "All"
      ? roundTable
      : roundTable.filter((d) => d.action === filter);
      ...
      <div>
        ...
        <div className="flex items-baseline gap-6 flex-wrap">
          <h1 className="text-[26px] font-bold tracking-tight2 text-white/92">
            {roundTable.length} decisions
          </h1>
          {/* UXC-1 S8 (#3177): neutral segmented filter — selection sits on white/12,
      ...
      <div className="surface-flat rounded-xl px-5 py-4 flex items-start gap-3">
        ...
        <div className="text-[13.5px] text-white/70 leading-relaxed space-y-2">
          <p>
            ...
            The action is a risk-weighted vote of the agents — not a simple
            head-count. A high-conviction, heavily-weighted agent can carry the
            verdict to BUY or SELL even when most agents read HOLD.
          </p>
          <p className="text-[13px] text-white/45">
            <span className="text-white/60">Tip:</span> Expand any card to see
            every agent's vote; click an agent's name for its role, tasks and
            criteria.
          </p>
        </div>
      </div>
