...
export function Settings() {
  ...
  // Emergency kill switch — shared arm→confirm halt + reset-and-resume logic (useKillSwitch, #1642).
  ...
  const rows: Row[] = [
    { l: "Require approval for all orders", d: "Recommended. autonomous_ never routes without your explicit go.", on: true, locked: true },
  ];
  ...
  return (
    <div className="px-8 py-7 space-y-6 max-w-[820px]">
      ...
      {/* ─── Risk / AI-transparency notice (GTM-1 T2 #1465) ───────────── */}
      <div className="surface p-5" style={{ borderColor: "rgba(255,69,58,0.22)" }}>
        <div className="text-[12px] text-white/80 leading-relaxed">
          <strong className="text-[#ff8a80]">
            Autonomous AI · not investment advice · capital at risk.
          </strong>{" "}
          autonomous_ makes trading decisions autonomously. Live trading can lose real money; past
          performance is no guarantee of future results. You are solely responsible for your trades.
          See the{" "}
      ...
      {/* ─── Broker API keys — separate paper + live slots (desktop · #1402/#1425) ────── */}
      <BrokerKeysCard mode="paper" />
      <BrokerKeysCard mode="live" />
      <LiveTradingSwitchCard />

      {/* ─── LLM · Gemini API key (desktop · #1705) ──────────────────────── */}
      <LlmKeysCard />
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="text-[12px] text-white/55 mt-1 max-w-md">
              Halts the engine immediately — no new orders are placed. Open positions are left untouched; you decide whether to liquidate. If a risk limit has tripped the kill switch, <span className="text-white/70">Reset</span> clears it and resumes trading.
            </div>
          ...
          <div className="flex flex-col gap-2 shrink-0">
            ...
            <button className="btn" onClick={() => void handleResetKill()} disabled={killResetting} style={{ opacity: killResetting ? 0.5 : 1 }}>
      ...
      <div className="surface p-6">
        ...
        <div className="text-[11px] text-white/40 mt-4">
          <a href="https://autonomous-trading.de" target="_blank" rel="noreferrer" className="text-[#7ce7b3] underline">
            autonomous-trading.de
          </a>
          {" · "}
          <a href="/legal/imprint" className="text-[#7ce7b3] underline">Legal &amp; imprint</a>
