    ...
    <div className="surface p-6" data-testid="engine-card" style={status === "error" ? { borderColor: "rgba(255,69,58,0.22)" } : {}}>
      ...
      {detailVisible && (
        <div
          className={
            status === "error"
              ? "flex items-start gap-2 px-4 py-3 rounded-lg border mb-4"
              : "surface-flat px-4 py-3 flex items-start gap-2 mb-4"
          }
          style={status === "error" ? { background: "rgba(255,69,58,0.10)", borderColor: "rgba(255,69,58,0.25)" } : undefined}
        >
          <span
            className={`text-[12px] leading-relaxed break-all${status === "error" ? "" : " text-white/70"}`}
            style={status === "error" ? { color: "rgba(255,122,114,0.9)" } : undefined}
          >
            {detail}
          </span>
        </div>
      ...
      <div className="flex gap-2 mb-4">
        <button className="btn" onClick={() => void start()} disabled={isStartDisabled}>
          Start engine
        </button>
        <button className="btn" onClick={() => void stop()} disabled={isStopDisabled}>
          Stop engine
        </button>
      </div>
...
function DiagnosticsCard() {
  ...
  return (
    <div className="surface p-6">
      <div className="eyebrow mb-2">Diagnostics</div>
...
export function Settings() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (typeof window !== "undefined" && window.location.hash) {
      ...
      if (
        ["trading", "trading-agents", "system", "notifications", "about", "legal"].includes(hash)
      ) {
        return hash;
      }
    }
  ...
  return (
    <div className="px-8 py-7 space-y-6 max-w-[1100px]">
      ...
      {/* Variante E (owner 03.09.): General is ITS OWN page — the Agents page lives
          under #trading-agents via the sidebar sub-navigation (no in-page switcher). */}
      ...
      {/* ─── Trading controls (#2863): individual neutral engine controls ── */}
      <TradingControlsCard />

      {/* #3071: Performance-Basis — neutral reporting setting (engine-persisted); the
          Overview card hint switches to "since <date>" while a baseline is set. */}
      ...
      {/* Emergency Safety Control removed from the Trading block (#2863): the Kill-Switch
          ...
          {activeTab === "trading-agents" && (
            <>
              {/* UXC-1 S4 (#3157): round-table agent settings — rows/roles/bounds from
                  the S2 engine GET; edits stay pending until the S3 WORM-audited apply. */}
              <RoundTableCard />
            </>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="surface p-6">
                ...
                <div className="text-[11px] text-white/40 mt-4">
                  <a href="https://autonomous-trading.de" target="_blank" rel="noreferrer" className="text-white/80 underline">
                    autonomous-trading.de
                  </a>
                  {" · "}
                  <a href="/legal/imprint" className="text-white/80 underline">Legal &amp; imprint</a>
