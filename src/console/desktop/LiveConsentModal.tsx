...
import { restartEngine, isDesktop, getKeychainStatus, validateAlpaca, saveSecret, getSetupState } from "@/lib/desktopBridge";
...
export function LiveConsentModal() {
  ...
  // #3364: the keep/reset PROMPT was removed from the modal (owner decision — it sat below the
  // fold and added go-live friction). We still COMPUTE the deviations so a silent "keep" snapshot
  // of the non-default settings in effect can be written to the audit chain on go-live.
  ...
  async function toLive() {
    ...
    // #3364: the keep/reset prompt was removed (owner decision). Custom settings simply STAY
    // in effect (keep). We still write a SILENT snapshot of the non-default settings in effect
    // at go-live onto the Art-14/WORM chain — BEST-EFFORT: a failed snapshot must NOT block the
    // go-live. This is a secondary, non-decisional audit note; the authoritative go-live record
    // is liveEnable(ACK+waiver) below. No reset path, nothing persisted.
    ...
    if (deviations.length > 0) {
      try {
        ...
        // fail-open: proceed with the go-live; the primary WORM record is liveEnable.
        console.warn("live go-live: settings snapshot could not be recorded (proceeding).");
      }
    }
