...
export function DailyUpdatesCard() {
  ...
  return (
    <div className="surface p-6">
      ...
      {locked && (
        <div className="surface-flat px-5 py-4 mb-5 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="shrink-0">
            {/* #3177: emphasis via `.btn btn-primary` — solid green fill is reserved for
                critical/arming flows, and an upgrade CTA is not one. */}
            <button
        ...
        <div className="pb-3 space-y-3">
          <div className="flex items-center justify-between">
            ...
            {scheduleTimes.length < 3 && (
              <button onClick={addTime} className="btn-link shrink-0">
                + Add time
              </button>
            )}
          </div>
          ...
          <div className="space-y-3">
            <div className="flex flex-wrap gap-2">
              {PROVIDERS.map((p) => {
                ...
                return (
                  // #3177: mini choice cards share the selectable-card idiom — neutral border/bg
                  // steps, and the chosen chip carries a StatusDot instead of a colour fill.
                  <button
                    key={p.id}
                    ...
                    className={`px-3 py-2 rounded-[10px] border text-[12px] font-medium transition-all ${
                      active
                        ...
                        : "border-white/10 hover:border-white/20 bg-white/[0.03] text-white/60"
                    }`}
                  >
                    {p.label}
                    {active && (
                      <StatusDot tone="on" className="ml-2 !text-[11px]">
                        active
                      </StatusDot>
                    )}
                  </button>
