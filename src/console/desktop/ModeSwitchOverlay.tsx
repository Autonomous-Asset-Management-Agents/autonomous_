...
export function ModeSwitchOverlay() {
  ...
  // frozen line. Each phase is derived from the SAME real store signals the reveal gate above
  // watches, so the copy can never claim progress the switch hasn't actually made. The switch
  // walks these in order: engine restarts → reconcile → account flips → snapshot syncs → ready.
  const dir = toLive ? "live" : "paper";
  let statusText: string;
  if (arriving) {
    statusText = "your account is ready";
  } else if (engineServing !== true) {
    // The engine is down, re-booting so it comes back up in the target mode.
    statusText = toLive
      ? "arming live trading — restarting the engine…"
      : "returning to paper — restarting the engine…";
  } else if (switchReconciling === true) {
    // Engine serves again; it is reconciling the account's real positions & orders.
    statusText = "reconciling your open positions & orders…";
  } else if (paperTrading !== targetPaper) {
    // Serving + reconcile done, but /health still reports the old mode — the account is flipping.
    statusText = `switching your account to ${dir}…`;
  } else if (!dataFresh) {
    // Account is the target; the fresh snapshot (equity/positions/cash) is still landing.
    statusText = `syncing your ${dir} account…`;
  } else {
    // Everything settled — the last beat before the wordmark reveals.
    statusText = `finalising your ${dir} account…`;
  }
