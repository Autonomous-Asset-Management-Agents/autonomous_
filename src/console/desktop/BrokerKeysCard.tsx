...
export function BrokerKeysCard({
  ...
  const wrapperCls = embedded
    ...
    : `surface p-6 ${isLive ? "border border-[#ff5a52]/30" : ""}`;
    ...
    <div className={wrapperCls}>
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          {/* #3177: system state = StatusDot with white text, never coloured status text. */}
          {keysPresent && !locked && <StatusDot tone="on">Keys stored</StatusDot>}
        </div>
      ...
      {saved && (
        <div className="flex items-center justify-between gap-3 text-[12px] text-white/70 bg-white/[0.05] border border-white/10 rounded-lg px-3 py-2 mb-3">
          ...
          <button className="btn shrink-0" onClick={() => void restart()} disabled={restarting}>
      ...
      <button className="btn" onClick={() => void submit()} disabled={submitDisabled}>
