...
const LOCAL: LocalOpt[] = [
  {
    ...
    model: "llama3.1:8b",
    blurb:
      "The model our research cards are tuned and validated on — recommended. Runs fully on your machine, private, offline, free. ~4.6 GB one-time download.",
  },
  {
    id: "phi",
    name: "Phi-3.5 (light)",
  ...
  },
  {
    id: "mistral",
    name: "Mistral 7B",
    blurb: "Alternative local model. Private, offline, free. ~4.4 GB one-time download.",
  },
];
...
// keychain. Purely visual — never the real value; an empty field on submit keeps the stored key
// (applyCloud early-returns on a blank key). Matches BrokerKeysCard's marker wording.
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-1.5">
        ...
        {installed === true && (
          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#00c27a]/10 text-[#00c27a]">
            {current ? "Installed & Active" : "Installed"}
          </span>
        )}
        {installed === false && (
          <span className="px-1.5 py-0.5 rounded text-[10px] font-medium bg-[#f0b429]/10 text-[#f0b429]">
            Not Installed
          </span>
        )}
        {selected && installed !== false && <StatusDot tone="on" className="ml-auto !text-[11px]">{current ? "in use" : "selected"}</StatusDot>}
      </div>
  ...
  return (
    <div className={bare ? "" : "surface p-6"}>
      ...
      {currentLocalMissing && (
        ...
        >
          <span aria-hidden className="shrink-0 text-[13px] leading-none mt-0.5 text-[#ffcc66]">
            ⚠
          </span>
          <span className="text-[12px] text-white/80 leading-relaxed">
            <span className="font-semibold text-[#ffcc66]">{currentName}</span> is selected but{" "}
      ...
      {/* On your machine */}
      <div className="text-[10px] text-white/30 uppercase tracking-wider mb-2">On-Device Local Models — private</div>
            ...
            {selected === o.id && (
              <div className="mt-4 pt-4 border-t border-white/10">
                ...
                <div className="mt-3 flex items-center gap-3">
                  <SolidButton onClick={() => void applyCloud(o)} disabled={busy || !key.trim()}>
