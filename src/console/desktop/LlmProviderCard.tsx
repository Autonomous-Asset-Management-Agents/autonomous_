 ...
 * `p-4 rounded-xl border` selectable cards (active = StatusDot + neutral pills, #3177), the same
 * info-box idiom, a lightbulb explanation. `.btn btn-primary` CTA — solid green fill is reserved
 * for critical/arming flows (#3177).
 *
...
// B10 (0.5.1 release check, mac): when Ollama is missing, the shell's own message already names the
// download page ("Download it from ollama.com/download/mac, install it, then retry." on darwin, the
// generic page on win32). The card used to print that AND its own "Install it from
// ollama.com/download, then retry." underneath — the same instruction twice, the second one on the
// wrong page. Now the page named in the shell text is linkified in place, and the generic line
// appears ONLY when the shell text names no page. The link is a plain target="_blank" anchor: the
// shell's setWindowOpenHandler routes every external http(s) window.open to shell.openExternal, so
// no openExternal bridge call exists or is needed (UpdateCard uses the same idiom).
const OLLAMA_DOWNLOAD_FALLBACK = "https://ollama.com/download";
/** The bare page mention as the shell writes it into its message: ollama.com/download[/mac|/windows]. */
const OLLAMA_DOWNLOAD_MENTION_RE = /ollama\.com\/download(?:\/[a-z]+)?/i;
/** The only hrefs accepted from the IPC payload — never an arbitrary URL from a result object. */
const OLLAMA_DOWNLOAD_HREF_RE = /^https:\/\/ollama\.com\/download(?:\/[a-z]+)?$/i;

/** Link target for the manual-install hint: the shell's platform URL when it is an ollama.com download
 *  page, else the page mentioned in the shell text (older shells), else the generic page. */
function resolveOllamaDownloadHref(fromShell: unknown, errorText: unknown): string {
  if (typeof fromShell === "string" && OLLAMA_DOWNLOAD_HREF_RE.test(fromShell)) return fromShell;
  const m = typeof errorText === "string" ? OLLAMA_DOWNLOAD_MENTION_RE.exec(errorText) : null;
  return m ? `https://${m[0]}` : OLLAMA_DOWNLOAD_FALLBACK;
}

/** The shell's error text with its ollama.com/download[/os] mention (if any) rendered as a link. */
function OllamaErrorText({ text, href }: { text: string; href: string }) {
  const m = OLLAMA_DOWNLOAD_MENTION_RE.exec(text);
  if (!m) return <>{text}</>;
  const start = m.index;
...
// Emphasis CTA — the shared `.btn` family carries geometry/colour (#3177: solid green fill is
// reserved for critical/arming flows; this is a standard action).
...
function ProviderCard({
  ...
  return (
    ...
    >
      ...
      {/* Badge row. Only the neutral `pill` primitive from console.css is used — badges here are
          categories, never state (#3177: state→dot, category→pill; the active/in-use state lives
          in the StatusDot on the title row). No hand-rolled hex tints, and no amber/gold anywhere:
          that is the operator's design decision, already recorded in HitlPolicyCard ("no gold/amber;
          red for the high-autonomy state"). "Not Installed" is a state, not a fault, so it stays
          neutral; the urgency for a selected-but-missing model lives in the notice above the grid,
          not in a colour. `Recommended` is short by design — the "for your machine" qualifier
          belongs to the section heading, which states it once with the measured RAM. */}
      {(recommended || installed !== undefined) && (
        <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
          ...
          {installed === true && <span className="pill">Installed</span>}
...
export function LlmProviderCard({
  ...
  // darwin (0.5.2): the shell installs Ollama itself before the model pull and streams that phase
  // as { stage: "install-ollama" } on the same channel. True while that phase is running, so the
  // bar reads "Installing Ollama… N%" instead of "Downloading… N%". Windows never sends the stage.
  const [installingOllama, setInstallingOllama] = useState(false);
  const [saved, setSaved] = useState(false);
  const [restarting, setRestarting] = useState(false);
  // B11: the shell restarted the running engine itself during the local pull (the pulled model
  // differed from the one the engine booted with) → say so instead of offering a second restart.
  const [shellRestarted, setShellRestarted] = useState(false);
  const [error, setError] = useState<string | null>(null);
  // Manual-install recovery: when auto-install of the local runtime fails, surface a clickable link.
  // The value is the resolved download page (see resolveOllamaDownloadHref); null = no install hint.
  const [ollamaDownloadHref, setOllamaDownloadHref] = useState<string | null>(null);
  ...
  function pick(id: string) {
    ...
    setError(null);
    setOllamaDownloadHref(null);
    setShellRestarted(false);
  }
  ...
  async function applyLocal(o: LocalOpt) {
    ...
    setInstallingOllama(false);
    try {
      const res = await deps.provisionOllama((p) => {
        setInstallingOllama(p.stage === "install-ollama");
        setProgress(p.percent ?? 0);
      }, o.model);
      setInstallingOllama(false);
      // B10: on either failure the bar must not linger at "Downloading… 0 %" — nothing is downloading.
      if (res.needsManual) {
        setError(res.error || "Couldn't auto-install the local AI runtime. Install Ollama, then retry.");
        setOllamaDownloadHref(resolveOllamaDownloadHref(res.downloadUrl, res.error));
        setProgress(null);
        return;
      }
      if (!res.ok) {
      ...
      setProgress(100);
      setSaved(true);
      setShellRestarted(res.restarted === true);
      onApplied?.(o.id);
  ...
  return (
    <div className={bare ? "" : "surface p-6"}>
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        {/* #3177: always the neutral strong pill — "Current: …" is a category label, not a state. */}
          ...
          >
            {selected === o.id && (
              <div className="mt-4 pt-4 border-t border-white/10">
                  ...
                  <div className="mb-3">
                    ...
                    <div className="text-[10.5px] text-white/45 num mt-1.5">
                      {saved && progress === 100
                        ...
                        : installingOllama
                          ? `Installing Ollama… ${progress}%`
                          : `Downloading… ${progress}% (first run only)`}
                    </div>
                  </div>
                )}
                ...
                {error && (
                  <div className="text-[11px] text-bear mt-2">
                    {ollamaDownloadHref ? <OllamaErrorText text={error} href={ollamaDownloadHref} /> : error}
                  </div>
                )}
                {/* B10: the generic line only when the shell's text names no download page itself —
                    otherwise the linkified shell text above already IS the instruction. */}
                {ollamaDownloadHref && !(error && OLLAMA_DOWNLOAD_MENTION_RE.test(error)) && (
                  <p className="text-[11px] text-white/55 mt-1">
                    Install it from{" "}
                    <a href={ollamaDownloadHref} target="_blank" rel="noreferrer" className="btn-link">
                      {ollamaDownloadHref.replace(/^https:\/\//, "")}
                    </a>
                    , then retry.
                  </p>
                )}
                {saved &&
                  (shellRestarted ? (
                    // B11: the shell already restarted the running engine onto the pulled model.
                    <div className="text-[11px] text-white/55 mt-2">Saved. Engine restarted with the new model.</div>
                  ) : showRestart ? (
                    <div className="text-[11px] text-white/55 mt-2">
                      Saved.{" "}
                      <button className="btn-link" onClick={() => void restart()} disabled={restarting}>
      ...
      {/* Cloud providers */}
      <div className="sublabel mb-2">Cloud Provider Models — bring your own key</div>
