...
/** One selectable stop-mode card (radio semantics). Unselected cards are borderless (design A3). */
function OptionCard({
  title,
  note,
  active,
  ...
  return (
    <button
      ...
      disabled={disabled}
      onClick={onSelect}
      className={`w-full text-left rounded-lg px-4 py-3 border transition-all flex items-start gap-3 disabled:opacity-60 disabled:cursor-not-allowed ${
        active ? "border-white/25 bg-white/[0.06]" : "border-transparent bg-white/[0.02] hover:bg-white/[0.05]"
      }`}
    >
      <span
        className={`mt-0.5 w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${
          active ? "bg-white text-black" : "border border-white/30"
        }`}
      >
...
/**
 * EmergencyStopModal — the shared "how do you want to stop?" confirm, used by BOTH the
 * Sidebar kill control and Settings › Trading (SafetyControlCard) so the two emergency
 * surfaces stay identical (previously two drifting inline copies).
 *
...
export function EmergencyStopModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="surface max-w-md w-full p-6 space-y-4 rounded-xl border border-white/10 relative">
        ...
        <button
          ...
          disabled={selling}
          onClick={confirm}
          className={`w-full rounded-full px-5 py-2.5 text-[13px] font-bold text-white transition-all transform active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2 ${
            mode === "sell" ? "bg-[#c0261f] hover:bg-[#d62d25]" : "bg-[#ff5a52] hover:bg-[#ff6c65]"
          }`}
        >
          {selling && (
        ...
        <p className="text-[11.5px] text-white/45 bg-white/[0.03] border border-white/10 p-2.5 rounded">
        ...
        <div className="flex justify-end pt-1">
          <button
            type="button"
            disabled={selling}
            className="btn rounded-full px-6 py-2.5 text-[13px] font-bold tracking-wide disabled:opacity-50"
            onClick={dismiss}
          >
            Cancel
          </button>
