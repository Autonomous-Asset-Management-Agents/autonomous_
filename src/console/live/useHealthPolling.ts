...
export function useHealthPolling(): void {
  ...
  const setStartupBlockedReason = useStore((s) => s.setStartupBlockedReason);
  useEffect(() => {
    let alive = true;
    ...
    const loop = async () => {
      ...
      try {
        ...
        if (alive && h) {
          ...
          // B11: WHY the loop is idle (the sidebar hint). Absent on older engines → null.
          setStartupBlockedReason(h.startup_blocked_reason ?? null);
        }
  ...
  }, [setStrategyRunning, setSystemHalted, setPaperTrading, setEngineServing, setStartupBlockedReason]);
}
