  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      try {
        ...
        if (alive && resp) {
          const view = adaptPortfolio(resp);
          setPortfolio(view);
          // Bug B (0.3.5): tag the freshness marker with the account this snapshot belongs to,
          // so the mode-switch overlay only reveals on TARGET-account data (not a stale cross-
          // account in-flight poll).
          markSynced(view.paperTrading);
        }
      } finally {
