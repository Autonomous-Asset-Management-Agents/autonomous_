...
export function useEntitlementPolling(): { refetch: () => Promise<void> } {
  ...
  const setPaywallEnabled = useStore((s) => s.setPaywallEnabled);
  const setCheckoutUrl = useStore((s) => s.setCheckoutUrl);
  const setPriceDisplay = useStore((s) => s.setPriceDisplay);
  ...
  const refetch = useCallback(async () => {
    ...
    if (s) {
      ...
      setCheckoutUrl(s.checkout_url ?? null);
      setPriceDisplay(s.price_display ?? null);
    }
  }, [setEntitlement, setPaywallEnabled, setCheckoutUrl, setPriceDisplay]);
