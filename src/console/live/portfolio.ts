...
export interface ConsolePosition {
  ...
  weight: number;
  heldDays: number;
  /** #3421: today's price move vs. yesterday's close (percent), or null when the
   *  broker didn't serve it (offline / older engine) — the row then shows just the
   *  price, never a fabricated 0%. Absolute $ move is derived from marketValue. */
...
export function adaptPortfolio(resp: PortfolioSummaryResponse | null | undefined): PortfolioView {
  ...
  const positions: ConsolePosition[] = raw.map((p) => {
    ...
    return {
      ...
      // #3421: null (not 0) when absent — the UI shows only the price then.
      changeTodayPct:
        typeof p.change_today_pct === "number" ? p.change_today_pct : null,
    };
  });
