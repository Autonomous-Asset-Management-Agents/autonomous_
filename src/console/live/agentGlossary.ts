...
const GLOSSARY: Record<string, AgentInfo> = {
  ...
  SkewSizeTilt: {
    ...
    role: "Volatility-skew input to position sizing — the 25-delta options risk-reversal, alongside VIX. A deterministic sizing input, not a directional vote. Ships dark — off by default.",
    tasks: "Turns the symbol's option risk-reversal into a bounded size factor.",
    criteria: "A bullish upside skew sizes the position up, a downside skew sizes it down; the cap bounds how far, up or down.",
  },
  LSTMSignalAgent: {
    label: "LSTM Model",
  ...
  UpsideSkewAgent: {
    label: "Upside Skew",
    role: "Reads the options market's asymmetry — the 25-delta risk reversal (call vs. put implied vol) across the cross-section. A directional vote (#3095). Ships dark — flag off by default.",
    tasks: "Ranks each symbol by how bullish its option risk-reversal is versus its peers.",
    criteria: "A bullish upside skew raises the vote, a downside skew lowers it. The same risk-reversal can separately drive an optional position-sizing tilt (#3199) — a sizing input, not a vote.",
  },
  TrendAgent: {
    label: "Trend (MACD)",
    role: "Directional voter — the MACD trend follower. Ships dark — off by default.",
    tasks: "Reads the MACD histogram from the shared feature panel.",
    criteria: "A rising, positive histogram leans bullish; a falling, negative one bearish.",
  },
  VolumeConfirmationAgent: {
    label: "Volume Confirmation",
    role: "Directional voter — volume confirmation. Ships dark — off by default.",
    tasks: "Reads the 5-day versus 20-day average volume ratio.",
    criteria: "Above-average volume confirms the prevailing direction; thin volume abstains toward neutral.",
  },
  FundamentalsAgent: {
    label: "Fundamentals",
    role: "Reads the business: growth and profitability from point-in-time SEC EDGAR filings.",
  ...
  ValuationAgent: {
    label: "Valuation",
    role: "Reads the price: trailing PEG and leverage from the same SEC EDGAR filings.",
  ...
  QualityAgent: {
    label: "Quality",
    role: "Reads how sound the business is, from the same point-in-time SEC EDGAR filings. A directional voter that ships dark — off by default.",
    tasks: "Blends gross profitability, return on assets, accrual quality and single-period financial-strength points into one composite, leaving margin and leverage to the other two fundamentals voices.",
    criteria: "A more profitable, cash-backed and solid business scores above neutral, a weaker one below; with fewer than two measures available it abstains.",
  },
};
