  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      try {
        ...
        // #2822: drop writes during a mode switch (see useOpenOrdersPolling).
        if (alive && resp && useStore.getState().modeSwitch == null) setActivities(adaptActivities(resp), resp.truncated);
      } finally {
