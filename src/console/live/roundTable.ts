...
export function adaptRoundTableDecisions(
  ...
  return rows.map((d) => {
    ...
    return {
      ...
      conviction: clamp(
        (typeof d.consensus_score === "number" ? d.consensus_score : 0.5) * 2 -
          1,
        -1,
        1,
      ),
      consensusScore: clamp(
        typeof d.consensus_score === "number" ? d.consensus_score : 0.5,
        0,
        1,
      ),
...
function weightedSums(arr: RoundTableSenator[]): {
  wSum: number;
  wTot: number;
  mean: number | null;
} {
...
export function partitionRoundTable(
  ...
  const isNonDir = (s: RoundTableSenator) =>
    NON_DIRECTIONAL_ROLES.has(s.role ?? "directional");
...
/**
 * Head-count of the DIRECTIONAL voters only (weight>0, not an overlay), by vote — the honest
 * tally that reconciles with the served consensus. Unlike the raw `votesFor/Abstain/Against`
 * on the decision (a full-board count that mixes in overlays and dark abstains), this is what
 * the vote bar beside the verdict should show (#3354).
 */
export function directionTally(d: ConsoleRoundTableDecision): {
  votesFor: number;
  votesAbstain: number;
  votesAgainst: number;
} {
  const { direction } = partitionRoundTable(d.senators, d.consensusScore);
  return {
    votesFor: direction.filter((s) => s.vote === "BULL").length,
    votesAbstain: direction.filter((s) => s.vote === "ABSTAIN").length,
    votesAgainst: direction.filter((s) => s.vote === "BEAR").length,
  };
}
