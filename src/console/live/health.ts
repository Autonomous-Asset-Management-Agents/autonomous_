  ...
  useEffect(() => {
    ...
    const loop = async () => {
      ...
      try {
        ...
        if (alive) {
          setMarketHealth(
            deep ? !!deep.is_market_open : null,
            brokerLabel(deep),
            // #2743/S3: carry the degraded flag so the sidebar's run-status is honest. null when the
            // engine is unreachable (unknown), not false — never claim "healthy" on a failed poll.
            deep ? !!deep.components_degraded : null,
          );
        }
      } catch {
