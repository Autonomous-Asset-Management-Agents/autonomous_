...
export function useEquityPolling(): void {
  ...
  useEffect(() => {
    ...
    // poll below silently overwrites it. EXCEPT during a Paper↔Live switch: the
    // switch just cleared the store+cache, so seeding a stale curve here would
    // repaint the OLD account (the ~-99% paper-baseline leak on the live page).
    const cached = readEquityCache();
    if (cached && useStore.getState().modeSwitch == null) setEquity(cached);
    let alive = true;
      ...
      try {
        ...
        if (alive && resp) {
          if (resp.message && DEGRADED_MESSAGES.has(resp.message)) {
            ...
            // Not while a switch is mid-flight: this could be the OLD account's in-flight poll,
            // which would leak its curve/baseline onto the freshly-switched page on reveal.
            if (useStore.getState().modeSwitch == null) {
              setEquity(view);
