...
interface ConsoleState {
  ...
  // #3071: user-chosen performance baseline (null = true inception) — drives the honest
  // "since <date>" card hint; inceptionDate bounds the Settings picker.
  ...
  // B11: /health `startup_blocked_reason`: WHY the trading loop is idle ("llm_unreachable" while
  // the engine waits for the local LLM, "redis_unreachable" after a hard abort), null when the loop
  // runs or the engine has not said. Drives the sidebar hint next to "Idle".
  startupBlockedReason: string | null;
  setStartupBlockedReason: (reason: string | null) => void;
  ...
  // GTM-2 (#1809): whether the paid Private/Senior paywall is active on the engine
  // (paywall_enabled from /api/entitlement/status). null until the first poll → the
  // desktop cards fail-safe to the current free-beta claim path. When true, a Junior
  // desktop is offered the paid checkout + license-key activation instead.
  paywallEnabled: boolean | null;
  setPaywallEnabled: (v: boolean | null) => void;
  // GTM-2 (#1809) T5: the Lemon Squeezy hosted checkout URL for "Get Senior" (from
  // /api/entitlement/status). null until provisioned → the button falls back to key activation.
  checkoutUrl: string | null;
  setCheckoutUrl: (v: string | null) => void;
  // GTM-2 (#1809): Finance-controlled display price for the paid Senior card (from
  // /api/entitlement/status). null until polled → the card shows a placeholder.
  priceDisplay: string | null;
  setPriceDisplay: (v: string | null) => void;
...
export const useStore = create<ConsoleState>((set) => ({
  ...
  setStartupBlockedReason: (reason) => set({ startupBlockedReason: reason }),
  ...
  paywallEnabled: null,
  setPaywallEnabled: (v) => set({ paywallEnabled: v }),
  checkoutUrl: null,
  setCheckoutUrl: (v) => set({ checkoutUrl: v }),
  priceDisplay: null,
  setPriceDisplay: (v) => set({ priceDisplay: v }),
