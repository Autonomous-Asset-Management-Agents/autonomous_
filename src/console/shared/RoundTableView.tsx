...
// #2067: the DrawdownGuard "VETO" badge read like a hard block. It only guards
// against buying into a falling name — it blocks new BUYs / freezes HOLD, but never
// a risk-reducing SELL (direction-aware, #2038). Explain that, and render the cryptic
// "peak-to-trough … drawdown=X%" reasoning as a plain sentence (raw string stays in the audit).
const VETO_TOOLTIP =
  "DrawdownGuard veto: the stock is >7% below its 30-day high. It blocks new BUYs / freezes HOLD — it does NOT block a risk-reducing SELL.";

function displayReasoning(s: RoundTableSenator): string {
  if (s.hardVeto) {
    const m = /drawdown=([\d.]+)\s*%/i.exec(s.reasoning);
    if (m) {
      const pct = Math.round(parseFloat(m[1]));
      return `Stock ${pct}% below its 30-day high — blocks new buys, not a risk-reducing sell.`;
    }
  }
  // #2134: a plain-English per-agent summary; fall back to the de-prefixed raw string.
  return humanizeReasoning(s.name, s.reasoning) ?? stripAgentPrefix(s.reasoning);
}
  ...
  return (
    <div className="text-[11.5px]">
      <div className="flex items-baseline gap-2">
        ...
        {s.hardVeto && (
          <span
            className="text-bear text-[9px] shrink-0 cursor-help"
            title={VETO_TOOLTIP}
            aria-label={VETO_TOOLTIP}
          >
            VETO
          </span>
        )}
        <span className="text-white/40 num leading-snug truncate min-w-0 flex-1">{displayReasoning(s)}</span>
