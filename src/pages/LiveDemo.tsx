...
import {
  PositionsTable,
  type PositionRow,
} from "@/console/shared/PositionsTable";
...
const toBenchSeries = (curve: DemoEquityPoint[]): CurvePt[] =>
  curve
    ...
    .map((p) => ({ t: new Date(p.date), eur: p.benchmark as number }));

// #3321 C1 — since-inception KPI math for the shared KpiBand, computed from the snapshot curve
// (never fabricated; nulls where the curve is too thin). Honest, standard definitions.
const benchmarkSinceInception = (curve: DemoEquityPoint[]): number | null => {
  const b = curve
    .filter((p) => p.benchmark != null)
    .map((p) => p.benchmark as number);
  return b.length >= 2 && b[0] > 0
    ? ((b[b.length - 1] - b[0]) / b[0]) * 100
    : null;
};
const maxDrawdownPct = (curve: DemoEquityPoint[]): number => {
  let peak = -Infinity;
  let mdd = 0;
...
const annualizedSharpe = (curve: DemoEquityPoint[]): number | null => {
  const eq = curve.map((p) => p.equity).filter((e) => e > 0);
  if (eq.length < 3) return null;
  const rets: number[] = [];
  for (let i = 1; i < eq.length; i++)
    rets.push((eq[i] - eq[i - 1]) / eq[i - 1]);
...
function demoOutcomeBadge(
  ...
  switch (code) {
    ...
    case "blocked:market_closed":
      return { label: "Blocked · market closed", cls: "pill-bear" };
    case "hitl_held":
...
// The overview P/L (hero line) follows the timeframe toggle instead of always "today".
...
function DashboardBody({
  ...
  const totalReturnPct =
    ((s.equity - INCEPTION_CAPITAL) / INCEPTION_CAPITAL) * 100;
  ...
  const exposurePct = s.equity
    ...
    : null;
  const investedEUR = s.positions.reduce((a, p) => a + p.market_value, 0);

  // #3321 C1: snapshot → shared-component props (the demo is read-only — plain symbols, no nav).
  const demoPositions: PositionRow[] = s.positions.map((p) => {
    const base = p.market_value / (1 + p.unrealized_pl_pct / 100); // cost basis
    return {
      symbol: p.symbol,
      name: getCompanyName(p.symbol) ?? undefined,
      avgEntry: base,
      last: p.market_value,
      marketValue: p.market_value,
      unrealizedPct: p.unrealized_pl_pct,
    };
  });
  // The Round Table is a compact teaser (top calls by consensus), not the full list.
  const topDecisions = [...s.decisions]
  ...
  return (
    <main className="aaa-console">
      <div className="mx-auto max-w-5xl px-6 sm:px-8 py-7 space-y-7">
        ...
        <div className="-mx-6 sm:-mx-8 border-y border-white/15 bg-white/[0.06] px-6 sm:px-8 py-3 text-xs leading-relaxed text-white/80">
          ⚠ {s.disclaimer}
        </div>
        ...
        <div className="flex items-end justify-between">
          <div>
            ...
            <p className="text-white/55 mt-2 text-[13px]">
              Autonomous paper portfolio — {s.positions.length} open{" "}
              {s.positions.length === 1 ? "position" : "positions"}.
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span className="pill">Paper Trading</span>
            <span className={`pill ${paused ? "pill-warn" : "pill-bull"}`}>
              {paused ? "paused" : "live"}
            </span>
          </div>
        </div>
        ...
        <div className="surface halo-soft -mx-6 sm:-mx-8 px-6 sm:px-8 py-7">
          <div className="flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <div className="mt-2 flex items-center gap-3 text-[13px]">
                <span
                  className={`num ${plAbs >= 0 ? "text-bull" : "text-bear"}`}
                >
                  {fmtUSD(plAbs, { sign: true })}{" "}
                  <span className="text-white/40">
                    ({fmtPct(plPct)} {RANGE_PL_SUB[range]})
                  </span>
                </span>
              </div>
              <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1 text-[12px] text-white/55">
                <span>
                  Since inception{" "}
                  <span
                    className={`num ${totalReturnPct >= 0 ? "text-bull" : "text-bear"}`}
                  >
                ...
                <span>
                  ...
                  <span className="num text-white/80">
                    {benchToday != null ? fmtPct(benchToday) : "—"}
                  </span>
                </span>
                <span className="hairline-v" />
                <span>
          ...
          <div className="mt-5 equity-grid rounded-xl">
            <EquityChart
              ...
              height={240}
              glowColor="#00c27a"
              benchColor="rgba(255,255,255,0.55)"
              currency="$"
            />
          </div>
          ...
          <div className="mt-3 flex items-center gap-5 text-[11px] text-white/45">
            ...
            {benchView.length >= 2 && (
              <span className="inline-flex items-center gap-1.5">
                <span className="inline-block h-0.5 w-4 rounded border-t border-dashed border-white/40" />{" "}
                S&amp;P 500
              </span>
            )}
          </div>
        ...
        {/* #3321 C1: the SAME Balances + KPI bands as the console dashboard, fed by the snapshot. */}
        <BalancesBand
          cashEUR={s.cash}
          currentEquity={s.equity}
          investedEUR={investedEUR}
          investedPct={exposurePct ?? 0}
          positionsCount={s.positions.length}
          netDeposits={INCEPTION_CAPITAL}
          startEquity={INCEPTION_CAPITAL}
          iso="USD"
          money={fmtUSD}
        />
        <KpiBand
          totalReturnPct={totalReturnPct}
          benchInception={benchmarkSinceInception(s.equity_curve)}
          maxDD={maxDrawdownPct(s.equity_curve)}
          sharpe={annualizedSharpe(s.equity_curve)}
          sinceHint={`since ${INCEPTION_LABEL}`}
          sharpeHint="realised curve"
        />

        {/* #3321 C1: positions-first — the portfolio leads (shared PositionsTable), the Round Table
            is a compact teaser below. Read-only: plain symbols, no "View all" navigation.
            The demo has no "View all", so the list must show EVERY position — otherwise the
            "{N} open" header disagrees with a capped list. limit = the full count. */}
        <PositionsTable
          positions={demoPositions}
          money={fmtUSD}
          investedPct={exposurePct ?? 0}
          limit={demoPositions.length}
        />

        {/* Round Table — compact teaser (top calls by consensus), not the full 60-item list. */}
        <div className="surface p-5">
          <div className="flex items-center justify-between mb-3">
            <div className="eyebrow">Round Table · latest calls</div>
            <span className="num text-[11px] text-white/40">
              {s.decisions.length} this snapshot
            </span>
          </div>
          <div className="space-y-2">
            {topDecisions.map((d) => {
              ...
              return (
                <div
                  key={d.symbol}
                  className="flex items-center gap-3 p-2.5 rounded-lg bg-white/[0.025] border border-white/5"
                >
                  <span className="w-20 font-bold text-[13px] tracking-tight2 text-white/92 truncate">
                    {d.symbol}
                  </span>
                  <span className={`pill ${pillCls}`}>{act}</span>
                  {outcome && (
                    <span
                      className={`pill ${outcome.cls} text-[10px] whitespace-nowrap`}
                    >
                      {outcome.label}
                    </span>
                  )}
                  <span className="ml-auto pill pill-strong">
                    Consensus {(d.consensus * 100).toFixed(0)}%
                  </span>
                </div>
              );
            })}
            {s.decisions.length === 0 && (
              <div className="text-[12px] text-white/30 p-4 text-center">
                No decisions in the current snapshot.
              </div>
            )}
          </div>
          {s.decisions.length > topDecisions.length && (
            <div className="text-[11px] text-white/35 pt-3">
              Showing the top {topDecisions.length} of {s.decisions.length}{" "}
              calls — the full round table runs in the app.
            </div>
          )}
        </div>
        ...
        <p className="pt-2 text-center text-[11px] text-white/30">
          Demo portfolio, fictitious · paper trading · no real orders ·
          autonomous_ trading solution
        </p>
      </div>
    </main>
  );
...
export function LiveDemo() {
  ...
  const view = useMemo(
    () => filterCurveByRange(series, range),
    [series, range],
  );
  const benchView = useMemo(
    () => filterCurveByRange(bench, range),
    [bench, range],
  );
  const period = useMemo(() => rangeReturn(series, range), [series, range]);

  const stamp = snapshot
    ? new Date(snapshot.generated_at).toLocaleTimeString("en-GB", {
        hour: "2-digit",
        minute: "2-digit",
      })
    : "—";
  ...
  return (
    <div className="min-h-screen bg-black text-slate-200">
      ...
      <SiteMarquee dark />
      {/* Public marketing header (shared) — dark variant. On the demo the top-right CTA is a
          white "Download" pill → the landing's Editions section (choose a variant), not "Live Demo". */}
      <SiteHeader dark cta="download" />
          ...
          {paused ? (
            <>
              ...
              <span className="text-xs text-slate-600">
                The agent is offline right now — back shortly.
              </span>
            </>
          ) : (
