...
interface LegalPageProps {
    kind: "imprint" | "privacy" | "risk-disclosure" | "notice" | "terms" | "inducements";
}
...
explicit commercial licensing terms and separate enterprise software licensing agreements.
...
========================================================================`,
    ...
    terms: {
        ...
        body: `# Terms of Service (ToS) of the AAAgents Trading Software

**Legal Entity:** Autonomous Asset Management Agents UG (haftungsbeschränkt), Germany
**Scope of Application:** Use of the AAAgents Software (Beta Version)

## § 1 Subject Matter and Beta Status
1. Autonomous Asset Management Agents UG (haftungsbeschränkt) (hereinafter "Provider") provides the user with a software application that acts strictly as a technical interface (Local Execution Engine) between the user's terminal device and third-party brokers connected by the user themselves. There is no central order routing by the Provider, and the Provider does not receive any Payment for Order Flow (PFOF) from the connected brokers.
2. The software is in the **beta stage**. It is provided strictly on an "as-is" basis. The user acknowledges that the software may contain untested features, bugs, or technical latencies that could lead to unexpected system behavior, delays, or incorrect order executions.

## § 2 Regulatory Status and Execution-Only
1. The Provider is a technology company and **does not provide any regulated financial services**. In particular, the Provider does not offer investment advice, investment brokerage, or financial portfolio management within the meaning of the German Banking Act (KWG), the Securities Institutions Act (WpIG), or the Securities Trading Act (WpHG). No discretionary management takes place.
2. The software acts exclusively as an "Execution-Only" tool. The Provider does not conduct any suitability or appropriateness assessments (Client Profiling under MiFID II). The final investment decision ("Human-in-the-loop") is made exclusively by the user under their own legal and financial responsibility.

## § 3 Decentralized Architecture, No Custody, and Data Protection
1. **Local Execution:** In the current phase, the execution of the software logic takes place completely decentralized and locally on the user's hardware.
2. **No Custody:** The Provider does not take custody or possession of any client funds, financial instruments, or crypto assets. The Provider never has access to broker accounts, wallets, or private API keys. The confidentiality and security of the access data are the sole responsibility of the user.
3. **Data Protection (No Phone-Home):** Since the software is executed locally, no personally identifiable financial or transaction data (PII) is transmitted to the Provider's servers. Accordingly, no processing of such data by the Provider within the meaning of the GDPR takes place.

## § 4 Limitation of Liability and Auto-Trading Risks
1. Trading financial instruments and crypto assets is highly speculative and carries the risk of a **total loss** of the invested capital. The use of automated trading functions involves additional system-related risks (e.g., latencies, network failures, faulty signals, slippage). Past performance is not a reliable indicator of future results.
2. The Provider is liable without limitation for damages resulting from injury to life, body, or health, as well as for damages caused by intent or gross negligence.
3. For slight negligence, the Provider is only liable for the breach of essential contractual obligations (cardinal obligations), the fulfillment of which is necessary for the proper execution of the contract. In this case, liability is limited to typical, foreseeable damages.
4. Any further liability of the Provider, in particular for indirect damages, lost profits, or pure financial losses resulting from technical failures of the beta software, faulty interface communication, or API disconnections, is excluded to the fullest extent permitted by law. Mandatory liability under the German Product Liability Act (ProdHaftG) remains unaffected by the aforementioned limitations of liability.

## § 5 Final Provisions
1. This agreement is governed exclusively by the laws of the Federal Republic of Germany, excluding the UN Convention on Contracts for the International Sale of Goods (CISG).
2. The exclusive place of jurisdiction for all disputes arising from or in connection with these Terms of Service is Mainz, Germany, provided the user is a merchant, a legal entity under public law, or a special fund under public law, or has no general place of jurisdiction in Germany.`,
    },
    inducements: {
        title: "Conflicts of Interest & Inducements",
        body: `# Disclosure of Conflicts of Interest and Inducements (WpHG / MiFID II)

To ensure the highest possible regulatory transparency, Autonomous Asset Management Agents UG (haftungsbeschränkt) provides information on the handling of inducements regarding broker interfaces:

**Strict Exclusion of Payment for Order Flow (PFOF)**
Autonomous Asset Management Agents UG (haftungsbeschränkt) acts purely as a technical interface provider. There is no central order routing to specific execution venues. The Provider expressly receives **no** transaction-based rebates, kickbacks, or so-called Payment for Order Flow (PFOF) for the transmission of client orders.

**Referral Remuneration (CPA/Affiliate)**
If interfaces to third-party brokers or crypto exchanges are provided within the software or on associated websites, Autonomous Asset Management Agents UG (haftungsbeschränkt) may receive a one-time Cost-Per-Action (CPA) payment or affiliate commission from the respective third-party provider for successful account openings (lead generation).

**Neutrality and Best Execution**
These potential inducements for new customer referrals have absolutely no technical or logical influence on local order transmission or execution quality (Best Execution). The decision as to which compatible broker you connect is solely yours. You will not incur any additional costs or disadvantages in your trading conditions as a result.`,
    },
};

export default function Legal({ kind }: LegalPageProps) {
    const navigate = useNavigate();
    // "Home" is context-aware: inside the desktop console, home = the console Overview (/console),
    // NOT the marketing landing (`/`, which in the desktop app resolves to the legacy dashboard views
    // — the "design dummies"). On the web/marketing site, home stays `/`.
    const goHome = () => {
        if (isDesktop()) {
            useStore.getState().setDesktopPage("overview");
            navigate("/console");
        } else {
            navigate("/");
        }
    ...
    return (
        <div className="landing-b-root">
            <nav className="lb-nav">
                <button className="lb-nav-logo" onClick={goHome} style={{ background: "none", border: "none", cursor: "pointer" }}>
                    aaagents<span style={{ color: "#00c27a" }}>_</span>
                </button>
                <div className="lb-nav-right">
                    <button className="lb-nav-link" onClick={goHome}>← Back to home</button>
                </div>
            </nav>
