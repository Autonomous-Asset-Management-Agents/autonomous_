...
import { getEngineKey } from "@/lib/desktopBridge";
...
export function useEngineWebSocket() {
    ...
    // C3/H4: the desktop engine now requires the engine key on the WS handshake (a browser
    // cannot send the X-Engine-Key header, so it travels as a query param). getEngineKey() returns
    // the key on desktop and null in the cloud/browser build (where the WS is unauthenticated).
    const _wsKey = getEngineKey();
    const wsUrl = API_BASE
        .replace(/^https/, "wss")
        .replace(/^http/, "ws")
        + "/ws/updates"
        + (_wsKey ? `?key=${encodeURIComponent(_wsKey)}` : "");
