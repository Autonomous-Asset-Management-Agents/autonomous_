...
interface AAAgentsBridge {
  ...
  validateAlpaca?: (keyId: string, secret: string) => Promise<{ ok: boolean; status: number }>;
...
export async function validateAlpaca(keyId: string, secret: string): Promise<AlpacaValidateResult> {
  const r = await bridge()?.validateAlpaca?.(keyId, secret);
