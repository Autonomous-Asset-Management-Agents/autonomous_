...
export interface Position {
  ...
  /** #3421: today's price move vs. yesterday's close, as PERCENT (Alpaca change_today
   *  ×100), or null/absent when the broker didn't serve it. Passthrough only. */
  change_today_pct?: number | null;
  /** #3421: the live quote (Alpaca current_price), or null/absent. */
...
export interface PortfolioSummaryResponse {
  ...
  positions?: Position[];
  equity?: number;
  /** Real broker settled cash (Alpaca account.cash), the authoritative Cash/margin figure — can be
   *  negative on a margin account (margin used). Absent on the error path / pre-0.4.4 engines, where
   *  the console falls back to the derived `equity − Σ market_value`. */
  cash?: number | null;
  last_equity?: number;
...
export async function fetchHealth(timeoutMs?: number): Promise<{
  ...
  /** B11: true once the engine's startup check verified the LLM provider. */
  llm_ready?: boolean;
  /** B11: why the trading loop is idle ("llm_unreachable" while it waits for the local LLM,
   *  "redis_unreachable" after a hard abort), null when it runs. Absent on older engines. */
  startup_blocked_reason?: string | null;
} | null> {
  try {
...
export interface EntitlementStatus {
  ...
  /** GTM-2 (#1809): whether the paid Private/Senior paywall is active (config
   *  PAYWALL_ENABLED). Absent on older engines → the console treats it as false and
   *  keeps the current free-beta claim path. */
  paywall_enabled?: boolean;
  /** GTM-2 (#1809) T5: the Lemon Squeezy hosted checkout URL for "Get Senior". null/absent
   *  until ops provisions it → the button falls back to the key-activation panel. */
  checkout_url?: string | null;
  /** GTM-2 (#1809): Finance-controlled display price for the paid Senior card (one config
   *  source, not hardcoded in the UI). null/absent → the card shows a placeholder. */
...
/** POST /api/entitlement/activate — GTM-2 (#1809) T4: activate a pasted license key
 *  OFFLINE on the local engine. The engine verifies the Ed25519 signature and, only on
 *  a pass, persists license.json so the next status poll resolves the tier. Returns the
 *  resolved tier on success, or `{ ok:false }` on any rejection (invalid/expired key,
 *  engine down, browser build) — the caller surfaces a visible note, never fails silent. */
export async function activateLicense(
  token: string,
): Promise<{ ok: true; tier: string } | { ok: false }> {
  try {
    const res = await fetchJson<{ status: string; tier: string }>(
...
export interface DeepHealth {
  ...
  strategy_running?: boolean;
  /** #2743/A2: true when a broker/model component is faulted. With `strategy_running` this lets the
   *  UI show an honest "degraded" instead of a misleading "running" (S3). The endpoint now answers
   *  HTTP 200 even when degraded, so this body field is readable (was a swallowed 500 before). */
  components_degraded?: boolean;
  components: {
...
export interface PortfolioShapeChange {
  key: string;
  from: string;
  to: string;
}

export interface PortfolioShapeResult {
  success: boolean;
  applied: PortfolioShape;
  changes: PortfolioShapeChange[];
  detail: string;
}

/** POST /api/portfolio-shape — record a deliberate portfolio-structure change on the same
 *  tamper-evident WORM chain as live enablement (#3132). Bounds are clamped SERVER-side, so a
 *  value outside the allowed range comes back corrected rather than rejected.
 *
 *  audit-before-change: the engine writes the record FIRST and only then reports success, so a
 *  throw here means nothing was recorded — and the caller must NOT persist. The engine cannot
 *  write setup.json itself (the Electron main process owns it), which is why persistence is the
 *  caller's step, conditional on this call succeeding. Throws on error. */
export async function savePortfolioShape(
  shape: Partial<PortfolioShape>,
  acknowledgment: string,
  nonce: string,
): Promise<PortfolioShapeResult> {
  return (await fetchJson("/api/portfolio-shape", {
    method: "POST",
    body: JSON.stringify({ ...shape, acknowledgment, nonce }),
  })) as PortfolioShapeResult;
}
...
export interface RoundTableVote {
  ...
  vetoed?: boolean;
  signal?: "BUY" | "SELL" | "HOLD" | string;
  /** #3084/#3094 audit role of this vote in the DIRECTION mean:
   *  "directional" (counted) · "conditioner"/"veto_guard" (base non-directional) ·
   *  "sizer" (VIXAware when IMPLIED_VOL_FORECAST_ENABLED redirects its IV to the
   *  size-scaler — excluded from the mean). Serialized by runner._serialize_votes
   *  from consensus_exclusions(); the console splits Direction vs Sizing on it. */
  role?: string;
}
...
// ── Reconciliation block (#3430) ─────────────────────────────────────────────
/** One mismatch between broker and engine — both sides as the engine recorded them. */
...
/** GET /api/reconciliation/block — the block and the finding that holds it. */
export interface ReconciliationBlock {
  available: boolean;
  entries_blocked: boolean;
  block_on_break: boolean;
  run_id: string | null;
  finished_at?: string | null;
  breaks: ReconciliationBreak[];
  last_run_id: string | null;
  last_run_clean: boolean | null;
}

export async function fetchReconciliationBlock(): Promise<ReconciliationBlock> {
  return fetchJson<ReconciliationBlock>("/api/reconciliation/block");
}

/** POST /api/reconciliation/release — a human lifts the block. The engine records who and when;
 *  the reason is optional and never replaces the author. */
export async function releaseReconciliationBlock(
  reason: string,
): Promise<ReconciliationBlock & { status: string; by: string }> {
  return fetchJson("/api/reconciliation/release", {
    method: "POST",
...
// ── HITL per-order approval queue (#2660, Epic #2667) ────────────────────────
// The engine's PR-0a-ii queue: orders the HITL gate HELD instead of executing. The console
// only reads + approves/rejects; execution stays with the engine's audited drain path
// (trading_loop._drain_hitl_approvals → execute_approved_order, Art. 14).

/** One order awaiting human approval (GET /api/hitl/pending item, engine DTO shape). */
...
/** Engine verdict for an approve/reject action — `success:false` means expired / already gone. */
...
/** GET /api/hitl/pending — every order currently awaiting human approval. */
export async function fetchHitlPending(): Promise<HitlPendingResponse> {
  return fetchJson<HitlPendingResponse>("/api/hitl/pending");
}

/** POST /api/hitl/approve — release one held order to the engine's audited execution drain. */
export async function approveHitlOrder(approvalId: string): Promise<HitlActionResponse> {
  return fetchJson<HitlActionResponse>("/api/hitl/approve", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
// ── Portfolio sector constraints (#2666, Epic #2655 Weg 1) ───────────────────
// The async EOD governance flow: the engine stages a RELATIVE sector-cap proposal
// (#2652) and the console reviews it read-only, then approves with the Ed25519
// token from the notification (#2653: expired -> 400 Token Expired).

export interface PendingConstraintsResponse {
  pending: {
...
/** GET /api/admin/portfolio-constraints/pending — read-only review data (never mutates). */
export async function fetchPendingConstraints(): Promise<PendingConstraintsResponse> {
  return fetchJson<PendingConstraintsResponse>("/api/admin/portfolio-constraints/pending");
}

/** POST /api/admin/portfolio-constraints/approve — four-eyes approve with the Ed25519 token. */
export async function approvePortfolioConstraints(
  token: string,
  approver: string,
): Promise<{ status: string; caps?: Record<string, number> }> {
  return fetchJson("/api/admin/portfolio-constraints/approve", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ token, approver }),
  });
}

/** POST /api/hitl/reject — remove one held order from the queue (audited, never silently dropped). */
export async function rejectHitlOrder(approvalId: string, reason = ""): Promise<HitlActionResponse> {
  return fetchJson<HitlActionResponse>("/api/hitl/reject", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export interface BenchmarkEquityPoint {
  date: string;
  equity: number;
  /** #1957: net external cash-flow that landed on this day (deposit +, withdrawal −), for markers. */
  cashflow?: number;
}
...
export interface BenchmarkEquityResponse {
  ...
  final_equity?: number;
  message?: string;
  // #1957: cash-flow-adjusted KPIs computed engine-side so a deposit/withdrawal never inflates
  // the return. twr_pct is the headline (time-weighted, deposit-neutral, SPY-comparable).
  twr_pct?: number;
  net_deposits?: number;
  pnl_net_of_deposits?: number;
  // #3071: user-chosen performance baseline (null = true inception). The served points/KPIs
  // are already re-anchored; these fields exist for honest labelling + dual reporting.
  baseline_date?: string | null;
  inception_date?: string | null;
  inception_twr_pct?: number | null;
}

/** GET /api/performance-baseline — the persisted baseline date (null = true inception). #3071 */
export async function getPerformanceBaseline(): Promise<{ baseline_date: string | null }> {
  return fetchJson<{ baseline_date: string | null }>("/api/performance-baseline");
}

/** POST /api/performance-baseline — persist (date) or clear (null) the baseline. #3071 */
export async function setPerformanceBaseline(
  baselineDate: string | null,
): Promise<{ baseline_date: string | null }> {
  return fetchJson<{ baseline_date: string | null }>("/api/performance-baseline", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
/**
 * UXC-1 S4 (#3157, Epic #3151) — round-table agent settings.
 * Served by the S2 endpoint (#3155) GET /api/trading-settings. The engine is the
 * single source of truth: row list, roles, bounds, defaults and EFFECTIVE values all
 * come from here — the UI never derives them (epic guardrail 4). Strict types by
 * audit action item: no loose Record shapes at this boundary.
 */
export type AgentRole = "mean_voter" | "veto_guard" | "conditioner" | "size_input";

export interface AgentSetting {
  name: string;
  /** Effective enable state in the running engine. */
  enabled: boolean;
  /** Shipped default for the enable flag (UpsideSkew ships dark). Owner decision
   *  2026-09-02: EVERY agent is toggleable, incl. the Drawdown Guard pre-trade veto. */
  default_enabled: boolean;
  /** Effective weight as resolved by the running engine (env seam already applied). */
  weight: number;
  default_weight: number;
  /** Slider bounds — per-agent min_weight/max_weight from base_agent; NEVER mirrored in the UI. */
  min_weight: number;
  max_weight: number;
  role: AgentRole;
  /** Industry role designation shown per row (e.g. "Fundamental Analyst") —
   *  server-authoritative like everything else on this boundary. */
  role_title: string;
  /** Engine config key of the enable flag; "" only if the agent has no gate. */
  enable_flag: string;
  /** Engine config key of the weight seam; "" for overlay rows (inert weights). */
  weight_key: string;
}

/** One registry key's bounds + shipped default from the S2 GET (client MIRROR only —
 *  the server clamps authoritatively, core/trading_settings.py). */
export interface TradingSettingMeta {
  key: string;
...
export interface TradingSettings {
  agents: AgentSetting[];
  /** IMPLIED_VOL_FORECAST_ENABLED — decides whether VIXAware is a size input or a mean voter. */
  implied_vol_forecast_enabled: boolean;
  /** Effective engine values per registry key (canonical strings, read at call time). */
  settings: Record<string, string>;
  /** Bounds + shipped defaults per key (S5 sliders, "Changed" badges, reset reference). */
  meta: TradingSettingMeta[];
  /** Read-only compliance guardrails — visible, never editable (e.g. HARD_STOP_LOSS_PCT). */
  guardrails: Record<string, string>;
}

export interface TradingSettingsChange {
  key: string;
  from: string;
  to: string;
}

export interface TradingSettingsApplyResult {
  success: boolean;
  /** Full server-clamped state (canonical strings). */
  applied: Record<string, string>;
  /** Only the actually changed keys — what the shell persists to setup.json. */
  changes: TradingSettingsChange[];
  detail: string;
}

/** POST /api/trading-settings — record a settings change on the WORM chain (S2, #3155).
 *  Audit-before-persist: the caller writes setup.json only after the 201. */
export async function postTradingSettings(
  settings: Record<string, string>,
  acknowledgment: string,
  nonce: string,
): Promise<TradingSettingsApplyResult> {
  return fetchJson<TradingSettingsApplyResult>("/api/trading-settings", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ settings, acknowledgment, nonce }),
  });
}

/** GET /api/trading-settings — engine-effective round-table configuration (S2, #3155). */
export async function getTradingSettings(): Promise<TradingSettings> {
  return fetchJson<TradingSettings>("/api/trading-settings");
}

/** GET /api/regime-preview — read-only market regime reading (#3361). Works while the
 *  throttle is off; a data gap reads as `available: false` / `factor: 1`. */
export interface RegimePreview {
  ...
  factor: number;
  enabled?: boolean;
  size_factor_setting?: number;
}

export async function getRegimePreview(): Promise<RegimePreview> {
  return fetchJson<RegimePreview>("/api/regime-preview");
}

/** UXC-1 S3 (#3156, Option A/Rev. 3): one deviation the operator decided about at the
 *  live switch — str-only fields (they land verbatim in the WORM record). */
export interface SettingsDeviationItem {
  key: string;
  current: string;
  default: string;
}

/** POST /api/settings-deviation-ack — records the operator's keep/reset decision on the
 *  Art-14 chain BEFORE live enablement; decision="reset" atomically appends the ack AND
 *  the regular old→new settings event server-side (Rev. 3 backend composition), the
 *  caller persists setup.json only after the 201. */
export async function postSettingsDeviationAck(payload: {
  decision: "keep" | "reset";
  deviations: SettingsDeviationItem[];
  nonce: string;
  switch_id?: string;
}): Promise<{ success: boolean }> {
  return fetchJson<{ success: boolean }>("/api/settings-deviation-ack", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export interface SpecialistReportDTO {
  ...
  // REPORTS-UX ⑥ (#2710): per-source freshness the engine already emits — true when
  // any source (filings/insider/events) is older than the cutoff. Absent → not stale.
  data_stale?: boolean;
}
...
/** GET /specialist-report/{symbol} response — the FULL, untruncated report. */
export interface SpecialistReportResponse {
  status: "ok" | "unavailable" | "error" | string;
  report: SpecialistReportDTO | null;
  message?: string;
}

/**
 * REPORTS-UX Wave 1 (#2710): the FULL, untruncated report for one symbol, fetched
 * lazily when a row is expanded. The bulk /specialist-reports list caps the note
 * body (payload); this per-symbol call serves it uncapped. Fail-soft → `{report:null}`.
 */
export async function fetchSpecialistReport(
  symbol: string,
): Promise<SpecialistReportResponse> {
  try {
    return await fetchJson<SpecialistReportResponse>(
      `/specialist-report/${encodeURIComponent(symbol)}`,
    );
  } catch {
