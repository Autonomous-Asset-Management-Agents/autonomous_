...
/** Build the GitHub release tag for a given desktop version.
 *
 *  #3302: this used to append a fixed `-beta`. Production releases are published as
 *  `desktop-v<X.Y.Z>` (see .github/workflows/desktop-release.yml and the `version.json` the update
 *  notifier reads, whose `download_url` names that exact tag). 0.4.10 happened to exist under BOTH
 *  tags, which hid the mismatch; 0.4.11 was published only as `desktop-v0.4.11`, and every public
 *  download link 404'd. Measured 10.09.: `desktop-v0.4.11-beta/autonomous_setup.exe` -> 404,
 *  `desktop-v0.4.11/autonomous_setup.exe` -> 200. */
export const desktopReleaseTag = (version: string): string => `desktop-v${version}`;
 ...
 *  #3302: no `-beta` suffix, for the same reason as the Windows tag above —
 *  `desktop-mac-v0.4.11-beta` -> 404, `desktop-mac-v0.4.11` -> 200. */
export const macDesktopReleaseTag = (version: string): string =>
  `desktop-mac-v${version}`;
