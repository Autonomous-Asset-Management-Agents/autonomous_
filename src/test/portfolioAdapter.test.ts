...
describe("adaptPortfolio", () => {
  ...
  it("derives cash as equity minus invested when the engine sends no cash field (legacy fallback)", () => {
    const out = adaptPortfolio(resp());
    expect(out.cashEUR).toBe(98_000); // 100000 - 2000
    expect(out.currentEquity).toBe(100_000);
  });

  it("uses the engine's real broker cash when present (authoritative over the derived value)", () => {
    // Real settled cash ≠ equity − invested (margin buying power, unsettled funds, etc.).
    const out = adaptPortfolio(resp({ cash: 12_345 }));
    expect(out.cashEUR).toBe(12_345); // real cash wins, NOT 98_000
  });

  it("passes a negative real cash straight through (margin used)", () => {
    const out = adaptPortfolio(resp({ cash: -4_000 }));
    expect(out.cashEUR).toBe(-4_000);
  });

  it("falls back to the derived cash when cash is null/non-finite (not masked to derived only when absent)", () => {
    expect(adaptPortfolio(resp({ cash: null })).cashEUR).toBe(98_000);
    expect(adaptPortfolio(resp({ cash: Number.NaN })).cashEUR).toBe(98_000);
  });
  ...
  it("resolves the company name from the bundled map, falling back to the symbol when unknown", () => {
    // Known ticker → full company name (SEC snapshot via getCompanyName).
    expect(adaptPortfolio(resp()).positions[0].name).toBe("Apple Inc.");
    // Unknown ticker → the symbol itself, so the row never shows a blank name.
    const unknown = adaptPortfolio(
      resp({ positions: [{ symbol: "ZZZZNOPE", qty: 1, market_value: 1, unrealized_pnl: 0, unrealized_pnl_pct: 0 }] }),
    ).positions[0];
    expect(unknown.name).toBe("ZZZZNOPE");
  });
