...
// Guard the DOM-globals install so this shared setup file is also safe under the `node` test
// environment (e.g. config/*.node.test.ts) where `window` is undefined. jsdom behaviour is unchanged.
...
// Global mock for react-i18next so component tests see ENGLISH copy without booting the
// full i18n runtime. Resolves keys generically against the real EN catalog (the previous
// per-key if-chain didn't scale past the header) and falls back to the key itself — a
// failing lookup is then visible in a test assertion instead of silently passing.
vi.mock('react-i18next', async () => {
  const en = (await import('@/locales/en/translation.json')).default as Record<string, unknown>;
