...
const dec = (
  over: Partial<ConsoleRoundTableDecision> = {},
...
describe("console Decisions page", () => {
  ...
  it("lists decisions collapsed, with an honest 'Autonom' source marker", () => {
    useStore.setState({
      roundTable: [
        dec({ symbol: "NVDA", action: "BUY" }),
        dec({ symbol: "TSLA", action: "SELL" }),
      ],
    });
    render(<Decisions />);
  ...
  it("the agent name is a link that reveals its role, tasks and criteria", () => {
    useStore.setState({
      roundTable: [
        dec({
          symbol: "MSFT",
          senators: [
            // weight>0 so the agent lands in the Direction breakdown (#3354 partitions by weight);
            // the click-to-reveal glass box (#569) is on the agent-label button either way.
            {
              name: "DrawdownGuardAgent",
              vote: "BULL",
              conviction: 0.92,
              weight: 0.5,
              reasoning: "DrawdownGuard: H=377 L=370 → score=0.92",
              hardVeto: false,
            },
          ],
        }),
    ...
    render(<Decisions />);
    // Wave 2 (#2732): the ticker itself is now a SymbolLink (→ report), so expand
    // the card via the card button, not the ticker text.
    fireEvent.click(screen.getByRole("button", { name: /MSFT/i })); // expand the decision card
    expect(screen.getByText(/score=0\.92/)).toBeTruthy(); // the agent's raw read is shown
    expect(screen.queryByText(/risk overlay/i)).toBeNull(); // profile hidden until the link is clicked

    fireEvent.click(screen.getByRole("button", { name: "Drawdown Guard" })); // the agent-name link
    expect(screen.getByText(/^role$/i)).toBeTruthy();
    expect(screen.getByText(/risk overlay/i)).toBeTruthy(); // role
    expect(screen.getByText(/from the recent high/i)).toBeTruthy(); // tasks
    expect(screen.getByText(/sharp sell-off can veto/i)).toBeTruthy(); // criteria
  });

  it("shows the HITL marker only when a decision carries source=hitl (future-ready)", () => {
    useStore.setState({
      roundTable: [dec({ symbol: "MSFT", action: "BUY", source: "hitl" })],
    });
    render(<Decisions />);
  ...
  it("shows a green Executed badge when the order reached the broker", () => {
    useStore.setState({
      roundTable: [
        dec({ symbol: "NVDA", action: "BUY", executionOutcome: "executed" }),
      ],
    });
    render(<Decisions />);
    ...
    useStore.setState({
      roundTable: [
        dec({
          symbol: "NVDA",
          action: "BUY",
          executionOutcome: "blocked:order_value",
        }),
      ],
    });
    render(<Decisions />);
  ...
  it("renders no execution badge for a decision without an outcome (e.g. HOLD)", () => {
    useStore.setState({
      roundTable: [dec({ symbol: "AAPL", action: "HOLD" })],
    });
    render(<Decisions />);
