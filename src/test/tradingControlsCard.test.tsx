...
/**
 * "Handels-Kontrollen" card (Settings → Trading). Two INDIVIDUAL, neutral engine
...
describe("Handels-Kontrollen card", () => {
  ...
  it("renders the two neutral controls with shipped defaults (sizing on → target shown)", async () => {
    ...
    expect(screen.getByText("Handels-Kontrollen")).toBeTruthy();
    const sw = screen.getByLabelText("Volatilitäts-Sizing") as HTMLElement;
    expect(sw.getAttribute("data-state")).toBe("checked"); // default ON
    expect((screen.getByLabelText("Vol-Tagesziel") as HTMLInputElement).value).toBe("0.015");
  });
  ...
  it("loads existing setup.json values (sizing off → target field hidden)", async () => {
    ...
    expect(screen.queryByLabelText("Vol-Tagesziel")).toBeNull();
  });

  it("toggling Volatilitäts-Sizing writes the flag to setup.json", async () => {
    render(<TradingControlsCard />);
    await waitFor(() => screen.getByLabelText("Volatilitäts-Sizing"));
    fireEvent.click(screen.getByLabelText("Volatilitäts-Sizing")); // ON -> OFF
    expect(saveMock()).toHaveBeenCalledWith({ VOL_TARGETING_SIZING_ENABLED: "false" });
    // and the target field disappears while off
    expect(screen.queryByLabelText("Vol-Tagesziel")).toBeNull();
  });

  it("a valid Vol-Tagesziel is persisted; an invalid one shows an error and writes nothing", async () => {
    render(<TradingControlsCard />);
    const input = await waitFor(() => screen.getByLabelText("Vol-Tagesziel"));
  ...
  it("Default Settings resets both controls to the shipped defaults", async () => {
    ...
    await waitFor(() => screen.getByLabelText("Vol-Tagesziel"));
  });
