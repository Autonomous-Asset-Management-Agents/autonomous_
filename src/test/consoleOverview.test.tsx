...
import { SafetyControlCard } from "../console/desktop/pages/Settings";
...
describe("console Overview page", () => {
  ...
  it("renders equity, daily P/L and the range toggles from seeded data", () => {
    ...
    expect(screen.getByText(/\$50\.00k/)).toBeTruthy(); // cash card (compact, account currency)
  });
  ...
  it("PR-3: live + empty equity curve + known equity shows the 'live history building' note", () => {
    useStore.setState({ paperTrading: false, currentEquity: 120_000, equityCurve: [] });
    render(<Overview />);
    expect(screen.getByText(/live history is building/i)).toBeTruthy();
  });

  it("PR-3: the live-history note is hidden once the live equity curve has points", () => {
    useStore.setState({
      paperTrading: false,
    ...
    render(<Overview />);
    expect(screen.queryByText(/live history is building/i)).toBeNull();
  });

  it("PR-3: the live-history note never shows in paper (empty curve + known equity)", () => {
    useStore.setState({ paperTrading: true, currentEquity: 120_000, equityCurve: [] });
    render(<Overview />);
    expect(screen.queryByText(/live history is building/i)).toBeNull();
  });

  it("PR-3: the live-history note is disjoint from the reconnect banner (null equity)", () => {
    useStore.setState({ paperTrading: false, currentEquity: null, equityCurve: [] });
    render(<Overview />);
    // null equity -> the reconnect banner path, never the live-history note
    expect(screen.queryByText(/live history is building/i)).toBeNull();
