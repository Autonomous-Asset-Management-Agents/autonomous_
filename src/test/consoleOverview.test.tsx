...
describe("console Overview page", () => {
  ...
  it("renders equity, daily P/L and the range toggles from seeded data", () => {
    ...
    // #overview-ia: the hero change follows the selected range. #3180: the default is 1D, so the
    // hero opens on "today" — and must NOT claim a week.
    expect(screen.getAllByText(/today/i).length).toBeGreaterThanOrEqual(1);
    expect(screen.queryByText(/past week/i)).toBeNull();
