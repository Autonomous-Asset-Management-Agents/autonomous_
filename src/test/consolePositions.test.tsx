...
vi.mock("../lib/api", () => ({
  fetchOpenOrders: vi.fn().mockResolvedValue({ status: "success", orders: [] }), cancelOrder: vi.fn().mockResolvedValue({ status: "success" }), fetchPortfolioSummary: vi.fn().mockResolvedValue(null) }));
...
describe("console Positions page", () => {
  ...
  it("renders an honest 2-point trend sparkline (no synthetic Math.sin) and no '30d' header", () => {
    ...
    const path = container.querySelector("tbody svg path");
