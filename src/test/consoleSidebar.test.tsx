...
// #2708 D3: the halted RESET branch wires useKillSwitch → resetKillSwitchAndResume. Stub it so the
// reset path is deterministic (default resolves; a per-test override drives the in-flight state).
vi.mock("../lib/killSwitch", () => ({
  resetKillSwitchAndResume: vi.fn().mockResolvedValue({ message: "reset ok" }),
}));
...
describe("console Sidebar", () => {
  beforeEach(() => {
    setBridge(undefined);
    claimBeta.mockClear();
    vi.mocked(resetKillSwitchAndResume).mockClear().mockResolvedValue({ message: "reset ok" });
    // B11: a per-test /health body must not leak into the next test's poll.
    vi.mocked(fetchHealth).mockResolvedValue(null);
    useStore.setState({
      ...
      // #2708: isolate the live-arming guard inputs so a switch flag can't leak between tests.
      switchReconciling: false, modeSwitch: null, requestLiveConsent: false,
  ...
  it("Agents is '—' and Live Trading is 'Idle' when the trading loop is off", () => {
    ...
    expect(screen.queryByText(/AI model missing/i)).toBeNull(); // no blocked reason → no hint
  });

  // B11: the engine's trading loop WAITS for the LLM provider on a fresh Mac without Ollama and
  // /health carries `startup_blocked_reason: "llm_unreachable"`. The bare "Idle" gets an actionable
  // hint that jumps to Settings → System (where the local model is installed).
  it("Idle + startup_blocked_reason llm_unreachable ⇒ 'AI model missing: Settings → System' hint", () => {
    useStore.setState({ strategyRunning: false, startupBlockedReason: "llm_unreachable", desktopPage: "overview" });
    render(<Sidebar />);
    expect(screen.getByText("Idle")).toBeTruthy();
    const hint = screen.getByRole("button", { name: "AI model missing: Settings → System" });
    fireEvent.click(hint);
    expect(useStore.getState().desktopPage).toBe("settings");
    expect(window.location.hash).toBe("#system");
  });

  it("strategy_running=true + llm_unreachable (the real wait state on a fresh Mac) still shows Idle + hint", () => {
    useStore.setState({ strategyRunning: true, startupBlockedReason: "llm_unreachable", desktopPage: "overview" });
    render(<Sidebar />);
    expect(screen.getByRole("button", { name: "AI model missing: Settings → System" })).toBeTruthy();
    expect(screen.queryByText(/^Paper$/)).toBeNull();
  });

  it("the hint disappears once the loop runs", () => {
    useStore.setState({ strategyRunning: true, startupBlockedReason: null });
    render(<Sidebar />);
    expect(screen.queryByText(/AI model missing/i)).toBeNull();
  });

  it("useHealthPolling writes startup_blocked_reason from /health into the store", async () => {
    vi.mocked(fetchHealth).mockResolvedValue({
      ...
      paper_trading: true,
      llm_ready: false,
      startup_blocked_reason: "llm_unreachable",
    });
    useStore.setState({ strategyRunning: null, startupBlockedReason: null });
    render(<Sidebar />);
    await screen.findByText("AI model missing: Settings → System");
    expect(useStore.getState().startupBlockedReason).toBe("llm_unreachable");
  });
  ...
  // ── #2708 D3: a halted engine offers RESET, not go-live (the trap that produced the re-arm) ──
  it("Senior, halted ⇒ RESET (not 'Start Live Trading', not Kill Switch)", () => {
  ...
  it("D3: RESET is not double-submittable while the reset is in flight", () => {
    // The reset never settles → the control disables (killResetting) so a second click cannot fire a
    // second /reset-kill-switch + /start-live pair. Kept synchronous + a halted /health so an async
    // poll can't re-render the branch out from under the assertions mid-reset.
    vi.mocked(fetchHealth).mockResolvedValue({ status: "healthy", system_halted: true, paper_trading: false, strategy_running: false });
    vi.mocked(resetKillSwitchAndResume).mockReturnValueOnce(new Promise(() => {}));
    useStore.setState({ tier: "PRO", canUpgrade: false, strategyRunning: false, systemHalted: true, paperTrading: false });
    render(<Sidebar />);
    const btn = screen.getByRole("button", { name: /^reset$/i });
    fireEvent.click(btn); // killResetting → true → disabled (state flush is synchronous under act)
    expect(btn).toBeDisabled();
    fireEvent.click(btn); // second click on the disabled control is a no-op
    expect(resetKillSwitchAndResume).toHaveBeenCalledTimes(1);
  });

  // ── #2708 D2: the sidebar never invites live-arming in an unsafe state ──
  it("D2: unknown account state ⇒ 'Start Live Trading' is disabled and raises no intent", () => {
    useStore.setState({ tier: "PRO", canUpgrade: false, strategyRunning: false, systemHalted: false, paperTrading: null });
    render(<Sidebar />);
    const btn = screen.getByRole("button", { name: /start live trading/i });
    expect(btn).toBeDisabled();
    fireEvent.click(btn);
  ...
  it("D2: a switch in flight ⇒ 'Start Live Trading' is disabled and raises no intent", () => {
    useStore.setState({ tier: "PRO", canUpgrade: false, strategyRunning: true, systemHalted: false, paperTrading: true, switchReconciling: true });
    render(<Sidebar />);
    const btn = screen.getByRole("button", { name: /start live trading/i });
    expect(btn).toBeDisabled();
    fireEvent.click(btn);
