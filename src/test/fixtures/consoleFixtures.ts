...
// ── setup.json (non-secret wizard state) ────────────────────────────────────
// B9: a finished installation carries the wizard's own done-mark. An install from before the
// mark has only LLM_PROVIDER; an abandoned one has only the name (keys live in the keychain).
export const setupStateComplete: Record<string, unknown> = {
  name: operator.name,
  LLM_PROVIDER: "ollama",
...
export const setupStateLegacy: Record<string, unknown> = {
  name: operator.name,
  LLM_PROVIDER: "ollama",
  LOCAL_LLM_MODEL: "llama3.2",
};
export const setupStateAbandoned: Record<string, unknown> = { name: operator.name };
