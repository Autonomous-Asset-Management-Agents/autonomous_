...
describe("mac download links (additive, Windows untouched)", () => {
  it("mac tag uses the separate desktop-mac-v* namespace", () => {
    // #3302: no -beta — the suffixed mac tag 404s just like the Windows one.
    expect(macDesktopReleaseTag("0.4.0")).toBe("desktop-mac-v0.4.0");
  });
  it("mac download url points at the fixed arm64 .dmg asset", () => {
    expect(macDownloadUrl("0.4.0")).toBe(
      "https://github.com/Autonomous-Asset-Management-Agents/autonomous_/releases/download/desktop-mac-v0.4.0/AAAgents-arm64.dmg",
    );
  });
  it("mac stable version points at a REAL published release, not the 0.0.0 no-release fallback (404 guard)", () => {
    // build-time define resolves from desktop/stable-release.json.mac_version. A mac PROD release
    // exists, so 0.0.0 here means the public .dmg link is a dead desktop-mac-v0.0.0 404.
    ...
    expect(STABLE_DESKTOP_VERSION_MAC, "mac download would 404 on desktop-mac-v0.0.0").not.toBe("0.0.0");
    expect(MAC_DOWNLOAD_URL).not.toContain("desktop-mac-v0.0.0-beta");
  });

  it("desktop/stable-release.json carries mac_version so the public .dmg link never regresses to a 0.0.0 404", async () => {
    const { readFileSync } = await import("node:fs");
    const { resolve } = await import("node:path");
    const raw = readFileSync(resolve(process.cwd(), "desktop/stable-release.json"), "utf8");
    const json = JSON.parse(raw) as { version?: string; mac_version?: string };
    expect(json.mac_version, "mac_version missing → vite falls back to 0.0.0 → dead download link").toBeDefined();
    expect(json.mac_version).toMatch(/^\d+\.\d+\.\d+$/);
    expect(json.mac_version).not.toBe("0.0.0");
  });
