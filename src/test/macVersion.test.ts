...
describe("mac download links (additive, Windows untouched)", () => {
  ...
  it("mac stable version points at a REAL published release, not the 0.0.0 no-release fallback (404 guard)", () => {
    ...
    // exists, so 0.0.0 here means the public .dmg link is a dead desktop-mac-v0.0.0-beta 404.
    ...
    expect(STABLE_DESKTOP_VERSION_MAC, "mac download would 404 on desktop-mac-v0.0.0-beta").not.toBe("0.0.0");
