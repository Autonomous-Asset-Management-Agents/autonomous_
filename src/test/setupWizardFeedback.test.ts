...
// The vendor-independent LLM picker (#1705) now owns local-AI provisioning + its manual-install recovery.
const cardSrc = readFileSync(path.join(dir, "..", "console", "desktop", "LlmProviderCard.tsx"), "utf8");
...
describe("C-B: setup wizard feedback + recovery", () => {
  ...
  it("a failed local-AI install surfaces a clickable Ollama download link", () => {
    // B10: the recovery state is the resolved download page (`ollamaDownloadHref`, formerly the
    // boolean `needsOllamaInstall`), and the page the shell names is linkified in its own text.
    expect(cardSrc).toMatch(/ollamaDownloadHref/);
    expect(cardSrc).toMatch(/ollama\.com\/download/);
  });
