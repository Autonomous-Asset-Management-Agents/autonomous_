...
export interface HarnessOptions {
  hasKeychain?: boolean;
  /** B9: non-secret setup.json. Default: a completed setup when `hasKeychain`, else `{}`. */
...
export async function installDesktopBridge(page: Page, opts: HarnessOptions = {}): Promise<void> {
  const hasKeychain = opts.hasKeychain ?? true;
  const cfg = {
    hasKeychain,
    setupState: opts.setupState ?? (hasKeychain ? fx.setupStateComplete : {}),
  ...
  await page.addInitScript((c) => {
    const noop = () => {};
    const unsub = () => noop;
    // B9: setup.json as the shell keeps it; the wizard's writes merge, the gate reads them back.
    let setupJson: Record<string, unknown> = { ...c.setupState };
    (window as unknown as { aaagents: Record<string, unknown> }).aaagents = {
      ...
      saveSetupState: async (partial: Record<string, unknown>) => {
        setupJson = { ...setupJson, ...(partial ?? {}) };
        return setupJson;
      },
      getSetupState: async () => ({ ...setupJson }),
