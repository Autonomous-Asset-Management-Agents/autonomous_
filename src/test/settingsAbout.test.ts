...
describe("Settings: About card (facts from NOTICE)", () => {
  ...
  it("wires the desktop app version into the About card (#1939)", () => {
    // The About card must surface the running app version, sourced from the
    // desktop bridge helper (getAppVersion) and rendered under a "Version" label.
    expect(settings).toMatch(/getAppVersion/);
    expect(settings).toMatch(/>Version</);
  });
});
