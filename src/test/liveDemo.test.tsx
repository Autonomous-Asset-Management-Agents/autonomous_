...
const snap = (overrides: Partial<DemoSnapshot> = {}): DemoSnapshot => ({
  ...
  status: "live",
  disclaimer:
    "Paper-Trading-Demo — keine Anlageberatung / keine Kauf- oder Verkaufsempfehlung.",
  equity: 10847.32,
  cash: 1203.55,
  day_pl_pct: 0.84,
  positions: [
  ...
  decisions: [
    {
      ...
      consensus: 0.71,
      conviction: 0.64,
      summary: "EDGAR-grounded",
    },
  ],
  report: {
    symbol: "AAPL",
    summary: "Stabiler Cashflow",
...
const mockFetch = (
  resp: Partial<Response> & { json?: () => Promise<unknown> },
...
describe("LiveDemo page", () => {
  ...
  it("renders the disclaimer, the shared header (LIVE DEMO), and a position", async () => {
    ...
    // the "Live Demo" hero heading is shown once the snapshot renders
    ...
    // #2047: the company name is shown next to the ticker, like the desktop app
    expect(screen.queryAllByText(/Apple Inc/i).length).toBeGreaterThan(0);
    // a round-table vote is shown
    expect(screen.queryAllByText(/buy/i).length).toBeGreaterThan(0);
  });

  it("shows EVERY position, not a capped list — the read-only demo has no 'View all' (matches the 'N open' header)", async () => {
    const many = [
      "AAPL",
      "MSFT",
      "NVDA",
      ...
      symbol,
      qty: 10 + i,
      market_value: 1000 + i * 100,
      unrealized_pl_pct: 1.0,
    }));
    mockFetch({ ok: true, json: async () => snap({ positions: many }) });
    renderDemo();
    await waitFor(() =>
      expect(screen.queryAllByText("AAPL").length).toBeGreaterThan(0),
    );
    // the header reflects the true count …
    expect(screen.getAllByText(/10 open/).length).toBeGreaterThan(0);
    // … and the 9th and 10th positions (beyond the old limit={8}) actually render.
    expect(screen.getAllByText("JPM").length).toBeGreaterThan(0);
    expect(screen.getAllByText("XOM").length).toBeGreaterThan(0);
  });

  it("shows the shared header (with the Download CTA, not Live Demo) before the snapshot arrives", () => {
    mockFetch({ ok: true, json: async () => snap() });
    renderDemo();
    // header (shared SiteHeader) is present immediately; content waits for the snapshot
    expect(screen.getByRole("button", { name: /To homepage/i })).toBeTruthy();
    // the demo's top-right CTA is the white "Download" pill → the Editions section on the landing
    ...
    // the "Live Demo" self-link must NOT appear on the demo page's own header
  ...
  it("renders the console-look dashboard (equity surface, KPIs, consensus) when a snapshot is present", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText(/10,847\.32/)).not.toBeNull(),
    );
    ...
    // the P/L card was replaced by KpiBand in #3321
  ...
  it("shows the execution-outcome badge on a decision when the engine provides it (RQ-1 #1516)", async () => {
    ...
    renderDemo();
    await waitFor(() =>
      expect(screen.queryByText(/blocked · order-value/i)).not.toBeNull(),
    );
  });
