...
describe("LiveTradingSwitchCard (#1425)", () => {
  ...
  it("#2454: the go-live confirm offers an autonomy limit, prefilled at 10% of the live equity", async () => {
    useStore.setState({ liveEquity: 50000 });
    render(<Harness />);
    await waitFor(() => expect(screen.getByText("Paper")).toBeTruthy());
    fireEvent.click(await screen.findByRole("button", { name: /^live$/i }));
    await waitFor(() => expect(screen.getByText(/autonomy limit/i)).toBeTruthy());
    // Prefilled to 10% of €50,000 = €5,000.
    expect((screen.getByLabelText("hitl-per-trade") as HTMLInputElement).value).toBe("5000");
    expect(screen.getByRole("button", { name: /10% of equity/i })).toBeTruthy();
    expect(screen.getByRole("button", { name: /full autonomous/i })).toBeTruthy();
  });

  it("#2454: enabling live persists the chosen per-trade limit BEFORE the WORM enable", async () => {
    useStore.setState({ liveEquity: 50000 });
    render(<Harness />);
    await waitFor(() => expect(screen.getByText("Paper")).toBeTruthy());
    fireEvent.click(await screen.findByRole("button", { name: /^live$/i }));
    await waitFor(() => expect(screen.getByLabelText("hitl-per-trade")).toBeTruthy());
    fireEvent.click(screen.getByLabelText("ack-live"));
    fireEvent.click(screen.getByLabelText("ack-advance-approval"));
    ...
    expect(updateHitlPolicy.mock.calls[0][0]).toMatchObject({
      HITL_MAX_VALUE_PER_TRADE: 5000, HITL_AUTONOMOUS_UNLIMITED: false,
    });
    await waitFor(() => expect(liveEnable).toHaveBeenCalled());
  });
  ...
  it("#2454: choosing Full autonomous persists Mode C (unlimited)", async () => {
    useStore.setState({ liveEquity: 50000 });
    render(<Harness />);
    await waitFor(() => expect(screen.getByText("Paper")).toBeTruthy());
    fireEvent.click(await screen.findByRole("button", { name: /^live$/i }));
