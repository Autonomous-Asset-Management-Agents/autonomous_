...
const mockRecoverToPaper = vi.fn().mockResolvedValue(undefined);
...
vi.mock("@/lib/desktopBridge", () => ({
    ...
    recoverToPaper: (...args: unknown[]) => mockRecoverToPaper(...args),
}));
...
describe("BackendOffline — Desktop vs Cloud", () => {
    ...
    // INC-3 (#2213): recovery-to-paper CTA — the way out of a dead LIVE boot.
    it("shows a 'Recover to Paper' CTA on desktop and calls recoverToPaper when clicked", () => {
        mockIsDesktop.mockReturnValue(true);
        render(<BackendOffline />);
        const cta = screen.getByRole("button", { name: /Recover to Paper/i });
        expect(cta).toBeTruthy();
        fireEvent.click(cta);
        expect(mockRecoverToPaper).toHaveBeenCalledTimes(1);
    });

    it("does NOT show the 'Recover to Paper' CTA for cloud users", () => {
        mockIsDesktop.mockReturnValue(false);
        render(<BackendOffline />);
        expect(screen.queryByText(/Recover to Paper/i)).toBeNull();
    });
});
