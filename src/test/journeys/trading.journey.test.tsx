...
describe("Journey · Operational trading (monitor the book)", () => {
  ...
  it("Positions: the engine book flows through the adapter into the table", async () => {
    ...
    expect(screen.getByRole("heading", { name: /3 Active Positions/i })).toBeTruthy();
  });

  it("Decisions: the latest Round-Table verdict per symbol renders", async () => {
    render(<DesktopApp />);
    goto(/decisions/i);
  ...
  it("warming up: empty engine responses show honest empty states, not zeros", async () => {
    ...
    goto(/decisions/i);
    await waitFor(() => expect(screen.getByText(/no decisions yet/i)).toBeTruthy());
  });
});
