...
describe("Journey · Onboarding (first-run setup)", () => {
  ...
  it("gate: an already-configured desktop skips the wizard and opens the console", async () => {
    ...
    render(<ConsoleApp />);
    await waitFor(() => expect(screen.getByPlaceholderText(/message the engine/i)).toBeTruthy(), { timeout: 3000 });
    expect(screen.queryByText(/set up aaagents/i)).toBeNull();
  });
  ...
  it("happy path: first run → wizard → Ollama → launch → console", async () => {
    ...
    // Welcome
    await waitFor(() => expect(screen.getByText(/set up aaagents/i)).toBeTruthy());
    ...
    fireEvent.click(screen.getByRole("button", { name: /launch aaagents/i }));
