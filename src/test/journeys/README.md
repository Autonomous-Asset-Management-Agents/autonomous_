...
- **Gate** — *Given* eine bereits eingerichtete Installation (Keychain gefüllt, `setup_completed` in setup.json) *When* die App startet *Then* wird der Wizard übersprungen und die Konsole geöffnet.
- **Gate B9 (Wiederaufnahme)**: *Given* Keys im Keychain, aber setup.json ohne Abschluss (Modellschritt abgebrochen) *When* die App startet *Then* erscheint der Wizard direkt auf dem Modellschritt; Modell → „Launch" → Konsole, und der Wizard schreibt `setup_completed: true`.
- **Gate B9 (Migration)**: *Given* eine Installation von vor der Abschlussmarke (nur `LLM_PROVIDER` in setup.json) *When* die App startet *Then* gilt sie als abgeschlossen, Konsole offen.
- **Gate B9 + Demo**: der wiederaufgenommene Wizard bietet auf dem Modellschritt „Skip for now" an; Demo-Dashboard mit „Finish setup"-Banner.
- **Happy Path (Ollama)** — Name → Alpaca live-validiert → lokales Ollama provisioniert (Fortschritt streamt) → „Launch" → Konsole erscheint; am Ende steht die Abschlussmarke in setup.json.
