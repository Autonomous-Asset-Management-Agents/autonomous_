...
describe("Journey · Settings (engine lifecycle & safety)", () => {
  ...
  it("shows the seeded status and replays the engine log on open", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/system stopped/i)).toBeTruthy());
  ...
  it("Start flips the status to running over IPC and tracks button state", async () => {
    ...
    await waitFor(() => expect(screen.getByRole("heading", { name: /System/i })).toBeTruthy());
    await waitFor(() => expect(screen.getByText(/system stopped/i)).toBeTruthy());
    const engineCard = screen.getByTestId("engine-card");
    const start = within(engineCard).getByRole("button", { name: /start engine/i });
    const stop = within(engineCard).getByRole("button", { name: /stop engine/i });
    expect(start).not.toBeDisabled();
    expect(stop).toBeDisabled();

    fireEvent.click(start); // startEngine() resolves + emits status:"running"

    // RC2-7: the engine PROCESS is up but the trading loop isn't started, so the sidebar footer
    // pill now reads "System ready" (⇔ Overview Start Engine) rather than the old always-"running".
    await waitFor(() => expect(screen.getByText(/system ready/i)).toBeTruthy());
    expect(within(engineCard).getByRole("button", { name: /start engine/i })).toBeDisabled();
    expect(within(engineCard).getByRole("button", { name: /stop engine/i })).not.toBeDisabled();
  ...
  it("an engine error surfaces with its diagnostic detail", async () => {
    ...
    await waitFor(() => expect(screen.getByText(/system error/i)).toBeTruthy());
