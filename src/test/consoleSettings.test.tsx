...
vi.mock("@/lib/api", () => ({
  ...
  // SafetyControlCard (#2327) — the Start Engine button calls startLive().
...
describe("console Settings page", () => {
  ...
  it("emergency-stop modal: select 'sell all' then confirm liquidates via panicSell", async () => {
    // Engine running → the SafetyControlCard shows the Kill Switch button.
    setBridge(desktopBridge({ getEngineStatus: vi.fn().mockResolvedValue({ status: "running" }) }));
    useStore.setState({ strategyRunning: true, systemHalted: false, engineServing: true });
    render(<Settings />);

    // Open the emergency-stop modal.
    const killBtn = await screen.findByRole("button", { name: /kill switch/i });
    fireEvent.click(killBtn);

    // Both stop modes are offered (safer "halt only" is preselected).
    expect(screen.getByText("Open positions stay in the account.")).toBeTruthy();
    const sellCard = screen.getByRole("button", { name: /stop live trading and sell all/i });

    // Selecting "sell all" alone must NOT execute — only the explicit Confirm does (select-then-confirm).
    fireEvent.click(sellCard);
    expect(api.panicSell).not.toHaveBeenCalled();
    fireEvent.click(screen.getByRole("button", { name: /^confirm/i }));
    await waitFor(() => expect(api.panicSell).toHaveBeenCalledOnce());
  });
