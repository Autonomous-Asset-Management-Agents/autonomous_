...
describe("adaptRoundTableDecisions (real SenateSession payload)", () => {
  ...
  it("maps signal_action + senators from votes (agent_name / signal / score / vetoed)", () => {
    ...
    // ISO → HH:MM in the user's LOCAL timezone (shortTime → fmtTime, by design).
    // Assert the FORMAT, not a fixed value, so the test is timezone-independent
    // (a UTC-only "10:00" failed on any non-UTC runner, e.g. CET → "12:00").
    expect(d.ts).toMatch(/^\d{2}:\d{2}$/);
  ...
  it("maps per-agent weight through (0 when missing, never NaN) — RPT-GOLD Wave 2 #2732", () => {
    const [d] = adaptRoundTableDecisions(resp());
    expect(d.senators[0].weight).toBe(1.0);
    expect(d.senators[2].weight).toBe(0.5);
    // A vote with no weight → 0, so it is excluded from Σweight (abstain), never NaN.
    const [e] = adaptRoundTableDecisions(
      resp({
        decisions: [{ symbol: "Z", signal_action: "HOLD", votes: [{ agent_name: "A", score: 0.5, signal: "HOLD" }] }],
      }),
    );
    expect(e.senators[0].weight).toBe(0);
  });
  ...
  it("exposes the raw consensus_score (0..1) for the honest percentage label, alongside the signed meter", () => {
    // The Overview shows "N% consensus" from this raw score — 0.72 must read as 72%, NOT the
    // meter's rebased (0.72·2−1)=0.44. Missing score → 0.5 (neutral), clamped to [0,1].
    const d = adaptRoundTableDecisions(resp({ decisions: [{ symbol: "X", signal_action: "BUY", consensus_score: 0.72 }] }))[0];
    expect(d.consensusScore).toBeCloseTo(0.72);
    expect(d.conviction).toBeCloseTo(0.44); // meter value is unchanged
    expect(adaptRoundTableDecisions(resp({ decisions: [{ symbol: "X", signal_action: "HOLD" }] }))[0].consensusScore).toBeCloseTo(0.5);
  });
  ...
  it("carries the per-vote role through (#3094 — Direction vs Sizing split)", () => {
        ...
        decisions: [
          {
            ...
            votes: [
              { agent_name: "MomentumAgent", score: 0.8, weight: 0.45, signal: "BUY", role: "directional" },
              { agent_name: "VIXAwareRiskAgent", score: 0.12, weight: 0.0, signal: "SELL", role: "sizer" },
              { agent_name: "RegimeDetectionAgent", score: 0.76, weight: 0.0, signal: "BUY", role: "conditioner" },
            ],
          },
        ],
      }),
    );
    expect(d.senators.map((s) => s.role)).toEqual(["directional", "sizer", "conditioner"]);
    // A record without role (very old engine) → undefined, never a crash.
    const [e] = adaptRoundTableDecisions(
      resp({ decisions: [{ symbol: "Y", signal_action: "HOLD", votes: [{ agent_name: "A", score: 0.5, signal: "HOLD" }] }] }),
    );
    expect(e.senators[0].role).toBeUndefined();
  });
