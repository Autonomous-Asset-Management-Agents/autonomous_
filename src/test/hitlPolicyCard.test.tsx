...
describe("HitlPolicyCard (#1425 — honest HITL policy, real /api/hitl/policy)", () => {
  ...
  it("shows NO value fields at all — two modes need no arithmetic (#2700)", async () => {
    // The per-trade / per-day inputs are gone on purpose. Every value-based mode had to convert
    // equity -> % -> $, and each conversion was a way to land in a mode the operator did not
    // choose: the wizard's 25% collapsed to 0 when the live equity was unknown, and a per-trade
    // budget was negated by a per-day budget of 0 (which hitl_gate.py:379 correctly reads as
    // "no autonomous budget"). Result on 2026-08-06: 13 held orders under "full autonomy".
    render(<HitlPolicyCard />);
    ...
    expect(screen.queryByLabelText(/max-per-trade/i)).toBeNull();
    expect(screen.queryByLabelText(/max-per-day/i)).toBeNull();
  });

  it("the mode tiles carry the policy — both limits stay 0, HITL_ENABLED never sent", async () => {
    // Both limits 0 is the documented "all-manual" pair (hitl_gate.py module docstring); the ONLY
    // thing that distinguishes the two modes is the Art-14 waiver flag.
    render(<HitlPolicyCard />);
    await waitFor(() => expect(getHitlPolicy).toHaveBeenCalled());
    ...
    expect(body.HITL_MAX_VALUE_PER_TRADE).toBe(0);
    expect(body.HITL_MAX_VALUE_PER_DAY).toBe(0);
