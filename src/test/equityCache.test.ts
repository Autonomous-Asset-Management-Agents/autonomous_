...
describe("equityCache (PR-A Instant-Data)", () => {
  ...
  // Paper→Live leak: a switch must drop the OLD account's persisted curves so the next
  // Overview mount / renderer reload can't repaint the stale paper ~100K baseline (which
  // renders a ~-99% fake loss against the small live equity).
  it("clearEquityCaches drops the equity cache AND every intraday range", () => {
    writeEquityCache(view());
    writeIntradayCache("1D", view().equityCurve);
    writeIntradayCache("1W", view().equityCurve);
    expect(readEquityCache()).not.toBeNull();
    expect(readIntradayCache("1D")).toHaveLength(2);
  ...
  it("clearEquityCaches leaves unrelated localStorage keys untouched + never throws", () => {
    localStorage.setItem("aaa.modeSwitch", "live");
    writeEquityCache(view());
    expect(() => clearEquityCaches()).not.toThrow();
    expect(localStorage.getItem("aaa.modeSwitch")).toBe("live");
    expect(readEquityCache()).toBeNull();
  });
});
