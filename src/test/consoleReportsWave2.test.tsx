...
vi.mock("../lib/api", () => ({
  ...
  fetchSpecialistReports: vi
    .fn()
    .mockResolvedValue({ status: "ok", reports: [] }),
  fetchSpecialistReport: vi
    .fn()
    .mockResolvedValue({ status: "ok", report: null }),
...
describe("Reports — Wave 2 list (Round-Table-authoritative row)", () => {
  ...
  it("renders the escalation as a lightning bolt (role=img), never a grey dot", () => {
    useStore.setState({
      specialistReports: [
        report({
          symbol: "AAPL",
          escalate: true,
          escalateReason: "Heavy insider activity (4 filings)",
        }),
      ],
    ...
    expect(
      screen.getAllByRole("img", { name: /escalated/i }).length,
    ).toBeGreaterThan(0);
  });
...
describe("GroundedSynthesisCard — Round-Table consensus transparency (#2732)", () => {
  ...
  it("renders consensus = Σ(score×weight)/Σ(weight) over the DIRECTION set, lists weight-0 as dormant, and never truncates a rationale", () => {
    ...
    expect(text).toContain("Specialist Alpha"); // dormant agent (fixture humanizes; glossary miss)
  ...
  it("shows the served 68% (NOT the 59% re-compute that re-includes VIXAware) and splits Direction vs Sizing & Risk", () => {
    const { container } = render(
      <GroundedSynthesisCard
        r={report({ symbol: "ANET" })}
        decision={anetDecision()}
      />,
    );
    const text = container.textContent ?? "";
    ...
    expect(text).toContain("VIX Risk"); // #3354: agentGlossary label, in Sizing & risk (not counted)
  });
});
  ...
  it("Reports consumes reportSymbol → opens that report expanded, then clears the target", () => {
    useStore.setState({
      specialistReports: [
        ...
        report({
          symbol: "TSLA",
          investmentThesis: "TSLA-UNIQUE-THESIS marker.",
        }),
      ],
      roundTable: [],
    ...
    expect(screen.getAllByText(/TSLA-UNIQUE-THESIS/i).length).toBeGreaterThan(
      0,
    );
