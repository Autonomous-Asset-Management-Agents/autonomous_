...
// White "Download" pill (black text) — mirrors the Editions "Download for Windows" CTA.
const DOWNLOAD_BTN: CSSProperties = {
  background: "#fff",
  color: "#000",
  borderRadius: "9999px",
  padding: "6px 18px",
...
export function SiteHeader({
  dark = false,
  cta = "demo",
}: { dark?: boolean; cta?: "demo" | "download" } = {}) {
    ...
    <header className={`site-header${dark ? " site-header--dark" : ""}`}>
      <nav className="lb-nav">
        ...
        <div className="lb-nav-social">
          ...
          {cta === "download" ? (
            // Demo page only: a white "Download" pill that sends the visitor to the
            // Editions section of the landing (choose one of the three variants).
            <a
              href="/preview#block-editions"
              style={DOWNLOAD_BTN}
              aria-label="Download — choose your edition"
              title="Download — choose your edition"
            >
              Download
            </a>
          ) : (
