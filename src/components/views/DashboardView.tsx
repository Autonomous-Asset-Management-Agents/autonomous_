...
export const DashboardView = ({ equity, lastEquity, positions = [], isConnected = false, agentStatuses }: DashboardViewProps) => {
  ...
  /* ├─ Computed ├─ */
  // #1957: prefer the engine's cash-flow-adjusted figures so a deposit/withdrawal is not counted
  // as P&L. Fail-soft to the legacy fixed-baseline math only when the engine did not supply them.
  const engTwrPct = typeof benchmarkData?.twr_pct === "number" ? benchmarkData.twr_pct : null;
  const engPnlNet =
    typeof benchmarkData?.pnl_net_of_deposits === "number" ? benchmarkData.pnl_net_of_deposits : null;
  const STARTING_CAPITAL = 100000;
  const totalPnL = engPnlNet != null ? engPnlNet : equity != null ? equity - STARTING_CAPITAL : null;
              ...
              <div>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <button onClick={handleSaveLimits} disabled={saving} className="flex items-center gap-1.5" style={{
                    ...
                    color: "#ffffff", cursor: saving ? "not-allowed" : "pointer", transition: "all 0.2s",
                  }}>
      ...
      >
        ...
        <div className="surface-card p-5">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-[11px] font-medium text-[#ffffff]">{positions.length} open</span>
          </div>
          {positions.length > 0 ? (
