...
class AgentRegistry:
    ...
    def get(self, name: str) -> Optional["BaseStrategy"]:
        """Return a registered strategy BY NAME — active or standby. None if absent.

        The registry has always been able to HOLD several strategies (``register(...,
        set_active=False)``), and it could never be ASKED for one: there was
        ``get_active()``, ``list_registered()``, ``swap()`` — and no way to say "give me
        the LSTM".

        Why that gap matters: the Round Table's two ML voices both resolve their source
        via ``get_active()``. Under the default ``ACTIVE_STRATEGY="RLAgent"`` that means
        LSTMSignalAgent and RLConfidenceAgent read the SAME object — two votes, 0.80
        combined weight, one source, one of them wearing the other's name in the audit
        trail. The gremium cannot weigh LSTM against RL, and no flag can fix it: an agent
        that asks for "whichever strategy is active" cannot be told which model to
        listen to. This is the accessor that makes the question answerable.

        Returns None rather than raising: an agent whose model is not registered must be
        able to ABSTAIN. A missing model must never become a dead round table.

        Read-only by construction — it never activates, swaps, or mutates anything.

        ⚠ ADDITIVE ONLY: nothing reads this yet. Pointing an agent at a named source
        changes its vote, which changes trading, and that needs its own flag and a
        walk-forward — never as a side-effect of adding an accessor.
        """
        return self._strategies.get(name)
