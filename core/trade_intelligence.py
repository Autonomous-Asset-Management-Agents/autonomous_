...
class TradeIntelligence:
    ...
    def __init__(self, clock: "ClockPort", data_file: str = TRADE_INTELLIGENCE_FILE):
        self.clock = clock
        ...
        self._session_start = self.clock.now().astimezone(None).replace(tzinfo=None)
        ...
        self._last_tuning_check = self.clock.now().astimezone(None).replace(tzinfo=None)
    ...
    def record_entry(
        ...
        now = self.clock.now().astimezone(None).replace(tzinfo=None)
    ...
    def _detect_patterns(self, trade: CompletedTrade):
        ...
        today = self.clock.now().astimezone(None).replace(tzinfo=None).date()
        today_symbol_trades = [
            t
    ...
    def _maybe_apply_forgiveness(self):
        ...
        symbols_forgiven = 0
        for _sym, intel in self._symbol_intelligence.items():
    ...
    def get_session_stats(self) -> Dict[str, Any]:
        ...
        session_hours = (
            self.clock.now().astimezone(None).replace(tzinfo=None) - self._session_start
        ).total_seconds() / 3600
