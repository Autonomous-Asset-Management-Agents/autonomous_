...
def _bc_config(name: str, default):
    """Edition-neutral config read (getattr on the config shell, edition-neutral).
...
def construct_book(
    ...
    """
    now = as_of or clock.now().astimezone(None).replace(tzinfo=None)
...
def should_rebalance(
    ...
    (intelligent exit / broker stop) protect positions; ranking churn does not trade."""
...
def maybe_construct_book(
    ...
    if not is_book_constructor_enabled():
        return None
    moment = now or clock.now().astimezone(None).replace(tzinfo=None)
    ranking, source = gather_ranking(clock=clock, as_of=moment)
    if not ranking:
        return None
