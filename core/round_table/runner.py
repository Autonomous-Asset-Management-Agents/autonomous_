...
def _serialize_votes(valid_votes) -> list:
    ...
    HONEST WEIGHTS in the audit trail: the two NON_DIRECTIONAL_AGENTS
    (consensus.py — DrawdownGuard acts via VETO, RegimeDetection via
    weight-coupling) are recorded with ``weight: 0.0`` + their ``role``,
    because their class-weight (0.60/0.50) NEVER enters the directional mean —
    recording it misled readers repeatedly (#1993, AGENT_CATALOG.md:158,
    owner query 2026-08-28). Every vote STAYS in the record (the console keeps
    showing the full board — owner directive); the LIVE VoteResult is not
    mutated (its weight must stay > 0 for the validator + agent-veto path).
    Directional voters record their real weight + role "directional".
    """
    serialized_votes = []
    for v in valid_votes:
        if v.score > SIGNAL_BUY_THRESHOLD:
            ...
            agent_signal = "HOLD"
        non_directional = v.agent_name in NON_DIRECTIONAL_AGENTS
        serialized_votes.append(
            {
                "name": v.agent_name,
                "agent_name": v.agent_name,
                "score": v.score,
                "weight": 0.0 if non_directional else v.weight,
                "role": _AGENT_RECORD_ROLES.get(v.agent_name, "directional"),
...
async def run_round_table(state: "SymbolEvalState") -> "SymbolEvalState":
    ...
    # #3210: directional-vote coverage for the flag-gated prudence size discount
    # (dark by default). Computed ONLY when armed (COVERAGE_SIZING_STRENGTH > 0) so
    # the dark path stays byte-identical (no extra weight reads). Fail-safe: any
    # error → None → the sizer/audit discount is a no-op (1.0). Uses the SAME
    # consensus_exclusions() the mean used, so the directional set is consistent.
    _vote_coverage = None
    try:
        import config as _cfg_cov

        if float(getattr(_cfg_cov, "COVERAGE_SIZING_STRENGTH", 0.0) or 0.0) > 0.0:
        ...
        logger.warning("coverage compute failed for %s: %s", symbol, exc)
    ...
    if gatekeeper_decision.approved:
        ...
        try:
            ...
            if _dc_cov is not None and _vote_coverage is not None:
                _dc_cov.vote_coverage = _vote_coverage
        except Exception as exc:  # noqa: BLE001 — never break the decision path
            logger.warning("coverage attach failed for %s: %s", symbol, exc)
    ...
    try:
        ...
        logger.warning("[runner] consensus_return_recorder hook failed: %s", exc)
