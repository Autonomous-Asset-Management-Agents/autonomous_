...
class VIXAwareRiskAgent(VotingAgent):
    ...
    Momentum, LSTM und NewsSentiment schätzen die *Richtung*; diese Stimme soll die
    *Schwankung* beisteuern — als Gegengewicht zur Vorrangregel „Kapitalerhalt vor
    kurzfristigem und risikobehaftetem Gewinn" (VISION_AND_GOALS §3).

    Zwei Eingänge, per Flag umschaltbar (#3038):

    * **AUS (Default):** marktweiter VIX durch eine Sigmoid um 25 — für alle Titel
      eines Tages derselbe Wert, bitgleich mit RegimeDetectionAgent.
    * **AN:** die implizite Volatilität des Titels, als Rang gegen die Verteilung des
      Vortages: ``score = 1 − Perzentil``. Ruhig heißt hoher Score.
...
class UpsideSkewAgent(VotingAgent):
    ...
    default_weight: float = 0.30  # config-tunable via UPSIDE_SKEW_WEIGHT (Step-2 arm)
    min_weight: float = 0.10
    max_weight: float = 1.50
