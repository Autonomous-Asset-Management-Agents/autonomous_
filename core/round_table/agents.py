...
# Default-Gewicht 0.0 ⇒ VoteResult(weight=0.0) ⇒ Field(gt=0.0) schließt sie aus dem
...
# (QUALITY_AGENT_ENABLED default false ⇒ Enthaltung ⇒ VoteResult(weight=0.0) ⇒ aus dem
...
# Redaktionelle Startkalibrierung (KEIN Modell-Fit): die Squashing-Skalen halten
# den Score in (0,1), streng monoton, mit 0.5 als neutralem Punkt. Kandidaten für
# einen Walk-forward-Review bei Aktivierung (analog _MOMENTUM_SCORE_SCALE).
_TREND_MACD_HIST_SCALE = 1.0
...
class TrendAgent(VotingAgent):
    """#3250: DIREKTIONALE Trendstimme aus dem MACD-Histogramm.

    Liest den abgeschlossenen TA-Skalar ``state["features"]["macd_hist"]``
    (``core/round_table/features.py`` via ``_compute_features_node``). ``macd_hist``
    = MACD-Linie − Signallinie: positiv = aufbauendes Aufwärtsmomentum, negativ =
    Abwärts. Score = ``0.5 + 0.5*tanh(macd_hist / _TREND_MACD_HIST_SCALE)`` —
    vorzeichenerhaltend, streng monoton, in (0,1), neutral 0.5 bei 0.

    Bewusst NICHT genutzt (#3250/#3251): ``macd_line`` wird nicht nach
    ``state["features"]`` geschrieben (nur ``macd_hist``), und ``adx_14`` ist derzeit
    ein Platzhalter-Default. Fehlt ``features`` / der Schlüssel / ist der Wert nicht
    endlich → Enthaltung (Gewicht 0.0 direkt im VoteResult). Flag OFF (Default) →
    Enthaltung (dark, byte-identisch).
    """
    ...
    async def vote(self, state: "SymbolEvalState") -> VoteResult:
        ...
        if macd_hist is None:
            return self._abstain(symbol, "no macd_hist in the feature snapshot")
        try:
            macd_hist = float(macd_hist)
        except (TypeError, ValueError):
            return self._abstain(symbol, f"macd_hist not usable ({macd_hist!r})")
        if not math.isfinite(macd_hist):
            return self._abstain(symbol, "macd_hist not finite")

        score = self._clamp(0.5 + 0.5 * math.tanh(macd_hist / _TREND_MACD_HIST_SCALE))
        direction = (
            "bullish" if macd_hist > 0 else ("bearish" if macd_hist < 0 else "flat")
        )
        reasoning = (
            f"MACD histogram {macd_hist:+.4f} — {direction} trend momentum "
            f"(score {score:.3f}). [src: MACD(12,26,9) on daily closes]"
        )
        return VoteResult(
            agent_name="TrendAgent",
            symbol=symbol,
