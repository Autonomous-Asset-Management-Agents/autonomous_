...
class SenateSession:
    ...
    #: #2783 Inkrement 1 — die Entscheidung, aus der dieser Datensatz stammt.
    #: Ohne sie laesst sich die hash-verkettete WORM-Datei nur ueber
    #: ``(session_id, symbol)`` mit ``decision_outcomes`` verbinden, und das ist kein
    #: Schluessel, sondern ein Zufall gemeinsamer Spalten: Eine Sitzung bewertet viele
    #: Symbole, die Eindeutigkeit erzwingt niemand.
    #:
    #: Optional mit Vorgabe ``None``, weil ein Aufrufer ohne Entscheidungskontext den
    #: Audit-Pfad **nicht** zum Stehen bringen darf. Ein Datensatz ohne Bezug ist
    #: schlechter als einer mit — aber unendlich viel besser als keiner.
...
class HITLExecutionEvent:
    ...
    # --- #2783 Inkrement 2: der wahre Order-Endzustand -----------------------
    # Bis hierhin belegte das Ereignis, dass eine Order den Broker ERREICHT hat
    # (`branch`) und welchen Nominalwert sie trug (`order_value`) — nicht, was aus
    # ihr geworden ist. Eine abgesetzte Order, die nie gefuellt wurde, war im
    # Pruefprotokoll von einer gefuellten nicht zu unterscheiden.
    #
    # Der Endzustand ist eine audit-relevante Tatsache und gehoert deshalb auf die
    # VERKETTETE Ebene, nicht nur in die analytische Tabelle daneben (Korrektur
    # gegenueber dem urspruenglichen #2909-Plan).
    #
    # Alle optional mit Vorgabe ``None``: Ein Zweig, der den Endzustand nicht kennt
    # — eine Ablehnung vor dem Absenden etwa —, soll ihn auch nicht erfinden.
    # ``None`` heisst „nicht bekannt"; eine Null hiesse „nichts gefuellt", und das
...
@dataclass
class PortfolioShapeEvent:
    """Immutable record of a deliberate portfolio-structure change (#3132).

    Position count and minimum holding period decide how capital is spread and how long
    it stays committed, so a change is a capital decision — recorded on the SAME
    tamper-evident SHA-256 chain as live enablement, BEFORE it is applied
    (audit-before-change, mirroring ``LiveEnableEvent``).

    All fields are ``str`` — NO floats, NO nested objects. ``changes`` carries the
    already-serialised JSON text of ``[{"key", "from", "to"}, ...]`` (built by
    ``core.portfolio_shape.describe_changes``, which emits text-only fields). The reason is
    the same as for ``LiveEnableEvent``: the SHA-256 preimage
    (``json.dumps(entry, sort_keys=True)``) must be byte-identical when the JS verifier
    (``audit-chain.cjs`` ``verifyAuditChain``) recomputes it, and float repr can diverge
    between ``json.dumps`` and ``JSON.stringify`` — which would permanently fail-close live
    trading.

    ``switch_id`` defaults to the ``nonce`` at the write boundary so the record is never
    ``null`` (D1: no null-noise) and a change self-correlates.
    """
...
class SettingsChangeEvent:
    """Immutable record of a deliberate trading-settings change (#3155, UXC-1 S2).

    The generic sibling of ``PortfolioShapeEvent`` (#3132): any registry parameter
    (agent weights/enable flags, thresholds, rotation, exits, frequency guards,
    stops/sizing — ``core.trading_settings.REGISTRY``) changes capital behaviour
    once applied, so the change is recorded on the SAME tamper-evident SHA-256
    chain BEFORE it is applied (audit-before-change, mirroring ``LiveEnableEvent``).

    All fields are ``str`` — NO floats, NO nested objects. ``changes`` carries the
    already-serialised JSON text of ``[{"key", "from", "to"}, ...]`` (built by
    ``core.trading_settings.describe_changes``, text-only fields) for the same
    byte-parity reason documented on ``PortfolioShapeEvent``: the SHA-256 preimage
    must be byte-identical when ``audit-chain.cjs`` recomputes it.

    ``switch_id`` defaults to the ``nonce`` at the write boundary (D1: no null-noise).
    """
...
@dataclass
class TradingSettingsEvent:
    """Immutable boot-seal record of the FULL effective trading-settings state (#3155 S2).

    Plan Rev. 2 Option B: a DEDICATED event with its own discriminator —
    ``TradingControlEvent`` (#2863) stays byte-identically frozen (its docstring
    contract covers exactly the two vol controls). ``settings`` carries the
    canonical, sorted JSON text of ALL ``core.trading_settings`` registry keys
    (the established ``PortfolioShapeEvent.changes`` string pattern), so every
    boot's governing parameter state is on-chain (A5) while the event schema stays
    stable when the registry grows. All fields ``str`` (float-free) for the JS
    verifier (``audit-chain.cjs`` re-hashes generically).
    """
...
@dataclass
class SettingsDeviationAckEvent:
    """Immutable record of the operator's keep/reset decision at the live switch
    (#3156, UXC-1 S3, epic guardrail 6).

    Before live enablement the console lists every user setting that deviates from
    the shipped defaults; the operator ACTIVELY chooses to keep or reset them. Both
    decisions are Art-14-relevant oversight facts, so they are sealed onto the SAME
    tamper-evident SHA-256 chain BEFORE ``LiveEnableEvent`` (audit-before-change).
    ``decision="reset"`` is additionally accompanied by the regular old→new
    ``SettingsChangeEvent`` — appended by the SAME backend request (Rev. 3 atomic
    composition, no split-brain).

    All fields are ``str`` (float-free) so the SHA-256 preimage is byte-stable for
    the JS verifier (``audit-chain.cjs``). ``deviations`` carries the pre-serialised
    JSON text of ``[{"key","current","default"}, ...]``; ``switch_id`` joins the
    decision to the live switch and the boot it armed (#2277).
    """
...
def _hitl_event_to_dict(event: _HITLEvent) -> dict:
    ...
    if isinstance(event, PortfolioShapeEvent):
        return {"event_type": "portfolio_shape_change", **asdict(event)}
    ...
    if isinstance(event, SettingsChangeEvent):
        return {"event_type": "settings_change", **asdict(event)}
    if isinstance(event, TradingSettingsEvent):
        return {"event_type": "trading_settings_seal", **asdict(event)}
    if isinstance(event, SettingsDeviationAckEvent):
        return {"event_type": "settings_deviation_ack", **asdict(event)}
...
class LocalJSONAuditLogger(IAuditLogger):
    ...
    async def _async_log_to_jsonl(self, session: SenateSession) -> None:
        entry = {
            # #2783 Inkrement 1: der Bezug auf die Entscheidung. Er steht bewusst
            # VORN — der Datensatz soll mit seiner Identitaet beginnen, nicht mit
            # einem Sitzungsmerkmal, das viele Symbole teilen.
            #
            # Fuer den JS-Verifier ist der zusaetzliche Schluessel durchsichtig:
            # `pyJsonDumps` (desktop/electron/audit-chain.cjs) serialisiert rekursiv
            # mit sortierten Schluesseln und hat KEINE Feldliste. Alte Datensaetze
            # ohne das Feld verifizieren darum weiter, weil jeder Datensatz sein
            # eigenes Preimage aus seinen eigenen Schluesseln bildet — gepinnt in
            # tests/unit/test_2783_decision_id_in_worm.py.
