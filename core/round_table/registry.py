...
class PluginRegistry:
    ...
    def load_plugins_from_directory(self, plugins_dir: str):
        # Guard I-2a (#943): Block plugin loading in production environment.
        # Structurally independent from ALLOW_UNTRUSTED_PLUGINS — production is
        # ALWAYS protected even if the env-var is set incorrectly in Cloud Run.
        if os.environ.get("ENVIRONMENT", "").lower() == "production":
            logger.warning(
                "SECURITY: Plugin loading BLOCKED — ENVIRONMENT=production. "
                "ALLOW_UNTRUSTED_PLUGINS is ignored in production. "
                "Possible rogue plugin injection attempt via Cloud Run env-var."
            )
            return
