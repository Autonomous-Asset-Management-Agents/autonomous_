...
if TYPE_CHECKING:
    ...
    from core.round_table.types import SymbolEvalState
...
class VotingAgent(ABC):
    ...
    @abstractmethod
    async def vote(self, state: "SymbolEvalState") -> "VoteResult | SignalCandidate":
        """
        Bewertet ein Symbol und gibt ein VoteResult oder SignalCandidate zurück.

        Args:
            state: SymbolEvalState mit OHLC-Skalaren (kein DataFrame erlaubt)

        Returns:
            VoteResult oder SignalCandidate
        """
        ...
...
class AsyncAIAgent(VotingAgent):
    ...
    @abstractmethod
    def _run_inference(
        self, state: "SymbolEvalState"
    ) -> "VoteResult | SignalCandidate":
        """
