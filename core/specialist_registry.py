...
# #3438: Zeitlimit einer einzelnen Symbol-Auffrischung.
#
# Ohne dieses Limit war `stop()` eine Behauptung: Das Shutdown-Event wird nur ZWISCHEN
# Symbolen geprueft, und `agent.research()` startet sechs Netzabrufe parallel (EDGAR
# Form 4 / 8-K / 13D, Congressional Trades, Polygon News, Wiki Pageviews) ohne
# umschliessende Frist. Steckte der Thread dort, verstrich der `join(timeout=15)`
# wirkungslos und `stop()` kehrte zurueck, als haette es gestoppt (RCA §4, Befund c).
#
# Der Wert ist bewusst GROSSZUEGIG. Der Plan warnt ausdruecklich: Ein zu knappes Limit
# bricht eine laufende Recherche ab, und die Karte zeigt dann "degraded" statt Inhalt.
# 120 s liegt weit ueber dem, was sechs parallele Abrufe brauchen, und weit unter der
# Ewigkeit, die vorher galt. Es ist eine Reissleine, keine Taktvorgabe.
REFRESH_SYMBOL_TIMEOUT_SECONDS = 120.0
...
class StockSpecialistRegistry:
    ...
    def __init__(
        ...
        Args:
            ...
            report_only:    A5 (RPT-8b) deterministic-first mode. When True, each
                            ...
                            feed the consensus (the agent's *score* does: SPECIALIST_ALPHA_WEIGHT
                            defaults to 0.40 since #2839; #3261).
        """
        ...
        # #3438: Handgriffe, mit denen `stop()` eine LAUFENDE Auffrischung abbricht,
        # statt nur das Ende der naechsten abzuwarten. Ohne sie kehrte `stop()` nach
        # seinem `join(timeout=15)` zurueck, waehrend der Thread weiterlief.
        self._loop: Optional[asyncio.AbstractEventLoop] = None
        self._laufende_auffrischung: Optional[asyncio.Task] = None
    ...
    def stop(self) -> None:
        ...
        #3438: Das Setzen des Events allein genuegte nicht. Es wird nur ZWISCHEN
        Symbolen geprueft — steckte der Thread in einer Auffrischung, verstrich der
        ``join`` wirkungslos und diese Methode kehrte zurueck, als haette sie gestoppt.
        Darum wird eine laufende Auffrischung jetzt ausdruecklich abgebrochen.
        """
        self._shutdown.set()

        # Eine laufende Auffrischung abbrechen. `call_soon_threadsafe` ist der einzige
        # erlaubte Weg, von aussen in eine fremde Event-Loop zu greifen.
        loop = getattr(self, "_loop", None)
        aufgabe = getattr(self, "_laufende_auffrischung", None)
        if loop is not None and aufgabe is not None and not aufgabe.done():
            try:
                loop.call_soon_threadsafe(aufgabe.cancel)
            except RuntimeError as exc:
                # Loop bereits geschlossen — dann ist der Thread ohnehin am Ende.
                logger.debug("StockSpecialistRegistry: Abbruch nicht noetig (%s)", exc)
        ...
        if self._refresh_thread:
            ...
            if self._refresh_thread.is_alive():
                # Ehrlich melden statt still zurueckkehren: Genau dieses stille
                # Zurueckkehren war der Defekt (RCA §4, Befund c).
                logger.error(
                    "StockSpecialistRegistry: Thread lebt nach 15s noch. Der Stopp "
                    "hat NICHT gewirkt — bitte melden (#3438)."
                )
                return
    ...
    def _refresh_loop(self) -> None:
        ...
        def _auffrischen(symbol: str) -> None:
            """Eine Auffrischung fahren — abbrechbar von aussen (#3438).

            ``run_until_complete(coro)`` gibt keinen Handgriff auf die laufende
            Aufgabe. Ohne einen solchen kann ``stop()`` nicht abbrechen, sondern nur
            warten — und genau das war der Defekt.
            """
            aufgabe = loop.create_task(self._refresh_symbol(symbol))
            self._laufende_auffrischung = aufgabe
            try:
                loop.run_until_complete(aufgabe)
            except asyncio.CancelledError:
                logger.info(
                    "[%s] SpecialistRegistry: Auffrischung durch Stopp abgebrochen "
                    "(#3438).",
                    symbol,
                )
            finally:
                self._laufende_auffrischung = None
        ...
        while not self._shutdown.is_set():
            try:
                ...
                if now - self._last_normal_tick >= normal_interval:
                    ...
                    # #3438 Schicht 1: Das Shutdown-Event wurde bisher nur im
                    # Hochprioritaets-Zweig geprueft (:424). Ein Stopp, der genau
                    # hier eintraf, musste erst noch eine volle Auffrischung
                    # abwarten. Jetzt gilt die Pruefung VOR JEDEM Symbol.
                    if normal_sym and not self._shutdown.is_set():
                        _auffrischen(normal_sym)
    ...
    async def _refresh_symbol(self, symbol: str) -> None:
        ...
        try:
            if self._report_only:
                ...
                # #3438 Schicht 1: `research()` startet sechs Netzabrufe parallel und
                # hatte keine umschliessende Frist. Ohne sie konnte `stop()` seine
                # Zusage nicht halten — siehe REFRESH_SYMBOL_TIMEOUT_SECONDS.
                report = await asyncio.wait_for(
                    agent.research(), timeout=REFRESH_SYMBOL_TIMEOUT_SECONDS
                )
            ...
            # Kein Fehler des Symbols, sondern eine gerissene Reissleine: benannt
            # melden, den letzten bekannten Bericht stehen lassen und weitergehen.
            # Ein halber Bericht waere schlechter als ein alter.
            logger.warning(
                "[%s] SpecialistRegistry: Auffrischung nach %.0fs abgebrochen "
                "(Zeitlimit). Der bisherige Bericht bleibt der letzte bekannte "
                "Stand (#3438).",
                symbol,
                REFRESH_SYMBOL_TIMEOUT_SECONDS,
            )
            self._refresh_failures[symbol.upper().strip()] = {
                "reason": "timeout",
                "seconds": REFRESH_SYMBOL_TIMEOUT_SECONDS,
