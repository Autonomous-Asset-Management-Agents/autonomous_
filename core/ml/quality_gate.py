...
Rejects symbols whose checkpoint should not be served at inference:
  ...
  3. (After walk-forward) walkforward_ic <= IC_MIN → no out-of-sample edge
     (MLR-9 #1909: demoted to diagnostic-only WARNING when
     TFT_IC_DIAGNOSTIC_ONLY is set — feedback_sharpe_over_ic doctrine)
  ...
  5. (MLR-9 #1909, ADR-ML-GATE-03, only when TFT_DEFLATED_SHARPE_GATE_ENABLED)
     deflated_sharpe must clear TFT_DSR_FLOOR. The field is the ANNUALIZED
     Sharpe-scale deflation margin SR - E[max SR] (Bailey/Lopez de Prado)
     stamped by scripts/apply_deflated_sharpe_layer.py (Layer 5) — negative ⇒
     the backtest Sharpe does not beat the expected maximum of n_trials
     zero-skill trials ⇒ overfitting. Orthogonal to FDR (multiple testing
     across the universe) and to net_sharpe (cost realism): all three must
     hold. Field absent → legacy behaviour + WARNING (fail-safe); flag off
     (default) → byte-identical to MLR-3.
...
def evaluate(symbol: str, model_dir: Path) -> GateResult:
    ...
    if wf_ic is not None:
        ...
        if wf_ic <= IC_MIN:
            # MLR-9 (#1909, ADR-ML-GATE-03): IC demotion. Doctrine
            # feedback_sharpe_over_ic — net-of-cost (deflated) Sharpe is the
            # price of a model, IC is diagnosis. When TFT_IC_DIAGNOSTIC_ONLY is
            # set the threshold reject is skipped and only WARNED (§5.6); the
            # field stays in metadata for report/DecisionContext consumers.
            # Default False → historical reject byte-identical.
            if get_config().TFT_IC_DIAGNOSTIC_ONLY:
                logger.warning(
                    "[TFTQualityGate] %s: %s=%.3f <= threshold %.2f — "
                    "IC diagnostic-only (TFT_IC_DIAGNOSTIC_ONLY), not rejecting",
                    symbol,
                    wf_ic_field,
                    wf_ic,
                    IC_MIN,
    ...
    # 4. MLR-9 (#1909, ADR-ML-GATE-03): Layer-5 deflated Sharpe — additive AND-gate
    # after FDR + net_sharpe, only when TFT_DEFLATED_SHARPE_GATE_ENABLED (default
    # OFF → this block is inert and the gate is byte-identical to MLR-3).
    #   * `deflated_sharpe` — annualized SR - E[max SR] margin written by
    #     scripts/apply_deflated_sharpe_layer.py inside the `layer5` block;
    #     promotion may flatten it to the top level (top level wins, net_sharpe
    #     idiom). Negative ⇒ backtest overfitting over n_trials (Bailey/LdP).
    # Fail-safe: field ABSENT → passthrough, tracked in the missing-fields
    # WARNING below so audit can see the model was served undeflated.
    dsr_enabled = bool(get_config().TFT_DEFLATED_SHARPE_GATE_ENABLED)
    dsr = None
    if dsr_enabled:
        dsr = meta.get("deflated_sharpe")
        if dsr is None:
            layer5 = meta.get("layer5")
            if isinstance(layer5, dict):
                dsr = layer5.get("deflated_sharpe")
        if dsr is not None:
            if not isinstance(dsr, (int, float)):
                return GateResult(False, f"deflated_sharpe is not numeric: {dsr!r}")
            dsr_floor = float(get_config().TFT_DSR_FLOOR)
            if dsr <= dsr_floor:
                return GateResult(
                    False,
                    f"deflated_sharpe={dsr:.3f} <= floor {dsr_floor:.2f} "
                    f"(SR - E[max SR] over n_trials={meta.get('n_trials')!r}, "
                    f"ADR-ML-GATE-03)",
                )
    ...
    # Only surface the deflated_sharpe gap when the DSR gate is active — flag off
    # must stay byte-identical to MLR-3 including log output (BORA).
    if dsr_enabled and dsr is None:
