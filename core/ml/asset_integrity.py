...
# that does not re-verify). Fail-closed on mismatch AND on an absent/incomplete manifest
# in every mode incl. LOCAL (#2984, sibling to #1886); a deliberate dev opt-out remains
# via AAA_REQUIRE_MANIFEST=false / TFT_REQUIRE_MANIFEST=false (logged loudly, CLAUDE.md 5.6).
...
def _require_manifest() -> bool:
    """Whether an ABSENT/incomplete manifest is fatal. Explicit AAA_REQUIRE_MANIFEST
    or TFT_REQUIRE_MANIFEST wins; otherwise strict EVERYWHERE (incl.
    DEPLOYMENT_MODE=LOCAL) to prevent unverified unpickling of LSTM/RL assets
    (CWE-502, sibling to the closed TFT fix #1886).

    LOCAL is no longer exempt (#2984): the install-time gate ships a git-tracked
    models_manifest.json co-located with every deserialised asset, so a clean
    Desktop/from-source boot still verify-and-passes. A deliberate developer who
    retrains a scaler without rebuilding the manifest can still opt out via the
    existing AAA_REQUIRE_MANIFEST=false (or TFT_REQUIRE_MANIFEST=false) escape hatch.
    """
    import config

    if not config.AAA_REQUIRE_MANIFEST:
        logging.warning(
            "model-integrity: manifest verification DISABLED via explicit "
            "opt-out - LSTM/RL assets will load UNVERIFIED (CWE-502). Dev-only; "
            "never set in a provisioned/live deployment."
        )
        return False
    return True
...
def verify_asset_bytes(data: bytes, asset_path: Union[str, Path]) -> None:
    ...
    Raises ModelIntegrityError on a hash mismatch (fail-closed). A missing/unreadable
    manifest, or an asset absent from it, is fatal by default in EVERY mode incl.
    LOCAL (#2984). Only an explicit dev opt-out (AAA_REQUIRE_MANIFEST=false /
    TFT_REQUIRE_MANIFEST=false) downgrades those to a logged WARNING + proceed.
    """
