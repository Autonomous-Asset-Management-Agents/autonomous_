...
def debiased_daily_vol_from_iv(
    ...
    Implied vol sits STRUCTURALLY above realized vol (a positive VRP). Feeding the
    raw ``iv_annual / sqrt(252)`` into the daily-calibrated scaler (target 0.015)
    would systematically under-size (median scaler pinned toward the 0.5 floor) —
    a chronic relative drag vs a ~fully-invested SPY. ``vrp_factor`` (median RV/IV,
    de-biased in the sweep; default 0.80) scales the level back to the realized
    operating point while KEEPING the forward-looking timing. Pass ``vrp_factor=1.0``
    for the raw Option-A comparison arm.
