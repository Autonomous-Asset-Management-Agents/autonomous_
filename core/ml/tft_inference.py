...
class TFTInferenceEngine:
    ...
    def _postprocess_quantiles(self, raw) -> Optional[TFTPrediction]:
        ...
        (config, default ON since #1885 / ADR-MLA-11 — serve on the trainer-validated
        scale). ON applies the adversarially-verified M1+M3a fix — see the inline
        comments at each gated step. The env override ``TFT_SERVING_FIX=0`` is the
        rollback path: it reproduces the historical behaviour byte-for-byte (AVERAGE
        across H and read the value as-is).
        """
