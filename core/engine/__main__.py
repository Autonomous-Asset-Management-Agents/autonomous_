...
if __name__ == "__main__":
    ...
    try:
        ...
        if root.is_cloud and os.path.exists(".env"):
            raise RuntimeError(
        ...
        _is_desktop = root.is_local_desktop
        ...
        try:
            # #2863 Inc 4: seal the operator's effective trading controls (vol-sizing switch +
            # daily-vol target) onto the WORM chain when they differ from the last-sealed state.
            from core.trading_control_seal import seal_trading_control

            asyncio.run(seal_trading_control())
        except Exception as exc:
            logging.warning("Trading-control seal skipped: %s", exc)
