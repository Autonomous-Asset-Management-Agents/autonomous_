        ...
        try:
            ...
            def strategy_cb(client, date):
                ...
                try:
                    if not getattr(self, "sim_risk_manager", None):
                        # #3485: eigener Halt — ein Verlust der Simulation haelt nie die
                        # echte Engine an, eine Erholung hebt nie deren Halt auf.
                        ...
                        if getattr(config, "SIMULATION_FALLBACK_BUY", False):
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    # #2980 (§5.6): a skipped fallback buy is a
                                    # fallback event — log at WARNING, not DEBUG.
                                    logging.warning("[SIM] Fallback buy skipped: %s", e)
                    else:
                        logging.info(
            ...
            if not shutdown_event.is_set():
                ...
                self.last_simulation_result = {
                    ...
                    # #1907 (RC-3): judge the claim against THIS backtest's window, not bare
                    # file existence — the no-arg call was the over-claim that suppressed the
                    # Console's limitation flag on a removals-only table.
                    "survivorship_adjusted": survivorship_adjusted(
                        symbol_sample_mode,
                        self.data_provider.has_point_in_time_membership(as_of=end_date),
                    ),
                    "metrics": metrics,
                }
                initial_cap = results.get("initial_cash", 100000)
