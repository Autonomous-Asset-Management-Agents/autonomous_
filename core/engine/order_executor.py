...
def _regime_throttled_size(
    ...
    try:
        ...
        try:  # audit mirror — observation only, never alters the flow
            setattr(context, "regime_throttle_factor", factor)
...
async def _outbox_sitzung():
    ...
    if not getattr(config.get_config(), "ORDER_OUTBOX_ENABLED", True):
        yield None
        return
    dauerhaft = (
...
async def _outbox_schliessen(stapel, art, fehler, spur) -> None:
    try:
        ...
        logging.exception("[Outbox] Ablage liess sich nicht schliessen (#3449)")
    ...
    try:
        ...
        gateway_for(client).submit(
            OrderIntent(
                ...
                halted=kill_switch.is_halted(user_id),
            ),
            request=req,
...
def halt_exempt_protective_exit(context, action) -> bool:
    ...
    ``trading_loop.py:1029``). Ein Halt soll neue Einstiege verhindern, nicht den Schutz
    ...
    * **Rotation und Trim nie.** Sie tragen ``triggered_by_stop=False``
      (``trading_loop.py:1216``, ``:1313``) — sie sind Meinungs-, keine Schutz-Exits.
...
class OrderExecutorMixin:
    ...
    async def _recover_displacement(
        ...
        try:
            ...
            re_buy_req = MarketOrderRequest(
                ...
                side=OrderSide.BUY,
                time_in_force=TimeInForce.DAY,
                client_order_id=_derived_coid(context, "displacement", 1),
            )
            ...
            await OrderExecutorMixin._sende_durchs_tor(
                client=client,
                request=re_buy_req,
                symbol=symbol_to_close,
    ...
    async def _submit_with_market_failsafe(
        ...
        *,
        ...
        try:
            ...
            return await OrderExecutorMixin._sende_durchs_tor(request=market_req, **tor)
    ...
    async def _sende_durchs_tor(
        ...
        **Was das Tor hier entscheidet — und was nicht.** Ohne Vorabentscheid blockiert
        ...
        intent = OrderIntent(
            ...
            halted=await asyncio.to_thread(kill_switch.is_halted, user_id),
        )
        ...
        async def _absenden(outbox):
            ...
            try:
                entscheidung, order = await asyncio.to_thread(
                    gateway_for(client).submit_with_result, intent, request=request
                )
        ...
        _konto = _lease.konto_schluessel(
            ...
            paper=bool(getattr(config.get_config(), "PAPER_TRADING", True)),
        )
        ...
        # #3449: erst festschreiben, dann senden. Schluessel ist der Idempotenz-Schluessel
        # der Anfrage (#3387) — eine Order, ein Schluessel, ein Eintrag. Die Sitzung
        # schliesst ihre Verbindung nach der Absendung wieder.
        if not (isinstance(coid, str) and coid):
            return await _absenden(None)
        async with _outbox_sitzung() as outbox:
            return await _absenden(outbox)
    ...
    async def _execute_tenant_order(
        ...
        try:
            ...
            _deferred_close_symbol = None  # #2712 Inc 2 (buy-first)
            _deferred_close_qty = 0.0
            ...
            if action == "SELL":
                ...
                apply_vol_targeting_audit(
                    context, forecast_vol, rr_percentile, coverage
                )
                ...
                size = _regime_throttled_size(symbol, action, size, context)
                # Issue 2935: Materialitaets-Riegel (Option A) - Tenant-Pfad
                import config as _cfg

                _cfg_obj = _cfg.get_config()
                _mat_pct = float(
                    getattr(_cfg_obj, "MATERIAL_ENTRY_MIN_PCT_OF_TARGET", 0.0)
                )
                ...
                _eg = _earnings_guard_veto(symbol, action)
                if _eg is not None:
                    ...
                    await _audit_skipped_signal(symbol, action, _eg_tag, _eg_reason)
                    return
                ...
                # BEFORE the displacement block below. Two reasons it cannot go later:
                #   1. the displacement SELL reaches the broker inside that block (before the
                ...
                if size > 0 and await self._hitl_holds_order(
                    event, context, symbol, user_id, size
                ):
                    return
                    ...
                    try:
                        ...
                        if old_pos:
                            ...
                            if old_qty > 0:
                                ...
                                _buy_first = bool(
                                    getattr(config, "DISPLACEMENT_BUY_FIRST", True)
                                )
                                buying_power_available = displacement_available_bp(
                                ...
                                if _buy_first:
                                    ...
                                    _deferred_close_symbol = symbol_to_close
                                    _deferred_close_qty = old_qty
                                else:
            ...
            if size <= 0:
                ...
                _zr = sizing_trace.get("zero_reason")
                reason_msg = (
                    f"size 0 — {_zr}"
                    if _zr
            ...
            if self.compliance_guardian:
                ...
                if action == "SELL" and _dust_floor_skips_exit(
                    event=event,
                    context=context,
                    qty=size,
            ...
            _current_price = (
                getattr(context, "current_price", None) if context else None
            )
            req = build_broker_order_request(
                symbol=symbol,
                qty=size,
                side_enum=side_enum,
                client_order_id=client_order_id,
            ...
            _is_limit_exit = isinstance(req, LimitOrderRequest) and (
                side_enum == OrderSide.SELL
            )

            # --- FIX: Kill Switch Gate ---
            # #3380 (ARC-E1.4): Ein Schutz-Exit darf am Halt nicht scheitern.
            # `risk_manager.py:751` setzt `trading_halted` und loest im selben Block
            # `:754` den Kill-Switch aus — ohne diese Freistellung waere die
            ...
            # trading_loop.py:1029). Ein Einstieg bleibt geblockt, eine Rotation oder
            # ein Trim ebenfalls — die tragen das Kennzeichen nicht
            # (trading_loop.py:1216, :1313).
            if halt_exempt_protective_exit(context, action):
                logging.warning(
            ...
            if getattr(config, "SHADOW_MODE", False):
                ...
                with tracer.start_as_current_span("broker.submit_order.live") as span:
                    ...
                    order = await OrderExecutorMixin._sende_durchs_tor(
                        client=DryRunOrderProxy(user_id),
                        request=req,
                        symbol=symbol,
                        side_enum=side_enum,
                        qty=size,
                ...
                if _deferred_close_symbol:
                    ...
                    submit_deferred_displacement_sell(
                        ...
                        user_id=user_id,
                        symbol=_deferred_close_symbol,
                        qty=_deferred_close_qty,
                        decision_id=getattr(context, "decision_id", None),
                    )
                    _deferred_close_symbol = None
            else:
                with tracer.start_as_current_span("broker.submit_order.live") as span:
                    span.set_attribute("trade.action", action)
                    for _k, _v in order_span_attributes(symbol, action, req).items():
                    ...
                    try:
                        ...
                        order = await self._submit_with_market_failsafe(
                            client=client,
                            primary_req=req,
                            symbol=symbol,
                            qty=size,
                            side_enum=side_enum,
                            user_id=user_id,
                            is_exit=_is_limit_exit,
                            decision_id=getattr(context, "decision_id", "") or "",
                            # Dasselbe Urteil wie das Halt-Tor weiter oben (#3380) —
                            # hereingereicht, damit das Tor es nicht neu faellt.
                            is_protective_exit=halt_exempt_protective_exit(
                                context, action
                            ),
                        )
                    except APIError as submit_err:
                    ...
                    max_wait_seconds = float(config.ORDER_FILL_TIMEOUT_SECONDS)
                    poll_interval = float(config.ORDER_FILL_POLL_SECONDS)
                    waited = 0
                    is_filled = False
                    if _deferred_close_symbol:
                    ...
                    if not is_filled:
                        ...
                        # oversell.
                        _failsafe_qty = exit_failsafe_remaining_qty(
                            is_limit_exit=_is_limit_exit,
                            order_qty=size,
            ...
            # Datensatz.
            await erfasse_order_endzustand(
                order=live_order if live_order is not None else order,
                symbol=symbol,
                action=action,
                ...
                order_value=abs(float(size or 0.0)) * float(curr or 0.0),
        ...
        return (
            {
                ...
                # schon geloest (`trading_loop.py:1068`), und genau diesen
                # Unterschied hat der Review zu #3452 (POLICY-01) getroffen.
                "equity": await asyncio.to_thread(
                    resolve_equity,
                    client,
                    getattr(config.get_config(), "DEFAULT_EQUITY", 0.0),
                ),
            },
            None,
        )
    ...
    async def execute_approved_order(
        ...
        if _mat_pct > 0 and action == "BUY" and order_value > 0:
            try:
                ...
                if tenant:
                    client = create_trading_client(tenant)
    ...
    async def _hitl_holds_order(
        ...
        """
        if not config.get_config().HITL_ENABLED:
            return False
        from core import hitl_gate
        ...
        if action in ["BUY", "SELL"] and not event.is_simulation:
            ...
            if await self._market_closed_blocks_order(symbol, action):
                ...
                if should_log and not event.is_simulation:
                    ...
                    _capture_outcome(context)
                return
            # #2704: the HITL autonomy-policy gate USED to sit here — 204 lines BEFORE the
            # position size is calculated. It judges an order's VALUE, so it was deciding on
            # `suggested_quantity`, which board signals leave at the 0.0 default
            # (round_table/runner.py: the board decides *whether*, the executor *how much*).
            # Every order therefore valued at 0 and routed to `unknown_value`: 13 live orders
            # queued with Qty 0 on 2026-08-06, none approvable, and the ADR-016 approval ceiling
            # skipped because it tests `_approved_qty > 0`.
            # The gate now sits in BOTH execution paths, after sizing and the portfolio decision
            # but before the first state mutation — see `_hitl_holds_order` and its two callers.

            try:
                _fallback_displacement_sell_order_id = None
                _fallback_displacement_sell_qty = 0.0
                _fb_deferred_close_symbol = None  # #2712 Inc 2 (buy-first)
                ...
                if not active_tenants:
                    ...
                    held_broker_qty = 0.0
                    # #2811: defined BEFORE the branching — the audit site far below reads
                    # it regardless of which path produced (or skipped producing) a qty.
                    _sizing_trace: dict = {}
                    _original_qty = qty
                    if _original_qty <= 0 or action == "BUY":
                        if action == "SELL" and _original_qty <= 0:
                            ...
                            try:
                                ...
                                rr_percentile = (
                                    getattr(context, "skew_percentile", None)
                                    if context
                                    else None
                                )
                                ...
                                # no-op in the sizer AND the audit mirror.
                                coverage = (
                                    getattr(context, "vote_coverage", None)
                                    if context
                                    else None
                                )
                                ...
                                max_allowed_qty = _regime_throttled_size(
                                ...
                                if _mat_pct > 0 and qty > 0 and action == "BUY":
                                    ...
                                    _order_val = qty * curr
                                    if is_immaterial_entry(
                                        _order_val, equity, _mat_pct
                                    ):
                                        await _audit_skipped_signal(
                                            symbol,
                                            action,
                    ...
                    if qty > 0:
                        ...
                        if _eg is not None:
                            ...
                            await _audit_skipped_signal(symbol, action, _eg[0], _eg[1])
                            qty = 0.0
                    ...
                    if qty > 0:
                        ...
                        if qty > 0 and await self._hitl_holds_order(
                            event,
                            context,
                            symbol,
                        ...
                        if approved:
                            ...
                            _schutz_exit = halt_exempt_protective_exit(context, action)
                            if _schutz_exit:
                                logging.warning(
                                    ...
                                    if _pc_oq > 0:
                                        ...
                                        if _fb_buy_first:
                                            ...
                                            if getattr(config, "SHADOW_MODE", False):
                                                ...
                                                _fb_sell_order = await OrderExecutorMixin._sende_durchs_tor(
                                                    client=trade_client,
                                                    request=_pc_sell_req,
                                                    symbol=_pc_symbol_to_close,
                                                    side_enum=OrderSide.SELL,
                        ...
                        _zr = _sizing_trace.get("zero_reason")
                        await _audit_skipped_signal(
                            symbol,
                            action,
