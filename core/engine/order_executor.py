...
def submit_deferred_displacement_sell(
    *, client, pm, guardian, user_id: str, symbol: str, qty: float
) -> bool:
...
class OrderExecutorMixin:
    ...
    async def _submit_with_market_failsafe(
        ...
        *,
        ...
        """
        try:
            return await asyncio.to_thread(client.submit_order, primary_req)
            ...
            return await asyncio.to_thread(client.submit_order, market_req)
    ...
    async def _execute_tenant_order(
        ...
        try:
            ...
            if action == "SELL":
                ...
                # Dark default (EARNINGS_GUARD_ENABLED=False) ⇒ the whole block is
                # skipped ⇒ byte-identical. `now` from the ENGINE clock (sim-safe).
                # Any error ⇒ fail-open (BUY proceeds); never touches SELL/risk exits.
                if action == "BUY" and getattr(
                    _cfg_obj, "EARNINGS_GUARD_ENABLED", False
                ):
                    try:
                        from core.engine.earnings_guard import earnings_guard_block
                        from core.sim.clock import engine_now

                        _eg = earnings_guard_block(
                            symbol,
                            engine_now(timezone.utc).date(),
                            int(getattr(_cfg_obj, "EARNINGS_GUARD_POST_DAYS", 2)),
                            int(getattr(_cfg_obj, "EARNINGS_GUARD_PRE_DAYS", 0)),
                        )
                    except (
                        Exception
                    ) as _eg_exc:  # noqa: BLE001 — guard never breaks the path
                        logging.warning(
                    ...
                    if not is_filled:
                        ...
                        if _failsafe_qty:
                            try:
                                ...
                                await asyncio.to_thread(
                                    client.submit_order, _mkt_failsafe
                                )
                                logging.warning(
