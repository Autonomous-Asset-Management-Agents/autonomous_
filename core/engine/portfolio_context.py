...
async def build_portfolio_context(api, compliance_guardian) -> Optional[Dict[str, Any]]:
    ...
    if intraday_timing_active:
        # #sim-clock: minutes-since-open (OpeningNoiseGuard) must use the ENGINE clock
        # (engine_now → sim time under SIM_MODE, else datetime.now — byte-identical in live),
        # NOT wall-clock time.time(). With the wall clock, an offline sim vetoed BUYs based on
        # the REAL time-of-day of the run (a run started in the real first-30m-after-open zeroed
        # every buy at score 0.795), making sim buy-pacing non-reproducible.
        from core.sim.clock import engine_now  # local: avoid import cycle

        _now = engine_now(timezone.utc).timestamp()
