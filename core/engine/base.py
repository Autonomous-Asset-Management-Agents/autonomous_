...
class BotEngine(
    ...
    def __init__(
        ...
        # Report-only decoupling (dormant, default OFF): boot the SpecialistRegistry for
        ...
        # (default) -> byte-identical. Never feeds trading (report_only + weight 0).
    ...
    def _boot_report_registry_standalone(self) -> None:
        ...
        Safety invariants:
          ...
          * SPECIALIST_ALPHA_WEIGHT (default 0.0) hard-clamps SpecialistAlphaAgent to
            weight 0 -> excluded from consensus regardless of injection -> NO trading change.
    ...
    def _build_heartbeat_status(self) -> str:
        ...
        # (api_routes.py::init_telemetry), never in this class module.
        from core.telemetry import get_service_version
            ...
            while not self._shutdown_event.is_set():
                try:
                    ...
                    verdict = _cw.evaluate_stall(
                        ...
                        strategy_running=self.strategy_running.is_set(),
    ...
    def start_live_strategy(self):
        ...
        try:
            ...
            # #2467 FIX 4c: ONLY auto-clear a STALE halt (a leftover Redis "system_halted" from a
            # previous process → this fresh KillSwitch instance has no in-process `_last_trip`). A
            # GENUINE safety trip in THIS process (panic/watchdog → `_last_trip` set) must NOT be
            # silently wiped on start — it requires an explicit operator /reset-kill-switch, so a
            # real emergency can never be auto-resumed. (Boot is Paper anyway under #2465.)
            if _ks is not None and _ks.is_halted():
                if getattr(_ks, "_last_trip", None) is None:
                    logging.warning(
                        "Kill Switch was HALTED (stale Redis state, no in-process trip). "
                        "Resetting before starting strategy — operator intent."
                    )
                    _ks.reset()
                else:
                    logging.warning(
                        "Kill Switch HALTED by a genuine safety trip (%s) — NOT auto-resetting on "
                        "start; explicit /reset-kill-switch required.",
                        (_ks._last_trip or {}).get("reason"),
                    )
