...
class MonitorLoopMixin:
    ...
    def _liquidate_for_switch(self, von, nach) -> None:
        """Schliesst den Bestand Position fuer Position ueber das Tor (#3383).

        Der Import steht in der Funktion, weil ``order_executor`` seinerseits Teile der
        Engine zieht; auf Modulebene ergaebe das einen Zyklus.
        """
        import uuid
        ...
        holen = getattr(self.api, "get_all_positions", None)
        if not callable(holen):
            logging.error(
                "Strategiewechsel %s -> %s: Broker kennt get_all_positions nicht — "
        ...
        liquidate_positions(
            gateway_for(self.api),
            positions,
            decision_id=f"switch-{von}-{nach}-{uuid.uuid4()}",
                        ...
                        if not skip_liquidation:
                            try:
                                if self.api:
                                    ...
                                    # #3383: war `close_all_positions(...)` — ohne
                                    # Symbol und ohne Menge. Ein Strategiewechsel
                                    # liquidierte damit den Bestand ohne einen einzigen
                                    # Entscheidungseintrag. Jetzt je Position ein
                                    # OrderIntent mit Grund-Code `strategy_switch`.
                                    self._liquidate_for_switch(
                                        current_strategy_name, target_strategy_name
                                    )
                        ...
                        try:
                            ...
                            if not self.live_risk_manager:
                                self.live_risk_manager = RiskManager(
                                    self.api, current_equity, clock=self.clock
                                )
                            with self.strategy_lock:
