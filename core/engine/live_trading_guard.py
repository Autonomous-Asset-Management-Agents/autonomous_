...
def assert_live_trading_config() -> None:
    ...
    # PR-1 WORM Fail-Closed: the audit dir MUST be writable before ANY live path executes —
    # including the entitlement-downgrade branch below, which writes a compensating 'disable'
    # record on the WORM chain.  A raise here is caught by __main__.py's fail-safe wrapper
    # (desktop → degrade to PAPER; cloud → exit 0).
    ...
    if CompositionRoot.get_instance().is_local_desktop:
        logger.info(
            "[LiveGuard] OSS own-account edition (is_local_desktop) — MiFID II Art. 27 SIP "
            "requirement N/A (own capital, not client orders); live data-feed check skipped."
        )
        import config

        # Respect an explicit opt-in (settings.py::ALPACA_DATA_FEED — Algo Trader Plus users
        # may set sip).
        # Only auto-correct an UNSET default that would crash the free Alpaca tier.
        if "ALPACA_DATA_FEED" not in os.environ and config.ALPACA_DATA_FEED != "iex":
            logger.warning(
                "[LiveGuard] No explicit ALPACA_DATA_FEED set; defaulting to iex for LOCAL mode "
                "to avoid a SIP-subscription broker crash. Set ALPACA_DATA_FEED=sip to override."
            )
            config.ALPACA_DATA_FEED = "iex"
        return
...
def _graceful_paper_downgrade(ent) -> None:
    ...
    # R3 Fix: Explizites Feedback für die UI, damit Kachel nicht still snappt
    config.LIVE_ENABLE_FAILED_REASON = "entitlement lapsed"
