...
async def health_check():
    ...
    _blocked = getattr(engine, "_startup_blocked_reason", None)
    startup_blocked_reason = _blocked if isinstance(_blocked, str) else None
    llm_ready = getattr(engine, "_llm_ready", False) is True
...
# ---------------------------------------------------------------------------
# #3430: Bedienstelle der Abgleich-Sperre (#3389). Ein Endpunkt fuer Konsole und Desktop —
# eine Pruefung, ein Protokolleintrag, eine Semantik fuer beide Editionen.
# ---------------------------------------------------------------------------
...
def _abgleich_dienst():
    return getattr(engine, "reconciler", None) if engine is not None else None
...
async def get_benchmark_equity():
    try:
        ...
        if (
            ...
            if not cfg_paper:
                ...
                if isinstance(_live_inc, (tuple, list)) and len(_live_inc) == 3:
                    ...
                    _live_full_metrics, _ = await _read_metrics_db_or_fallback(
                    ...
                    live_metrics, _ = await _read_metrics_db_or_fallback(
                        live_points, _baseline, False, "CH6_READ_METRICS_DB_3394"
                    )
            ...
            if not records:
                ...
                metrics, _ = await _read_metrics_db_or_fallback(
                    points, "ALL", cfg_paper, "CH6_READ_METRICS_DB_3542"
                )
                alpaca_only = {
                    "points": points,
            ...
            _full_metrics, _ = await _read_metrics_db_or_fallback(
                points, "ALL", cfg_paper, "CH6_READ_METRICS_DB_3634"
            )
            ...
            metrics, _ = await _read_metrics_db_or_fallback(
                points, _baseline, cfg_paper, "CH6_READ_METRICS_DB_3649"
            )
            reconstructed_data = {
                "points": points,
                "spy_points": spy_points,
        ...
        metrics, _ = await _read_metrics_db_or_fallback(
            points, data.get("baseline_date"), cfg_paper, "CH6_READ_METRICS_DB_3799"
        )
        return {
            "points": points,
            "spy_points": spy_points,
    ...
    try:
        ...
        if not getattr(cfg, "SIM_MODE", False):
            state = await asyncio.to_thread(
                ensure_fresh_state,
                provider_fetch(engine.data_provider, now),
                now.date(),
            )
        ...
        reading["enabled"] = bool(getattr(cfg, "REGIME_THROTTLE_ENABLED", False))
