...
def _get_inception_equity():
    ...
    try:
        ...
        for ts, eq in zip(timestamps, equities):
            ...
            backfill.append({"date": day, "equity": round(eq, 2)})
...
async def get_benchmark_equity():
    try:
        ...
        if not _cached or not _cached.get("initial_capital"):
            ...
            if not records:
                # Fresh engine / empty DB: no recorded snapshots yet. Still surface the account's
                # REAL equity history from Alpaca (period=all) so the Console + demo render the full
                # curve from inception on day one, instead of an empty chart. Only a truly empty
                # account (no Alpaca history either) returns the "no run yet" placeholder.
                # (#1790, follow-up to #1782 — the records path below is unchanged.)
