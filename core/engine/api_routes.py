...
def _get_live_cashflows(period: str = "all") -> dict:
    ...
    canonical source for CSD (cash deposit) / CSW (cash withdrawal) transfers is
    ``GET /v2/account/activities/{CSD,CSW}``. Paper and live are distinct Alpaca accounts/keys, so
    ...
    try:
        if engine is None or not engine.api:
            return {}
        from core.engine.perf_metrics import net_activities_by_date

        activities: list = []
        for atype in ("CSD", "CSW"):
            try:
        ...
        if not _cached or not _cached.get("initial_capital") or _cache_mode_stale:
            ...
            if acct_cf:
                ...
                buckets = bucket_cashflows_to_points(
                    [p.get("date") for p in points], acct_cf
                )
...
async def get_portfolio_intraday(
    ...
    try:
        ...
        if acct_cf:
            ...
            buckets = bucket_cashflows_to_points([p["date"] for p in points], acct_cf)
            for p, cf in zip(points, buckets):
                if cf:
                    p["cashflow"] = round(cf, 2)
