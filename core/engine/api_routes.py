...
def _get_inception_equity():
    ...
    try:
        ...
        from core.engine.perf_metrics import net_cashflow_by_date

        # cashflow_types is a *string* in alpaca-py 0.43.2 (Union[str, None]); a list raises a
        # pydantic ValidationError. "CSD,CSW" = cash deposits + cash withdrawals so the response's
        # `cashflow` dict lets us strip external capital from the return (#1957 deposit-KPI bug).
        hist = engine.api.get_portfolio_history(
            GetPortfolioHistoryRequest(
                period="all", timeframe="1D", cashflow_types="CSD,CSW"
            )
        )
        ...
        if not timestamps or len(timestamps) != len(equities):
            ...
            return None
        cf_by_date = net_cashflow_by_date(
            timestamps, getattr(hist, "cashflow", None) or {}
        )
        backfill: list = []
        inception_date = None
            ...
            backfill.append(
                {
                    "date": day,
                    "equity": round(eq, 2),
                    "cashflow": round(cf_by_date.get(day, 0.0), 2),
                }
            )
...
def _get_live_cashflows(period: str = "all") -> dict:
    """LIVE-path cash-flows only: ``{"YYYY-MM-DD": net_flow}`` from Alpaca (#1957).

    In LIVE mode the equity curve is built from DB ``PortfolioSnapshot`` rows (which carry NO
    cash-flow), and the Alpaca equity backfill is deliberately NOT used (PR-3: it would drag in the
    account's pre-live/paper-era equity). But the *cash-flow* amounts of the LIVE account are a
    separate, mode-safe signal — paper and live are distinct Alpaca accounts/keys — so we fetch
    them alone and align them by date onto the DB snapshot points. We read ONLY ``timestamp`` and
    ``cashflow`` here; equity is intentionally ignored. Fail-soft: ``{}`` on any error.
    """
    try:
        if engine is None or not engine.api:
            return {}
        ...
        hist = engine.api.get_portfolio_history(
            GetPortfolioHistoryRequest(
                period=period, timeframe="1D", cashflow_types="CSD,CSW"
            )
        )
        timestamps = list(getattr(hist, "timestamp", None) or [])
        return net_cashflow_by_date(timestamps, getattr(hist, "cashflow", None) or {})
    except (
        Exception
    ) as exc:  # noqa: BLE001 — cash-flow markers are best-effort; never kill the curve
        logging.warning(
            "benchmark: LIVE cash-flow fetch failed (%s) — TWR falls back to raw return "
            "for this cycle. (#1957)",
            exc,
            exc_info=True,
        )
...
def _return_metrics_from_points(points: list) -> dict:
    ...
    return compute_cashflow_adjusted_returns(dates, equity, cashflows)
...
async def get_benchmark_equity():
    try:
        ...
        if not _cached or not _cached.get("initial_capital") or _cache_mode_stale:
            ...
            if cfg_paper:
                ...
                # #1957/#2717 LIVE: DB-snapshot points carry NO cash-flow, and the snapshot series
                # is SPARSE (a day the engine was off has no point). Fetch the live account's
                # CSD/CSW flows (equity intentionally ignored — no paper-era pollution) and bucket
                # each into the first point on/after its date, so a deposit that landed in a
                # snapshot gap still lands in the sub-period that spans it instead of being dropped
                # (which left the deposit-inflated TWR on screen).
                live_cf = await asyncio.to_thread(_get_live_cashflows)
                if live_cf:
                    from core.engine.perf_metrics import bucket_cashflows_to_points

                    buckets = bucket_cashflows_to_points(
                        [p.get("date") for p in points], live_cf
                    )
