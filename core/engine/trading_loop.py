    ...
    async def live_trading_loop(self):  # noqa: C901
        ...
        while self.strategy_running.is_set() and not self._shutdown_event.is_set():
            ...
            try:
                ...
                # cycle. ROUND_TABLE_TOP_K_EVAL<=0 → no-op → byte-identical (default is 20 = ACTIVE
                # since the funnel armed; an earlier revision of this comment claimed "<=0 (default)"
                # — stale, fixed with #2672). It falls back to
                ...
                if _top_k > 0:
                    try:
                        ...
                        if _held is not None and not _stale:
                            ...
                            _xsec = active_cross_section(
                                _store, datetime.now(timezone.utc)
                            )
                    ...
                    if is_lstm:
                        ...
                        if _graph is not None:
                            ...
                            if _otel_trace is not None:
                                try:
                                    _cycle_span = _obs_tracer.start_span(
                                        "trading.cycle"
                                    )
