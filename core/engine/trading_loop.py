...
class TradingLoopMixin:
    ...
    async def live_trading_loop(self):  # noqa: C901
        ...
        while self.strategy_running.is_set() and not self._shutdown_event.is_set():
            ...
            # #2046: durable entry-time on desktop. The #2042 reconcile hook runs only on
            # the tenant path (order_executor), which the desktop/OSS fallback never enters
            # — so days_held would stay 0. Reconcile the active strategy's PM once/session.
            ...
            try:
                ...
                # Dormant by default (GATEKEEPER_PORTFOLIO_CONTEXT_ENABLED=False ⇒ None ⇒
                # the runner sees an empty context = today's byte-identical behaviour). When
                # armed, the snapshot is built ONCE here (not per symbol) and injected into
                # every symbol's state, so all parallel evaluations share one consistent
                # portfolio view. build_portfolio_context never raises (fail-open to None).
                    ...
                    graph_states.append(
                        {
                            ...
                            "error": None,
                            "vix": (
                                getattr(self, "current_market_data", {}).get("vix")
