...
This module does NOT execute anything. It is called every cycle from
``trading_loop._run_position_stop_checks`` (Increment 2); execution stays behind the
compliance gates there. Keeping it pure makes the stop logic unit-testable in the
deterministic agent loop without broker mocks. #3632: ``analyze_exit`` is the ONE exit
authority of the engine (Smart Exit retired).
"""
            ...
            plans.append(
                {
                    ...
                    # #3180: carry the risk/opinion tier so the loop can apply the
                    # consensus-retention gate to OPINION exits (take-profit / composite)
                    # only — RISK stops (hard/trailing/loss) are never gated. Fail-safe
                    # default 'risk' if the field is somehow absent.
