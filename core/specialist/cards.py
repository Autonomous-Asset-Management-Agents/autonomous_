...
def build_summary(
    *,
    ...
    # #2772: the report summary is a NEWS/signal digest only — it no longer appends an
    # "Overall outlook: <rec> (sentiment N/100)" verdict. That per-agent sentiment could contradict
    # the authoritative Round-Table decision shown on the same card (e.g. summary "hold" vs board
    # "SELL"), which read as the report disagreeing with itself. `recommendation`/`sentiment_score`
    # stay in the signature for call-site compatibility.
    return lead
