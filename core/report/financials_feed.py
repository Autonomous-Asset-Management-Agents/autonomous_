...
* **Report-only.** Nothing here is read by the decision path. (The SpecialistAlphaAgent's
  *score* does vote — ``SPECIALIST_ALPHA_WEIGHT`` defaults to 0.40 since #2839; #3261.)
...
def _resolve_cache_dir() -> str:
    """Where the WRITABLE fundamentals cache lives.

    #3145 (2b): on the desktop the engine CWD is the app bundle, which the
    bootstrap REPLACES on every upgrade — so a cache under ``data/`` there is wiped
    each update. When the desktop launcher sets ``AAA_USER_DATA_DIR`` (the same
    persistent root as aaagents.db / the audit logs) the cache goes there and
    survives upgrades; cloud/dev (env unset) stay byte-identical CWD-relative.
    """
    user_data_dir = os.environ.get("AAA_USER_DATA_DIR", "").strip()
    if user_data_dir:
        try:
            os.makedirs(user_data_dir, exist_ok=True)
            return os.path.join(user_data_dir, "financials_cache")
        except OSError:  # boot must never break on a path issue
            pass
    return os.path.join("data", "financials_cache")
...
def _pb(close: Any, shares: Any, equity: Any) -> Optional[float]:
    """Price/Book = market cap / equity = ``close * shares / equity`` (#3146).

    ``None`` without a price/shares or a non-positive book value (an honest gap).
    """
    if close is None or shares is None or equity in (None, 0) or float(equity) <= 0:
        return None
    return float(close) * float(shares) / float(equity)
...
def _liabilities_or_identity(reported: Any, assets: Any, equity: Any) -> Any:
    """#3337: ``liabilities``, ersatzweise aus ``assets - equity``.

    Der gemeldete Wert hat Vorrang. Die Identitaet greift nur, wenn er fehlt UND
    beide Seiten vorliegen — fehlt eine, bleibt es ``None`` (keine Erfindung, der
    bestehende Fail-open im ``ValuationAgent`` gilt dann unveraendert weiter).
    Negatives Eigenkapital ist kein Sonderfall: ABBV 133,96 Mrd - (-3,27 Mrd) =
    137,23 Mrd.
    """
    ...
    if assets is None or equity is None:
        return None
    try:
        return float(assets) - float(equity)
...
def derive_fundamentals(
    ...
    *,
    ...
    # #3337: Bilanzidentitaet als Rueckfall. SEC_TAG_MAP bildet die Verbindlichkeiten
    # auf EINEN Tag ab ("Liabilities"), ohne Rueckfallvariante — viele Emittenten weisen
    # aber keine Summenzeile aus, sondern nur Assets und StockholdersEquity. Gemessen
    # ueber den vollstaendigen Cache (476 Titel): 158 ohne total_liabilities, davon 128
    # aus assets - equity ableitbar. Folge des Fehlens: _debt_to_equity (agents.py:1997)
    # liefert None, und der Aufrufer wertet None als GESUNDE Verschuldung
    # (agents.py:2261) — der Waechter des armierten ValuationAgent greift dann nie.
    # Der GEMELDETE Wert hat Vorrang; fehlt equity ebenfalls, bleibt es None (nichts
    # wird erfunden). Liabilities = Assets - Equity ist eine Identitaet, keine Schaetzung.
    total_liabilities = _liabilities_or_identity(
    ...
    # #3275 (Part 1): expose the prior-period LEVELS additively (not just the YoY
    # ratios). The full 9-point Piotroski F-score needs prior-year balance-sheet /
    # share / cash-flow levels (ΔROA, ΔLeverage, ΔCurrent-Ratio, ΔTurnover, no
    # dilution). These use EXACTLY the same ``prior`` record that already drives the
    # YoY above — purely additive, so no existing consumer breaks (no consumer
    # asserts the exact key set; verified against the report adapter + fact_set).
    p_gross_profit = _stmt_value(pfinc, "income_statement", "gross_profit")
    p_diluted_shares = _stmt_value(pfinc, "income_statement", "diluted_average_shares")
    p_total_assets = _stmt_value(pfinc, "balance_sheet", "assets")
    # prior total liabilities: needed for the Piotroski ΔLeverage point (leverage =
    # total_liabilities / total_assets). Same additive `prior`-record pattern.
    p_total_liabilities = _stmt_value(pfinc, "balance_sheet", "liabilities")
    # #3337: dieselbe Identitaet fuer den Vorjahressatz — ohne ihn faellt der
    # Piotroski-Delta-Leverage-Punkt aus, auch wenn der aktuelle Wert vorliegt.
    p_total_liabilities = _liabilities_or_identity(
        p_total_liabilities,
        p_total_assets,
        _stmt_value(pfinc, "balance_sheet", "equity"),
    )
    p_current_assets = _stmt_value(pfinc, "balance_sheet", "current_assets")
    p_current_liabilities = _stmt_value(pfinc, "balance_sheet", "current_liabilities")
    p_operating_cash_flow = _stmt_value(
        pfinc, "cash_flow_statement", "net_cash_flow_from_operating_activities"
    )
    ...
    return {
        ...
        # #3275 (Part 1): prior-period LEVELS (same `prior` record as the YoY above)
        "prior_revenue": p_revenue,
        "prior_net_income": p_net_income,
        "prior_gross_profit": p_gross_profit,
        "prior_total_assets": p_total_assets,
        "prior_total_liabilities": p_total_liabilities,
        "prior_current_assets": p_current_assets,
        "prior_current_liabilities": p_current_liabilities,
        "prior_diluted_shares": p_diluted_shares,
        "prior_operating_cash_flow": p_operating_cash_flow,
        # valuation (price-dependent — None without a close)
        "pe": pe,
        "ps": _ps(close, shares, revenue),
        "pb": _pb(close, shares, total_equity),
    }
