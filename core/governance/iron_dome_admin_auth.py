...
def require_iron_dome_admin(
    ...
    Raises 501 if the admin token is not configured (fail-closed); 403 if the request
    is not from a loopback/private address or the token is missing/invalid.
    """
    expected = os.environ.get(ADMIN_TOKEN_ENV, "")
    if not expected:
        # DEF-2: Desktop edition single-tenant posture. Administrative override is
        # intentionally disabled unless an admin token is explicitly provided.
        raise HTTPException(
            status_code=501,
            detail=f"Desktop edition single-tenant posture: Iron Dome admin override is disabled. Set {ADMIN_TOKEN_ENV} to enable.",
        )
