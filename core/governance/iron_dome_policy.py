...
# ADR-C01 — MAX_ORDER_VALUE_CEILING (ratified #1599; basis revised #3219)
#   Basis: single-order notional ceiling — an OPERATIONAL blast-radius limit, not a
#   regulatory duty. The MiFID II / ESMA anchor (large-in-scale / extended-reporting)
#   requested in #1599 was never delivered and is withdrawn: Art. 57 governs position
#   limits for COMMODITY DERIVATIVES, large-in-scale is a pre-trade transparency
#   WAIVER (not a cap), and DISCLAIMER.md states MiFID II investment-firm duties do
#   not bind an own-account operator.
#   Rationale: 10x the 10,000 per-order operating default (config.py). The ceiling
#   itself stays ratified and is frozen by test_iron_dome_caps_ratified.py.
...
STRICT_DEFAULT = IronDomePolicy(
    max_daily_trades=10,  # settings.py#L220 (the tighter of 10 / 50)
    wash_trade_window_seconds=60,  # compliance.py#L60
    max_order_value=10_000.0,  # settings.py#L217 (ADR-C01)
...
def _operating_default(attr: str, strict_value: Any) -> Any:
    """Der konfigurierte Betriebswert als Rückfallebene (#3220/#3221).

    Drei Ebenen, sauber getrennt:

    * ``config`` (env / ``setup.json``, kundenseitig über die Settings-Registry
      setzbar) = **Betriebs-Default**
    * gespeicherte Iron-Dome-Policy = **Admin-Übersteuerung zur Laufzeit**
    * ``*_CEILING`` = **ratifizierte, unverrückbare Obergrenze**

    Vorher fiel ein gültiger Store ohne diesen Key auf ``STRICT_DEFAULT`` zurück.
    Da ``apply_policy`` bei JEDEM ``/start-live`` läuft, überschrieb das den
    konfigurierten Betriebswert lautlos — eine kundenseitige Einstellung wäre
    wirkungslos geblieben.

    Fail-closed bleibt: ist der Wert unlesbar oder untypisierbar, gilt weiterhin
    die strenge Konstante (CLAUDE.md §5.6 — die Abweichung wird geloggt).
    """
    try:
        cfg = __import__("config", fromlist=[attr])
        v = _coerce(getattr(cfg, attr, None), strict_value)
    except (
        Exception
    ) as exc:  # noqa: BLE001 — ein Config-Fehler darf nie den Boot brechen
        logger.warning(
            "IronDomePolicy: Betriebswert %s unlesbar (%s) — strenger Default %s gilt.",
            attr,
            exc,
            strict_value,
        )
        return strict_value
    return strict_value if v is None else v


def _clamp_ceiling(value: Any, default: Any, ceiling: Any) -> Any:
    # #3220/#3221: auch der DEFAULT wird geklemmt. Solange er immer STRICT_DEFAULT war,
    # lag er per Konstruktion unter dem Ceiling; seit er aus der Config kommen kann
    # (_operating_default), muss die ratifizierte Obergrenze auch diesen Pfad binden —
    # sonst hebelte ein zu grosser Konfigurationswert sie aus.
    v = _coerce(value, default)
    return min(default, ceiling) if v is None else min(v, ceiling)
    ...
    **clamped** to its immutable hard-floor cap (ADR-SEC-06 §2) — a submitted value can
    ...
    return IronDomePolicy(
        max_daily_trades=_clamp_ceiling(
            ...
            _operating_default(
                "COMPLIANCE_MAX_DAILY_TRADES", STRICT_DEFAULT.max_daily_trades
            ),
        ...
        max_order_value=_clamp_ceiling(
            ...
            _operating_default(
                "COMPLIANCE_MAX_ORDER_VALUE", STRICT_DEFAULT.max_order_value
            ),
