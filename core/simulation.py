...
try:
    from config import COMMISSION_PER_TRADE, SLIPPAGE_PERCENT
except ImportError:
    # ⚠ ADR-COST-01: these MUST track config.py/config.oss.py. A second, silently
    # divergent copy is worse than the original wrong number, because the config would say
    # one thing while every backtest charged another — and nothing would flag it.
    # tests/unit/test_cost_model_honesty.py pins them together.
    SLIPPAGE_PERCENT = 0.0002  # 2bps/side — half-spread, S&P 500 large caps
    COMMISSION_PER_TRADE = 0.0  # Alpaca charges no commission on US equities
...
class RealisticSimulationClient:
    ...
    def get_open_position(self, symbol: str) -> Optional[Dict]:
        """Alias for get_position to match alpaca-py client interface."""
