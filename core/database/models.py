...
class Decision(Base):
    ...
    # Größe erzeugte ("conviction" | "a" | "b") und die Ziel-Basis-Gewichtung (vor Caps).
    # Ohne diese Felder ist risk_size_scaler modus-mehrdeutig, seit clean-weight "b" Default ist.
    # Additiv/nullable — Legacy-Zeilen bleiben None; der DB-Writer lässt unbekannte Keys fallen.
    sizing_mode = Column(String)
    sizing_target_weight = Column(Float)
    # #3619 Teil 1: welcher Deckel die Groesse setzte und sein Dollarwert (None = das
    # Zielgewicht). Additiv/nullable; Desktop-Bootstrap zieht die Spalten nach (#3373).
    sizing_binding_limit = Column(String)
    sizing_cap_value = Column(Float)
...
class PortfolioMetricsReport(Base):
    """#3406 ARC-E5.5: Abgelegter Datensatz für Berichtskennzahlen,
    rückführbar auf decision_id.
    """

    __tablename__ = "portfolio_metrics_reports"

    id = Column(String, primary_key=True)
    timestamp = Column(DateTime(timezone=True), index=True)
    start_date = Column(DateTime(timezone=True))
    end_date = Column(DateTime(timezone=True))
    baseline = Column(String)  # 1M, 1Y, ALL
    paper_trading = Column(Boolean)

    # Kennzahlen
    initial_capital = Column(Float)
    final_equity = Column(Float)
    twr_pct = Column(Float)
    max_drawdown_pct = Column(Float)
    win_rate_pct = Column(Float)

    # Rückführbarkeit
    decision_ids = Column(JSON_TYPE, default=list)
    order_ids = Column(JSON_TYPE, default=list)
    coverage_gap_map = Column(JSON_TYPE, default=dict)
...
class DecisionOutcome(Base):
    ...
    (DEFAULT ON since #2840 — local DB, no egress; #3261). Desktop: bootstrapped via
    ...
    gatekeeper_reason = Column(Text)
    votes_json = Column(JSON_TYPE)
    # #3618: risk-off damping audit — applied factor + undamped consensus (None =
    # not damped). Additive/nullable; alembic 0017; desktop bootstrap heals (#3373).
    regime_damp_factor = Column(Float)
    consensus_undamped = Column(Float)
...
class LemonSqueezyLicense(Base):
    """GTM-2 (#1809) T7: an offline license token minted after a paid Lemon Squeezy order.

    One row per successful ``order_created`` (or ``subscription_created``). CLOUD-ONLY — the
    Lemon Squeezy webhook that writes it runs only on Cloud Run; the desktop never touches
    this table (it verifies the pasted token offline). Deliberately SEPARATE from
    ``entitlement_tokens`` (the Stripe path) so neither destabilises the other (SoC).

    ``order_identifier`` is the Lemon Squeezy order UUID: it is BOTH the idempotency key
    (webhook retries must not re-mint) AND the capability the success page presents to fetch
    the token (unguessable UUID, so it is not enumerable like a numeric order id).

    BORA: portable, SQLite-safe types ONLY. The raw licensee id is never stored (only
    ``issued_to_hash``); the signed token carries the authoritative expiry. Never log ``token``.
    """

    __tablename__ = "lemonsqueezy_licenses"

    id = Column(Integer, primary_key=True, autoincrement=True)
    # LS order UUID — idempotency key + the success-page lookup capability (unguessable).
    order_identifier = Column(String, nullable=False, unique=True)
    issued_to_hash = Column(String, nullable=False)
    tier = Column(String, nullable=False)
    token = Column(Text, nullable=False)
    expires_at = Column(DateTime(timezone=True))
