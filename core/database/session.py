...
def _guard_cloud_sqlite_fallback() -> None:
    ...
    if CompositionRoot.is_cloud.fget(None):
        raise RuntimeError(
...
async def ensure_local_db_ready() -> None:
    ...
    # #3468: erst NACH der Schema-Heilung — vorher scheiterten die abgelegten Saetze
    # genauso wie beim ersten Mal. Ausserhalb der Sperre und fehlertolerant: Das
    # Nachlesen darf den Start nie anhalten.
    try:
        from core.database.fallback_nachlesen import (
            ablage_verzeichnis,
            automatisch_zulaessig,
            lies_nach,
        )

        if automatisch_zulaessig():
            await lies_nach(engine, ablage_verzeichnis())
    except Exception as exc:  # noqa: BLE001 — Beobachtung, nie den Start brechen
        logging.getLogger(__name__).warning(
            "Fallback-Nachlesen beim Start fehlgeschlagen: %s (#3468)", exc
        )
