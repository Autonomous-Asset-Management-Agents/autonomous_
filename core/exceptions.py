"""
exceptions.py — Custom Exception Hierarchy for the ai_trading_bot.
...
Hierarchy:
    ...
    ├── TradingHaltedError      — An order was held back at the send point (#3500)
...
class TradingBotError(Exception):
    """Base exception for all ai_trading_bot errors.
...
class TradingHaltedError(TradingBotError):
    """Eine Order wurde an der Absendestelle zurueckgehalten — Tor-Ablehnung, keine
    Schreibberechtigung (Lease) oder Abgleich-Sperre (#3500, Review zu #3495).

    Die Meldung beginnt immer mit ``TRADING HALTED:``. Unterklasse von ``Exception``:
    jedes bestehende ``except Exception`` greift unveraendert; neue Aufrufer koennen einen
    Halt gezielt fangen, ohne jede andere Ausnahme mitzufangen.
    """
