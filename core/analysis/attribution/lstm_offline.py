...
class OfflineLstmStrategy:
    ...
    def cross_section_for(self, current_time: Any) -> Dict[str, float]:
        """#3228 Phase 2: public accessor for the per-date offline LSTM cross-section
        ({symbol: raw_score}, descending). The replay records this into the live
        ``LstmPanelStore`` per date so the REAL ``LSTMSignalAgent._vote_on_rank``
        (which reads the panel, not ``evaluate_for_symbol``) can produce a
        cross-sectional standing offline instead of abstaining."""
        return self._cross_section_for(current_time)
