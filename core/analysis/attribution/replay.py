...
EXCLUDED_AGENTS: Dict[str, str] = {
    ...
    "SpecialistAlphaAgent": (
        "external LLM behind the StockSpecialistRegistry (Gemini), non-deterministic "
        "and network-bound — an offline benchmark must not call it. Same class as "
        "NewsSentimentAgent above; activation needs a separate evidence source. "
        "NOTE (corrected 2026-09-08): the previous reason claimed 'DORMANT "
        "(default_weight 0.0) — excluded from consensus in production'. Both halves "
        "are false since #3195/#3164: SPECIALIST_ALPHA_WEIGHT is 0.40 (config.py) and "
        "the agent is NOT in NON_DIRECTIONAL_AGENTS (consensus.py:184), i.e. it is a "
        "real directional voter carrying 0.40 of 2.30 armed direction weight. The "
        "exclusion still stands — but on the LLM ground, not a dormancy that ended."
    ),
}
...
def build_symbol_eval_state(
    ...
    quality_reference: Optional[List[float]] = None,
    quality_reference_date: Optional[str] = None,
) -> Dict[str, Any]:
    ...
    return {
        ...
        # #3275: composite-quality channels — this symbol's feed-derived composite
        # today + the PREVIOUS session's cross-section QualityAgent ranks against
        # (agents.py QualityAgent, AC-7). Default None so a run without a quality
        # table stays byte-identical (QualityAgent — dark by default — abstains).
        "quality_score": None if quality_score is None else float(quality_score),
        "quality_reference": quality_reference,
        "quality_reference_date": quality_reference_date,
    }
...
# ---------------------------------------------------------------------------
# #3275 Part 4 — FEED-derived quality cross-section (QualityAgent attribution)
# ---------------------------------------------------------------------------


def build_quality_table(provider: "ReplayBarsProvider", start: str, end: str):
    """Build the per-date composite-quality cross-section from the PIT FEED.

    Unlike IV/RR (external corpus tables), the composite is derivable purely from the
    fundamentals feed (Novy-Marx gross profitability + 9-point Piotroski) AS OF each
    replay date — no look-ahead, no external file. Returns ``(quality_by_date,
    sorted_dates)`` with ``quality_by_date[normalized_ts][SYMBOL] = composite ∈ [0,1]``.
    FAIL-SAFE: any error degrades to ``({}, [])`` so the caller stays byte-identical
    (QualityAgent abstains honestly) — never raises into the run.
    """
    try:
        from core.quality_score import composite_quality
        ...
        reader = CachedFundamentalsReader()
        by_date: Dict[pd.Timestamp, Dict[str, float]] = {}
        for d in dates:
            row: Dict[str, float] = {}
            for sym in provider.symbols:
                try:
                    # close=None: the composite is price-independent (balance-sheet /
                    # income only), so pe/ps are never read.
                    fund = reader.get_fundamentals(sym, d.date())
                    if not fund:
                        continue
                    c = composite_quality(dict(fund))
                    if c is not None:
                        row[sym.upper()] = float(c)
                except Exception as e:
                    logger.warning("Quality parse fail %s: %s", sym, e)
                    continue
            if row:
                by_date[d] = row
        return by_date, sorted(by_date)
    except Exception as exc:  # noqa: BLE001 — degrade to the no-quality path
        logger.warning(
            "Quality panel build failed — QualityAgent abstains offline: %s",
            exc,
            exc_info=True,
        )
        return {}, []
...
def quality_reference_for(sorted_dates, quality_by_date, ts):
    """``(reference_list, reference_date_iso)`` = the quality cross-section of the LAST
    session STRICTLY BEFORE ``ts``. QualityAgent ranks today's composite against the
    previous session's distribution (agents.py QualityAgent, AC-7 no-look-ahead).
    ``(None, None)`` if no earlier session exists.
    """
    day = pd.Timestamp(ts).normalize()
    i = bisect.bisect_left(sorted_dates, day)
    if i == 0:
    ...
    ref = [
        float(v)
        for v in quality_by_date.get(prev, {}).values()
        if v is not None and pd.notna(v)
    ]
    return (ref or None), prev.date().isoformat()
...
async def replay_votes(
    ...
    *,
    ...
    quality_table: Optional[tuple] = None,
    ...
    # #3275: FEED-derive the quality cross-section for the window when the QualityAgent
    # is armed and no explicit table was supplied. Gated on the enable flag so the
    # default (dark) path builds nothing and stays byte-identical. FAIL-SAFE inside
    # build_quality_table (degrades to no panel → QualityAgent abstains honestly).
    if quality_table is None:
        try:
            from core.round_table.agents import _quality_agent_enabled

            if _quality_agent_enabled():
                quality_table = build_quality_table(provider, start, end)
        except Exception as exc:  # noqa: BLE001 — never break the run on setup
            logger.warning(
                "Quality panel auto-build skipped (%s) — QualityAgent abstains.",
                exc,
                exc_info=True,
            )
    ...
    try:
        for symbol in provider.symbols:
            ...
            for ts, bar in window.iterrows():
                ...
                q_val = q_ref = q_ref_date = None
                if quality_table is not None:
                    _q_by_date, _q_dates = quality_table
                    _day = pd.Timestamp(ts).normalize()
                    q_val = _q_by_date.get(_day, {}).get(symbol.upper())
                    q_ref, q_ref_date = quality_reference_for(_q_dates, _q_by_date, ts)
                state = build_symbol_eval_state(
                    ...
                    quality_score=q_val,
                    quality_reference=q_ref,
                    quality_reference_date=q_ref_date,
                )
                for agent in agents:
                    vote = await agent.vote(state)
                    score_val = (
                        float(vote.score) if vote.score is not None else float("nan")
                    )
                    rows.append(
                        (
                            pd.Timestamp(ts),
