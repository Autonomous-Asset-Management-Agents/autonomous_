...
class LatencyWatchdog:
    ...
    def _sim_mode(self) -> bool:
        """#2632 (P2b): is this process an offline Sim-Day run?

        Delegates to the single shared definition (``core.sim.is_sim_mode``): call-time,
        env-first, and fail-safe to **live** — a config that cannot be read must never
        disable a safety component. Kept as a method so it stays the patchable seam for
    ...
    def start(self):
        ...
        # #2632 (P2b) — SIM network isolation. The sim runner boots through
        # ``BotEngine.start_live_strategy`` (core/engine/base.py), which starts this
        # watchdog; its ping loop then hits the REAL Alpaca clock endpoint (401 against
        # paper-api under sim credentials). Two reasons that must not happen offline:
        #   1. the pinned corpus is meant to be the ONLY data source of a sim run;
        #   2. determinism — a slow/failing ping can trip the halt (_ping_loop → halt after
        #      N breaches), so two arms of an A/B could diverge on network weather instead
        #      of on the parameter under test. An A/B that can be decided by a socket is
        #      worthless.
        # The PASSIVE path (record_passive_latency) is untouched: it only observes latencies
        # that a real order actually produced, and the sim submits through the virtual broker.
        if self._sim_mode():
            self.logger.info(
                "Skipping LatencyWatchdog start: SIM_MODE (offline sim run — the pinned "
                "corpus is the only data source; no Alpaca ping, deterministic run)."
            )
            return
