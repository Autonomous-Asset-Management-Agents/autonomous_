...
#   - Nodes: _fetch_context_node → _compute_features_node → _run_strategy_node
...
class SymbolEvalState(TypedDict):
    ...
    # #2389 (dormant default): last-row TA-feature SCALARS from _compute_features_node
    # (rsi_14/atr_14d/macd_hist/bb_position/vol_anomaly/... — §5.9: Dict[str, float],
    # never the DataFrame). MUST be a declared channel here or LangGraph drops it before
    # run_round_table maps it into the MiFID DecisionContext. None when the flag is off
    # (default), history is missing, or the node failed open → runner keeps today's
    # neutral default snapshot.
    features: Optional[Dict[str, float]]
    # #1949 (RTR-2, dormant default): per-cycle market VIX value + regime label from
    # MarketRegimeModel (monitor_loop → current_market_data), threaded by the trading
    # loop. Scalars only (§5.9). MUST be declared channels: LangGraph drops undeclared
    # input keys before any node runs (verified against langgraph==1.0.10 — the
    # previously undeclared top-level "vix" the loop emitted NEVER reached the agents).
    # The producer (trading_loop._regime_conditioner_state_keys) emits None for BOTH
    # while REGIME_CONDITIONER_ENABLED is off, so agents observe exactly the old
    # dropped-key behaviour. (The flag is DEFAULT ON since #2516 / ADR-018; #3261.)
    vix: Optional[float]
    regime: Optional[str]
    # RTR-1 (#1948; NEWS_SENTIMENT_NLP_ENABLED DEFAULT ON by owner activation, #3261):
    # per-symbol point-in-time headlines
    # [{"title": str, "published_utc": ISO str, "source": str}], filled by the
    # trading loop from the FREE Google-News-RSS producer (core/nlp/headlines.py)
    # ONLY when NEWS_SENTIMENT_NLP_ENABLED — the producer keeps solely items with
    # published_utc <= current_time (no look-ahead). MUST be a declared channel
    # here or LangGraph drops it before NewsSentimentAgent reads it (same
    # precedence as _portfolio_context/features above). Absent/None when off.
    news_headlines: Optional[List[Dict[str, Any]]]
    # #2672 (DEFAULT ON — owner decision 2026-08-05, #3261): per-symbol broker position
    # context from the
    # trading loop's shared per-cycle snapshot (_fetch_position_snapshot →
    # _position_context_state_keys). Scalars only (§5.9). MUST be declared
    # channels — langgraph==1.0.10 drops undeclared input keys before any node
    # runs (same precedence as vix/regime above). Producer emits None for ALL
    # five while ROUND_TABLE_POSITION_CONTEXT_ENABLED is off (it is DEFAULT ON, #3261). Fail-closed contract: in_position is only
    # meaningful when position_context_confirmed is True; an UNCONFIRMED book
    # (broker outage) keeps the position fields None ("unknown", never "flat").
    in_position: Optional[bool]
    position_qty: Optional[float]
    position_avg_price: Optional[float]
    unrealized_pnl: Optional[float]
    position_context_confirmed: Optional[bool]
    # #3038 (TRD-10; DEFAULT ON since #3057, #3261): the symbol's own ATM-30d implied
    # volatility plus the PREVIOUS day's cross-section it is ranked against.
    # Scalars and a flat float list only (§5.9). MUST be declared channels —
    # langgraph==1.0.10 drops undeclared input keys before any node runs; that is
    # exactly how the old top-level "vix" never reached the agents (see above).
    # Were it to happen here it would fail silently in the worst way: the agent
    # would abstain on every symbol, the sim would show a clean null result, and
    # nobody would know why. A unit test pins the declaration.
    # `implied_vol_reference_date` is not decoration — without it the audit trail
    # cannot say afterwards WHICH distribution a symbol was scored against
    # (MiFID II). Producer emits None for all three while the flag is off.
    implied_vol: Optional[float]
    implied_vol_reference: Optional[List[float]]
    implied_vol_reference_date: Optional[str]

    # #3095 (b): the UpsideSkewAgent's declared channels — 25Δ risk-reversal (today),
    # the previous session's cross-section (rank reference, AC-7), and its date.
    # Producer (trading_loop._risk_reversal_state_keys) emits None for all three
    # while UPSIDE_SKEW_AGENT_ENABLED is off; declared here so LangGraph keeps them.
    risk_reversal: Optional[float]
    risk_reversal_reference: Optional[List[float]]
    risk_reversal_reference_date: Optional[str]

    # #3275: the QualityAgent's declared channels — the symbol's composite-quality
    # score today (Novy-Marx gross profitability + 9-point Piotroski, feed-derived),
    # the PREVIOUS session's cross-section it is ranked against (AC-7), and its date.
    # Producer (trading_loop._quality_state_keys) emits None for all three while
    # QUALITY_AGENT_ENABLED is off; declared here so LangGraph keeps them (undeclared
    # keys are dropped before any node runs — same precedence as the channels above).
    quality_score: Optional[float]
    quality_reference: Optional[List[float]]
    quality_reference_date: Optional[str]
...
# #2389: OHLCV-Fenster mit dem DrawdownGuardAgent TEILEN (beide 30 Tage) — der
# data_provider cached per `{symbol}_{end_date}_{days}`, so kostet der zweite
# Abruf im selben Zyklus nur einen Dict-Lookup statt eines zweiten Feeds
# (Cycle-Latenz ist bereits median ~14,8 s, Plan §12.2).
_FEATURE_WINDOW_DAYS = 30

# compute_technical_features erwartet die kapitalisierten OHLCV-Spalten.
_FEATURE_OHLCV_COLUMNS = ("Open", "High", "Low", "Close", "Volume")


def _feature_scalars_from_ohlcv(df: Any, symbol: str) -> Optional[Dict[str, float]]:
    """Reduziert einen OHLCV-DataFrame auf die letzte Zeile der TA-Features als
    reine SKALARE (§5.9: Dict[str, float], niemals der DataFrame selbst).

    Nicht-finite Werte (NaN aus zu kurzer Historie, z.B. ma200 im 30d-Fenster)
    werden verworfen — der Runner fällt für fehlende Keys feld-weise auf die
    heutigen Default-Werte zurück (fail-open). Zusätzlich wird aus `atr_pct`
    (ATR als Preis-Anteil) das absolute `atr_14d` am aktuellen Close abgeleitet —
    das Feld, das Sizer und MiFID-Record konsumieren.
    """
    import math
    ...
    # Provider-Frames können lowercase-Spalten tragen (der DrawdownGuardAgent
    # toleriert beide Schreibweisen) — hier auf die erwarteten Namen normieren.
    rename = {
        col: col.capitalize()
        for col in df.columns
        if col.capitalize() in _FEATURE_OHLCV_COLUMNS and col != col.capitalize()
    }
    if rename:
        df = df.rename(columns=rename)
    if any(col not in df.columns for col in _FEATURE_OHLCV_COLUMNS):
        logger.warning(
            "_compute_features_node: OHLCV history for %s lacks required columns "
            "(%s) — features skipped (fail-open to default snapshot).",
            symbol,
            list(df.columns),
        )
        return None

    feat_row = compute_technical_features(df).iloc[-1]
    ...
    # Absolutes 14d-ATR für Sizer + Decision-Record: features.py liefert ATR nur
    # als Preis-Anteil (atr_pct) — am aktuellen Close in den Absolutwert umrechnen.
    close = float(df["Close"].iloc[-1])
    if "atr_pct" in scalars and close > 0:
        scalars["atr_14d"] = scalars["atr_pct"] * close

    return scalars or None


async def _compute_features_node(state: SymbolEvalState) -> SymbolEvalState:
    """
    Node 1b (#2389): TA-Feature-Snapshot — gegated via DESKTOP_FEATURE_SNAPSHOT_ENABLED.

    Holt die 30d-OHLCV-Historie über den data_provider der aktiven Strategie
    (Fenster mit dem DrawdownGuardAgent geteilt → Provider-Cache-Hit), rechnet
    compute_technical_features und legt NUR die Last-Row-Skalare in
    state["features"] (§5.9). Der Round-Table-Runner mappt sie in den
    MiFID-DecisionContext.

    Fehler-Isolation: ein TA-/Feed-Fehler darf NIE die Decision kippen — jeder
    Fehlpfad geht fail-open auf state["features"]=None (heutige Default-Snapshot)
    mit logging.warning (§5.6), niemals state["error"].
    """
    if state.get("error"):
        return state
    ...
    try:
        ...
        enabled = bool(getattr(get_config(), "DESKTOP_FEATURE_SNAPSHOT_ENABLED", False))
    except Exception as exc:
        logger.warning(
            "_compute_features_node: config unreadable (%s) — features skipped "
            "(fail-open to default snapshot).",
            exc,
        )
        return {**state, "features": None}

    if not enabled:
        # Dormant default (BORA): kein Fetch, kein Compute — byte-identisch.
        return state
    ...
    try:
        ...
        if data_provider is None:
            logger.warning(
                "_compute_features_node: no data_provider for %s — features skipped "
                "(fail-open to default snapshot).",
                symbol,
            )
            return {**state, "features": None}
        ...
        df = await asyncio.to_thread(
            data_provider.get_data, symbol, current_time, _FEATURE_WINDOW_DAYS
        )
        features = _feature_scalars_from_ohlcv(df, symbol)
        if features is None and (df is None or getattr(df, "empty", True)):
            logger.warning(
                "_compute_features_node: no OHLCV history for %s — features skipped "
                "(fail-open to default snapshot).",
                symbol,
            )
        ...
        logger.warning(
            "_compute_features_node error for %s: %s — features skipped "
            "(fail-open to default snapshot).",
            symbol,
            exc,
        )
...
async def _run_strategy_node(state: SymbolEvalState) -> SymbolEvalState:
    ...
    Delegiert an Round Table V2 (run_round_table).
    Fehlt dieser oder schlägt er fehl, wird eine Enthaltung (HOLD) erzeugt
    und als Signal abgelegt (Council unavailable).
    ...
    if _ROUND_TABLE_AVAILABLE and _run_round_table is not None:
        try:
            ...
            logger.warning(
                "_run_strategy_node: Round Table V2 Fehler für %s, Fallback auf Abstain: %s",
                state.get("symbol"),
                e,
            )
            # Fallback auf Enthaltung

    logger.debug("_run_strategy_node: Round Table nicht verfügbar -> Abstain")
    return {
        **state,
        "signal": SignalEvent(
            symbol=state.get("symbol", "?"),
...
def build_symbol_eval_graph():
    ...
    Topology:
        START → fetch_context → compute_features → run_strategy
              → process_signal → END
    ...
    builder.add_node("compute_features", _compute_features_node)
    ...
    builder.add_edge("fetch_context", "compute_features")
    builder.add_edge("compute_features", "run_strategy")
...
def _build_checkpointer():
    ...
    with _CHECKPOINTER_LOCK:
        ...
        # The symbol_eval graph is LINEAR (START → fetch_context → compute_features →
        # run_strategy → process_signal → END): no interrupts, no human-in-the-loop, no resume, and it
