...
class SymbolEvalState(TypedDict):
    ...
    # while REGIME_CONDITIONER_ENABLED is off (default) so agents observe exactly the
    # old dropped-key behaviour — byte-identical dark ship.
    vix: Optional[float]
    regime: Optional[str]
    # RTR-1 (#1948, dormant default OFF): per-symbol point-in-time headlines
    ...
    # #2672 (dormant default OFF): per-symbol broker position context from the
    ...
    # five while ROUND_TABLE_POSITION_CONTEXT_ENABLED is off (default) —
    # byte-identical dark ship. Fail-closed contract: in_position is only
    ...
    # #3038 (TRD-10, dormant default OFF): the symbol's own ATM-30d implied
