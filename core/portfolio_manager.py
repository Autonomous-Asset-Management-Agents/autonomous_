...
class PortfolioManager:
    ...
    def refresh_positions(self) -> Dict[str, PositionScore]:
        ...
        try:
            ...
            try:
                ...
                Exception
            ):  # noqa: BLE001 — a config read must never break the refresh
            ...
            for pos in positions:
                ...
                days_held = 0
                age_known = False
                if _cur_lot and symbol in self._position_opened_at:
    ...
    def _topup_dead_band_reason(self, opportunity) -> Optional[str]:
        ...
        # tranches and THIS check is the only authority that ends the build. Without the
        ...
        if target_pct - current_pct < self._topup_dead_band_pct:
            ...
            target_kind = "clean-weight" if clean_target is not None else "conviction"
            return (
                f"{opportunity.symbol} within top-up dead-band "
                f"(held {current_pct:.1f}% vs {target_kind} target {target_pct:.1f}%, "
                f"band {self._topup_dead_band_pct:.0f}%{regime_note})"
            )
        return None
