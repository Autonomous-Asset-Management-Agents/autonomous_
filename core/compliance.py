...
# ADR-C09: Mengen-Toleranz der Exit-Freistellung = 1e-6 Stueck
# Basis: Keine regulatorische Pflicht — eine Rechengroesse der #2065-Freistellung.
# Begruendung: Der Vergleich prueft, ob eine SELL-Menge die gehaltene Menge nicht
#   uebersteigt. Bruchteilige Positionen bleiben erhalten (Owner-Entscheid), und ihre
#   Dezimaldarstellung traegt float-Rundungsunschaerfe: ohne Toleranz wuerde ein
#   Verkauf der VOLLEN Position gelegentlich als Ueberschreitung gelesen und die
#   Freistellung verweigert — also ein Schutz-Exit blockiert.
#   1e-6 liegt unter der kleinsten Menge, die ein Broker annimmt (0,001), und ueber der
#   Rundungsunschaerfe von float; die Toleranz kann damit keine echte Ueberschreitung
#   durchlassen. Einseitig sicher: sie kann einen Exit erlauben, nie einen erzwingen.
# Jaehrliche Pruefung.
_QTY_EPS = 1e-6

# ADR-C10: Haltefenster des BUY-Zeitstempel-Puffers = 7200 s (2 h)
# Basis: Keine regulatorische Frist — ABGELEITET aus den beiden Pacing-Regeln, die
#   darauf zugreifen: 30 min Cooldown je Symbol und 60 min Rate-Fenster.
# Begruendung: Das Fenster muss die laengere der beiden Regeln vollstaendig abdecken,
#   sonst laeuft ein Zeitstempel aus dem Puffer, bevor die Regel ihn noch braucht — die
#   Regel waere dann still wirkungslos. 2 h = 60 min + Reserve fuer Zyklusversatz und
#   Neustart. Ein kuerzeres Fenster verkuerzt die Regeln unbemerkt; ein laengeres kostet
#   nur Speicher.
#   Dieser Puffer ist BEWUSST getrennt vom 60-s-Wash-Trade-Puffer (_recent_trades):
#   jenen fuer das 30/60-min-Pacing zu lesen liesse den Cooldown nach 60 s verfallen
#   (Archon CRITICAL-1). Wer die Pacing-Fenster aendert, muss diesen Wert mitfuehren.
# Jaehrliche Pruefung.
...
# ADR-C11: Obergrenze der Ablehnungsgrund-Schluessel = 32
# Basis: Keine regulatorische Pflicht — eine Speicher- und Auswertbarkeitsgrenze.
# Begruendung: `_REJECT_REASONS` ist ein prozessweiter Counter ueber Grund-Codes. Ohne
#   Obergrenze koennte ein unerwarteter, variabler Grund-Code (etwa mit einem Symbol im
#   Text) den Zaehler unbegrenzt wachsen lassen — ein langsames Speicherleck im
#   Compliance-Pfad, und eine Auswertung, die im Rauschen untergeht.
#   32 liegt deutlich ueber der Zahl der bekannten Grund-Codes (11 in
#   core/contracts/reason_code.py nach #3383) und laesst Raum fuer neue, ohne dass die
#   Grenze im Normalbetrieb greift. Greift sie doch, ist das selbst ein Befund: es heisst,
#   dass irgendwo Grund-Codes erzeugt statt gewaehlt werden.
#   Die Zaehlung aendert NIE eine GO/NO-GO-Entscheidung (siehe Kommentar oben).
# Jaehrliche Pruefung.
_MAX_REJECT_REASON_KEYS = 32
...
# ADR-C09: Das Tagesbudget (ADR-C04) liegt je NY-Handelstag unter eigenem Schluessel.
# Basis: #3449 Schritt 5 (Plan docs/3449-schritt5-desktop-zustaende) — der Stand ueberlebt
#        den Neustart; der Tageswechsel braucht keinen Loeschvorgang.
# Frist 3 Tage: der Vortag bleibt fuer Diagnose lesbar, danach raeumt die Ablage selbst.
BUDGET_PRAEFIX = "compliance:daily_trades:"
BUDGET_FRIST_S = 3 * 24 * 3600


def _ny_handelstag() -> str:
    """Der NY-Kalendertag nach der Engine-Uhr — unter SIM_MODE die simulierte (#3317)."""
    import pytz

    from core.sim.clock import engine_now

    return engine_now(pytz.timezone("America/New_York")).date().isoformat()
...
class ComplianceGuardian:
    ...
    def __init__(self):
        # ADR-C01 (revidiert #3219): Max Order Value = 10.000 USD/EUR (ENTRY-/Sizing-Limit)
        # Basis: internes Risikopolicy v1.0 — operative Begrenzung der Schadenswirkung
        # EINER fehlerhaften Order (Fat-Finger, Preis-/Datenfehler, Modellausreißer).
        # Begründung: begrenzt die *Order*, nicht die *Position* — Konzentration steuert
        # MAX_POSITION_PERCENT (belegt: ein Titel erreichte über zwei Orders ~20.000 USD
        # und später 22,9 % des Depots). Review jährlich oder bei einer Änderung des
        # typischen Kontovolumens um mehr als eine Größenordnung.
        # KEIN regulatorisches Gebot: die frühere Basis "ESMA Position Limit Guidelines
        # (MiFID II Art. 57)" trägt nicht — Art. 57 regelt Positionslimits für
        # WARENDERIVATE, nicht Ordergrößen bei Aktien; der konkrete Anker wurde in #1599
        # angefragt und nie geliefert. Siehe #3219.
        ...
        # #3392: Das Betriebslimit ist einstellbar (Owner-Entscheid 05.09.2026, #3220),
        # die ratifizierte Obergrenze nicht. apply_policy klemmt erst beim Start; ohne diese
        # Klemme galt bis dahin jeder Wert aus der Umgebung — auch einer weit darueber.
        _konfiguriert = get_config().COMPLIANCE_MAX_ORDER_VALUE
        if _konfiguriert > MAX_ORDER_VALUE_CEILING:
            logger.error(
                "COMPLIANCE_MAX_ORDER_VALUE=%s liegt ueber der ratifizierten Obergrenze "
                "%s (ADR-C01) — Verstoss, es gilt die Obergrenze.",
                _konfiguriert,
                MAX_ORDER_VALUE_CEILING,
            )
        self.max_order_value = min(_konfiguriert, MAX_ORDER_VALUE_CEILING)

        # ADR-C02: Restricted Instrument List — BEWUSST LEER (Owner-Entscheid 05.09.2026, #3222)
        # Basis: MAR Art. 5 (Market Abuse Regulation). Eine Restricted List sperrt Papiere aus
        # RECHTLICHEN Gründen — Insiderwissen, Sanktionen, laufende Verfahren. Sie ist KEINE
        # Qualitäts- oder Risikoauswahl und hat nichts mit dem Handelsuniversum zu tun (das
        # kommt aus der S&P-500-Mitgliedschaft im data_provider).
        # Begründung für leer: ohne institutionelle Anleger besteht derzeit keine Sperrpflicht.
        # Leer ist die EHRLICHE Aussage — bis 05.09.2026 standen hier "SCAM_TOKEN"/"EVIL_CORP",
        # Platzhalter aus dem Scaffold-Commit vom 16.02.2026 ("# Example restricted
        # instruments"). Eine Kontrolle, die im Audit aktiv aussieht und nichts Reales sperrt,
        # ist schlechter als eine sichtbar leere (AUDIT_VORGEHENSVORSCHLAG.md F.1.3, High).
        # Das Gate darunter bleibt scharf und sofort nutzbar: ein hier eingetragenes Symbol
        # wird als ERSTE Prüfung in check_order abgelehnt.
        # KEIN Laufzeit-Pflegeweg: die frühere Notiz "aus Cloud-Config/DB erweiterbar" war
        # unzutreffend — ein Lesepfad dafür existiert nicht; die Iron-Dome-Policy trägt nur
        # Zahlenwerte, keine Symbolliste. Erweitern geht heute nur per Code-Änderung.
        ...
        # #3449 Schritt 5: das verbrauchte Tagesbudget ueberlebt den Neustart. Ohne dauerhafte
        # Ablage (weder REDIS_URL noch AAA_USER_DATA_DIR) zaehlt der Waechter wie bisher nur im
        # Arbeitsspeicher.
        self._budget_ablage = None
        try:
            from core.state import synchron

            self._budget_ablage = synchron.synchrone_ablage()
        except Exception:  # noqa: BLE001 — ohne Ablage wie bisher
            logger.exception("ComplianceGuardian: keine Ablage fuer das Tagesbudget.")
        self.daily_trades = self._budget_von_heute()

        # INC-8 / #3529: Apply Iron Dome ceilings immediately at boot.
        # This clamps max_order_value and max_daily_trades loaded from get_config()
        # against the true policy ceilings, preventing a split-brain state where
        # desktop signals bypass the ceiling until /start-live is called.
        self.reload_policy({})
    ...
    def reload_policy(self, config_value=None):
        ...
        # #3220/#3221 (CLAUDE.md §5.6): übersteuert eine gespeicherte Admin-Policy den
        # konfigurierten Betriebswert, ändert das Geldverhalten — das muss sichtbar sein.
        # Der RiskManager tut das seit #3217; der Guardian schwieg. Ohne diesen Log
        # wundert sich ein Operator, warum sein über die Settings gesetzter Wert nach
        # dem Neustart nicht greift.
        for _attr, _new in (
            ("COMPLIANCE_MAX_ORDER_VALUE", policy.max_order_value),
            ("COMPLIANCE_MAX_DAILY_TRADES", policy.max_daily_trades),
        ):
            try:
                _cfg_val = getattr(__import__("config", fromlist=[_attr]), _attr, None)
            except (
                Exception
            ) as _exc:  # noqa: BLE001 — Diagnose darf den Policy-Load nie brechen
                # CLAUDE.md §5.6: auch der Ausfall der DIAGNOSE wird laut. Sonst ist
                # eine defekte Config von "Policy stimmt mit Config überein" nicht zu
                # unterscheiden — beides bliebe still, und der Vergleich unten fiele
                # unbemerkt aus.
                logging.warning(
                    "Betriebswert %s für den Policy-Vergleich unlesbar (%s) — "
                    "eine Abweichung zur Iron-Dome-Policy bleibt diesen Durchlauf "
                    "unentdeckt.",
                    _attr,
                    _exc,
                    exc_info=True,
                ...
                _cfg_val = None
            if _cfg_val is not None and _new != _cfg_val:
                logging.warning(
                    "Iron-Dome-Policy setzt %s auf %s — der konfigurierte Betriebswert "
                    "(%s) greift ab jetzt nicht mehr.",
                    _attr.lower().replace("compliance_", ""),
                    _new,
                    _cfg_val,
                )
    ...
    def reset_daily_limit(self):
        """Stellt den Zaehler auf den Stand des heutigen NY-Handelstags (#3449 Schritt 5).

        Aufgerufen beim Tageswechsel — aber auch bei **jedem** Strategiestart
        (``base.py``) und im ersten Schleifendurchlauf (``trading_loop.py``). Ein blindes
        ``= 0`` loeschte dort das nach einem Neustart wiederhergestellte Budget. Der neue Tag
        hat einen neuen Schluessel und beginnt damit von selbst bei null.
        """
        self.daily_trades = self._budget_von_heute()
        self._daily_limit_alert_sent = False
    ...
    # ── #3449 Schritt 5: dauerhaftes Tagesbudget ─────────────────────────────

    def _budget_schluessel(self) -> str:
        return f"{BUDGET_PRAEFIX}{_ny_handelstag()}"

    def _budget_von_heute(self) -> int:
        """Der verbrauchte Stand des heutigen NY-Handelstags. Ohne Ablage: 0 wie bisher.

        **Unlesbar heisst ausgeschoepft** (Plan #3449 Schritt 5, fail-closed wie beim Halt):
        Neue Einstiege werden abgelehnt; risikomindernde Exits nimmt
        ``COMPLIANCE_ALLOW_RISK_REDUCING_EXITS`` wie bisher vom Deckel aus.
        """
        if self._budget_ablage is None:
            return 0
        try:
            wert = self._budget_ablage.get(self._budget_schluessel())
            return max(0, int(round(float(wert)))) if wert is not None else 0
        except Exception:  # noqa: BLE001 — unlesbar ist nicht null
            logger.exception(
                "ComplianceGuardian: Tagesbudget nicht lesbar — gilt als ausgeschoepft, bis "
                "es wieder lesbar ist (#3449)."
            )
            return int(self.max_daily_trades)

    def _budget_buchen(self, betrag: float) -> None:
        """Bucht einen Handel (oder eine Rueckgabe) dauerhaft. Scheitert das, wird es laut
        gemeldet; der Zaehler im Arbeitsspeicher gilt weiter."""
        if self._budget_ablage is None:
            return
        schluessel = self._budget_schluessel()
        try:
            self._budget_ablage.incrbyfloat(schluessel, betrag)
            self._budget_ablage.expire(schluessel, BUDGET_FRIST_S)
        except Exception:  # noqa: BLE001 — der Handel selbst ist gebucht
            logger.exception(
                "ComplianceGuardian: Tagesbudget nicht dauerhaft gebucht — ein Neustart "
                "heute saehe einen zu niedrigen Stand (#3449)."
            )
