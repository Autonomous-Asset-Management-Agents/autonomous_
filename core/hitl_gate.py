...
async def log_settings_change_event(
    *,
    changes: list,
    acknowledgment: str,
    nonce: str,
    ...
    """Record a deliberate trading-settings change on the Art-14 hash chain (#3155 S2).

    Same contract as ``log_portfolio_shape_event``: **audit-before-change**, written
    BEFORE the setting is persisted; with ``strict=True`` (the API default) a failed
    WORM write **re-raises** so the endpoint cannot report success for an unaudited
    change. ``changes`` is the text-only list from
    ``core.trading_settings.describe_changes``; an EMPTY list is a programming error
    at the call site (the caller must skip the chain when nothing changed).
    """
    if not changes:
        raise ValueError(
            "log_settings_change_event called with no changes — the caller must skip "
            "the WORM write when nothing changed."
        )
    ...
    try:
        ...
        logger.warning("[HITL] trading-settings audit failed: %s", exc)
        if strict:
            raise


async def log_settings_deviation_ack(
    *,
    decision: str,
    deviations: list,
    nonce: str,
    ...
    Same contract as ``log_portfolio_shape_event``: **audit-before-change** — written
    BEFORE live enablement; ``strict=True`` (the API default) re-raises a failed WORM
    write so the endpoint cannot report success for an unrecorded decision. An EMPTY
    ``deviations`` list is a programming error at the call site (no deviations ⇒ the
    consent shows no review block and nothing must reach the chain).
    """
    if decision not in ("keep", "reset"):
        raise ValueError(f"invalid deviation decision: {decision!r}")
    if not deviations:
        raise ValueError(
            "log_settings_deviation_ack called with no deviations — the caller must "
            "skip the WORM write when nothing deviates."
        )
    payload = json.dumps(deviations, sort_keys=True, separators=(",", ":"))
    try:
        await _resolve_audit_logger().log_hitl_event(
            SettingsDeviationAckEvent(
                timestamp=_now(),
        ...
        logger.warning("[HITL] settings-deviation-ack audit failed: %s", exc)
        if strict:
            raise


async def log_portfolio_shape_event(
    *,
    changes: list,
    acknowledgment: str,
    nonce: str,
    ...
    """Record a deliberate portfolio-structure change on the Art-14 hash chain (#3132).

    **audit-before-change**, the same contract as ``log_live_enablement_event``: written
    BEFORE the setting is persisted, and with ``strict=True`` (the API default) a failed
    WORM write **re-raises** so the endpoint cannot report success. A change without a
    record is what this ordering rules out; the reverse (a record without a change) is the
    safe direction.

    ``changes`` is the list from ``core.portfolio_shape.describe_changes`` — text-only
    fields. It is serialised here with ``sort_keys`` and no whitespace so the chain
    preimage is byte-stable for the JS verifier (``audit-chain.cjs``); see
    ``PortfolioShapeEvent`` for why floats may never reach the chain.

    An EMPTY ``changes`` list is a programming error at the call site: the caller must not
    reach the chain when nothing changed (that would be noise obscuring real decisions).
    """
    if not changes:
        raise ValueError(
            "log_portfolio_shape_event called with no changes — the caller must skip "
            "the WORM write when nothing changed."
        )
    payload = json.dumps(changes, sort_keys=True, separators=(",", ":"))
    try:
        ...
        logger.warning("[HITL] portfolio-shape audit failed: %s", exc)
        if strict:
            raise
