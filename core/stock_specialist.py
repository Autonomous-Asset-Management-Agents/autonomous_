...
def _now_utc() -> datetime:
    """Specialist "current time" — routed through the engine clock seam so a Sim-Day run reads
    AS OF the sim day, not today (`core.sim.clock.engine_now` returns the SimClock under SIM_MODE,
    wall-clock otherwise; #2548). Used ONLY for the data-window / `as_of` calls (leakage-relevant);
    internal bookkeeping stamps (`_last_refresh`, `updated_at`) keep wall-clock. Fail-safe: any
    import/lookup error → wall-clock, so telemetry/sim wiring never breaks the specialist.
    """
    try:
...
def _resolve_polygon_key() -> str:
    """The configured Polygon key, or "" when none is set (RPT-R7).

    Never raises and never guesses: a missing key must surface as an honest "no feed
    attached" gap in report §5, not as a crash and not as a silent "no news".
    """
    import config as _config

    try:
        return _config.get_secret_str(getattr(_config, "POLYGON_API_KEY", None)) or ""
    except Exception as _exc:  # noqa: BLE001 — a keyless agent beats a dead registry
        logging.getLogger(__name__).warning(
            "POLYGON_API_KEY could not be resolved (%s) — specialist runs without a "
            "news feed; report §5 will show an honest gap.",
            _exc,
        )
        return ""
...
# Sim-Day offline contract (#2548 / SpecialistAlpha activation, runbook addition 2): in a sim the
# specialist must NOT hit the network (offline/deterministic) and must not read anachronistic live
# alt-data over a historical day. The 9 live-only sources are skipped + honestly marked
# sources_unavailable; only ml_prediction (offline, as-of) runs. Purely SIM_MODE-gated → live is
# byte-identical.
_SIM_NETWORK_SOURCES = _SPECIALIST_SOURCE_NAMES[:9]  # all but ml_prediction (index 9)


def _sim_offline() -> bool:
    """True when the specialist must stay offline (Sim-Day SIM_MODE). Fail-safe: any error → False
    (live behaviour), so a config glitch can never accidentally silence the LIVE specialist.
    """
    try:
        return bool(getattr(get_config(), "SIM_MODE", False))
    except Exception:
        return False


async def _sim_skip(sentinel):
    """A resolved no-op that stands in for a skipped network fetcher under the sim offline contract
    (keeps the gather tuple shape; no coroutine that would open a socket is ever created).
    """
    return sentinel
...
class StockSpecialistAgent:
    ...
    def __init__(
        ...
        # ⚠ RPT-R7 — this used to default to "" and NO production caller ever passed a
        # key: `specialist_registry.py:328` is the only construction site repo-wide and
        # it passes the gemini key alone. So `_fetch_polygon_news` returned before any
        # I/O for every symbol, in every cycle, forever. That silently killed BOTH the
        # Polygon headline path (long-standing, pre-dating this work) and report §5.
        # The empty-string default is the bug: it is indistinguishable from a
        # deliberately keyless agent, so the failure was invisible instead of loud.
        # Resolving from config here fixes every caller at once; `get_secret_str` is the
        # authoritative extractor and handles both editions (Enterprise `SecretStr` vs
        # Desktop plain str). Pass "" explicitly to force a keyless agent (tests do).
        if polygon_api_key is None:
            polygon_api_key = _resolve_polygon_key()
        self._polygon_api_key = polygon_api_key
        # RPT-R7 — report §5's catalyst feed, filled as a side-channel by
        # ``_fetch_polygon_news`` (which already fetches these articles and used to keep
        # only their titles). ⚠ None and [] mean DIFFERENT things and the report renders
        # them differently: None = "no news source attached" (we never asked), [] = "we
        # asked and nothing qualified". Never collapse the two into a falsy check.
        self._news_items: Optional[List[Any]] = None
    ...
    async def research(self) -> SpecialistReport:
        ...
        # Sim-Day offline contract (#2548): the network alt-data sources are skipped below, so the
        # shared CIK refresh (which only feeds the skipped EDGAR fetchers) is skipped too — no I/O.
        _sim_off = _sim_offline()
        if not _sim_off:
            await maybe_refresh()
        ...
        (
            ...
            self._fetch_edgar_form4() if not _sim_off else _sim_skip([]),
            self._fetch_edgar_8k() if not _sim_off else _sim_skip([]),
            self._fetch_edgar_13d() if not _sim_off else _sim_skip([]),
            self._fetch_congressional_trades() if not _sim_off else _sim_skip(None),
            self._fetch_polygon_news() if not _sim_off else _sim_skip([]),
            self._fetch_wiki_pageviews() if not _sim_off else _sim_skip({}),
            self._fetch_reddit_mentions() if not _sim_off else _sim_skip({}),
            self._fetch_finra_short_interest() if not _sim_off else _sim_skip(None),
            self._fetch_google_trends() if not _sim_off else _sim_skip(None),
            self._fetch_ml_prediction(),  # offline (as-of LSTM panel) — always runs
        ...
        # #2587 dead-source honesty: congressional trades / reddit signal
        # "source unavailable" as None (vs. a REAL checked-and-empty result).
        # Collect those names (roster spelling, _SPECIALIST_SOURCE_NAMES) so the
        # serializer can null the counts and the card shows "—" instead of a
        # fabricated 0. Internals keep the historical [] / 0 shapes, so scoring,
        # prompt and escalation behave exactly as before on a dead source.
        political_trades = _safe(political_trades, None)
        ...
        if political_trades is None:
            sources_unavailable.append("congressional_trades")
            political_trades = []
        if reddit_data.get("mentions") is None:
            sources_unavailable.append("reddit_mentions")
        # Sim-Day (#2548): the live-only sources were skipped (offline contract) — mark them ALL
        # unavailable so the coverage card honestly shows "—", never a fabricated "checked, found
        # nothing". Only ml_prediction (offline, as-of) really ran.
        if _sim_off:
            for _src in _SIM_NETWORK_SOURCES:
                if _src not in sources_unavailable:
                    sources_unavailable.append(_src)
        ...
        if getattr(get_config(), "SPECIALIST_NEWS_V2", False):
            google_news = [] if _sim_off else await self._fetch_google_news()
        ...
        gathered = {
            ...
            # #2587: unavailable (None) normalises to 0 for the internal scoring /
            # prompt paths (identical to the historical dead-source behaviour);
            # the honesty signal travels via sources_unavailable only.
            "reddit_mentions_24h": reddit_data.get("mentions") or 0,
        ...
        # Rich-synthesis card (Direction A, dormant): honest HAR-RV vol band. Flag-
        # FIRST inside the fetcher -> None (no I/O) unless SPECIALIST_GROUNDING_ENABLED,
        # so flag OFF is byte-identical (the field stays its None default).
        # Fetched BEFORE the synthesis call so the model view can feed the prompt
        # as CITABLE SOURCES (below) — the same values the card's Modell-Sicht shows.
        vol_scenario = await self._fetch_vol_scenario()

        # Model signals as citable sources: the synthesis must read as ONE system
        # (news + own models), not an LLM news wrapper (live feedback 2026-07-23).
        # Each line carries the symbol token (survives the subject-corpus filter)
        # and is numbered [H#] after the news headlines by prompt.py; the grounding
        # gate then verifies any echoed rank/band number against EXACTLY these
        # lines. Flag-first: grounding OFF -> no lines -> prompt byte-identical.
        if getattr(get_config(), "SPECIALIST_GROUNDING_ENABLED", False):
            ...
            try:
                ...
                # #2587: the STORE method is cross_section_at (cross_section is
                # the reader adapter's name) — the old call AttributeError'd into
                # this except and silently dropped the rank line every cycle.
                _pct, _rank, _n = cross_section_standing(
                    get_store().cross_section_at(_now_utc()),
                    self.symbol,
                )
            except Exception as e:  # noqa: BLE001 — panel unavailable = abstain
                logger.warning(
                    "[%s] LSTM panel unavailable for model-signal line: %s",
                    self.symbol,
                    e,
                )
                _pct, _rank, _n = None, None, None
            if _rank is not None and _n:
                model_lines.append(
                    f"{self.symbol} model signal: our LSTM ranks this stock "
                    f"#{_rank} of {_n} in the tracked universe (scores above "
                    f"{_pct:.0f}% of peers)"
                )
            if vol_scenario and vol_scenario.get("vol_band_sigma_pct") is not None:
                if vol_scenario.get("price_band_low") is not None:
                    model_lines.append(
                        f"{self.symbol} model signal: our volatility model expects "
                        f"the stock to trade between "
                        f"${vol_scenario['price_band_low']:.0f} and "
                        f"${vol_scenario['price_band_high']:.0f} over the next 20 "
                        f"trading days (last close ${vol_scenario['last_close']:.0f}, "
                        f"a range call, not a direction call)"
                    )
                else:
                    model_lines.append(
                        f"{self.symbol} model signal: our volatility model expects a "
                        f"20-day range of about "
                        f"{vol_scenario['vol_band_sigma_pct']:.1f}% "
                        "either way (a risk spread, not a direction call)"
                    )
            if model_lines:
                gathered["model_signal_lines"] = model_lines
        ...
        report = self._build_report(
            gathered, synthesis, integrity=integrity, vol_scenario=vol_scenario
        )
        ...
        # RPT-6 (#1998, Epic #1998, dormant default OFF): attach the auditable v2
        # research report (fact_set -> render -> narrate(get_llm_provider) ->
        # audit). REPORT-ONLY: it writes ONLY report.report_markdown /
        # report_audit_* — never score/recommendation/reasons/escalate — and is
        # fully flag-gated, so flag OFF is a no-op == byte-identical to today
        # (grep + test). The agent's *score* still votes (SPECIALIST_ALPHA_WEIGHT
        # defaults to 0.40 since #2839; #3261) — the report does not.
        self._maybe_attach_v2_report(report, gathered)
        self._last_report = report
        self._last_refresh = datetime.now(timezone.utc)
        return report

    def _maybe_attach_v2_report(
        self, report: SpecialistReport, gathered: Dict[str, Any]
    ) -> None:
        """Flag-gate for the RPT-6 auditable report. No-op unless the flag is ON.

        With ``REPORT_GENERATOR_V2_ENABLED`` OFF (default) this returns
        immediately, so ``research()`` is byte-identical to today: nothing else
        is imported, no field is written, the decision path is untouched.
        """
        if not getattr(get_config(), "REPORT_GENERATOR_V2_ENABLED", False):
            return
        self._attach_v2_report(report, gathered)

    def _default_report_reader(self, as_of, gathered: Dict[str, Any]) -> Any:
        """Build the live FactSet reader (RPT-8): real bars/fundamentals/model card
        plus the engine's live LSTM panel and the specialist's ``gathered``
        alt-signals. Shared by the deterministic attach and the lazy re-narrate so
        both read exactly the same sources."""
        from core.report.engine_reader import EngineFactSetReader
        from core.report.lstm_panel_store import EngineLstmPanelReader, get_store

        return EngineFactSetReader(
            as_of,
            alt_signals=gathered,
            lstm_reader=EngineLstmPanelReader(get_store(), as_of),
            # RPT-R7: §5's catalysts. Passing None (no key, cold cache, failed fetch) is
            # a real state, not a bug — the report says so in words rather than
            # inventing "no relevant news". The FactSet then PIT-filters these against
            # as_of, so an article published after the as_of cannot leak backwards.
            news=self._news_items,
        )

    def _attach_v2_report(
        self,
        ...
        *,
        ...
        """Build + attach the auditable v2 report (report-only, fully guarded).

        Lazy imports keep the module load byte-identical when the flag is OFF.
        The FactSet reader wires the REAL engine sources (bars via the canonical
        data provider, PIT fundamentals cache, model card) and the specialist's
        already-``gathered`` alt-signals for the deterministic pros/cons; the LLM
        is reached ONLY via RPT-5's ``get_llm_provider`` seam (BORA). Any failure
        leaves the report's decision fields untouched and ``report_markdown``
        ``None`` (an honest gap, never a crash of the research cycle).

        ``narrate=False`` (RPT-8b) skips the LLM entirely — a complete, audit-clean
        report with deterministic prose placeholders — so the full ~500-symbol
        universe can be populated at zero LLM cost and narrated lazily (see
        :meth:`narrate_report`)."""
        try:
            ...
            as_of = _now_utc().date()
            if reader is None:
                reader = self._default_report_reader(as_of, gathered)
            bundle = build_report_bundle(
                self.symbol, as_of, reader=reader, llm=llm, narrate=narrate
            )
            report.report_markdown = bundle.markdown
            report.report_audit_ok = bundle.audit.integrity_ok
            report.report_audit_summary = bundle.audit.summary()
            # The reader badge must reflect the note's OWN cross-sectional verdict,
            # not the uncomputed sentiment default: report-only paths never set
            # `recommendation`, so it stays "hold" and would paint a fabricated
            # HALTEN over an abstaining note. rec_code mirrors the note's abstention
            # EXACTLY (no rank -> None -> the badge shows nothing). Report-only: this
            # writes report_recommendation, never the consensus `recommendation`.
            from core.report.render import rec_code

            report.report_recommendation = rec_code(bundle.fact_set)
        except Exception as exc:  # noqa: BLE001 — report-only must never break research
            logger.warning(
                "[%s] RPT-6 report generation failed (report-only): %s",
                self.symbol,
                exc,
            )
    ...
    def narrate_report(
        ...
        *,
        ...
        """RPT-8b lazy narration: upgrade a deterministically-attached report's
        prose in place with one LLM pass for this single symbol.

        The FactSet is rebuilt from the live sources (deterministic — same numbers)
        and re-rendered WITH narration, so the ``<!-- SLOT: -->`` placeholders
        become real prose. Report-only and fully guarded: on any failure the
        existing (deterministic) markdown is left untouched. This is the on-open /
        background-upgrade seam that keeps the whole-universe pass LLM-free while
        still delivering rich prose for the reports a user actually looks at."""
        self._attach_v2_report(
            report, gathered or {}, reader=reader, llm=llm, narrate=True
        )

    def refresh_report_only(self) -> SpecialistReport:
        """A5 (RPT-8b) deterministic-first refresh: produce a fresh
        :class:`SpecialistReport` carrying ONLY the auditable v2 report
        (``report_markdown`` + audit fields), with **no LLM** (``narrate=False``)
        and **no** ``research()`` gather / Gemini synthesis.

        This is the cheap whole-universe path: the report is built from the live
        engine sources (bars, PIT fundamentals cache, model card, LSTM panel) via
        the FactSet reader — the alt-signal enrichment (news/insider pros-cons)
        that ``research()`` fetches is skipped, so §4/§5 degrade to honest gaps
        until a full refresh or lazy narration enriches them. The decision fields
        (score/recommendation/reasons/escalate) stay at their neutral defaults —
        this path is report-only and never feeds the consensus."""
        report = SpecialistReport(symbol=self.symbol)
        self._attach_v2_report(report, {}, narrate=False)
        return report
    ...
    async def _fetch_vol_scenario(self) -> Optional[Dict[str, Any]]:
        ...
        if not getattr(get_config(), "SPECIALIST_GROUNDING_ENABLED", False):
            return None
        try:
            bars_df = await asyncio.to_thread(
                _get_data_provider().get_data,
    ...
    async def _fetch_edgar(
        ...
        *,
        ...
        try:
            ...
            cutoff = (_now_utc() - timedelta(days=cutoff_days)).strftime("%Y-%m-%d")
    ...
    async def _fetch_polygon_news(self) -> List[str]:
        ...
        RPT-R7 — this call ALSO feeds report §5. It already fetches full articles and
        keeps only ``title``; the rest (publisher, url, published_utc) was discarded.
        The report needs exactly those discarded fields, so they are now retained in
        ``self._news_items`` as a SIDE-CHANNEL. Two constraints shaped that choice:

        * The ``List[str]`` return shape is load-bearing — the sentiment path and the
          Google-News merge both consume it. It does not change.
        * ``_CACHE_KEY`` holds ``List[str]`` in a live Redis. Changing that value's shape
          would make a rolling deploy read new-shaped data with old code (and vice
          versa) for one TTL. So the items get their OWN key and the old one is left
          exactly as it is.

        Net cost of §5's news: zero extra requests.
        """
        # ⚠ RESET FIRST. This agent is CACHED ACROSS CYCLES by the registry, so
        # ``_news_items`` outlives a single research pass. Without this line, a fetch
        # that fails THIS cycle (non-200, timeout, exception) leaves the PREVIOUS
        # cycle's articles in place and §5 renders them stamped with today's as_of —
        # stale news presented as current, carrying a provenance badge. Every early
        # return below therefore means "we have nothing", never "keep what we had".
        self._news_items = None

        if not self._polygon_api_key:
            # No key -> no feed. self._news_items stays None, and §5 says "keine
            # Nachrichtenquelle angebunden" — which is the truth, not a fallback.
            return []

        # --- Redis cache check (async) ---
        _CACHE_KEY = f"polygon:news:{self.symbol}"
        _ITEMS_CACHE_KEY = f"polygon:news:items:{self.symbol}"  # RPT-R7, separate shape
        ...
        try:
            ...
            if r is not None:
                ...
                if cached is not None:
                    ...
                    # RPT-R7: a headline cache-HIT must not starve the report. Without
                    # this, any symbol warm in cache would render "no news source
                    # attached" while the headlines it just served sit right there.
                    await self._load_cached_news_items(r, _ITEMS_CACHE_KEY)
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                # RPT-R7: keep the provenance this call already paid for. Articles that
                # cannot carry a title/timestamp/link are dropped by the mapper — §5
                # renders each item as a source the reader can open, and an unsourceable
                # headline would be struck by the auditor anyway.
                self._news_items = self._reconcile_news_items(results)
            ...
            try:
                ...
                if r is not None:
                    ...
                    # RPT-R7: separate key, separate shape — see the docstring.
                    # ⚠ Only cache a REAL list. `self._news_items or []` would write []
                    # for None and freeze "asked, nothing qualified" into the cache for
                    # a full TTL — serving that false claim to every warm reader. None
                    # means unusable/not-wired and must stay uncached, so the next
                    # reader re-asks instead of inheriting a lie.
                    if self._news_items is not None:
                        await r.set(
                            _ITEMS_CACHE_KEY,
                            _json.dumps([_dc.asdict(i) for i in self._news_items]),
                            ex=_CACHE_TTL,
                        )
                    logger.debug(
    ...
    def _reconcile_news_items(self, results: Any) -> Optional[List[Any]]:
        """Map raw Polygon articles to ``NewsItem``s and reconcile the counts.

        ⚠ The live article shape is NOT the shape of our fixtures: ``publisher`` is a
        dict, the link lives in ``article_url``, and the stamp is Z-suffixed. All three
        divergences are handled in ``core.report.news_feed`` and pinned by tests there.

        THE RECONCILIATION IS THE POINT. There are THREE outcomes, and collapsing them
        into two produces a lie:

        * ``[]``  — the feed genuinely returned nothing. §5: "keine relevanten
          Meldungen im Fenster". True.
        * ``None`` — we asked and got articles we could not use AT ALL. Returning ``[]``
          here would make §5 state "keine relevanten Meldungen" — a false claim about
          the WORLD, carrying a ``[src:]`` badge and passing the auditor with 0 flags,
          because the auditor checks that a citation EXISTS, never that anything
          survived the mapper. That is exactly the bug class #2151 removed. Any systemic
          cause (an ``article_url`` -> ``articleUrl`` rename, a plan tier that omits a
          field, a partial outage) would silently turn every report into that lie. So
          "unusable feed" degrades to the honest not-wired gap instead.
        * a partial list — logged at WARNING with both counts, because §5 reads as an
          exhaustive catalyst list and a silent 7-of-10 is a quiet overstatement.

        Drops are logged at WARNING, never DEBUG: a fallback that hides is a fallback
        that rots (CODING_POLICY §5.6).
        """
        from core.report.news_feed import map_polygon_article

        if not isinstance(results, list):
            logger.warning(
                "[%s] polygon:news returned %s, not a list — treating the feed as "
                "unusable (report renders an honest gap, not 'no news')",
                self.symbol,
                type(results).__name__,
            )
            return None
        if not results:
            return []  # asked, feed had nothing — a true and renderable statement

        items = [m for m in (map_polygon_article(a) for a in results) if m is not None]
        if not items:
            logger.warning(
                "[%s] polygon:news: all %d article(s) lacked title/published_utc/"
                "article_url — feed unusable, NOT 'no news'. Did the response schema "
                "change?",
                self.symbol,
                len(results),
            )
        ...
        if len(items) < len(results):
            logger.warning(
                "[%s] polygon:news: kept %d of %d article(s); %d lacked provenance and "
                "cannot be rendered as a checkable source",
                self.symbol,
                len(items),
                len(results),
    ...
    async def _load_cached_news_items(self, redis: Any, key: str) -> None:
        """Rehydrate ``self._news_items`` from the items cache (RPT-R7).

        Stays ``None`` on any miss or error — which renders §5's honest "not wired" gap
        rather than an empty-and-therefore-'no news' claim. Those two are different
        statements and the report must not confuse them.
        """
        try:
            import json as _json
            ...
            self._news_items = [NewsItem(**d) for d in _json.loads(raw)]
        except (
            Exception
        ) as _e:  # noqa: BLE001 — a cold report beats a crashed specialist
            logger.debug(
                "[%s] polygon:news items cache read failed: %s", self.symbol, _e
            )
    ...
    async def _fetch_google_news(self) -> List[str]:
        ...
        try:
            ...
            # Google-News RSS search for the ticker. Query "<SYMBOL> stock", NOT the bare ticker:
            # a bare 2-letter ticker collides with a US-state abbreviation and drags in local news
            # (e.g. "WY" -> Wyoming: wildfires, University-of-Wyoming sports, Casper events — nothing
            # to do with Weyerhaeuser). The " stock" suffix pins the query to the equity and is the
            # SAME posture the NewsSentiment path already uses (headlines.py ADR-NS-06). URL-encode so
            # special-char tickers (BRK.A, BF-B) cannot corrupt the query (RPAR T3 review #1312 F-03).
            url = (
                "https://news.google.com/rss/search"
                f"?q={quote(f'{self.symbol} stock')}&hl=en-US&gl=US&ceid=US:en"
            )
            async with httpx.AsyncClient(timeout=8.0) as client:
    ...
    async def _fetch_reddit_mentions(self) -> Dict:
        ...
        #2587 dead-source honesty: ``mentions`` is ``None`` when the SOURCE is
        unavailable (non-200 / exception — live finding 2026-07-30: Reddit answers
        403 to this unauthenticated search endpoint), an int only when the search
        really ran. The caller maps ``None`` into ``sources_unavailable`` so the
        coverage card shows "—", never a fake 0.
        """
        try:
            import httpx

            headers = {"User-Agent": "AI-Trading-Bot/1.0 research@aaagents.de"}
            # Reddit search API (no auth needed for basic search). "<SYMBOL> stock" (not the bare
            # ticker) for the same reason as the Google-News fetch above — a bare ticker like "WY"
            # pulls state/community posts (r/wyoming) unrelated to the equity.
            url = (
                f"https://www.reddit.com/search.json"
                f"?q={quote(f'{self.symbol} stock')}&restrict_sr=false&sort=new&t=day&limit=25"
            )
            async with httpx.AsyncClient(timeout=8.0) as client:
                r = await client.get(url, headers=headers)
                if r.status_code != 200:
                    logger.warning(
                        "[%s] Reddit mentions HTTP %s — source unavailable "
                        "this cycle (unauthenticated search is blocked)",
                        self.symbol,
                        r.status_code,
                    )
                    return {"mentions": None, "sentiment": "neutral"}
            ...
            logger.warning(f"[{self.symbol}] Reddit mentions unavailable: {e}")
            return {"mentions": None, "sentiment": "neutral"}
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=8.0) as client:
                ...
                if r.status_code == 200:
                    ...
                    # #2587: the dataset path is dead (live 2026-07-30: HTTP 404 —
                    # group/name ``OTCmarket/otcShortInterest`` does not exist on
                    # the FINRA Query API; ``otcMarket/consolidatedShortInterest``
                    # answers 200 but needs a metric-semantics decision before a
                    # switch, see #2587). Until then: WARN, serve honest None ("—").
                    logger.warning(
                        "[%s] FINRA short interest HTTP %s — short interest "
                        "unavailable this cycle (endpoint/dataset mismatch)",
                        self.symbol,
                        r.status_code,
                    )
            return None
            ...
            logger.warning(
                f"[{self.symbol}] FINRA short interest unavailable: {e}"
            )  # noqa: E501
            return None
    ...
    def _synthesis_prompt(self, gathered: Dict[str, Any]) -> str:
        ...
        if getattr(get_config(), "SPECIALIST_PROMPT_V2", False):
            ...
            # Direction-A grounding: when SPECIALIST_GROUNDING_ENABLED is ON the
            # prompt numbers headlines [H#] and asks the model to cite the single
            # supporting headline per bull/bear/thesis sentence (verified later by
            # core.specialist.grounding). OFF -> the existing V2 prompt, unchanged.
            require_tags = getattr(get_config(), "SPECIALIST_GROUNDING_ENABLED", False)
            return build_prompt_v2(
                self.symbol, gathered, require_headline_tags=require_tags
            )
    ...
    def _build_report(
        ...
        vol_scenario: Optional[Dict[str, Any]] = None,
    ) -> SpecialistReport:
        ...
        # #2249 FIX 1: the RAW parser prose lands in these locals, NEVER directly in
        # the displayed bull_case_v2/bear_case_v2/investment_thesis_v2. Grounding
        # (below) is the ONLY writer of the displayed prose — so with PROMPT_V2 ON
        # but GROUNDING OFF the card shows "" (honest empty) instead of the raw,
        # possibly-fabricated prose (the live DIA "Tate Museums" class).
        raw_bull = ""
        raw_bear = ""
        raw_thesis = ""
        if getattr(get_config(), "SPECIALIST_PROMPT_V2", False):
            ...
            raw_bull = parsed.bull_case
            raw_bear = parsed.bear_case
            raw_thesis = parsed.investment_thesis
        else:
            (
                news_summary,
                alt_signals,
        ...
        # Rich-synthesis card (Direction A, dormant): mechanically ground the V2
        # prose. ONLY when SPECIALIST_GROUNDING_ENABLED is ON (which also requires
        # the V2 prose to exist). Each displayed bull/bear/thesis sentence is bound
        # to a real headline of THIS symbol; ungrounded/fabricated sentences (the
        # #2202 "Cerebras" class) are dropped. Display-only: the score/recommendation/
        # reasons above are untouched. Flag OFF -> these stay their DTO defaults.
        synthesis_citations: List[Dict[str, Any]] = []
        synthesis_dropped: List[Dict[str, Any]] = []
        grounding_ok_v2: Optional[bool] = None
        if getattr(get_config(), "SPECIALIST_GROUNDING_ENABLED", False) and getattr(
            get_config(), "SPECIALIST_PROMPT_V2", False
        ):
            ...
            grounded = verify_grounding(
                ...
                gathered,
                self.symbol,
                company_tokens=self._load_company_tokens(),
            )
            bull_case_v2 = grounded.bull
            bear_case_v2 = grounded.bear
            investment_thesis_v2 = grounded.thesis
            synthesis_citations = grounded.citations
            synthesis_dropped = grounded.dropped
            grounding_ok_v2 = grounded.grounding_ok
        ...
        return SpecialistReport(
            ...
            wiki_spike=gathered.get("wiki_spike", False),
            # #2587: dead-source names (roster spelling) -> serializer nulls the
            # counts -> the coverage card renders "—" instead of a fabricated 0.
            sources_unavailable=gathered.get("sources_unavailable") or [],
            ...
            summary=summary,
            headlines=headlines,
            # Rich-synthesis card (Direction A). Grounding fields stay their
            # None/[] defaults unless SPECIALIST_GROUNDING_ENABLED; the vol band
            # stays None unless the scenario fetch produced one. P0-1: pass the
            # band values through with .get() (NEVER `or` — a 0.0 base is real).
            synthesis_citations=synthesis_citations,
            synthesis_dropped=synthesis_dropped,
            grounding_ok=grounding_ok_v2,
            vol_band_low_pct=(vol_scenario or {}).get("vol_band_low_pct"),
            vol_band_base_pct=(vol_scenario or {}).get("vol_band_base_pct"),
            vol_band_high_pct=(vol_scenario or {}).get("vol_band_high_pct"),
            vol_band_sigma_pct=(vol_scenario or {}).get("vol_band_sigma_pct"),
            scenario_basis=(vol_scenario or {}).get("scenario_basis", ""),
            last_close=(vol_scenario or {}).get("last_close"),
            price_band_low=(vol_scenario or {}).get("price_band_low"),
            price_band_high=(vol_scenario or {}).get("price_band_high"),
            chg_20d_pct=(vol_scenario or {}).get("chg_20d_pct"),
            dist_52w_high_pct=(vol_scenario or {}).get("dist_52w_high_pct"),
        )
