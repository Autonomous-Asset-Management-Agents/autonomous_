...
# The Round-Table agent class names (source of truth: core/round_table/agents.py
# ALL_AGENTS). Hard-coded here to keep the registry a pure, import-cheap data table
# and to avoid instantiating the (heavy) agent objects just to read their names.
# runner.py filters the active agents by THIS list, so a module missing here never
# runs at all — test_entitlement_resolve is the drift guard that enforces the mirror.
_ALL_ROUND_TABLE_AGENTS: tuple[str, ...] = (
    "DrawdownGuardAgent",
    ...
    "RLConfidenceAgent",
    "NewsSentimentAgent",
    # (#1951: PatternRecognitionAgent deleted from the gremium — mirror kept in
    # sync with ALL_AGENTS; test_entitlement_resolve enforces the drift guard.)
    # Phase B — the fundamentals voices. Granted to EVERY tier, exactly like the
    # others, so the #1877 invariant holds: BASIC is content-identical to PRO
    # except allow_live. They are dormant at weight 0, so granting them changes no
    # tier's decisions — it only lets them speak and justify in the audit trail.
    "FundamentalsAgent",
    "ValuationAgent",
    # #3095 (b): granted to every tier like the others; dark by default
    # (UPSIDE_SKEW_AGENT_ENABLED off → abstains), so it changes no tier's decisions.
    "UpsideSkewAgent",
    # #3250: TA-Feature-Voter — granted to every tier like the others; dark by
    # default (TREND_/VOLUME_CONFIRM_AGENT_ENABLED off + weight 0.0 → abstain), so
    # they change no tier's decisions. Mirror of ALL_AGENTS (drift guard:
    # test_entitlement_resolve).
    "TrendAgent",
    "VolumeConfirmationAgent",
    # #3275: QualityAgent — granted to every tier like the others; dark by default
    # (QUALITY_AGENT_ENABLED off + weight 0.0 → abstain), so it changes no tier's
    # decisions. Mirror of ALL_AGENTS (drift guard: test_entitlement_resolve).
    "QualityAgent",
)
...
TIER_REGISTRY: dict[Tier, Entitlement] = {
    ...
    Tier.BASIC: Entitlement(
        tier=Tier.BASIC,
        agent_names=_ALL_ROUND_TABLE_AGENTS,
