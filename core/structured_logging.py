...
def _force_utf8_stdio():
    """INC-9: force UTF-8 on process stdio so non-ASCII log records never crash.

    The Round-Table logs glyphs like ``→``/``€``/umlauts (runner.py:298). On Windows
    the console stream defaults to cp1252, whose strict codec raises
    ``UnicodeEncodeError`` on those — logging then swallows the record via
    ``handleError`` and prints a ``--- Logging error ---`` traceback instead. We
    reconfigure stdout/stderr to UTF-8 with ``backslashreplace`` as a last-resort
    escape hatch (never raise, never drop a line). ``reconfigure`` exists on the real
    ``TextIOWrapper`` streams; anything else (pytest capture, pipes) is left untouched.
    """
    for stream in (sys.stdout, sys.stderr):
        ...
        if callable(reconfigure):
            try:
                ...
                # Stream already detached / not reconfigurable — safe to skip.
                pass
