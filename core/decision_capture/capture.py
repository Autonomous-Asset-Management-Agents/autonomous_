...
Design (plan Rev 2, §5):
* Flag-gated: ``DECISION_CAPTURE_ENABLED`` (DEFAULT ON since #2840, desktop profile;
  #3261) — flag off ⇒ every
...
def build_outcome_row(ctx: Any) -> dict:
    ...
    return {
        ...
        # #3618: risk-off damping audit (None = not damped)
        "regime_damp_factor": getattr(ctx, "regime_damp_factor", None),
        "consensus_undamped": getattr(ctx, "consensus_undamped", None),
