...
class VirtualLiveBroker:
    ...
    def __init__(self):
        ...
        # #3377 (ARC-E1.1): test-only fixtures for the CH-3 acceptance. Both default to
        # "off", so an untouched broker behaves exactly as before. They are set by a test,
        # never by configuration — nothing in config.py / config.oss.py refers to them.
        self.sim_reject_symbols: set = set()  # symbols the broker refuses
        self.sim_partial_fill_ratio: Dict[str, float] = (
            {}
        )  # symbol -> share filled per pass
