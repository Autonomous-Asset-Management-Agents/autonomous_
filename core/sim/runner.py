...
# Wie lange ein gesetztes Shutdown-Ereignis bestehen muss, bevor es als Halt
# zaehlt. Deckt das Startup-Flackern ab (Millisekunden) ohne einen echten Halt
# spuerbar zu verzoegern.
_HALT_KARENZ_S = 1.0
_HALT_SEEN: dict = {}


def _sim_wait_reason(engine, t0: float, budget_s: float):
    """Warum die Warteschleife enden darf — ``None`` heisst weiterwarten.

    Drei Ausgaenge, bewusst unterscheidbar:

    * ``"complete"`` — das regulaere Fensterende (#2548 setzt ``_sim_complete``).
    * ``"halted"``   — die Engine hat abgeschaltet, ohne dass der Sim fertig ist.
    * ``"timeout"``  — Zeitbudget erschoepft.

    #2956/#3038: Der Halt-Ausgang fehlte, und das hat an einem Tag zwei Laeufe
    gekostet. ``trading_loop.py:1478-1483`` bricht beim Kill-Switch die
    Handelsschleife ab und setzt ``_shutdown_event``; ``_sim_complete`` wird dabei
    NIE gesetzt, weil der Sim-Hook das nur am regulaeren Fensterende tut. Die
    Schleife pollte danach bis zum Budget — bei 60 Tagen fuenf Stunden — und schrieb
    dann ein Ergebnis mit ``timed_out``.

    Warum das schlimmer ist als ein Absturz: Der Lauf schreibt ein Ergebnis, es
    traegt ``status = OK``, und nur ``timed_out`` verraet den Unterschied. Genau
    davor warnt der Timeout-Zweig unten selbst — und genau das ist trotzdem zweimal
    passiert (#3038 Tag 43 von 60, #2956 Tag 23 von 60).

    ``complete`` gewinnt gegen ``halted``, weil die Engine beim regulaeren Ende
    ebenfalls abschaltet. Andernfalls truege jeder saubere Lauf die Halt-Markierung
    und waere von einem Abbruch nicht mehr zu unterscheiden — die Umkehrung des
    Defekts, nicht seine Behebung.

    Nicht geprueft wird ``strategy_running``: Der Monitor loescht es beim ersten
    Strategiewechsel (None -> RLAgent) kurzzeitig mitten im Lauf, und genau diese
    Loeschung wurde einmal fuer "Sim fertig" gehalten (#2630, 0-Zyklen-Abbruch).
    """
    if engine._sim_complete.is_set():
        return "complete"

    shutdown = getattr(engine, "_shutdown_event", None)
    if shutdown is not None and shutdown.is_set():
        # ZWEI Bedingungen, beide aus einem echten Fehlschlag gelernt.
        #
        # (1) Mindestens ein gespielter Zyklus. ``start_live_strategy`` SETZT
        #     ``_shutdown_event`` beim Hochfahren kurz und loescht es sofort wieder
        #     (``base.py:898-901``: ``stop_strategy()`` setzt, dann clear). Die erste
        #     Fassung dieses Fixes fiel genau darauf herein und brach den Lauf nach
        #     30 Sekunden ab: "Sim HALTED after 1/60 day(s)", null gespielte Tage.
        #     Das ist dieselbe Falle, vor der der Code fuer ``strategy_running``
        #     warnt (#2630) -- nur mit dem Nachbar-Flag.
        #
        # (2) Die Bedingung muss BESTEHEN BLEIBEN. Ein echter Halt ist endgueltig
        #     (#3038 Tag 43, #2956 Tag 23 -- danach lief nichts mehr); ein
        #     Startup-Artefakt ist nach Millisekunden weg. Die Karenz kostet im
        #     Ernstfall eine Sekunde und schuetzt gegen jedes weitere Flackern, das
        #     ich nicht kenne.
        if int(getattr(engine, "_sim_cycles", 0) or 0) <= 0:
            _HALT_SEEN.pop(id(engine), None)
            return None
        seit = _HALT_SEEN.setdefault(id(engine), time.time())
        if time.time() - seit >= _HALT_KARENZ_S:
            return "halted"
        return None
    _HALT_SEEN.pop(id(engine), None)
...
def run_sim(timeout_s: float = None, seed_positions: dict = None):
    ...
    while True:
        grund = _sim_wait_reason(engine, t0, budget)
        if grund == "complete":
            break
        if grund == "halted":
            # #2956/#3038: Die Engine hat abgeschaltet (Kill-Switch, Shutdown).
            # Frueher lief die Schleife hier bis zum Budget weiter und schrieb ein
            # timed_out-Ergebnis — drei Stunden Leerlauf und eine falsche Beschriftung.
            try:
                from core.kill_switch import kill_switch as _ks

                halt_reason = str(getattr(_ks, "halt_reason", "") or "engine shutdown")
            except Exception as exc:  # noqa: BLE001 — Grund ist Beiwerk, nie fatal
                logger.warning("Sim: Halt-Grund nicht lesbar (%s).", exc)
                halt_reason = "engine shutdown"
            logger.warning(
                "Sim HALTED after %d/%d day(s): %s — stopping now instead of polling to "
                "the %.0fs budget. The result is marked halted; the replayed days are "
                "valid up to this point, but do NOT compare it against a complete run.",
