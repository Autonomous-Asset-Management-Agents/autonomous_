...
def _add_run_env(args):
    ...
    os.environ["SIM_DAYS"] = str(getattr(args, "days", 1) or 1)
...
def main(argv=None):
    ...
    r.add_argument(
        ...
        default=1,
        help=(
            "TRADING days to replay (default 1). >1 is what makes hold-vs-rotate "
            "measurable at all: a single static-price day values selling at the mark and "
            "holding at the mark identically, so its P&L cannot see exit policy. With "
            "--date the day is the START; without one, the LAST N corpus days are replayed."
        ),
    )
