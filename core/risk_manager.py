...
class RiskManager:
    ...
    def reload_policy(self, config_value=None):
        ...
        # DEF-1 (#3217): der Order-Deckel gehört zu den Werten, die diese Methode
        # übernehmen MUSS. Vorher setzte nur ComplianceGuardian.reload_policy ihn —
        ...
        # der Sizer baute weiter zu große Orders, die der Guardian hart blockte —
        # der Ausfall vom 14.07.2026).
        self._policy_max_order_value = policy.max_order_value

        # CLAUDE.md §5.6: eine Abweichung vom Config-Wert ändert das Geldverhalten
        # und muss sichtbar sein — sonst wundert sich ein Operator, warum sein
        # gesetztes COMPLIANCE_MAX_ORDER_VALUE nach dem Start nicht mehr greift.
        try:
            _cfg_value = float(
                getattr(
        ...
        except Exception:  # noqa: BLE001 — Diagnose darf den Policy-Load nie brechen
            _cfg_value = 0.0
        if _cfg_value > 0 and policy.max_order_value != _cfg_value:
            logging.warning(
                "Iron-Dome-Policy setzt max_order_value auf %s — der Config-Wert "
        ...
        if current_price > 0:
            try:
                if self._policy_max_order_value is not None:
                    # DEF-1 (#3217): eine angewandte Iron-Dome-Policy hat Vorrang —
                    # derselbe Wert, den ComplianceGuardian blockt. Ohne Policy
                    # (None) bleibt der Config-Read unten byte-identisch, damit
                    # die bestehenden Tests, die ``config.COMPLIANCE_MAX_ORDER_VALUE``
