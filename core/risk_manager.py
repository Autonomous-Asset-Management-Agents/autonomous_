...
def _bull_exposure_cfg() -> Tuple[bool, float]:
    """#3099 (H2): (enabled, calm_scaler) for the bounded bull-regime risk-on step.

    Fail-safe: any config read error → (False, 1.0), i.e. the pure ADR-R08 down-only
...
def vix_ladder_scaler(
    ...
    Down-only ABOVE 18 (unchanged): >40→0.3, >35→0.4, >25→0.65, >18→0.9 — so any
    volatility rise de-risks immediately (fast forward-looking gauge). AT/BELOW 18
    (calm regime): 1.0, OR ``calm_scaler`` (>1.0) when ``bull_enabled`` — the ONLY
...
class RiskManager:
    ...
    def calculate_position_size(
        ...
        # pure down-only ladder (byte-identical). The >1.0 step is capped downstream
