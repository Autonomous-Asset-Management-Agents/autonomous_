...
# --- #3096 (Hebel 5): benchmark-relative metrics vs SPY ----------------------
# Active Return / Tracking Error / Information Ratio measured against a benchmark
# daily-return series (SPY). These are the ONLY measures commensurable with a
# ">SPY" goal — the absolute Sharpe family cannot express benchmark-relative
# alpha. All annualized with sqrt(252), consistent with the Sharpe convention
# below. A missing/too-short benchmark yields None — never fabricated.


def active_return_series(
    strat_returns: Optional[Sequence[float]],
    bench_returns: Optional[Sequence[float]],
) -> Optional[List[float]]:
    """Elementwise strategy-minus-benchmark daily returns.

    None on empty inputs or a length mismatch (the two series must cover the same
    aligned days; alignment is the caller's job — here we only refuse the mismatch).
    """
    if not strat_returns or not bench_returns:
        return None
    if len(strat_returns) != len(bench_returns):
        return None
    return [float(s) - float(b) for s, b in zip(strat_returns, bench_returns)]


def _sample_std(values: Sequence[float]) -> Optional[float]:
    """Sample standard deviation (ddof=1). None for < 2 observations."""
    n = len(values)
    if n < 2:
        return None
    mean = sum(values) / n
...
def tracking_error(
    active_returns: Optional[Sequence[float]], periods_per_year: int = 252
) -> Optional[float]:
    """Annualized sample stdev of the active-return series (× sqrt(252)).

    None when fewer than 2 observations. Zero when the active return is constant.
    """
    if not active_returns:
        return None
    sd = _sample_std([float(v) for v in active_returns])
    if sd is None:
        return None
    return round(sd * math.sqrt(periods_per_year), 6)


def information_ratio(
    active_returns: Optional[Sequence[float]], periods_per_year: int = 252
) -> Optional[float]:
    """Annualized IR = mean(active) * sqrt(252) / stdev(active).

    None when fewer than 2 observations OR the active return has zero variance
    (IR undefined — never a ZeroDivisionError).
    """
    if not active_returns:
        return None
    vals = [float(v) for v in active_returns]
    sd = _sample_std(vals)
    if sd is None or sd == 0.0:
        return None
    mean = sum(vals) / len(vals)
    return round(mean * math.sqrt(periods_per_year) / sd, 6)
