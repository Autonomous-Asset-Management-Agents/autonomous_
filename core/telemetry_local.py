...
# --- OBS-7 (#2640): db.statement parametrization + cardinality caps -----------
# Applied at the export choke point so EVERY DB-span emitter (present and future)
# is covered before anything is written — the source-agnostic egress guarantee.
# Regex is a HEURISTIC, not a SQL parser: it covers signed / decimal / scientific
# numbers and single-quoted strings ('' = escaped quote), and is identifier-safe
# via look-around so digits INSIDE identifiers (col1, table2) are NOT parametrized.
_DB_STMT_VALUE = re.compile(
    r"'(?:[^']|'')*'"  # single-quoted string literal
    r"|(?<![\w.])[-+]?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(?![\w.])"  # numeric literal
)

# Known high-cardinality, trade-adjacent attribute keys → digest instead of raw
# value (e.g. bot.decision = "AAPL=BUY,MSFT=SELL,…"). The digest keeps records
# distinguishable without exposing symbols/actions.
_HIGH_CARD_HASH = frozenset({"bot.decision"})


def parametrize_db_statement(stmt: str) -> str:
    """Reduce a DB statement to its parametrized shape: string & numeric literals
    → ``?``. Bounds cardinality and removes value leakage. Pure; never raises."""
    return _DB_STMT_VALUE.sub("?", stmt)


def cap_attr(key, value):
    """Value-level hygiene for one span attribute BEFORE it is written/exported
    (INF-13 #1371 line: clean before write). Parametrizes ``db.statement`` and
    digests known high-cardinality trade-adjacent keys. Pure; never raises."""
    try:
        if key == "db.statement" and isinstance(value, str):
            return parametrize_db_statement(value)
        if key in _HIGH_CARD_HASH and isinstance(value, str):
            # NON-cryptographic: bucketing high-cardinality trade-adjacent values,
            # not a security digest → usedforsecurity=False (documents intent + B324).
        ...
        # failure here means an attribute reaches the export UN-parametrized /
        # UN-hashed, i.e. a scrubbing gap the #1371 line exists to prevent.
        logger.warning(
            "cap_attr: hygiene failed for key %r — value left uncapped: %s", key, exc
        )
        return value
    return value
...
def _scrub_attrs(attrs) -> dict:
    ...
    for k, v in dict(attrs or {}).items():
        v = cap_attr(k, v)  # OBS-7: parametrize / cap cardinality first
...
# --- OBS-2 (#2635): drop high-volume, low-signal spans from the local store ----------
_HEALTH_PREFIXES = ("GET /health", "HEAD /health")


def _is_droppable_noise(record: dict) -> bool:
    """True for a span safe to drop: a SUCCESSFUL /health poll or an OPTIONS CORS-preflight
    (together ~35 % of the export, near-zero diagnostic value). An ERROR is NEVER droppable —
    the "keep 100 % errors + trading spans" invariant. Pure; never raises."""
    try:
        if record.get("status") == "ERROR":
            return False
        name = record.get("name") or ""
        return name.startswith("OPTIONS ") or name.startswith(_HEALTH_PREFIXES)
    except Exception:
        return False
...
class LocalScrubbingSpanExporter(SpanExporter):
    ...
    def __init__(
        ...
        # OBS-2 (#2635): drop successful /health + OPTIONS noise (config-gated, default on).
        try:
            from config import get_config

            self._drop_noise = bool(
                getattr(get_config(), "TELEMETRY_DROP_HEALTH_SPANS", True)
            )
        except Exception:
            self._drop_noise = True
    ...
    def _span_to_record(self, span) -> dict:
        ...
        # OBS-1 (#2634): the SDK already holds the trace/span/parent IDs, the SpanKind and
        # the Resource in-process — serialise them so the local diagnostics are correlatable
        # (trace waterfalls) and source-identifiable. IDs are hex (no PII); the Resource is
        # scrubbed like any attribute. All reads are defensive — telemetry must never raise.
        trace_id = span_id = parent_span_id = kind = None
        try:
        ...
        try:
            ...
            pass
        try:
            sk = getattr(span, "kind", None)
            kind = sk.name if sk is not None else None
        except Exception:
            pass
        try:
            res = getattr(span, "resource", None)
            resource = _scrub_attrs(dict(res.attributes)) if res is not None else {}
        except Exception:
            resource = {}
        return {
    ...
    def export(self, spans) -> SpanExportResult:
        try:
            records = (self._span_to_record(s) for s in spans)
            if self._drop_noise:
                records = (r for r in records if not _is_droppable_noise(r))
