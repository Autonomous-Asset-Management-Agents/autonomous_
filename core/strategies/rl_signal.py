...
# from the model's own metadata (RLStrategy._load_torch_model_assets; v1 ships
# with 20, v2 with 60). This module default applies when metadata is invalid.
SEQUENCE_LENGTH = 60
...
class RLSignalMixin:
    ...
    def _calculate_conviction_score(
        ...
        ADR-CONV-01: ``action`` defaults to "BUY" so the three-arg callers keep working.
        It is only read on the CONVICTION_SIGNED_ENABLED path; the legacy body below is
        untouched and runs verbatim when the flag is off.
        """
        if features is None:
            return 0.2

        # ADR-CONV-01. Read in-function so the flag is monkeypatchable and re-read per
        # call (same pattern as risk_manager's config reads).
        import config as _cfg

        if getattr(_cfg, "CONVICTION_SIGNED_ENABLED", False):
            return self._conviction_signed(features, model_pred, market_data, action)
    ...
    # ── ADR-CONV-01: agreement-gated conviction ───────────────────────────────

    def _conviction_quality(
        self, features: pd.Series, market_data: Dict[str, Any]
    ) -> float:
        """Direction-agnostic evidence quality (0.0–0.55): RSI + ADX + VIX.

        Thresholds are verbatim from the legacy body above — this is a extraction, not a
        re-tuning. These terms say "the setup is readable", not "buy" or "sell", so they
        are the same regardless of which side we are taking.
        """
        q = 0.0
        rsi = features.get("rsi_14", 50.0)
        if rsi < 30:
    ...
    def _conviction_signed(
        ...
        """Conviction that means "evidence FOR ``action``" (ADR-CONV-01).

        The legacy score adds ``min(abs(model_pred)/1.2, 1.0) * 0.40`` — so a forecast
        arguing AGAINST the trade still contributes its full weight toward taking it.
        Measured: pred=-5.764 -> 0.950 vs pred=+5.764 -> 0.800 on a BUY.

        Here, agreement gates the whole score MULTIPLICATIVELY rather than adding a signed
        term. That choice is measured, not aesthetic: conviction's entire effect on order
        size is a single step at conv ~= 0.067 (above it COMPLIANCE_MAX_ORDER_VALUE, the
        stop-loss constraint, or the cash slot binds first). A signed term moves 0.950 to
        0.550 — both still above the step, so zero orders change. Only a gate that reaches
        exactly 0.0 can cross it.

        ``/1.2`` is the legacy normaliser, kept so a strongly-agreeing trade scores as it
        did before. HOLD (direction 0.0) yields 0.0 — we are not sizing anything.
        """
        direction = 1.0 if action == "BUY" else (-1.0 if action == "SELL" else 0.0)
        agreement = max(0.0, min((model_pred * direction) / 1.2, 1.0))

        score = self._conviction_quality(features, market_data)

        # MACD keyed on the ACTION, not on sign(model_pred): the legacy version at
        # :151-161 asks "does MACD agree with the FORECAST", which credits a bearish
        # MACD/forecast pair on a BUY. Here it must agree with what we are DOING.
        macd = features.get("macd", 0.0)
        macd_signal = (
            features.get("macd_signal", 0.0) if "macd_signal" in features.index else 0.0
        )
        if direction > 0 and macd > macd_signal:
            score += 0.15
        elif direction < 0 and macd < macd_signal:
            score += 0.15

        # The gate. Evidence against the action cannot argue for it: agreement=0 -> 0.0.
        # Multiplying (never adding) is what guarantees the never-increase property.
        return max(0.0, min(1.0, score * agreement))
    ...
    def _stabilize_signal(
        ...
        # #1878 Fix 2: pred is now a real return (inverse-transformed); rescale the
        # z-space thresholds with the same affine so every decision stays identical.
        from models.torch_model import z_to_return

        LSTM_BUY = z_to_return(
            getattr(self, "scaler_y", None), 0.25, context="RLStrategy"
        )
        LSTM_SELL = z_to_return(
            getattr(self, "scaler_y", None), -0.25, context="RLStrategy"
        )
        LSTM_STRONG_BUY = z_to_return(
            getattr(self, "scaler_y", None), 0.6, context="RLStrategy"
        )
        LSTM_STRONG_SELL = z_to_return(
            getattr(self, "scaler_y", None), -0.6, context="RLStrategy"
        )

        lstm_buy = pred > LSTM_BUY
        lstm_sell = pred < LSTM_SELL
        lstm_strong_buy = pred > LSTM_STRONG_BUY
        lstm_strong_sell = pred < LSTM_STRONG_SELL
        rl_buy = raw_action == 1
    ...
    async def _get_torch_prediction(
        ...
        # (metadata via RLStrategy._load_torch_model_assets), not the module default.
        # Bare test doubles built via __new__ carry no instance attribute — the
        # module fallback keeps them patchable.
        seq_len = getattr(self, "sequence_length", None) or SEQUENCE_LENGTH

        try:
            days = seq_len + 200
            # #1878 review (Finding 1): feature warm-up floor — see
            # models/torch_model.py FEATURE_WARMUP_ROWS (sma_50 needs 50 rows,
            # MACD 26+9). seq_len alone (v1: 20) would serve placeholder features.
            ...
            # is absent. A constant across the window breaks a retrained 34-feature model.
            ...
            if features_df is None:
                return 0.0, None
            if len(features_df) < min_rows:  # #1878 review: warm-up floor, see above
                return 0.0, None
            ...
            # #1878 Fix 2: scaler_y inverse — return a real 5-day return, not a z-score.
            from models.torch_model import z_to_return

            raw_z = float(pred.cpu().numpy()[0][0])
            return (
                z_to_return(
                    getattr(self, "scaler_y", None), raw_z, context="RLStrategy"
                ),
                features_df.tail(1),
            )
