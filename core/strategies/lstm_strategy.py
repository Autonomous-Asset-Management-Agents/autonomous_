...
class LSTMDynamicStrategy(BaseStrategy):
    ...
    async def evaluate_for_symbol(
        ...
        if in_top_n and not in_pos:
            ...
            # #2965: own map first, else (flag ON) the durable reconciled
            # entry-time — only when BOTH are unknown does the #1952 fail-open
            # below apply (risk rules unaffected either way).
