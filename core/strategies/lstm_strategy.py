...
from core.entry_time_bridge import bridged_entry_time
from core.events import SignalEvent
from core.hold_policy import rank_drop_exit, resolve_hold_hours
...
from core.telemetry_attrs import inference_span_attributes
...
class _ExitVerdict(NamedTuple):
    """#3632: what the strategy's exit step decided (action + audit reason)."""

    action: str
    reason: str


# ADR-ML-COLLAPSE-01 — below this cross-sectional standard deviation the model's
# predictions carry no ordering, and the "ranking" derived from them is the input
# order (sorted() is stable). See the guard in update_lstm_rankings for the full
# rationale and the sigma=1.79 basis.
_LSTM_COLLAPSE_STD_FLOOR: float = 1e-6


class ModelCollapseError(RuntimeError):
    """The model emitted an effectively constant cross-section (ADR-ML-COLLAPSE-01).

    Raised as a model-HEALTH event for the ml_watchdog, not thrown at the caller: a
    collapse must stop capital being placed, never break the trading loop.
    """
...
class LSTMDynamicStrategy(BaseStrategy):
    """LSTM-Only Strategy: Ranking → Positionsgröße via Conviction Weight → Exit.

    Kein RL-Agent. LSTM treibt Entry + Ranking; der Rang-Exit (core/hold_policy.py,
    #1952/#2711) rotiert nach dem Min-Hold, jeder Preis-Exit ist der Intelligent Exit
    (#3632: eine Exit-Autoritaet). Manuell waehlbare Ersatzstrategie — der Round Table
    laeuft nur ueber RLStrategy.
    ...
    async def _get_torch_prediction(
        ...
        np = self.np
        pd = self.pd
        from models.feature_pool import (  # #2499: GIL-offload of create_live_features
            featurize,
        )
        ...
        try:
            ...
            try:
                features_df = await featurize(  # #2499: off-thread on the live path (inline in backtests)
                    hist, offload=not isinstance(self.client, SimulationAdapter)
                )
            ...
            with tracer.start_as_current_span("model.inference") as span:
                for _k, _v in inference_span_attributes(
                    symbol,
                    market_data,
                    model_name=getattr(self, "strategy_name", "LSTMDynamic"),
                ).items():
                ...
                span.set_attribute("aaa.model.raw_output", raw_z)
    ...
    async def update_lstm_rankings(
        ...
        for symbol in symbols:
            ...
            if snap is None:
                continue
            # PANEL-0 BUGFIX: the LIVE Alpaca SDK ``Trade`` (latest_trade) exposes the
            # trade price as ``.price``; only the simulation adapter / legacy raw shape
            # carries ``.p`` (simulation_adapter.py:160). Gating on ``.p`` ALONE dropped
            # EVERY live symbol — ``getattr(sdk_trade, "p", None)`` is None on the real
            # model — so the ranking came back empty, ``record_cycle`` never ran, and the
            # cross-sectional panel the report/vote read stayed at 0 stocks (the whole
            # "0 decisions" outage). Read BOTH names, exactly as the trading path's
            # _extract_ohlc_from_snapshot does (trading_loop.py:115). Still a tradeability
            # gate: no live trade price at all -> skip the symbol.
            trade_price = getattr(snap, "price", None) or getattr(snap, "p", None)
            if not trade_price:
                continue
            ...
            if pred is None:
                ...
                # buy list). Rank stays None → the rank-drop exit does not fire
                # either (core/hold_policy.py rank_drop_exit).
                logging.warning(
        ...
        #
        # sorted() is STABLE. If the model degenerates and emits one identical
        # prediction for every name, the sort above reorders NOTHING — it hands back
        # the input order, so the "Top-N" is just the first N symbols of the universe
        # list. The weighting below then maps identical predictions onto identical
        # weights, and the engine equal-weights an arbitrary handful of names while
        # logging "Ranked top N" as though it had ranked them. Verified: five tied
        # names produce {AAPL: .2, MSFT: .2, NVDA: .2, XOM: .2, ZTS: .2}.
        #
        # This is the cross-section twin of #1878 (per-symbol feature failure ->
        # abstain, never rank), and it takes the same line: trading on a
        # demonstrably broken input is not a strategy choice, it is a defect. So no
        # flag — the behaviour only changes in the defect case, and refusing to
        # place capital on a dead model is not a strategy change anyone opted out of.
        #
        # Threshold basis: the OOS prediction sigma is 1.79 (2024-01..2026-05,
        # N=285,445 — see _LSTM_VOTE_SCALE in core/round_table/agents.py). A healthy
        # cross-section's standard deviation sits within an order of magnitude of
        # that; 1e-6 is six-plus orders BELOW it, so a false positive (refusing to
        # trade a working model) is not reachable for any realistic dispersion, while
        # numerically-constant output — where the ranking is float noise rather than
        # bit-identical ties — is still caught. Annual review.
        if len(self._lstm_rank_cache) >= 2:
            _preds = [p for _, p in self._lstm_rank_cache]
            _spread = float(self.np.std(_preds))
            if _spread < _LSTM_COLLAPSE_STD_FLOOR:
                # ⚠ This used to call ml_watchdog.record_error("LSTMDynamic", exc).
                # That was wrong twice over, and it shipped:
                #
                # 1. CATEGORY. record_error means "the INFERENCE CRASHED". A collapsed
                #    model did not crash — it answered, and the answer carries no
                #    information. Routing "useless output" through the crash channel
                #    borrows an escalation nobody chose.
                # 2. SEVERITY. That channel ends at ml_watchdog.py:99-108 in
                #    kill_switch.trip() once errors persist past kill_threshold_sec —
                #    and this branch fires on EVERY cycle a collapse lasts, so the trip
                #    was not a risk, it was a certainty. The kill switch is GLOBAL: it
                #    halts exits, stop-losses and every healthy path too, until a human
                #    resets it. Refusing to allocate on a dead model must never cost the
                #    operator the ability to SELL what they already hold.
                #
                # The guard's job is exactly one thing: place no capital on a ranking
                # that is really the universe's input order. It says so at ERROR — loud,
                # because silence is what made the original defect expensive — and stops
                # there. Halting the bot is a separate, deliberate decision and does not
                # belong to a per-cycle data check.
                exc = ModelCollapseError(
                    f"LSTM cross-section carries no ordering: std={_spread:.3e} over "
                    f"{len(_preds)} symbols (floor {_LSTM_COLLAPSE_STD_FLOOR:.0e}). "
                    f"The model emitted an effectively constant prediction."
                )
                logging.error(
                    "LSTMDynamic: MODEL COLLAPSE — %s No allocation this cycle "
                    "(ranking would be the universe's input order, not a ranking). "
                    "Exits and every other path stay live.",
                    exc,
                )
                self._lstm_rank_cache = []
                self._allocation_weights = {}
                return
        # RPT-8: observe this cycle's cross-section for the auditable report.
        # Read-only telemetry, flag-gated + lazy-imported so update_lstm_rankings
        # is byte-identical when REPORT_GENERATOR_V2_ENABLED is OFF (the ship
        # default). It never reads/writes the decision path below, and any error
        # is swallowed — panel recording can NEVER break the trading loop.
        if self._lstm_rank_cache and getattr(
            config.get_config(), "REPORT_GENERATOR_V2_ENABLED", False
        ):
            try:
                from core.report.lstm_panel_store import get_store

                get_store().record_cycle(current_time, self._lstm_rank_cache)
            except Exception:  # noqa: BLE001 — telemetry must never break the loop
                logging.debug("RPT-8 panel record skipped", exc_info=True)
    ...
    async def evaluate_for_symbol(
        ...
        if in_top_n and not in_pos:
            ...
            # #3632 one exit authority: the rank-drop rebalance (#1952/#2711 min-hold +
            # hysteresis, core/hold_policy.py) stays a strategy rule; every price exit
            # is the intelligent exit. #2965: own map first, else (flag ON) the durable
            # reconciled entry-time — only when BOTH are unknown does the #1952
            # fail-open in resolve_hold_hours apply.
            hours_held = resolve_hold_hours(
                bridged_entry_time(self, symbol), current_time
            )
            hwm = self.high_water_marks.get(symbol, avg)
            decision = self._exit_decision(
                symbol, avg, curr, hwm, hours_held, in_top_n, lstm_rank
            )
    ...
    def _exit_decision(
        ...
        """#3632: rank-drop rebalance (strategy rule), then the ONE exit authority.

        A failing intelligent exit is logged as WARNING with its stack trace and holds
        this cycle; there is no second price rule set (the broker stop caps the risk).
        """
        rank = rank_drop_exit(
            in_top_n=in_top_n,
            lstm_rank=lstm_rank,
        ...
        if rank.sell:
            return _ExitVerdict("SELL", rank.reason)
        try:
            ctx = IntelligentPositionContext(
                ...
                high_water_mark=hwm,
                hours_held=hours_held,
                lstm_prediction=float(self._get_pred_for_symbol(symbol) or 0.0),
            )
            analysis = analyze_exit(ctx)
        except Exception as exc:  # noqa: BLE001 — kein zweites Regelwerk
            logging.warning(
                "LSTMDynamic [%s]: intelligent exit failed -> HOLD this cycle: %s",
                symbol,
                exc,
                exc_info=True,
            )
            return _ExitVerdict("HOLD", f"exit error: {exc}")
        if analysis.should_sell:
            return _ExitVerdict("SELL", analysis.reason)
        return _ExitVerdict("HOLD", analysis.reason)
    ...
    async def run_for_symbol(
        ...
        if in_pos:
            ...
            # #3632 one exit authority (see evaluate_only): rank-drop rebalance first,
            # then the intelligent exit. #2965 entry-time bridge, #1952 fail-open.
            hours_held = resolve_hold_hours(
                bridged_entry_time(self, symbol), current_time
            )
            decision = self._exit_decision(
