...
from core.ml.asset_integrity import safe_joblib_load, safe_read_bytes, safe_torch_load
...
from models.torch_model import LSTMModel, get_lstm_paths, resolve_sequence_length
...
# v2 with 60). This module default applies when metadata carries no valid value.
SEQUENCE_LENGTH = 60
_MODELS_MANIFEST_NAME = "models_manifest.json"
...
def _stats_filename_from_manifest(version: str) -> Optional[tuple]:
    """VecNormalize-Stats-Dateiname für *version* aus data/models_manifest.json (#1875).

    Kandidaten sind ``.pkl``-Einträge, deren filename ODER purpose auf Stats
    hinweist. Rückgabe ``(filename, version_matched)``: ``version_matched`` ist
    True, wenn der Eintrag die *version* (bzw. deren Suffix) im Dateinamen
    trägt; ein einzelner Kandidat ohne Versionsbezug kommt als schwacher Treffer
    (False) zurück. ``AAA_MODELS_MANIFEST`` überschreibt den Manifest-Pfad
    (gleiche Semantik wie core/ml/asset_integrity.py). Ein fehlendes oder
    beliebig kaputtes Manifest liefert None — es darf nie den Boot bricken.
    """
    override = os.environ.get("AAA_MODELS_MANIFEST")
    manifest_path = (
        override
        if override and os.path.isfile(override)
        else os.path.join(_data_dir, _MODELS_MANIFEST_NAME)
    )
    try:
        ...
        candidates = [
            ...
            and entry["filename"].endswith(".pkl")
            and (
                "stats" in entry["filename"].lower()
                or "stats" in str(entry.get("purpose", "")).lower()
            )
        ]
    except Exception:  # malformed manifest must never brick the engine boot
        return None
...
def _rl_stats_file(version: str) -> str:
    """Pfad der VecNormalize-Stats für *version* — erste existierende Datei gewinnt.

    Auflösungskette (#1875 / MLA-1 — der alte Loader riet ``rl_stats_{suffix}.pkl``
    und verfehlte das models-v1.0-Bundle ``rl_agent_v3_dsr_stats.pkl``):
      1. Manifest-Eintrag, der die *version* im Dateinamen trägt (Bundle-Wahrheit),
      2. ``{version}_stats.pkl`` (Bundle-Namenskonvention),
      3. ``rl_stats_{suffix}.pkl`` (Legacy-Trainer-Konvention, z.B. v5-Trainer),
      4. einzelner Manifest-Stats-Eintrag OHNE Versionsbezug (letzter Ausweg —
         nachrangig, damit z.B. ein v5-Agent nie still die v3-Stats eines
         Fremd-Eintrags vor seinen eigenen ``rl_stats_v5.pkl`` zieht).

    Greift nur ein Fallback (3. oder 4.), wird WARNING geloggt (CODING_POLICY
    §5.6). Existiert keine Datei, kommt der primäre Kandidat zurück (Caller
    loggt ERROR, siehe RLStrategy._load_vec_normalize).
    """
    manifest_hit = _stats_filename_from_manifest(version)
    candidates = []
    if manifest_hit is not None and manifest_hit[1]:
        candidates.append(os.path.join(_data_dir, manifest_hit[0]))
    candidates.append(os.path.join(_data_dir, f"{version}_stats.pkl"))
    legacy = os.path.join(_data_dir, f"rl_stats_{version.split('_')[-1]}.pkl")
    candidates.append(legacy)
    weak = None
    if manifest_hit is not None and not manifest_hit[1]:
        weak = os.path.join(_data_dir, manifest_hit[0])
    ...
    for path in candidates:
        if os.path.exists(path):
            if path == legacy and path != candidates[0]:
                logging.warning(
                    "⚠️ RL stats resolved via legacy filename %s — expected "
                    "bundle name %s not found (#1875).",
                    path,
                    candidates[0],
                )
            elif path == weak:
                logging.warning(
                    "⚠️ RL stats resolved via manifest entry %s WITHOUT a "
                    "version match for '%s' — verify the stats belong to this "
                    "agent (#1875).",
                    path,
                    version,
                )
...
class RLStrategy(RLSignalMixin, RLExecutionMixin, BaseStrategy):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if os.path.exists(rl_agent_file):
            try:
                ...
                try:
                    ...
                    rl_bytes = safe_read_bytes(rl_agent_file)
        ...
        self._load_vec_normalize(rl_stats_file)
        ...
        try:
            max_positions = 10
            try:
                # #3371: use the SHARED effective-cap helper (the same one order_executor's
                # sizing relies on) instead of reading config.MAX_POSITIONS directly. With the
                # full universe (FULL_UNIVERSE_TRADING_ENABLED, the default) it returns the
                # operator's FULL_UNIVERSE_MAX_POSITIONS; else config.MAX_POSITIONS. Reading the
                # bare MAX_POSITIONS (=10) here left the enforcing PM pinned at 10, so raising
                # the Console position count had no effect (setup.json was saved + injected,
                # just never read on this path) — the two disjoint PMs disagreed on the cap.
                from core.risk_manager import effective_max_positions

                max_positions = effective_max_positions()
            except (ImportError, ValueError, TypeError) as _cap_exc:
                # §5.6 / WoW §2.2: a fallback value MUST be visible at WARNING, never
                # silent — otherwise a mispinned cap (the #3371 class of bug) hides again.
                logging.warning(
                    "RLStrategy: could not read effective_max_positions (%s) — "
                    "falling back to max_positions=%d.",
                    _cap_exc,
                    max_positions,
                    exc_info=True,  # OTel: preserve the stack trace (review FINDING-01)
                )
    ...
    def _load_vec_normalize(self, rl_stats_file: str) -> None:
        """Lädt die VecNormalize-Stats; macht fehlende Stats sichtbar (#1875).

        Die RecurrentPPO-Policy wurde ausschließlich auf normalisierten
        Beobachtungen trainiert (norm_obs=True, clip_obs=10). Läuft sie ohne
        Stats auf Raw-Scale-Obs, degradieren die RL-Votes still zu Rauschen —
        deshalb ERROR (nie still), wenn die Policy geladen ist, aber keine
        Stats verfügbar sind.
        """
        if os.path.exists(rl_stats_file):
            try:
                logging.info("🔄 Loading RL stats from %s...", rl_stats_file)
                self.vec_normalize = safe_joblib_load(rl_stats_file)
                logging.info("✅ RL stats loaded successfully!")
            except Exception as e:
                logging.error("❌ FAILED to load RL stats: %s: %s", type(e).__name__, e)
        if self.rl_model is not None and self.vec_normalize is None:
            logging.error(
                "❌ VecNormalize stats missing or unloadable (%s) — RecurrentPPO "
                "policy was trained with norm_obs=True; raw-scale observations "
                "degrade RL votes to noise (#1875).",
                rl_stats_file,
            )
    ...
    def _load_torch_model_assets(self) -> None:
        ...
        try:
            ...
            self.sequence_length = resolve_sequence_length(
                self.torch_metadata, SEQUENCE_LENGTH, "RLStrategy"
            )
    ...
    async def evaluate_for_symbol(
        ...
        """Evaluate-only entry point with per-symbol lock. No order execution.
        ...
        if symbol not in self._per_loop_symbol_locks[loop_id]:
            self._per_loop_symbol_locks[loop_id][symbol] = asyncio.Lock()
        async with self._per_loop_symbol_locks[loop_id][symbol]:
            return await self._evaluate_for_symbol_impl(
                symbol, ohlc_data, market_data, current_time
            )
