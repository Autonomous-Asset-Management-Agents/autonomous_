        ...
        try:
                        ...
                        if expected_cost > max(0, cash_available):
                            ...
                            if self.compliance_guardian:
                                self.compliance_guardian.log_execution_outcome(
                                    compliance_order,
                                    False,
                                    f"cash gate: order ${expected_cost:.2f} exceeds cash ${reg_cash:.2f}",
                                )
                            return False
                        current_bp = reg_cash
                    else:
            ...
            if inspect.iscoroutinefunction(submit_method):
                ...
                if is_simulation:
                    ...
                    _schutz_exit = bool(is_protective_exit) and side == "sell"
                    try:
                    ...
                    def _durchs_tor(auftrag):
                        ...
                        art = (
                            "stop"
                            if _schutz_exit
                            else ("entry" if side == "buy" else "trim")
                        )
                        ...
                        entscheidung, antwort = gateway_for(
                            self.client
                        ).submit_with_result(intent, request=auftrag)
            ...
            if not is_simulation:
                ...
                if self.compliance_guardian:
                    ...
                    self.compliance_guardian.log_execution_outcome(
                        compliance_order, True, "submitted to broker"
                    )
