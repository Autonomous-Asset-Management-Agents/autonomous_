...
class RLExecutionMixin:
    ...
    _run_for_symbol_impl
      ...
      ├── _check_exit()             → triggered, updated signal (#3632: one exit authority)
    ...
    def _check_exit(
        ...
        """Prueft die Exit-Bedingungen einer Position (#3632: EINE Exit-Autoritaet).

        ``core.intelligent_exit.analyze_exit`` ist das einzige Regelwerk (Verlust-Stufen
        aus den Settings, Hard Stop, Trailing-Stufen, Momentum). Es gibt keinen zweiten
        Pfad mehr: wirft die Analyse, wird der Fehler als WARNING mit Stacktrace geloggt,
        der Zyklus haelt (HOLD) und ``exit_error`` steht im Ergebnis fuer den
        Decision-Record; die Position bleibt vom Broker-Stop gedeckelt (ADR-020,
        ``BROKER_STOPS_ENABLED``).

        Returns:
            dict mit keys: triggered (bool), signal (str), optional tier / reason /
            exit_error
        """
        if not in_position:
        ...
        # #2965: own map first, else (flag ON) the durable reconciled entry-time.
        # Unknown stays current_time (hours=0) — the pre-existing conservative
        # default; with the bridge a reconciled position gets its REAL age, so
        # the min-hold gate neither freezes nor fail-opens for it.
        _bridged = bridged_entry_time(self, symbol)
        entry_time = _bridged if _bridged is not None else current_time
        hours_held = (current_time - entry_time).total_seconds() / 3600

        try:
            momentum_history = list(
                getattr(self, "_signal_history", {}).get(symbol, [])
            )[-5:]
            ...
            Exception
        ) as exc:  # noqa: BLE001 — kein zweites Regelwerk, kein stiller Pfad
            logger.warning(
                "[%s] intelligent exit failed -> HOLD this cycle; the broker stop "
                "(BROKER_STOPS_ENABLED) remains the cap: %s",
                symbol,
                exc,
                exc_info=True,
            )
            return {"triggered": False, "signal": "HOLD", "exit_error": str(exc)}
        ...
        if analysis.should_sell:
            ...
            # #3180: carry the risk/opinion tier so the caller can apply the
            # consensus-retention gate to OPINION exits only (RISK never gated).
            # Fail-safe default 'risk' if the field is somehow absent.
            return {
                "triggered": True,
                "signal": "SELL",
                "tier": getattr(analysis, "tier", "risk"),
    ...
    def _log_decision_trace(
        ...
        # #2958: monitor_loop writes {"vix": None} on a double outage
        # (market_regime value=None) — float(None) raised TypeError and killed
        # the decision trace. Non-numeric/absent readings keep the defaults.
        _md_vix = market_data.get("vix")
        _vix_level = (
            float(_md_vix)
            if isinstance(_md_vix, (int, float)) and not isinstance(_md_vix, bool)
            else 20.0
        )
        _md_regime = market_data.get("regime")
        _market_regime = (
            _md_regime if isinstance(_md_regime, str) and _md_regime else "normal"
        )
    ...
    async def _run_for_symbol_impl(  # noqa: C901
        ...
        if signal == "SELL" and in_pos:
            from core.consensus_retention import consensus_retention_veto

            _exit_kind = exit_info.get("tier", "risk") if triggered_exit else "model"
            if consensus_retention_veto(
                symbol, _exit_kind, getattr(self, "portfolio_manager", None)
            ):
                self.log_thought(
                    f"[{symbol}] 🛑 SELL RETAINED — live round-table consensus still "
        ...
        if signal == "BUY" and not in_pos and size > 0:
            ...
            order_success = await self._submit_order_safe(
                ...
                is_protective_exit=ist_schutz_exit(triggered_exit, exit_info),
            )
            if order_success:
                if self.portfolio_manager:
