...
# #3632: die EINE Exit-Autoritaet der Engine (smart_exit.py wurde zurueckgebaut).
...
def _runtime_config():
    """Laufzeit-Config (ein WORM-Apply wirkt sofort); getrennt, damit Tests sie
    patchen koennen (#3632 Verlust-Stufen, #3662 Panik-Fenster)."""
    import config
...
# #3632: die drei Verlust-Stufen sind Settings (settings.py LOSS_WATCH_PCT / LOSS_CUT_PCT /
# LOSS_ESCALATION_PCT; Registry-Klemme WATCH > CUT > ESCALATION > HARD_STOP) und werden je
# Aufruf ueber ``_loss_tiers()`` gelesen. Die Modul-Werte sind der Auslieferungsstand
# (byte-identisch: -2 / -4 / -6 %) und der Rueckfall, wenn keine Config lesbar ist.
LOSS_TIER_1_PCT = -2.0  # Beobachtungsmodus (Basisdruck 40)
LOSS_TIER_2_PCT = -4.0  # Loss-Cut (Basisdruck 70 -> verkauft ab 24 h Haltedauer)
LOSS_TIER_3_PCT = -6.0  # Eskalation (Basisdruck 90 -> verkauft sofort)
HARD_STOP_LOSS_PCT = _config("HARD_STOP_LOSS_PCT", -8.0)  # Absoluter Stop (fest)


@dataclass(frozen=True)
class LossTiers:
    """Die in einem Aufruf wirksamen Verlust-Stufen (Prozent, negativ)."""

    watch: float = LOSS_TIER_1_PCT
    cut: float = LOSS_TIER_2_PCT
    escalation: float = LOSS_TIER_3_PCT
    hard_stop: float = HARD_STOP_LOSS_PCT


def _loss_tiers() -> LossTiers:
    """Stufen zur Aufrufzeit aus der Laufzeit-Config (ein WORM-Apply wirkt sofort).

    Rueckfall auf den Auslieferungsstand nur, wenn die Config nicht lesbar ist — dann
    als WARNING mit Stacktrace (CODING_POLICY 5.6), nie still.
    """
    try:
        cfg = _runtime_config()
        return LossTiers(
            watch=float(cfg.LOSS_WATCH_PCT),
            cut=float(cfg.LOSS_CUT_PCT),
            escalation=float(cfg.LOSS_ESCALATION_PCT),
            hard_stop=float(cfg.HARD_STOP_LOSS_PCT),
        )
    except Exception:  # noqa: BLE001 — Config nicht lesbar: Auslieferungsstand
        logger.warning(
            "intelligent_exit: loss tiers unreadable from config -> shipped defaults "
            "%.1f / %.1f / %.1f / %.1f",
            LOSS_TIER_1_PCT,
            LOSS_TIER_2_PCT,
            LOSS_TIER_3_PCT,
...
)  # Auslieferungsstand des Panik-Fensters (Rueckfall; #3662 liest je Aufruf)


@dataclass(frozen=True)
class PanicWindow:
    """#3662: Panik-Schutz dieses Aufrufs — Schalter und Fenster in Stunden."""

    enabled: bool = True
    hours: float = PANIC_PROTECTION_HOURS


def _panic_window() -> PanicWindow:
    """Schalter ``PANIC_PROTECTION_ENABLED`` und Fenster ``PANIC_PROTECTION_HOURS``
    zur Aufrufzeit (Settings, Registry-geklemmt) — dieselbe Leseroutine wie
    ``_loss_tiers``. Rueckfall auf den Auslieferungsstand (an, 2,0 h) nur, wenn die
    Config nicht lesbar ist, dann als WARNING mit Stacktrace (CODING_POLICY 5.6)."""
    try:
        cfg = _runtime_config()
        return PanicWindow(
            enabled=bool(getattr(cfg, "PANIC_PROTECTION_ENABLED", True)),
            hours=float(getattr(cfg, "PANIC_PROTECTION_HOURS", PANIC_PROTECTION_HOURS)),
        )
    except Exception:  # noqa: BLE001 — Config nicht lesbar: Auslieferungsstand
        logger.warning(
            "intelligent_exit: panic window unreadable from config -> shipped "
            "default (enabled, %.1f h)",
            PANIC_PROTECTION_HOURS,
            exc_info=True,
        )
        return PanicWindow()
...
class PositionContext:
    ...
    hours_held: float
    # #3632: optional - the analysis works on hours_held; callers without a durable
    # entry time (LSTMDynamic, acceptance tests) need not fabricate one.
...
def analyze_exit(ctx: PositionContext) -> ExitAnalysis:
    ...
    # #3632: Verlust-Stufen dieses Aufrufs (Settings, Registry-geklemmt).
    tiers = _loss_tiers()

    # === PANIK-SCHUTZ (#3662: Schalter + Fenster je Aufruf): im Fenster nur Hard Stop ===
    panic = _panic_window()
    if panic.enabled and ctx.hours_held < panic.hours:
        if pnl_pct <= tiers.hard_stop:
            analysis.should_sell = True
            analysis.reason = f"HARD STOP: {pnl_pct:.1f}% (unter {tiers.hard_stop}%)"
            analysis.confidence = 1.0
            analysis.total_score = 100
            analysis.tier = "risk"  # #3180: hard stop — never gate-eligible
            return analysis
        else:
            analysis.reason = f"Panic protection: Position unter {panic.hours:.1f}h alt"
            return analysis

    # === VERLUST-MANAGEMENT ===
    analysis.loss_pressure_score = _calculate_loss_pressure(
        pnl_pct, ctx.hours_held, tiers
    )
    ...
    if analysis.loss_pressure_score >= 90:
        ...
        # #3632 Audit: der angewandte Stufenwert steht im Grund (Decision-Record).
        applied = (
            tiers.hard_stop
            if pnl_pct <= tiers.hard_stop
            else tiers.escalation if pnl_pct <= tiers.escalation else tiers.cut
        )
        analysis.reason = (
            f"LOSS CUT: {pnl_pct:.1f}% Verlust (Stufe {applied:.1f}%) "
            f"(Score: {analysis.loss_pressure_score:.0f})"
        )
        return analysis
...
def _calculate_loss_pressure(
    pnl_pct: float, hours_held: float, tiers: Optional[LossTiers] = None
) -> float:
    """
    Berechnet Verkaufsdruck für Verlierer.
    Je tiefer im Minus + je länger gehalten = mehr Druck.
    #3632: Stufen aus den Settings (``tiers``); ohne Argument zur Aufrufzeit gelesen.
    """
    if pnl_pct >= 0:
        return 0.0
    t = tiers if tiers is not None else _loss_tiers()

    # Basis-Score nach Verlust-Tiefe (more aggressive)
    base_score = 0.0
    if pnl_pct <= t.hard_stop:
        return 100.0  # Absoluter Stop
    elif pnl_pct <= t.escalation:
        base_score = 90.0
    elif pnl_pct <= t.cut:
        base_score = 70.0  # -4% should be serious
    elif pnl_pct <= t.watch:
        base_score = 40.0
    else:
