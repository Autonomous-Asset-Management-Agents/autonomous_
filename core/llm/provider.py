...
class OllamaProvider:
    ...
    def _payload(self, prompt: str, max_output_tokens: int) -> Dict[str, Any]:
        return {
            ...
            # default unloads it after ~5 min idle, so each timed cycle (news 5m,
            # round-table 30m) cold-reloads the runner — on older Ollama builds
            # that runner restart flashes a console window. "30m" spans the
            # slowest cycle; OLLAMA_KEEP_ALIVE overrides ("-1" = never unload).
