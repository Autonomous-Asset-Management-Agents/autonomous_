...
class MarketRegimeModel:
    """Classifies current market regime based on VIX data with SPY fallback."""

    def __init__(self, data_provider: HistoricalDataProvider, clock: ClockPort):
        self.clock = clock
    ...
    def get_market_regime(
        ...
        if current_date is None:
            current_date = self.clock.now().astimezone(None).replace(tzinfo=None)
        ...
        if not sim_client:
            if cache_key in self.vix_cache:
                ...
                if (
                    self.clock.now().astimezone(None).replace(tzinfo=None) - cached_time
                    < ttl
                ):
                    return cached_data
        ...
        try:
            ...
            if (
                ...
                if not close_series.empty:
                    ...
                    if not sim_client:
                        self.vix_cache[cache_key] = (
                            result,
                            self.clock.now().astimezone(None).replace(tzinfo=None),
                        )
