...
class HistoricalDataProvider:
    ...
    def get_batch_data(
        ...
        try:
            ...
            if results:
                ...
                try:
                    pd.concat(results, names=["symbol", "timestamp"]).to_parquet(
                        cache_file_path
                    )
    ...
    def get_nasdaq_symbols(self) -> List[str]:
        """
        Tries to fetch NASDAQ-100 symbols from Wikipedia, falling back to a static list of tech leaders.
        """
        try:
            logging.info("Fetching NASDAQ-100 symbol list from Wikipedia...")
            url = "https://en.wikipedia.org/wiki/Nasdaq-100"
            headers = {"User-Agent": "AI-Trading-Bot/1.0 (contact@aaagents.de)"}
            response = requests.get(url, headers=headers, timeout=30)
            response.raise_for_status()
            ...
            logging.info(
                f"Successfully loaded {len(symbols)} NASDAQ-100 symbols from Wikipedia."
            )
            return symbols
        except Exception as e:
            logging.warning(
                "Failed to fetch NASDAQ symbols from Wikipedia: %s. Using fallback list.",
                e,
            )
            return [
                "AAPL",
