...
class HitlQueue:
    ...
    async def count_approved() -> int:
        """Read-only count of approved-but-undrained orders (ADR-OBS-01 / PR B).

        Counts ``hitl:approved:*`` keys for the /engine-diagnostics ``hitl.approved``
        field. UNLIKE ``claim_approved`` this NEVER mutates: no move to inflight, no
        delete — pure observation. ``0`` without Redis.
        """
        redis = await RedisClient.get_redis()
        if redis is None:
            return 0
        return len(await redis.keys(_APPROVED_PREFIX + "*"))
