...
# #2711 (P0): min-hold is a HARD gate — rank-collapse cannot override it. False = legacy OR.
ROTATION_MIN_HOLD_HARD_GATE = bool(
    _smart_exit_config("ROTATION_MIN_HOLD_HARD_GATE", True)
)
...
def derive_position_hwm(
    ...
    RESTART-PROOF path (the V-2 finding): the strategies' in-memory HWM maps
    (lstm_strategy.py:107/:635, rl_execution.py:198) die on restart. The broker
    is the source of truth: the #1994 reconcile pattern
    (core/engine/entry_time_reconcile.py) already reconstructs a durable
    ``entry_time`` from Alpaca fills — the daily bar HIGHS since that entry_time
    then yield a restart-surviving HWM with NO new DB table. This function is the
    PURE derivation; the broker bar-fetch wiring is B3/follow-up.

    HONEST LIMITATION (documented, never silent-wrong): until the bar wiring
    lands, callers pass only ``session_hwm`` — after a restart the trailing
    policy then arms from the restore-time price (max of entry/current/session),
    i.e. it can only be TOO CONSERVATIVE (a later exit proposal), never a
    fabricated peak. None-safe and never raises: malformed bars are skipped.
    """
    candidates = []
...
def should_sell_smart(
    ...
    min_hold_hard_gate: bool = ROTATION_MIN_HOLD_HARD_GATE,
    trailing_from_peak_enabled: Optional[bool] = None,
    trailing_from_peak_pct: Optional[float] = None,
) -> ExitDecision:
    """
    ...
    3b. RTR-5/B2 (#2401, flag EXIT_POLICY_TRAILING_ENABLED, default OFF):
       trailing-from-peak profit-taking policy — position in profit AND price
       fallen >= TRAILING_FROM_PEAK_PCT below the position HWM -> SELL proposal
       with reason "trailing_from_peak". Gated by min_hold_hours (min-hold WINS,
       churn-safe); flag OFF -> this rule does not exist (byte-identical).
       HITL-konform: an ExitDecision PROPOSAL through this existing contract —
       downstream risk/compliance/HITL gates and PortfolioManager
       can_sell_position stay binding; no new autonomous SELL path.
    ...
    if not in_top_n and lstm_rank is not None and lstm_rank > top_n_size:
        ...
        # #2711 hard gate: inside min-hold, a rank collapse alone may NOT exit — normal
        # rank volatility was flushing 60-second-old positions (order forensics 04.-07.08).
        if min_hold_ok or (rank_collapsed and not min_hold_hard_gate):
    ...
    # 3b. RTR-5/B2 (#2401) — trailing-from-peak profit-taking policy (the 159k
    #     gap: at the account peak the static TP never fired and no policy
    #     secured the gain from the position HWM). Flag-gated at CALL time
    #     (EXIT_POLICY_TRAILING_ENABLED, default OFF => this block returns
    #     nothing and the legacy rule order below is byte-identical). Fires only
    #     when the position is IN PROFIT (losses are stop-loss jurisdiction) and
    #     only past min_hold_hours (min-hold WINS — churn-safe; the execution
    #     layer's can_sell_position stays binding on top). Checked BEFORE the
    #     legacy 3% trail so an armed >=TRAILING_FROM_PEAK_PCT collapse is
    #     audited under its policy name "trailing_from_peak"; smaller dips still
    #     fall through to rule 4 unchanged. HWM source: see derive_position_hwm
    #     (V-2 — session-HWM until the #1994-pattern bar wiring lands in B3).
    if trailing_from_peak_enabled is None:
        trailing_from_peak_enabled = bool(
            _smart_exit_config("EXIT_POLICY_TRAILING_ENABLED", False)
        )
    if trailing_from_peak_enabled:
        if trailing_from_peak_pct is None:
            trailing_from_peak_pct = float(
                _smart_exit_config("TRAILING_FROM_PEAK_PCT", 10.0)
            )
        if (
            hours_held >= min_hold_hours
            and pnl_pct > 0
            and drawdown_from_high_pct >= trailing_from_peak_pct
        ):
            return ExitDecision(
                "SELL",
                f"trailing_from_peak: {drawdown_from_high_pct:.1f}% below position "
                f"HWM {high_water_mark:.2f} (limit {trailing_from_peak_pct:.1f}%, "
                f"still +{pnl_pct:.1f}% vs entry) — exit proposal (HITL gates apply)",
            )
