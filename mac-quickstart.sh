...
# LSTM/RL models come from GitHub Releases via scripts/gcs_sync_on_start.py (SHA-256-verified against
# data/models_manifest.json). The Docker CMD + Windows installer run it; the from-source path must too,
# or the ML agents vote a neutral 0.5 forever and /health/deep stays "degraded" (finding A10). The sync
# script re-downloads its manifest files (~7 MB) on every run and ALWAYS exits 0 (failures only WARN in
# its own log) — hence the local presence gate and the artifact post-check below.
sync_models() {
  if [ -f "data/lstm_model.pth" ] && [ -f "data/rl_agent_v3_dsr.zip" ]; then
    info "ML models present — skipping sync (delete data/*.pth to force a re-sync)."
    return 0
  fi
  info "Syncing ML models (SHA-256-verified, ~7 MB)…"
  python scripts/gcs_sync_on_start.py || true
  if [ ! -f "data/lstm_model.pth" ]; then
    warn "Model sync incomplete — engine boots ML-degraded (LSTM/RL vote neutral 0.5). Check network."
  fi
}

# A stray .env is loaded by config.py:13 via load_dotenv() — WITHOUT override=True, so it only fills
# variables that are otherwise UNSET; it does NOT beat the shell-injected env or the OS keychain. The
# remaining trap: if the shell does not already set PAPER_TRADING / broker keys, a leftover .env (e.g.
# from a Docker/cloud experiment) can silently flip the engine to live or inject live credentials.
# Informational only — never fails the doctor.
check_stray_env() {
  if [ -f ".env" ] && grep -qE '^[[:space:]]*(ALPACA_|PAPER_TRADING)' ".env"; then
    warn ".env file with broker settings found — config.py loads it (load_dotenv, no override) so it"
    warn "  fills any UNSET var (could force PAPER_TRADING=false or inject live keys). Review/remove it."
  fi
}
...
PY
  ...
  # Fix A10: pull the SHA-256-verified LSTM/RL models before boot, else /health/deep stays "degraded".
  sync_models
