...
try:
    ...
    try:
        ...
        settings._config_state.DATA_DIR = mock_dir
        settings._config_state.USER_DATA_DIR = mock_dir
    except ImportError:
        pass
except ImportError:
    pass
...
# ---------------------------------------------------------------------------
# #3438 — kein Test hinterlaesst einen Specialist-Registry-Thread
# ---------------------------------------------------------------------------
#
# Neun Tests in drei Dateien booten eine ``BotEngine``; deren Boot-Pfad startet seit
# #2408 die ``StockSpecialistRegistry`` mit einem daemonischen Thread. Der lief nach
# dem Ende seines Tests weiter, machte echte HTTP-Aufrufe und schrieb auf prozessweite
# Zaehler — und erzeugte damit Fehlschlaege in voellig unbeteiligten Tests. Belegt sind
# drei verschiedene Auspraegungen (RCA §4): ``requests_mock.called`` ist ``True`` in
# ``test_slack_webhook``, ein ``AsyncClient`` ruft ``api.finra.org`` in
# ``test_specialist_edgar_grounding``, und ein globaler Zaehler steht unter ``-n 4``
# auf ``ok: 2`` statt ``ok: 1``.
#
# **Diese Fixture ruft den ECHTEN Teardown.** Sie unterdrueckt den Registry-Start
# nicht — das waere Option B des Plans und wurde verworfen, weil sie den Boot-Pfad aus
# der Pruefung genommen und den Produktionsdefekt dauerhaft verdeckt haette.
#
# Sie war in einer frueheren Fassung schon einmal gebaut und **gemessen wirkungslos**
# (Laufzeit 5,7 s → 12,4 s, die Threads ueberlebten), weil ``stop()`` selbst defekt war.
# Genau deshalb ist die Reihenfolge des Plans zwingend: Erst mit dem Abbruch aus
# Schicht 1 kann diese Fixture ueberhaupt etwas ausrichten.


# Gemessen, nicht erschlossen: Ein Engine-Boot hinterlaesst DREI Threads, nicht einen.
# Die RCA nannte nur den ersten.
_ENGINE_THREADS = (
    "SpecialistRegistryThread",
    "SpecialistWarmup",
    "StallMonitorThread",
    "HeartbeatThread",
)
...
def _lebende_engine_threads():
    import threading

    return [
        t
        for t in threading.enumerate()
        if any(t.name.startswith(n) for n in _ENGINE_THREADS)
    ]


def _besitzer(thread):
    """Das Objekt hinter einem Thread — gebundene Methode oder Closure.

    ``_refresh_loop`` ist eine gebundene Methode (``__self__``); ``_check`` und
    ``stall_thread`` sind Closures, die ``self`` in einer Zelle tragen. Beide Wege
    werden probiert, damit die Fixture den ECHTEN Teardown rufen kann statt zu raten.
    """
    ziel = getattr(thread, "_target", None)
    if ziel is None:
        return None
    selbst = getattr(ziel, "__self__", None)
    if selbst is not None:
        return selbst
    for zelle in getattr(ziel, "__closure__", None) or ():
        try:
            inhalt = zelle.cell_contents
        except ValueError:  # noch nicht gebundene Zelle
            continue
        if hasattr(inhalt, "stop_strategy") or hasattr(inhalt, "stop"):
            return inhalt
    return None


@pytest.fixture(autouse=True)
def stoppe_engine_threads(request):
    """Ruft nach jedem Test den echten Teardown jeder Engine bzw. Registry."""
    yield

    for thread in _lebende_engine_threads():
        besitzer = _besitzer(thread)
        if besitzer is None:
            continue
        try:
            # Die Engine zuerst: ``stop_strategy()`` setzt ``_shutdown_event`` (womit
            # Warmup und Stall-Monitor sich beenden) UND haelt die Registry an (#3438).
            if hasattr(besitzer, "stop_strategy"):
                besitzer.is_simulation = True  # erzwingt den Teardown-Pfad
                besitzer.stop_strategy()
            elif hasattr(besitzer, "stop"):
                besitzer.stop()
        except Exception as exc:  # noqa: BLE001 — darf keinen Test kippen
            # Review-Finding POLICY-02 zu #3448: NIE stillschweigend schlucken.
            # Ein Aufraeumfehler darf den Test nicht kippen — aber er muss sichtbar
            # sein, sonst ist er von einem geglueckten Teardown nicht zu
            # unterscheiden. Genau dieser stille Fehlschlag ist der Defekt, den
            # #3445 an der Reviewer-Automatik beschreibt.
            import logging

            logging.warning(
                "#3438: Teardown von %s nach %s fehlgeschlagen (%s) — der Thread "
                "bleibt womoeglich stehen und kippt einen spaeteren Test.",
                type(besitzer).__name__,
                request.node.nodeid,
                exc,
                exc_info=True,
            )

    uebrig = _lebende_engine_threads()
    if uebrig:
        import logging

        # Laut melden statt still hinnehmen. Ein hinterlassener Thread ist die Ursache
        # von Fehlschlaegen in FREMDEN Tests — er darf nicht unbemerkt bleiben.
        logging.error(
            "#3438: Nach %s leben noch %d Engine-Thread(s). Der Teardown "
            "hat nicht gewirkt; nachfolgende Tests koennen aus diesem Grund scheitern.",
            request.node.nodeid,
            len(uebrig),
        )


# ── #3396 (ARC-E3): Stufen- und Uebergabe-Marker sind die Quelle ─────────────
# Das Allure-Label "feature" wird aus @pytest.mark.vc0 … vc6 abgeleitet; von Hand gesetzt
# wird es nicht mehr. --stufenbericht zaehlt die gesammelten Tests je Stufe und je
# Uebergabe und weist Luecken aus (tests/stufen.py).
...
def pytest_addoption(parser):
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="#3396: gesammelte Tests je Value-Chain-Stufe und Uebergabe ausgeben.",
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    if config.pluginmanager.hasplugin("allure_pytest"):
        for item in items:
            vorhanden = {
                (m.args, m.kwargs.get("label_type"))
                for m in item.iter_markers("allure_label")
            }
            namen = [m.name for m in item.iter_markers()]
            for marke in stufen.abgeleitete_allure_marken(dict.fromkeys(namen)):
                if (marke.args, marke.kwargs.get("label_type")) not in vorhanden:
                    item.add_marker(
                        pytest.mark.allure_label(*marke.args, **marke.kwargs)
                    )

    if config.getoption("--stufenbericht", default=False):
        bericht = stufen.auswertung(
            [{m.name for m in item.iter_markers()} for item in items]
        )
        config._stufenbericht = bericht.als_text()


def pytest_terminal_summary(terminalreporter, config):
    text = getattr(config, "_stufenbericht", None)
    if text:
        terminalreporter.write_sep("-", "Stufenbericht")
        terminalreporter.write_line(text)


@pytest.fixture(autouse=True)
def reset_composition_root():
    """Resets the CompositionRoot singleton before and after every test."""
