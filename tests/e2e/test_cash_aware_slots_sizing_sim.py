...
def simulate(signals, start_cash, start_held, flag_on):
    ...
    rm = RiskManager(client=MagicMock(), total_capital=EQUITY, clock=MagicMock())
    cash = float(start_cash)
    held = int(start_held)
    rows = []
    for sym, price, conv in signals:
        divisor = _divisor(held, flag_on)
        with patch("config.CLEAN_WEIGHT_SIZING", "off", create=True):
...
def simulate_cycle(candidates, cash, held, proportional):
    ...
    for cand in candidates:
        ...
        ), patch("config.VOL_TARGETING_SIZING_ENABLED", True, create=True), patch(
            "config.CLEAN_WEIGHT_SIZING", "off", create=True
        ):
            qty = rm.calculate_position_size(
                stop_loss_atr_multiplier=3.0,
                atr=cand.price * 0.03,
