...
def risk_manager():
    with patch("core.risk_manager.CLOUD_LOGGING_AVAILABLE", False):
        with patch("core.risk_manager.AILearnedRules") as MockRules:
            ...
            clock_mock.now.side_effect = lambda: datetime.now().astimezone()
            rm = RiskManager(
                client=mock_client,
...
# ---------------------------------------------------------------------------
# Step Definitions: STORY-01/02 exit authority (#3632: core.intelligent_exit)
# ---------------------------------------------------------------------------
...
def price_changes(test_context, symbol, new_price, hours):
    ...
    test_context["exit_result"] = analyze_exit(ctx)


@then(parsers.parse('the exit authority triggers a "{action}" with reason "{reason}"'))
def check_exit_authority(test_context, action, reason):
    result = test_context["exit_result"]
    got = "SELL" if result.should_sell else "HOLD"
    assert got == action, f"Expected {action}, got {got} ({result.reason})"
    assert (
        reason.lower() in result.reason.lower()
    ), f"Expected reason to contain {reason}, got {result.reason}"


@then("the exit authority keeps the position")
def exit_authority_holds(test_context):
    result = test_context["exit_result"]
    assert not result.should_sell, f"unexpected SELL: {result.reason}"
...
def sell_signal_generated(test_context, risk_manager, symbol):
    ...
    test_context["execution"]._check_exit = MagicMock(
