...
Feature: Golden Path and Compliance
  ...
  I want to process trades through the ComplianceGuardian, RiskManager, and the exit authority
  So that I can safely execute trades, let winners run, cut losses, and prevent wash trades

  Scenario: STORY-01 Profitable Trade -- BUY -> HOLD -> winner is held (#3632)
  ...
  Scenario: STORY-02 Loss cut bei Kursverlust (#3632)
    Given a position in "AAPL" was entered at 150.0
    When the price of "AAPL" drops to 138.5 after 3 hours
    Then the exit authority triggers a "SELL" with reason "LOSS CUT"
