...
class TestReportOnlyNeverFeedsConsensus:
    @pytest.mark.anyio
    async def test_specialist_alpha_excluded_at_default_weight_even_with_report(self):
        from core.round_table.agents import SpecialistAlphaAgent

        agent = SpecialistAlphaAgent()
        # Default posture SPECIALIST_ALPHA_WEIGHT=0.0 -> max_weight 0.0 -> weight clamped 0.0.
        assert agent.weight == 0.0
        report = MagicMock(sentiment_score=80.0, recommendation="buy", escalate=False)
        registry = MagicMock()
        ...
        # Even a bullish report yields weight 0.0 -> ConsensusEngine filters weight>0 -> excluded.
