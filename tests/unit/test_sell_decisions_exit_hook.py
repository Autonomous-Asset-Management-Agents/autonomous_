...
class TestConfigParity:
    ...
    def test_flag_defaults(self):
        """Owner directive 2026-09-08: the ROTATION lever ships OFF by default (reverted from
        the 2026-08-03 ON directive after it churned the live book — BUY-rated names force-sold
        at a loss, ~85% turnover in 8 days). It stays an independent, env-reversible lever
        (ROTATION_EXIT_ENABLED=true) with its per-cycle/per-session caps intact when re-enabled.
        The de-concentration TRIM lever also stays OFF (the 'flatten every name to 10%' half of
        the 2026-07-28 whole-book-flush incident).
        """
        import config as ent
