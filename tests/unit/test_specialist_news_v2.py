...
def test_news_v2_flag_default_false():
    ...
    assert RuntimeConfigState().SPECIALIST_NEWS_V2 is False


@pytest.mark.mutates_global_state
def test_news_v2_flag_default_true_oss():
    # 0.2.14 desktop defaults: the grounded card's [H#] corpus needs NEWS_V2 —
    # a fresh install must produce cards out of the box (first customer install
    # 2026-07-23 shipped with the flag unset and showed no cards).
    oss = _load_oss_config()
    assert oss.SPECIALIST_NEWS_V2 is True
...
# ---------------------------------------------------------------------------
# 4a. _fetch_google_news - query pins to "<SYMBOL> stock", NOT the bare ticker.
#     A bare 2-letter ticker collides with a US-state abbreviation and drags in local news
#     (e.g. "WY" -> Wyoming wildfires / University-of-Wyoming sports, not Weyerhaeuser).
# ---------------------------------------------------------------------------
@pytest.mark.anyio
async def test_fetch_google_news_query_pins_to_stock_not_bare_ticker():
    agent = _agent()  # symbol "AAPL"
    cfg = types.SimpleNamespace(SPECIALIST_NEWS_V2=True)
    ...
    assert "AAPL%20stock" in called_url  # "<SYMBOL> stock", URL-encoded
    assert "q=AAPL&" not in called_url  # never the bare ticker
