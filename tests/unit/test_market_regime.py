...
def regime_model(mock_data_provider):
    ...
    clock_mock.now.side_effect = lambda: datetime.datetime.now().astimezone()
    return MarketRegimeModel(mock_data_provider, clock_mock)
...
def test_failure_result_uses_short_ttl_and_self_heals(monkeypatch):
    ...
    model = mr.MarketRegimeModel(dp, clock_mock)
