...
def test_bucket_datetime_point_keys_intraday():
    # The intraday (1D/1W) curve keys points by ISO DATETIME, but cash-flows are dated by DAY.
    # A deposit on 08-09 (a weekend — no intraday point) must land on the first intraday point
    # whose timestamp is >= 08-09 (the 08-10 open), so /portfolio-intraday strips it and shows the
    # marker exactly like the daily /benchmark-equity curve. Lexical ISO compare = chronological,
    # and "2026-08-09" < "2026-08-10T13:30:00+00:00".
    dates = [
    ...
    assert bucket_cashflows_to_points(dates, {"2026-08-09": 115.66}) == [
        0.0,
        0.0,
        115.66,
        0.0,
