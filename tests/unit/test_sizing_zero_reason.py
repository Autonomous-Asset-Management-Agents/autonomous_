...
def _pin_legacy_conviction_sizing(monkeypatch):
    # #3284 (Owner-Waiver 2026-09-09): the DEFAULT sizing is now clean-weight "b". These golden
    # values were generated from the unchanged CONVICTION path, which survives as the switchable
    # "off" mode — pin it so this byte-identity guard keeps validating that legacy path
    # (off == unchanged main), not the new default.
    import config
...
def _size(case, sink=None):
    ...
    with patch("core.kill_switch.kill_switch") as ks, patch(
        "config.PROPORTIONAL_SLOT_CASH_ENABLED", False, create=True
    ):
