...
class TestReportCachePersist:
    ...
    def test_stale_persisted_report_is_restored_on_boot(self, tmp_path, monkeypatch):
        """Reports older than 2h are restored on boot so the user never sees an empty card on restart."""
        monkeypatch.setenv("AAA_USER_DATA_DIR", str(tmp_path))
        with patch.object(reg_mod, "_persist_enabled", return_value=True):
            r1 = StockSpecialistRegistry(["AAPL"], gemini_api_key="x")
    ...
    def test_refresh_symbol_blocks_degraded_overwrite_in_ram_and_disk(
        self, tmp_path, monkeypatch, caplog
    ):
        """A degraded/keyless refresh must not overwrite an existing rich report in RAM or on disk."""
        import logging

        monkeypatch.setenv("AAA_USER_DATA_DIR", str(tmp_path))
        with patch.object(reg_mod, "_persist_enabled", return_value=True):
            r = StockSpecialistRegistry(["AAPL"], gemini_api_key="x")
            rich = _mk_report("AAPL")
            r._reports["AAPL"] = rich
            r._persist_report(rich)

            # Degraded report with placeholders / degraded flag
            degraded_report = SpecialistReport(
                symbol="AAPL",
                updated_at=datetime.now(timezone.utc),
                bull_case="No supported bull case from this news cycle yet — updated with the next research round.",
                degraded=True,
                sentiment_score=50.0,
            )

            with caplog.at_level(logging.WARNING):
                assert reg_mod._should_preserve_existing(rich, degraded_report) is True
                with patch.object(
                    reg_mod.StockSpecialistAgent,
                    "research",
                    return_value=degraded_report,
                ):
                    import asyncio

                    asyncio.run(r._refresh_symbol("AAPL"))

            assert (
                r.get_report("AAPL").bull_case
                == "The upgrade cycle strengthens the bull case."
            )
            disk_data = json.loads(
                (tmp_path / "report_cache" / "AAPL.json").read_text(encoding="utf-8")
            )
            assert (
                disk_data["bull_case"] == "The upgrade cycle strengthens the bull case."
            )
            assert "Preserving rich cached report for AAPL" in caplog.text

    def test_report_cache_persist_defaults_on(self, monkeypatch):
        """Reports must survive an engine restart OUT OF THE BOX. Persistence used to default
        OFF, so every restart wiped the Reports page ('reports alle verschwunden nach Neustart').
        The default is now ON in both editions — the persist/reload mechanism itself is already
        covered by the round-trip test above; this pins the DEFAULT that makes it effective.
        """
        monkeypatch.delenv("REPORT_CACHE_PERSIST_ENABLED", raising=False)
        import config

        assert config.get_config().REPORT_CACHE_PERSIST_ENABLED is True
