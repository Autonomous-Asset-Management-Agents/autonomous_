...
def test_flag_off_maybe_construct_returns_none():
    ...
    # mock_clock.now.return_value removed
    """Default config (flag OFF) -> maybe_construct_book is a no-op (dark, BORA)."""
    assert maybe_construct_book(clock=mock_clock, held=["A1"], max_positions=10) is None
...
def test_exactly_one_target_book_top_n():
    ...
    proposal = construct_book(
        clock=mock_clock, ranking=RANKING, held=[], max_positions=10
    )
    assert proposal.target_book == TOP_10
...
def test_target_book_smaller_universe_than_n():
    ...
    small = {"X": 0.9, "Y": 0.8}
    proposal = construct_book(
        clock=mock_clock, ranking=small, held=[], max_positions=10
    )
...
def test_construct_book_is_deterministic():
    ...
    # mock_clock.now.return_value removed
    p1 = construct_book(
        clock=mock_clock, ranking=RANKING, held=["A1", "A12"], max_positions=10
    )
    p2 = construct_book(
        clock=mock_clock, ranking=RANKING, held=["A1", "A12"], max_positions=10
    )
...
def test_no_diff_when_book_already_matches():
    ...
    proposal = construct_book(
        clock=mock_clock, ranking=RANKING, held=list(TOP_10), max_positions=10
    )
...
def test_max_one_trade_per_symbol_per_window():
    ...
    proposal = construct_book(
        clock=mock_clock, ranking=RANKING, held=["A1", "A11", "A12"], max_positions=10
    )
    buckets = (
        list(proposal.buys)
        + list(proposal.sells)
...
def test_gather_ranking_lstm_primary():
    ...
    try:
        ...
        ranking, source = gather_ranking(
            clock=mock_clock, as_of=datetime(2026, 7, 22, 16, 0)
        )
...
def test_gather_ranking_consensus_fallback(caplog):
    ...
    try:
        ...
        with caplog.at_level(logging.WARNING):
            ranking, source = gather_ranking(clock=mock_clock)
