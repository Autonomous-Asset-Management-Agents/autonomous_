...
class SpecialistReportsRoute(unittest.TestCase):
    ...
    def test_dto_keyset_matches_bundle_fixture(self):
        ...
        with (
            patch.object(get_config(), "REPORT_GENERATOR_V2_ENABLED", False),
            patch.object(get_config(), "SPECIALIST_SYNTHESIS_CARD_ENABLED", False),
            patch.object(api, "engine", stub_engine),
        ):
            r = client.get("/specialist-reports", headers=_headers())
        self.assertEqual(r.status_code, 200)
        body = r.json()
