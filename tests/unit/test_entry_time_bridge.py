...
class TestStrategyReadSitesUseBridge:
    def test_rl_exit_uses_bridge_when_own_map_empty(self):
        """rl_execution._check_exit defaulted to current_time (hours=0). With the
        flag ON and a durable entry two days back, the ONE exit authority (#3632)
        must see ~48h in its PositionContext."""
        ...
        def _fake_analyze(ctx):
            captured["hours_held"] = ctx.hours_held
            return SimpleNamespace(should_sell=False, reason="", total_score=0.0)

        fake._signal_history = {}
        with patch(
            "core.strategies.rl_execution.analyze_exit",
            side_effect=_fake_analyze,
        ), patch("config.ENTRY_TIME_RECONCILE_PER_CYCLE", True, create=True):
            RLExecutionMixin._check_exit(
                fake,
                symbol="HWM",
                in_position=True,
                qty=2.0,
