...
class OllamaGenerateSync(unittest.TestCase):
    ...
    def test_happy_path_posts_deterministic_payload(self):
        ...
        # #2424: keep the model resident between cycles (Ollama's 5-min default
        # unload → cold-reload the runner each cycle → the window-flash class).
        self.assertEqual(payload["keep_alive"], "30m")

    def test_keep_alive_default_when_env_unset(self):
        resp = MagicMock(status_code=200)
        resp.json.return_value = {"response": "x"}
        with _no_kservice_env(), patch("httpx.post", return_value=resp) as post:
            os.environ.pop("OLLAMA_KEEP_ALIVE", None)
            self._provider().generate_content("hi")
        self.assertEqual(post.call_args.kwargs["json"]["keep_alive"], "30m")

    def test_keep_alive_env_override_is_forwarded(self):
        resp = MagicMock(status_code=200)
        resp.json.return_value = {"response": "x"}
        with _no_kservice_env({"OLLAMA_KEEP_ALIVE": "-1"}), patch(
...
class OllamaGenerateAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_async_payload_carries_keep_alive(self):
        resp = MagicMock(status_code=200)
        resp.json.return_value = {"response": "ok"}
        client = MagicMock()
        client.post = AsyncMock(return_value=resp)  # §12.5 AsyncMock
        with patch("httpx.AsyncClient", return_value=self._client_cm(client)):
            await self._provider().generate_content_async("a")
        # #2424: the async cycle path must keep the model resident too.
        self.assertEqual(client.post.call_args.kwargs["json"]["keep_alive"], "30m")
...
class SeamConsumerGate(unittest.TestCase):
    ...
    _MIGRATED = {
        ...
        "report/narrator.py",  # RPT-5 (#2004): report prose slots via the seam
        "report/pipeline.py",  # RPT-6 (#2075): orchestrates the single narrate() seam call; documents the seam, no direct provider import
    }
