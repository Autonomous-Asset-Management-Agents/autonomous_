...
def _make_decision_context(**kwargs):
    ...
    ctx.action = kwargs.get("action", 1)
    return ctx
...
async def test_tenant_path_margin_account_precheck_allow():
    ...
    with (
        ...
        patch(
            ...
            False,
            create=True,
        ),  # legacy sell-first semantics under test (#2712)
    ):
        rc.get_redis = AsyncMock(return_value=None)
        event = _make_signal(action="BUY", symbol="AAPL", qty=2.0)
        tenant = {"user_id": "tenant-1", "client": api, "equity": 10000.0}
...
# ---- #2712 Inc 2: buy-first displacement (flag default ON) --------------------------


def _buyfirst_api(buy_fails=False):
    api = MagicMock()
    api.get_account.return_value = MagicMock(
        multiplier="2", buying_power=100000.0, cash=100000.0
    )
    api.get_open_position.return_value = MagicMock(
        symbol="MSFT", qty="5", current_price="100.0"
    )
    if buy_fails:
        api.submit_order.side_effect = _make_api_error(403)
    return api
...
def _run_swap(api):
    ...
    rm = MagicMock()
    rm.calculate_position_size.return_value = 2.0
    return executor, pm, rm


@pytest.mark.anyio
async def test_buy_first_sells_only_after_the_buy(monkeypatch):
    """Default (buy-first): the BUY goes to the broker FIRST; the displaced position is
    sold only afterwards - order of submit calls proves the resequencing."""
    api = _buyfirst_api()
    executor, pm, rm = _run_swap(api)
    with (
        patch.object(executor, "_get_tenant_portfolio_manager", return_value=pm),
        patch.object(executor, "_get_tenant_risk_manager", return_value=rm),
        patch("core.engine.order_executor.RedisClient") as rc,
        ...
        tenant = {"user_id": "tenant-1", "client": api, "equity": 10000.0}
        await executor._execute_tenant_order(tenant, event)
    sides = [str(c.args[0].symbol) for c in api.submit_order.call_args_list]
    assert sides[0] == "AAPL"  # the BUY first
    assert "MSFT" in sides[1:]  # the displaced SELL after


@pytest.mark.anyio
async def test_buy_first_failed_buy_changes_nothing(monkeypatch):
    """Buy-first: a failed swap BUY leaves the old position untouched - exactly ONE
    submit attempt (the BUY), no SELL, no recovery re-BUY, no spread loss."""
    api = _buyfirst_api(buy_fails=True)
    executor, pm, rm = _run_swap(api)
    with (
        patch.object(executor, "_get_tenant_portfolio_manager", return_value=pm),
        patch.object(executor, "_get_tenant_risk_manager", return_value=rm),
        ...
        await executor._execute_tenant_order(tenant, event)
    assert (
        api.submit_order.call_count == 1
    )  # only the failed BUY ever reached the broker


@pytest.mark.anyio
async def test_buy_first_insufficient_cash_skips_swap_without_liquidation(monkeypatch):
    """Archon Auflage 3: strict cash guard - proceeds do NOT count; insufficient BP
    skips the whole swap (no SELL, no BUY, old position untouched)."""
    api = _buyfirst_api()
    api.get_account.return_value = MagicMock(
        multiplier="2", buying_power=50.0, cash=50.0
    )
    executor, pm, rm = _run_swap(api)
    with (
        patch.object(executor, "_get_tenant_portfolio_manager", return_value=pm),
