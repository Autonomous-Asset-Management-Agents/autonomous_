...
Since #1885 (MLA-11) the fix is the DEFAULT (TFT_SERVING_FIX default True): a config
built without overrides serves on the trainer-validated scale. The env override
``TFT_SERVING_FIX=0`` remains as the rollback path and reproduces the historical
mean+no-scale behaviour byte-for-byte (the rollback tests below prove it).
"""
...
# --- e) ROLLBACK: flag OFF reproduces the historical behaviour byte-for-byte ---------
def test_fix_off_is_byte_identical_old_behaviour(tmp_path):
    eng = _engine(tmp_path)
    raw = _raw(
        [
            [-0.012, 0.018, 0.045],
...
# --- f) #1885 (MLA-11): the fix is the DEFAULT ----------------------------------------
def test_serving_fix_defaults_on(monkeypatch):
    """A config built without overrides has TFT_SERVING_FIX ON (#1885)."""
    monkeypatch.delenv("TFT_SERVING_FIX", raising=False)
    # _env_file=None → hermetic: a developer's local .env/.env.oss cannot flip this test.
    assert RuntimeConfigState(_env_file=None).TFT_SERVING_FIX is True


def test_default_config_serves_trainer_scoring_scale(tmp_path, monkeypatch):
    """Regression (#1885): with the DEFAULT config — no patch, no env override — a
    synthetic [N, H, Q] quantile tensor is served on the trainer's scoring scale:
    decoder step-0 (what train_tft_per_symbol._score_fold validated) ×100 to percent."""
    monkeypatch.delenv("TFT_SERVING_FIX", raising=False)
    eng = _engine(tmp_path)
    # Step-0 bullish; horizon mean negative — separates step-0/×100 from the old path.
    raw = _raw(
        [
            [-0.012, 0.018, 0.045],  # step 0 → trainer-scored step
            [-0.03, -0.02, -0.005],
            [-0.03, -0.02, -0.005],
            [-0.03, -0.02, -0.005],
            [-0.03, -0.02, -0.005],
    ...
    with patch(
        "core.ml.tft_inference.get_config",
        return_value=RuntimeConfigState(_env_file=None),
    ):
        pred = eng._postprocess_quantiles(raw)
    assert pred is not None
    assert pred.base_return_pct == pytest.approx(1.8)  # 0.018 × 100 — trainer unit
    assert pred.bear_return_pct == pytest.approx(-1.2)
    assert pred.bull_return_pct == pytest.approx(4.5)
    assert pred.direction == "up"  # old default: neutral w/ fake ~0.99 confidence
    assert pred.confidence < 0.99  # spread 5.7 → 1 - 5.7/8 = 0.2875, not fake-saturated


def test_env_rollback_restores_legacy_behaviour(tmp_path, monkeypatch):
    """Rollback path (#1885): env TFT_SERVING_FIX=0 restores the pre-fix read
    (horizon mean, no ×100) byte-for-byte."""
    monkeypatch.setenv("TFT_SERVING_FIX", "0")
    cfg = RuntimeConfigState(_env_file=None)
    assert cfg.TFT_SERVING_FIX is False
    eng = _engine(tmp_path)
    raw = _raw(
        [
            [-0.012, 0.018, 0.045],
    ...
    with patch("core.ml.tft_inference.get_config", return_value=cfg):
        pred = eng._postprocess_quantiles(raw)
    assert pred.base_return_pct == pytest.approx(-0.0124)  # mean across H, unscaled
    assert pred.direction == "neutral"  # the historical (broken) result
