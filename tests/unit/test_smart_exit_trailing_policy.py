...
FLAG_OFF_BATTERY = [
    ...
    # (#2839: min-hold default is 20d now — 21d clears the gate like 5d cleared 5d)
    (
        {"in_top_n": False, "lstm_rank": 12, "hours_held": 21 * DAY},
...
def test_flag_off_byte_identical_pinned(monkeypatch, over, expected):
    """EXIT_POLICY_TRAILING_ENABLED off -> exact legacy output. #2839: default is
    ON now (desktop master), so OFF is forced explicitly (rollback guarantee)."""
    import config as _config

    monkeypatch.setattr(_config, "EXIT_POLICY_TRAILING_ENABLED", False, raising=False)
...
def test_flag_off_explicit_param_matches_config_off(monkeypatch):
    """Explicit trailing_from_peak_enabled=False == config-OFF resolution
    (#2839: default is ON, so the config side is forced OFF explicitly)."""
    import config as _config
