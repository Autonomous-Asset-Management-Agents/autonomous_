...
def _weakest(days_held=3, pnl_pct=0.0, score=40.0, symbol="PG"):
...
def test_unknown_holding_age_does_not_block():
    """days_held defaults to 0 when the PM has no trade history for a symbol (e.g. a position
    restored from the broker without a reconciled entry time). Treating "unknown" as "brand new"
    would lock such a position in for the whole period on no evidence at all — the veto must
    fail OPEN, matching the existing `no trade history -> can sell` posture of
    can_sell_position (portfolio_manager.py:733)."""
    pm = _pm()
    weak = _weakest(days_held=0, score=40.0)
