...
def test_config_exposes_sim_days_in_both_editions():
    """BORA parity â€” a sim knob missing from the OSS config diverges the editions."""
    from settings import RuntimeConfigState

    assert (
        "SIM_DAYS" in RuntimeConfigState.model_fields
    ), "SIM_DAYS must be in settings.py"
...
def test_sim_days_default_is_one():
    ...
    # Default is evaluated by Pydantic
    assert RuntimeConfigState().SIM_DAYS == 1
