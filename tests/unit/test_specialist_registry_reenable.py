...
class TestRegistryDisabledByteIdentical:
    def _run_start(self, engine: BotEngine) -> None:
        ...
        # #2839: SPECIALIST_REGISTRY_ENABLED defaults ON now (desktop master) —
        # the OFF/dormant path is forced explicitly to stay tested.
        with patch("core.engine.base.config.ENVIRONMENT", "production"), patch(
            "core.engine.base.RiskManager"
        ), patch.object(config.get_config(), "SPECIALIST_REGISTRY_ENABLED", False):
...
# ---------------------------------------------------------------------------
# (2b) RTR-3 (#1950): Kohaerenz-Guard — Gewicht>0 ohne Registry = stiller toter
#      Voter (jeder Vote abstained, permanenter Konsens-Ausschluss ohne Log).
#      Der Guard in start_live_strategy macht die Mis-Config als WARNING laut.
# ---------------------------------------------------------------------------


class TestSpecialistAlphaCoherenceGuard:
    def test_weight_active_registry_off_warns(self, caplog):
        """R7: SPECIALIST_ALPHA_WEIGHT=0.55 + SPECIALIST_REGISTRY_ENABLED=False
        -> start_live_strategy loggt WARNING (permanenter Ausschluss, keine
        Report-Quelle). §5.6: WARNING, nie DEBUG."""
        engine = _make_engine()
        with patch("core.engine.base.config.ENVIRONMENT", "production"), patch(
            "core.engine.base.RiskManager"
        ), patch("core.engine.base.config.get_config") as mock_get_config:
            mock_get_config.return_value = MagicMock(
                SPECIALIST_REGISTRY_ENABLED=False, SPECIALIST_ALPHA_WEIGHT=0.55
            )
            with caplog.at_level(logging.WARNING):
                result = engine.start_live_strategy()
        assert result is True
        assert engine.specialist_registry is None  # registry stayed OFF
        warnings = [
            r.getMessage()
            for r in caplog.records
            if r.levelno == logging.WARNING and "SpecialistAlpha" in r.getMessage()
        ]
        assert any(
            "permanent aus dem Konsens ausgeschlossen" in m for m in warnings
        ), f"Kohaerenz-Guard-WARNING fehlt — captured: {warnings}"

    def test_registry_on_weight_zero_no_warning(self, caplog):
        """R8: Registry ON + Gewicht 0 ist der legitime Report-only-/Shadow-
        Zustand (#76) -> KEIN Kohaerenz-Warning."""
        engine = _make_engine()
        fake_registry = MagicMock()
        with patch("core.engine.base.config.ENVIRONMENT", "production"), patch(
            "core.engine.base.RiskManager"
            ...
            mock_get_config.return_value = MagicMock(
                SPECIALIST_REGISTRY_ENABLED=True, SPECIALIST_ALPHA_WEIGHT=0.0
            )
            with caplog.at_level(logging.WARNING):
                result = engine.start_live_strategy()
        assert result is True
        assert not any(
            "permanent aus dem Konsens ausgeschlossen" in r.getMessage()
            for r in caplog.records
        ), "Report-only/Shadow-Zustand darf NICHT warnen"

    def test_weight_dormant_registry_off_no_warning(self, caplog):
        """Default-Ship (beide 0/OFF in der Cloud-Edition) -> kein Warning,
        byte-identischer Boot."""
        engine = _make_engine()
        with patch("core.engine.base.config.ENVIRONMENT", "production"), patch(
            "core.engine.base.RiskManager"
        ), patch("core.engine.base.config.get_config") as mock_get_config:
            mock_get_config.return_value = MagicMock(
                SPECIALIST_REGISTRY_ENABLED=False, SPECIALIST_ALPHA_WEIGHT=0.0
            )
            with caplog.at_level(logging.WARNING):
                result = engine.start_live_strategy()
        assert result is True
