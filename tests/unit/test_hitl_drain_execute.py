...
#
# Decision-2 Option B (N11) galt: auf einer OSS-Engine ohne OAuth-Mandanten HOLD +
# warn + audit. #3391 hat das ersetzt — ohne Mandanten gilt der Broker-Zugang der
# Engine. Der Zweck der alten Zusage bleibt: der Inline-Global-Pfad wird weiterhin
# nicht benutzt. Siehe test_oss_no_tenant_fuehrt_ueber_den_engine_zugang_aus.
# (alter Wortlaut) Decision-2 Option B (N11): on an OSS engine with no OAuth tenant, it HOLDs + warns + audits
...
def test_oss_no_tenant_fuehrt_ueber_den_engine_zugang_aus():
    """#3391 ersetzt die frühere Zusage — mit Absicht und unter Wahrung ihres Zwecks.

    **Vorher (Decision-2 Option B, N11):** kein OAuth-Mandant ⇒ HOLD, niemals
    ausführen, ``rejected`` protokollieren.

    **Warum das geändert wird:** Auf dem Desktop gibt es nie Mandanten. Jede
    HITL-Freigabe lief damit ins Leere — der Mensch gab frei, es wurde als
    ``no_oauth_tenant`` verbucht, und nichts passierte. Die Aufsicht nach EU AI Act
    Art. 14 war dort protokolliert, aber nicht wirksam. Verschärfend: Im
    Standardmodus wandert wegen der Vorgabewerte ``0.0`` **jede** Order in die
    Warteschlange.

    **Der Zweck der alten Zusage bleibt gewahrt.** Sie lautete ausdrücklich „never
    reuse the inline global path" — der Inline-Global-Pfad ist autonom-only
    (``order_executor.py:3155-3159``). Der neue Weg benutzt ihn **nicht**: Er baut
    einen ordentlichen Tenant-Eintrag und geht durch ``_execute_tenant_order`` mit
    ``source="human_approved"``, also denselben source-geschützten Pfad wie
    Enterprise. Risiko-, Portfolio-, ``check_order``- und Kill-Switch-Prüfungen
    gelten unverändert.

    Die beiden Fälle, die weiterhin abgelehnt werden, stehen in
    ``test_unknown_user_id_holds_and_audits_rejected`` (Mandanten vorhanden, keiner
    passt) und in ``test_hitl_desktop_ohne_mandant.py`` (gar kein Broker-Zugang).
    """
    executor = _make_executor()
    ...
    assert result is True
    executor._execute_tenant_order.assert_awaited_once()
    assert executor._execute_tenant_order.await_args.kwargs.get("source") == (
        "human_approved"
    ), "Der Inline-Global-Pfad darf weiterhin nicht benutzt werden (Decision-2 N11)."
    assert _last_branch(audit) == "approved"
