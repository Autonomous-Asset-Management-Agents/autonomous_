...
async def test_startup_health_check_passes_when_all_ok(engine):
    ...
    ), patch.object(engine, "_check_llm", AsyncMock(return_value=True)), patch.object(
...
@pytest.mark.anyio
async def test_startup_health_check_llm_down_waits_then_aborts_on_stop(engine):
    """LLM-Provider nicht erreichbar → Wartezustand (B11), kein sofortiger Abbruch.

    Erst ein Stop/Shutdown beendet das Warten, dann wie bisher RuntimeError mit
    'llm' im Message. Der Wartezustand selbst: tests/unit/test_llm_wait_state.py.
    """

    async def shutdown_during_wait(_seconds):
    ...
    ) as check_llm, patch.object(
        engine, "_check_model_files", return_value=True
    ), patch(
        "core.engine.base.send_slack_alert", MagicMock()
    ), patch(
        "core.engine.base.asyncio.sleep", AsyncMock(side_effect=shutdown_during_wait)
    ):
        with pytest.raises(RuntimeError, match="llm"):
            await engine._startup_health_check()
    assert check_llm.await_count == 1  # first probe, then the wait was stopped
...
async def test_startup_health_check_lists_all_failed(engine):
    ...
    with patch.object(
        engine, "_check_redis", AsyncMock(return_value=False)
    ), patch.object(engine, "_check_llm", AsyncMock(return_value=False)), patch.object(
        engine, "_check_model_files", return_value=True
    ), patch(
        "core.engine.base.send_slack_alert", MagicMock()
    ):
