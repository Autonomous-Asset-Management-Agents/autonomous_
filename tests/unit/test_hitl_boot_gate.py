...
def test_boot_gate_live_without_hitl_degrades_to_paper_on_desktop_oss(monkeypatch):
    ...
    settings._config_state.PAPER_TRADING = False

    settings._enforce_hitl_boot_gate(False, False, False)  # must NOT raise
    assert (
        settings._config_state.PAPER_TRADING is True
...
def test_apply_remote_config_refuses_live_without_hitl(monkeypatch):
    ...
    monkeypatch.setattr(settings, "_config_state", safe)
...
def test_apply_remote_config_allows_live_with_hitl(monkeypatch):
    ...
    monkeypatch.setattr(settings, "_config_state", ok)
