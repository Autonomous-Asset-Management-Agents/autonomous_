...
def test_boot_gate_live_without_hitl_raises_config_py_cloud(monkeypatch):
    # CLOUD (non-LOCAL): still fail-closed HARD — a container restart-loop is a visible ops signal,
    # not a stranded user; the deploy must not run live without HITL.
    monkeypatch.delenv("DEPLOYMENT_MODE", raising=False)
    with pytest.raises(RuntimeError, match="EU AI Act"):
        config._enforce_hitl_boot_gate(False, False, False)


def test_boot_gate_live_without_hitl_raises_config_oss_cloud(monkeypatch):
    monkeypatch.delenv("DEPLOYMENT_MODE", raising=False)
    oss = _load_oss()
    with pytest.raises(RuntimeError, match="EU AI Act"):
...
# --- HOTFIX: DESKTOP (DEPLOYMENT_MODE=LOCAL) must NOT crash the boot on live-without-HITL.
#     A raise here bricks the desktop in a restart loop the operator can't escape (no way to reach
#     /api/live/disable). Refuse live by DEGRADING to PAPER — compliant (no unsupervised live) AND
#     the engine boots managed. Mirrors the #1918 live-guard philosophy. ---


def test_boot_gate_cloud_config_py_raises_even_on_local(monkeypatch):
    # The enterprise config.py is NEVER the desktop edition (the OSS snapshot swaps in config.oss.py).
    # It stays fail-closed HARD regardless of DEPLOYMENT_MODE — the desktop fail-safe lives in
    # config.oss.py's gate (below) + the __main__ boot wrapper.
    monkeypatch.setenv("DEPLOYMENT_MODE", "LOCAL")
    with pytest.raises(RuntimeError, match="EU AI Act"):
        config._enforce_hitl_boot_gate(False, False, False)


def test_boot_gate_live_without_hitl_degrades_to_paper_on_desktop_oss(monkeypatch):
    monkeypatch.setenv("DEPLOYMENT_MODE", "LOCAL")
    oss = _load_oss()
    oss.PAPER_TRADING = False  # arm live in the loaded module
    oss._enforce_hitl_boot_gate(False, False, False)  # must NOT raise
    assert (
        oss.PAPER_TRADING is True
    ), "desktop must degrade to PAPER (live refused), not crash"
