...
def test_specialist_alpha_weight_desktop_master_parity_across_editions():
    """#2839 (ersetzt die RTR-3-Dormanz-Pinnung): Owner-Entscheid 13.08. — die
    Defaults tragen das getestete Desktop-Launcher-Profil (0.40, wie von der
    Electron-Shell seit dem Arm injiziert). BORA: beide Editionen identisch."""
    import config as enterprise

    ent = enterprise.get_config()
    oss = _load_oss_config().get_config()
    assert float(ent.SPECIALIST_ALPHA_WEIGHT) == 0.40
    assert float(oss.SPECIALIST_ALPHA_WEIGHT) == 0.40


def test_specialist_registry_default_parity_restored():
    """#2839 (bewusste Governance-Entscheidung, ersetzt die RTR-3-Divergenz-
    Pinnung): Owner 13.08. — Desktop ist Master, beide Editionen default True
    (= das getestete Profil). Kostenfolge fuer Cloud-Deploys (per-call LLM-
    Kosten der Registry) ist im PR ausgewiesen; Cloud ist aktuell nicht im
    Testbetrieb."""
    import config as enterprise

    ent = enterprise.get_config()
    oss = _load_oss_config().get_config()
    assert ent.SPECIALIST_REGISTRY_ENABLED is True
    assert oss.SPECIALIST_REGISTRY_ENABLED is True
