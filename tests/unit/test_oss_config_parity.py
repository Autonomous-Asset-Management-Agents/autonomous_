...
def test_oss_required_flags_default_off():
    """Dormancy: the new gatekeeper flags must default False so the OSS merge is byte-identical."""
    cfg = _load_oss_config().get_config()
    assert cfg.GATEKEEPER_PORTFOLIO_CONTEXT_ENABLED is False
