...
@pytest.fixture(autouse=True)
def _pin_legacy_conviction_sizing(monkeypatch):
    # #3284 (Owner-Waiver 2026-09-09): the DEFAULT sizing is now clean-weight "b". This suite
    # validates the CONVICTION sizer (VIX/Kelly/cash/diversified-slot behaviour), which
    # survives as the switchable "off" mode — pin it so these legacy-behaviour tests keep
    # exercising that path rather than the new default.
    import config
...
def risk_manager():
    ...
    with patch("core.risk_manager.CLOUD_LOGGING_AVAILABLE", False):
        ...
        rm = RiskManager(client=client_mock, total_capital=100_000.0, clock=clock_mock)
        return rm
...
class TestRiskManager:
    ...
    def test_position_size_capped_at_compliance_max_order_value(self):
        ...
        with patch("core.risk_manager.CLOUD_LOGGING_AVAILABLE", False):
            rm = RiskManager(
                client=MagicMock(), total_capital=1_000_000.0, clock=clock_mock
            )
    ...
    def test_circuit_breaker_tier2(self, mock_trip, risk_manager):
        ...
        # #3383: war `close_all_positions(cancel_orders=True)`. Der Sammelaufruf kennt
        # weder Symbol noch Menge — im Audit waere hinterher nicht belegbar, WAS der
        # Breaker geschlossen hat. Er laeuft jetzt je Position ueber das Gateway.
        risk_manager.client.close_all_positions.assert_not_called()
        assert risk_manager.client.get_all_positions.called
    ...
    def test_calculate_position_size_vix_scaling(self, risk_manager):
        ...
        sl_multiplier = 3.0
        price = 100.0
        # ADR-R10: ample cash, so the cash cap cannot bind and this test keeps measuring
        # VIX scaling alone. Previously this relied on the `account_cash=0.0` DEFAULT
        # meaning "unconstrained" — it no longer does: zero cash means zero cash. Every
        # production call site passes real broker cash (order_executor.py:547 / :1377,
        # rl_execution.py:784), so supplying it here matches reality rather than dodging
        # the guard.
        cash = 1_000_000.0
    ...
    def test_calculate_position_size_cash_constraint(self, risk_manager):
        ...
        size = risk_manager.calculate_position_size(
            ...
            # #2980: VIX now resolves FAIL-CLOSED, so an unset VIX would apply the
            # 0.3 crash scaler and change this cash-cap fixture. Pin the (former
            # implicit) calm regime explicitly to isolate the CASH constraint.
            market_data={"vix": 15.0},
        )

        # ADR-R09 (revised 2026-07-27): cash buffer reduced $50 -> $1 (Alpaca minimum).
        # Max shares by cash = (10000 - 1) / 100 = 99.99  (single-symbol -> slots=1)
        # PR 3135: With PROPORTIONAL_SLOT_CASH_ENABLED=True, conviction=1.0 yields a demand_fraction
        # of 25000 / 30000 = 0.83333. 99.99 * 0.83333 = 83.325 shares.
        assert size == pytest.approx(83.325, abs=0.01)
    ...
    def test_calculate_position_size_diversified_cash_allocation(self, risk_manager):
        ...
        all. 10k cash / 10 slots / $100 = 9.999 shares (vs 99.99 for a single-symbol strategy).
        """
        size = risk_manager.calculate_position_size(
            stop_loss_atr_multiplier=3.0,
            atr=2.0,
        ...
        # ADR-R09 (revised 2026-07-27): $1 buffer. (10000 - 1) / 10 slots / $100 = 9.999 shares
        # PR 3135: With PROPORTIONAL_SLOT_CASH_ENABLED=True, conviction=1.0 yields a demand_fraction
        # of 25000 / 30000 = 0.83333. 9.999 * 0.83333 = 8.3325 shares.
        assert size == pytest.approx(8.3325, abs=0.01)
    ...
    def test_evaluate_new_trade_ai_rules(self, mock_get_rules, risk_manager):
        ...
        is_allowed, reason, mods = risk_manager.evaluate_new_trade(
            symbol="AAPL",
            side="BUY",
            market_data={"vix": 20.0, "indicators": {"features": {"rsi_14": 75}}},
    ...
    def test_evaluate_new_trade_ai_rules_additional_actions(
        ...
        is_allowed, reason, mods = risk_manager.evaluate_new_trade(
            ...
            market_data={
                "vix": 20.0,
                "indicators": {"features": {"rsi_14": 20, "macd": 1}},
            },
            current_sl_multiplier=3.0,
        )
        assert is_allowed is True
    ...
    def test_malformed_ai_rule_is_logged_not_swallowed(
        ...
        with caplog.at_level(logging.WARNING):
            is_allowed, _reason, _mods = risk_manager.evaluate_new_trade(
                symbol="AAPL",
                side="BUY",
                market_data={"vix": 20.0, "indicators": {"features": {}}},
                current_sl_multiplier=3.0,
            )
    ...
    def test_calculate_position_size_total_exposure_cap(self, risk_manager):
        ...
        size = risk_manager.calculate_position_size(
            ...
            atr=2.0,
            current_price=100.0,
            account_cash=1_000_000.0,  # ADR-R10: isolate the exposure cap from the cash cap
            conviction_score=1.0,
            market_data={"vix": 20.0},  # #2980: pin calm regime, isolate exposure cap
        )
    ...
    def test_exposure_cap_disabled_does_not_fail_closed(self, risk_manager):
        ...
        size = risk_manager.calculate_position_size(
            ...
            atr=2.0,
            current_price=100.0,
            account_cash=1_000_000.0,  # ADR-R10: isolate "cap disabled" from the cash cap
            conviction_score=1.0,
        )
    ...
    def test_calculate_position_size_kelly_fraction(self, risk_manager):
        ...
        size = risk_manager.calculate_position_size(
            ...
            atr=2.0,
            current_price=100.0,
            account_cash=1_000_000.0,  # ADR-R10: isolate the Kelly cap from the cash cap
            conviction_score=1.0,
            # #2980: the docstring's "VIX scaling = 0.9" was the OLD implicit
            # missing-VIX default (20.0). Pin it explicitly now that missing VIX
            # fails closed (0.3), so this test still exercises the Kelly cap.
...
# ===========================================================================
# #2153 divisor fix — the cash constraint must split available cash into the
# POSITION-CAP count (effective_max_positions), NOT len(live_universe)/len(self.symbols)
# (the ~500-name SCAN universe). The book is hard-capped at max_positions holdings
# (portfolio_manager.should_open_new_position), so dividing by ~500 under-sized every
# order ~50x in production/full-universe and forced the fractional-share churn. The
# author's own test (test_calculate_position_size_diversified_cash_allocation) already
# equates "slots" with the position count (10 = MAX_POSITIONS).
# ===========================================================================
...
def _stub_cfg(monkeypatch, **kw):
    ...
    base = dict(
        ...
        MAX_POSITIONS=10,
        FULL_UNIVERSE_MAX_POSITIONS=50,
    )
    base.update(kw)
    monkeypatch.setattr(config, "get_config", lambda: _SNS(**base))


def test_effective_max_positions_is_the_book_cap_not_the_scan_universe(monkeypatch):
    from core.risk_manager import effective_max_positions

    _stub_cfg(monkeypatch)  # normal mode
    assert effective_max_positions() == 10  # MAX_POSITIONS, not len(universe)

    _stub_cfg(monkeypatch, FULL_UNIVERSE_TRADING_ENABLED=True)  # full-universe
    assert effective_max_positions() == 50  # FULL_UNIVERSE_MAX_POSITIONS


def test_effective_max_positions_clamps_to_at_least_one(monkeypatch):
    from core.risk_manager import effective_max_positions

    _stub_cfg(monkeypatch, MAX_POSITIONS=0)
    assert effective_max_positions() == 1  # never a divide-by-zero slot count


def test_sizing_call_sites_use_max_positions_not_scan_universe():
    """All three production sizing callers must divide cash by effective_free_slots(held)
    (O3: the REMAINING free slots, bounded by the book cap effective_max_positions()),
    NEVER by the scan-universe size. The three sites are the tenant path, the desktop
    fallback path, and RLStrategy._run_for_symbol_impl (the DEFAULT active/round-table
    strategy, ACTIVE_STRATEGY='RLAgent')."""
    import inspect
    import linecache
    import os
    ...
    # Pre-populate linecache to prevent encoding mismatch line offset bugs on Windows under Python 3.12+
    for mixin in [OrderExecutorMixin, RLExecutionMixin]:
        filename = inspect.getfile(mixin)
        try:
    ...
    # AST guard (not substring): bind the num_stocks_in_strategy argument itself to
    # effective_free_slots(<held-from-_position_scores>). The old substring form only checked that the
    # token `effective_free_slots(` appeared and could NOT see the ARGUMENT — a regression to
    # effective_free_slots(len(live_universe)) (the ~500-name scan universe) would collapse the divisor
    # to 1 and route ~all free cash into one order, yet still pass (adversarial-review TEST VALIDITY finding).
    import ast
    import textwrap

    def _divisor_arg(method):
        # num_stocks_in_strategy is the 6th positional parameter of calculate_position_size
        # (stop_loss_atr_multiplier, atr, confidence, size_scaler, market_data, num_stocks_in_strategy, ...)
        # — the tenant/fallback sites pass it by keyword, the RL site passes it POSITIONALLY. Handle both.
        tree = ast.parse(textwrap.dedent(inspect.getsource(method)))
        for node in ast.walk(tree):
            if (
                isinstance(node, ast.Call)
                and isinstance(node.func, ast.Attribute)
                and node.func.attr == "calculate_position_size"
            ):
                for kw in node.keywords:
                    if kw.arg == "num_stocks_in_strategy":
                        return kw.value
                if len(node.args) > 5:
                    return node.args[5]
        return None
    ...
    sites = {
        "tenant": OrderExecutorMixin._execute_tenant_order,
        "fallback": OrderExecutorMixin._process_signal_event,
        "rl": RLExecutionMixin._run_for_symbol_impl,
    }
    for name, method in sites.items():
        arg = _divisor_arg(method)
        assert (
            arg is not None
        ), f"{name}: no calculate_position_size(num_stocks_in_strategy=...) call"
        assert (
            isinstance(arg, ast.Call)
            and isinstance(arg.func, ast.Name)
            and arg.func.id == "effective_free_slots"
        ), f"{name}: divisor is not effective_free_slots(...)"
        held_src = ast.unparse(arg)
        # held count MUST come from the live position map, NEVER the scan universe or the raw fixed cap.
        assert (
            "_position_scores" in held_src
        ), f"{name}: held not from _position_scores: {held_src}"
        assert (
            "symbols" not in held_src and "universe" not in held_src.lower()
        ), f"{name}: divisor references the scan universe: {held_src}"
        assert (
            "effective_max_positions" not in held_src
        ), f"{name}: divisor is the raw fixed cap, not remaining slots: {held_src}"
