...
ALL_AGENT_NAMES = tuple(a.__class__.__name__ for a in ALL_AGENTS)
...
def test_basic_tier_features():
    ...
    assert set(ent.agent_names) == set(ALL_AGENT_NAMES)


def test_pro_tier_grants_every_round_table_module_and_live():
...
def test_shipped_beta_license_resolves_to_pro(local_env):
    ...
    assert ent.tier is Tier.PRO
    assert ent.allow_live is True
    # Mirror ALL_AGENTS rather than a magic count: runner.py filters the active
    # modules by this list, so a module added to the gremium but missing here would
    # never run. Asserting the mirror keeps that drift a test failure, not a silent
    # "module is registered but mute".
    assert set(ent.agent_names) == set(ALL_AGENT_NAMES)
