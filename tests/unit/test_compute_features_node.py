...
def test_flag_default_true_enterprise(monkeypatch):
    # #2839: default False->True = shipped desktop launcher profile (desktop is master).
    monkeypatch.delenv("DESKTOP_FEATURE_SNAPSHOT_ENABLED", raising=False)
    from config import RuntimeConfigState

    assert RuntimeConfigState().DESKTOP_FEATURE_SNAPSHOT_ENABLED is True


def test_flag_default_true_oss(monkeypatch):
    # #2839: parity — OSS carries the same desktop-master default.
    monkeypatch.delenv("DESKTOP_FEATURE_SNAPSHOT_ENABLED", raising=False)
    cfg = _load_oss_config()
    assert cfg.DESKTOP_FEATURE_SNAPSHOT_ENABLED is True
