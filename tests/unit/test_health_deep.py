...
def test_deep_health_endpoint_structure():
    ...
    with patch("core.engine.api_routes.engine") as mock_engine:
        ...
        with patch("core.cloud_logger.get_cloud_logger") as mock_logger:
            ...
            # #2743/A2: explicit degraded flag for the desktop UI (healthy → False).
            assert data["components_degraded"] is False
...
def test_deep_health_degraded_if_alpaca_down():
    """#2743/A2: a degraded engine (broker/model fault) is UP and answering — the endpoint now
    returns HTTP **200** with an authoritative ``status='degraded'`` + ``components_degraded=True``,
    so the desktop UI can tell 'degraded' from 'engine down' (was a swallowed 500)."""
    from fastapi.testclient import TestClient
    ...
    with patch("core.engine.api_routes.engine") as mock_engine:
        ...
        with patch("core.cloud_logger.get_cloud_logger") as mock_logger:
            ...
            with patch("os.path.exists", return_value=False):
                ...
                assert data["components_degraded"] is True
                assert data["components"]["alpaca"]["status"] == "unavailable"


@allure.feature("VC-0 Platform Infrastructure")
@allure.story("Core Utilities")
def test_deep_health_starting_when_engine_none():
    """DASH-1 T7 (#1472): during the boot window the module-level ``engine`` may
    not be constructed yet. ``/health/deep`` must report an honest 200
    ``starting`` status instead of an uncaught AttributeError -> bare 500."""
    from fastapi.testclient import TestClient
    ...
    with patch("core.engine.api_routes.engine", None):
        with patch("core.cloud_logger.get_cloud_logger") as mock_logger:
            ...
            with patch("os.path.exists", return_value=False):
                ...
                assert data["strategy_running"] is False
                assert data["components"]["alpaca"]["status"] == "unavailable"
