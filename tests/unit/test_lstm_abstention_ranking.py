...
#
# Exit safety (verified against core/hold_policy.py rank_drop_exit): an abstained in-position
...
class TestFailedSymbolNeverInTopN:
    ...
    async def test_short_history_held_position_gets_no_rank_sell(self, caplog):
        ...
        symbol not ranked → rank None → rank_drop_exit does not SELL.
        """
        import pandas as pd

        from core.hold_policy import rank_drop_exit
        ...
        assert rank is None
        decision = rank_drop_exit(
            in_top_n=in_top_n, lstm_rank=rank, hours_held=1.0, top_n_size=10
        )
        assert (
            decision.sell is False
    ...
    def test_abstained_symbol_rank_is_none_so_no_forced_rank_exit(self):
        ...
        missing from the cache; rank_drop_exit requires a non-None rank
        (core/hold_policy.py) -> no forced SELL on data failure."""
        from core.hold_policy import rank_drop_exit
...
# ---------------------------------------------------------------------------
# RPT-8 (Epic #1998) — the live LSTM panel record is flag-gated + dormant.
# update_lstm_rankings must be byte-identical for the decision path whether the
# report flag is ON or OFF; the panel store is only written when it is ON.
# ---------------------------------------------------------------------------
@pytest.mark.vc1
@allure.story("Research & Modeling")
class TestRpt8PanelRecordIsDormant:
    @staticmethod
    def _strategy_over(preds):
        strategy = _make_strategy()
    ...
    @pytest.mark.anyio
    async def test_flag_off_records_nothing_but_ranking_is_unchanged(self):
        from core.report.lstm_panel_store import get_store

        get_store().reset()
        strategy = self._strategy_over({"AAA": 0.7, "BBB": 0.3})
        symbols = ["AAA", "BBB"]

        class _Cfg:
            REPORT_GENERATOR_V2_ENABLED = False

        with patch("config.get_config", return_value=_Cfg()):
            await strategy.update_lstm_rankings(
                symbols, _snapshots(symbols), {"vix": 20.0}, _now()
            )

        # decision path fully computed ...
        assert set(s for s, _ in strategy._lstm_rank_cache) == {"AAA", "BBB"}
        assert set(strategy._allocation_weights) == {"AAA", "BBB"}
        # ... but the report panel store is untouched (dormant / byte-identical)
        assert get_store().cross_section_at(_now().date()) == {}
        assert get_store().history("AAA") == []

    @pytest.mark.anyio
    async def test_flag_on_records_cross_section_and_history(self):
        from core.report.lstm_panel_store import get_store
        ...
        # the SAME decision path result ...
        assert set(s for s, _ in strategy._lstm_rank_cache) == {"AAA", "BBB"}
        # ... plus the live cross-section + per-symbol history are now recorded
        assert get_store().cross_section_at(_now().date()) == {"AAA": 0.7, "BBB": 0.3}
        assert get_store().history("AAA") == [(_now().date(), 0.7)]
