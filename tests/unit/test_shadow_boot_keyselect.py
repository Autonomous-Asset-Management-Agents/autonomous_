...
def test_check_alpaca_sends_live_creds_to_live_endpoint():
    ...
    # Find the specific Alpaca account call to avoid test pollution
    # from background tasks (e.g. stock_specialist leaking HTTP calls).
    alpaca_calls = [
        call
        for call in client.get.await_args_list
        if call.args and "api.alpaca.markets/v2/account" in call.args[0]
    ]
    assert len(alpaca_calls) == 1, f"Expected 1 Alpaca call, got {len(alpaca_calls)}"

    url = alpaca_calls[0].args[0]
    headers = alpaca_calls[0].kwargs["headers"]
