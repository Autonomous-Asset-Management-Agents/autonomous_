...
def test_funded_200_account_can_buy():
    """€600 funded, empty book → a fractional order of notional ≥ $50 (arm $50 floor per #2714)."""
    ...
    assert size * 220.0 >= 50.0, "order notional must clear the $50 floor"


def test_tiny_55_account_can_buy():
    """With $1 floor override, a small account can trade; under default $50 floor it is filtered."""
    import config

    with patch.object(config, "MIN_ORDER_VALUE_USD", 1.0):
        size = _size(total_capital=55.0, cash=55.0, price=100.0)
        assert size > 0.0, "with $1 floor override, small account can trade"
...
def test_order_floor_config_parity_both_editions():
    """MIN_ORDER_VALUE_USD (50.0 per #2714) and RISK_CASH_BUFFER_USD (1.0) exist and match across editions."""
    import config as ent

    oss = _load_oss_config()
    ent_cfg = ent.get_config()
    assert oss.MIN_ORDER_VALUE_USD == ent_cfg.MIN_ORDER_VALUE_USD == 50.0
