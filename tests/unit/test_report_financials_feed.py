...
# ===========================================================================
# 2b. Prior-period LEVELS (#3275 Part 1 — enables the full 9-point Piotroski)
# ===========================================================================
def test_prior_levels_exposed_additively():
    """derive_fundamentals reicht die Vorjahres-LEVELS additiv durch (nicht nur YoY).

    Equivalenz-Test ohne Magic-Numbers: die FY2025-Werte, die bei as_of=2026-05-01
    als PRIOR gelten, müssen exakt denen entsprechen, die derive_fundamentals bei
    as_of=2026-01-01 (FY2025 = current) liefert. Nutzt exakt die `prior`-Werte, die
    schon die YoY treibt (real NVDA-Fixture, kein Netz)."""
    cur = derive_fundamentals(_load_results(), AS_OF, close=NVDA_CLOSE)
    prior_as_current = derive_fundamentals(_load_results(), date(2026, 1, 1))
    assert cur is not None and prior_as_current is not None

    assert cur["prior_revenue"] == prior_as_current["revenue"]
    assert cur["prior_net_income"] == prior_as_current["net_income"]
    assert cur["prior_gross_profit"] == prior_as_current["gross_profit"]
    assert cur["prior_total_assets"] == prior_as_current["total_assets"]
    assert cur["prior_total_liabilities"] == prior_as_current["total_liabilities"]
    assert cur["prior_current_assets"] == prior_as_current["current_assets"]
    assert cur["prior_current_liabilities"] == prior_as_current["current_liabilities"]
    assert cur["prior_diluted_shares"] == prior_as_current["diluted_shares"]
    assert cur["prior_operating_cash_flow"] == prior_as_current["operating_cash_flow"]
    # consistency with the YoY the same prior already drives
    assert round(cur["revenue_yoy"], 6) == round(
        cur["revenue"] / cur["prior_revenue"] - 1.0, 6
    )


def test_prior_levels_none_without_prior_period():
    """Ohne PIT-gültige Vorperiode sind die prior_* Level None (kein geratener Wert)."""
    f = derive_fundamentals(_load_results(), date(2026, 1, 1), close=NVDA_CLOSE)
    assert f["fiscal_year"] == "2025"  # FY2025 current, no prior in-window
    for k in (
        "prior_revenue",
        "prior_net_income",
        "prior_gross_profit",
