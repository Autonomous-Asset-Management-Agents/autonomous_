...
REVISED by #2784 for point 3: raising the floor to $50 was the wrong instrument — the
only exit-side consumer of that knob is the trim lever, which is default-OFF, so it
suppressed no dust while blocking every account under ~$1,000. The floor is back at
Alpaca's $1 minimum; exit dust needs a floor at the submit paths (still open)."""
...
def test_reentry_lockout_armed_by_default():
    # #3604: one trading day, owner decision 2026-09-23 (replaces the 240-min brake).
    assert int(get_config().REENTRY_LOCKOUT_DAYS) == 1


def test_min_order_value_floor_is_small_account_safe():
    # Rev. of #2714: a flat $50 floor blocked small accounts (fractional shares below
    # $50 are legitimate position sizes there). Default = Alpaca's $1 technical
    # minimum; large-account operators raise it via env.
    assert float(get_config().MIN_ORDER_VALUE_USD) == 1.0
