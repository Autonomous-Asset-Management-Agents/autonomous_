...
class TestRLRunForSymbolAntiChurn:
    ...
    async def test_run_for_symbol_sell_blocked_by_portfolio_manager(
        ...
        # Mock check_smart_exit -> no exit triggered
        rl_execution._check_smart_exit = MagicMock(
...
class TestRLConsensusRetentionGate:
    ...
    def _arm(self, rl_execution, features_df, *, smart_exit, consensus):
        pm = MagicMock()
        ...
        rl_execution._check_smart_exit = MagicMock(return_value=smart_exit)
        rl_execution._submit_order_safe = AsyncMock(return_value=True)
        return pm
    ...
    async def test_model_sell_retained_when_consensus_high(
        ...
        self._arm(
            rl_execution,
            features_df,
            smart_exit={"triggered": False, "signal": "HOLD"},
            consensus=0.80,
        )
    ...
    async def test_risk_tier_smart_exit_never_gated(
        ...
        self._arm(
            rl_execution,
            features_df,
            smart_exit={"triggered": True, "signal": "SELL", "tier": "risk"},
            consensus=0.99,
        )
        event = await self._run(rl_execution, market_data)
        assert event.action == "SELL"
    ...
    async def test_opinion_tier_smart_exit_retained(
        ...
        self._arm(
            rl_execution,
            features_df,
            smart_exit={"triggered": True, "signal": "SELL", "tier": "opinion"},
            consensus=0.80,
        )
        event = await self._run(rl_execution, market_data)
        rl_execution._submit_order_safe.assert_not_called()
