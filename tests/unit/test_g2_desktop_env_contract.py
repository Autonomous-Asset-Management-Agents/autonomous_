...
class DesktopWorkspaceExists(unittest.TestCase):
    def test_manager_module_exists(self):
        self.assertTrue(
            _MANAGER.is_file(),
            f"G2 contract: {_MANAGER} missing — the desktop shell must live in "
    ...
    def test_workspace_is_isolated_from_root_package_json(self):
        ...
        if root_pkg_path.is_file():
            ...
            self.assertNotIn(
                ...
                "root package.json must not adopt desktop/ as an npm workspace",
            )
...
class MandatoryEnvBlock(unittest.TestCase):
    ...
    def test_loopback_bind(self):
        ...
        # after G0b flips the engine default — defense in depth on every spawn.
        self._has(
    ...
    def test_paper_trading_is_the_worm_gated_live_switch(self):
        # LIVE-1 T1 (#1424) + BORA: PAPER_TRADING is the ONE live gate — the same mechanism as the
        # cloud edition (it also drives ALPACA_BASE_URL paper-api↔api, config.py:53). It is the
        # confirmed-live ternary, DEFAULTING to "true" (the Alpaca PAPER account — fake money,
        # fail-closed) and is "false" (real money) ONLY when verifyAuditChain confirms an un-revoked
        # Art.-14 WORM live-enablement (T4). A live boot is impossible without that verified record.
        self._has(
            r'env\.PAPER_TRADING\s*=\s*\w+\s*\?\s*"false"\s*:\s*"true"',
            "PAPER_TRADING must be the confirmed-live ternary defaulting to true (fail-closed paper)",
        )
        # The live (false) branch must be gated on the audit-chain verifier, not a bare flag.
        self._has(
            r"verifyAuditChain\s*\(",
            "the PAPER_TRADING live branch must be gated on verifyAuditChain (WORM verifier)",
        )
        # NEGATIVE: PAPER_TRADING must never be an UNCONDITIONAL "false" (no ungated live trading).
        self.assertNotRegex(
            self.src,
            r'env\.PAPER_TRADING\s*=\s*"false"\s*;',
            "PAPER_TRADING must never be an unconditional false (fail-closed default required)",
        )

    def test_shadow_mode_forced_off_honest_paper(self):
        # BORA + honesty: paper trading round-trips to the Alpaca PAPER account (fake money) and is
        # already safe — it needs NO shadow interception. SHADOW_MODE (a pre-broker bypass) is NOT a
        # paper safety gate and was a desktop-only divergence from the cloud. It is forced OFF so
        # paper AND live both execute honestly; the single live gate is PAPER_TRADING above.
        self._has(
            r'env\.SHADOW_MODE\s*=\s*"false"',
            "SHADOW_MODE must be forced off (honest paper execution; PAPER_TRADING is the gate)",
        )
        # NEGATIVE: SHADOW_MODE must never be "true" — the dishonest shadow-sim (paper that never
        # reaches the broker) the cloud edition never uses.
        self.assertNotRegex(
            self.src,
            r'env\.SHADOW_MODE\s*=\s*"true"',
            "SHADOW_MODE must never be forced true (dishonest shadow-sim; use PAPER_TRADING)",
        )

    def test_live_boot_forces_hitl_enabled(self):
        # EU AI Act Art. 14: a live boot (PAPER_TRADING=False) MUST carry HITL_ENABLED — config.py
        # _enforce_hitl_boot_gate raises otherwise. The shell forces it on the verified-live path,
    ...
    def test_auto_start_strategy_false(self):
        ...
        self._has(
            ...
            "AUTO_START_STRATEGY must be forced to false on live (env default is True!)",
        )
    ...
    def test_local_state_switch_is_redis_url(self):
        # main's switch is an EMPTY REDIS_URL (core/redis_client.py:43), not the
    ...
    def test_senate_log_dir_anchored_in_user_data(self):
        ...
        # SENATE_LOG_DIR at instance time (senate_log.py:96) and would otherwise
        # write audit_log_<date>.jsonl to a CWD-relative `oss_audit_logs` under
        # ai_trading_bot/ — read-only in a Program Files install. The shell must
        # anchor it under the per-user data dir so (a) audit logs persist and
        # (b) the console's audit-chain reader (audit-chain.cjs::resolveAuditDir)
        # resolves the SAME path and the Audit Chain page is not empty.
        self._has(
            r'env\.SENATE_LOG_DIR\s*=\s*path\.join\(\s*userDataDir\s*,\s*"cloud_fallback_logs"\s*\)',
            "SENATE_LOG_DIR must be <userDataDir>/cloud_fallback_logs so writer "
            "(LocalJSONAuditLogger) and reader (audit-chain.cjs) agree",
        )

    def test_cors_both_vars_with_renderer_origin(self):
        # The engine reads ALLOWED_ORIGINS (api_routes.py:182), the public proxy
        # reads CORS_ALLOWED_ORIGINS (serve_public_api.py:347). Both must be the
        ...
        self._has(
            ...
            "CORS_ALLOWED_ORIGINS must be the renderer-origin list (proxy parity)",
        )
        self.assertNotRegex(
            self.src,
    ...
    def test_xai_agent_core_enabled(self):
        # XAI-T9a (#1401): the desktop OSS build enables the glass-box agent-core
        # so /chat routes through the 4-domain Zero-Hallucination router instead
        ...
        self._has(
            ...
            "XAI_AGENT_CORE must default to '1' on the desktop spawn (glass-box chat)",
        )
...
class MainShellContract(unittest.TestCase):
    ...
    def test_renderer_origin_flows_into_manager(self):
        # The ephemeral static-server port must reach _childEnv (CORS contract).
        self.assertRegex(
            self.src,
            r"rendererOrigin",
    ...
    def test_desktop_opens_the_operator_console(self):
        ...
        self.assertRegex(
            self.src,
            r"loadURL\([^)]*/console",
            "desktop shell must open /console (the operator console), not /dashboard",
        )
