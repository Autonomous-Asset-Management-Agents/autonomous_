...
class MandatoryEnvBlock(unittest.TestCase):
    ...
    def test_desktop_arm_sets_the_whole_lstm_and_exit_group(self):
        ...
        # #3632: the EXIT_POLICY_TRAILING_ENABLED arm is gone with the smart-exit rule set;
        # profit protection is the intelligent exit's own trailing tiers (profile below).
        # #2681: the THIRD exit subsystem (core/intelligent_exit.py) has its own trailing
        # tiers — the legacy 1.5%-from-+2%-after-1h tier churned the book on 2026-08-05
        # (HPE sold 30 min after a top-up, re-bought higher). The desktop profile must
        # select the mid-term tier table; a silent drop of this arm fails CI here.
        self._has(
            r'env\.EXIT_TRAIL_PROFILE\s*=\s*"midterm"',
            "the desktop profile must set EXIT_TRAIL_PROFILE=midterm (intelligent_exit tiers, #2681)",  # noqa: E501
        )
        # #2678 (owner directive 2026-08-05, Stufe 0 of the #1957 mid-term plan): rotation on a
        # MID-TERM cadence for every desktop/test install. July forensics: daily rank noise
        # (cross-section rank 15→~300 in 2 days) exited held names with min-hold 5d/hysteresis 3;
        # KW31 churn = 51 fills / 375k$ volume on a 147k book. Independent of the LSTM arm toggle
        # (same reasoning as the trailing arm above); explicit pre-set env still WINS.
        self._has(
            r'env\.SMART_EXIT_MIN_HOLD_DAYS\s*=\s*"20"',
            "the desktop profile must set SMART_EXIT_MIN_HOLD_DAYS=20 (mid-term hold, #2678)",
        )
        self._has(
            r'env\.SMART_EXIT_EXIT_RANK_HYSTERESIS\s*=\s*"5"',
            "the desktop profile must set SMART_EXIT_EXIT_RANK_HYSTERESIS=5 (rank-noise immunity, #2678)",  # noqa: E501
        )
        self._has(
            r'env\.ROTATION_MAX_EXITS_PER_SESSION\s*=\s*"2"',
            "the desktop profile must set ROTATION_MAX_EXITS_PER_SESSION=2 (max two rotations/day, #2678)",  # noqa: E501
        )
        self._has(
            r'env\.ROUND_TABLE_TOP_K_EVAL\s*=\s*"30"',
            "the desktop profile must set ROUND_TABLE_TOP_K_EVAL=30 (top 30 funnel, #2678)",
        )
        # #2680 "Patient AI": the ranking BASIS. Without this arm the mid-term posture above
        # sits on a point-in-time signal that moved ~230 places in three hours (IVZ,
        # 2026-08-05) — the smoothing is what makes min-hold 20 coherent.
        self._has(
            r'env\.LSTM_RANK_SMOOTHING_ENABLED\s*=\s*"True"',
            "the desktop profile must arm LSTM_RANK_SMOOTHING_ENABLED=True (trend ranking, #2680)",  # noqa: E501
        )
        ...
        # #2631: the registry's startup high-priority set is the STATIC _CARD_REGISTRY_STOCKS
        # list (base.py:524/532). Without SPECIALIST_COVERAGE_DYNAMIC the retarget returns at
        # trading_loop.py:610 and held positions NEVER get a report — measured 05.08.: 33
        # cached reports (mega caps), of 10 holdings only JNJ + XOM covered, so
        # SpecialistAlphaAgent abstained (weight 0.0) for 8 of 10 positions.
        self._has(
            r'env\.SPECIALIST_COVERAGE_DYNAMIC\s*=\s*"True"',
            "the desktop profile must set SPECIALIST_COVERAGE_DYNAMIC=True so held positions "  # noqa: E501
            "enter the specialist registry (#2631 is inert without it)",
        )
        self._has(
