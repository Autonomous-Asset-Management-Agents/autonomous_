...
async def test_live_deposit_does_not_inflate_twr():
    ...
    # _get_live_cashflows() reads this: a 5000 CSD deposit aligned to day 2.
    live_hist = SimpleNamespace(
        timestamp=[_TS_D1, _TS_D2],
        equity=[10_000.0, 15_000.0],
        cashflow={"CSD": [0.0, 5_000.0], "CSW": [0.0, 0.0]},
    )
    mock_engine = MagicMock()
    mock_engine.api.get_portfolio_history.return_value = live_hist
...
async def test_live_deposit_in_snapshot_gap_still_neutralised():
    ...
    ts_gap = int(datetime(2026, 6, 2, 0, 0, 0, tzinfo=timezone.utc).timestamp())
    ...
    # Alpaca reports the deposit on the GAP day 06-02.
    live_hist = SimpleNamespace(
        timestamp=[ts_gap],
        equity=[0.0],
        cashflow={"CSD": [5_000.0], "CSW": [0.0]},
    )
    mock_engine = MagicMock()
    mock_engine.api.get_portfolio_history.return_value = live_hist
    mock_engine.data_provider.get_data.return_value = spy_df
