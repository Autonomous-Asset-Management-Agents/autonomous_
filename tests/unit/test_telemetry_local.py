...
def test_local_exporter_serializes_trace_context_and_resource(tmp_path):
    # OBS-1 (#2634): the OTel SDK already holds trace/span/parent IDs + kind + Resource
    # in-process; the exporter must NOT drop them, or the local diagnostics stay isolated
    # (no waterfalls, no source identity) — the exact gap in the 2026-08-03 export.
    from opentelemetry.sdk.trace import TracerProvider
    from opentelemetry.sdk.trace.export import SimpleSpanProcessor

    tele_dir = tmp_path / "telemetry"
    exporter = LocalScrubbingSpanExporter(str(tele_dir))
    provider = TracerProvider(resource=minimal_resource("aaa-desktop", "v9"))
    provider.add_span_processor(SimpleSpanProcessor(exporter))
    tracer = provider.get_tracer("test")

    with tracer.start_as_current_span("parent.op") as parent:
        pctx = parent.get_span_context()
        parent_trace_id = format(pctx.trace_id, "032x")
        parent_span_id = format(pctx.span_id, "016x")
    ...
    prec, crec = by_name["parent.op"], by_name["child.op"]

    # identity
    assert prec.get("trace_id") == parent_trace_id
    assert len(prec.get("span_id") or "") == 16
    assert prec.get("kind")  # e.g. "INTERNAL"
    # correlation: same trace, child links to parent
    assert crec.get("trace_id") == parent_trace_id
    assert crec.get("parent_span_id") == parent_span_id
    # source identity (Resource)
    res = prec.get("resource") or {}
    assert res.get("service.name") == "aaa-desktop"
    assert res.get("service.version") == "v9"


def test_local_exporter_drops_health_and_options_noise_but_keeps_errors(tmp_path):
    # OBS-2 (#2635): successful /health polls + OPTIONS CORS-preflights dominate the export
    # (~35 %) with near-zero diagnostic value. Drop them; NEVER drop an ERROR or a trading
    # span (the "keep 100 % errors + trading" invariant).
    from opentelemetry.sdk.trace import TracerProvider
    from opentelemetry.sdk.trace.export import SimpleSpanProcessor
    ...
    with tracer.start_as_current_span("GET /health"):
        pass  # OK health poll -> dropped
    with tracer.start_as_current_span("OPTIONS /portfolio-summary"):
        pass  # CORS preflight -> dropped
    with tracer.start_as_current_span("GET /health") as s:
        s.set_status(StatusCode.ERROR)  # ERROR health -> KEPT
    with tracer.start_as_current_span("model.inference"):
        pass  # trading span -> KEPT
    provider.force_flush()
    ...
    assert "model.inference" in names  # trading span kept
    assert not any(n.startswith("OPTIONS ") for n in names)  # preflight dropped
    health = [r for r in recs if r["name"] == "GET /health"]
    assert len(health) == 1  # only the ERROR one survived (OK poll dropped)
