...
def test_flag_off_default_emits_no_span(span_capture):
    # Default posture (BORA byte-identity): no flag -> no span, exact today's behaviour.
...
def test_config_flag_default_off():
    assert config.EXECUTION_BLOCK_TELEMETRY_ENABLED is False


def test_config_oss_flag_parity_default_off():
    ...
    assert cfg.EXECUTION_BLOCK_TELEMETRY_ENABLED is False
