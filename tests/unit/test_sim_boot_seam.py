...
def test_boot_seam_routes_through_factory_under_sim_mode(monkeypatch):
    ...
    monkeypatch.setattr(api_routes, "engine", None, raising=False)

    class _StubEngine:
        def __init__(self, trading_client=None, data_client=None, **_kw):
...
def test_boot_seam_sim_mode_ignores_missing_alpaca_key(monkeypatch):
    ...
    monkeypatch.setattr(
        api_routes,
        "BotEngine",
        lambda trading_client=None, data_client=None, **_kw: captured.update(
