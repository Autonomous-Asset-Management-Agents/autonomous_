...
class TestMomentumAbstainCoupling:
    ...
    def test_abstained_momentum_not_halved_and_excluded(self):
        ...
        # Only LSTM carries the mean (regime excluded, momentum abstained). #3618: with
        # the conditioner ON (default) the bearish regime (0.30 -> severity 0.4, factor
        # 0.8) shrinks that BUY-leaning mean toward neutral: 0.5 + 0.8 * (0.8 - 0.5).
        assert abs(result - (0.5 + 0.8 * 0.3)) < 1e-6
