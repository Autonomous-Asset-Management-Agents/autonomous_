...
# ---------------------------------------------------------------------------
# 1. Market Closed → INC-6 (continuous operation, Option A):
#    research/analysis runs CONTINUOUSLY (no full-cycle sleep-SKIP), at a modest
#    cadence. Order PLACEMENT stays market-gated downstream (order_executor).
# ---------------------------------------------------------------------------


def _make_rlagent_cycle_mixin(market_open):
    """Full RLAgent cycle mixin (mirrors TestLangGraphDispatch) so the loop can
    run a real analysis cycle. `_process_signal_event` is mocked — the ORDER gate
    is exercised separately in test_order_executor.py::TestMarketClosedOrderGate."""
    import threading
    ...
    lt.p = 150.0
    lt.size = 100
    # #3381: Alpaca liefert am latest_trade immer einen Zeitstempel. Ohne ihn gilt das
    # Kursbild als 'Alter unbekannt' und der Zyklus enthaelt sich (stale_quote).
    lt.timestamp = datetime.now(timezone.utc)
    bar = MagicMock()
    bar.open = 148.0
    bar.high = 152.0
    bar.low = 147.0
    ...
    return mixin, data_api, shutdown_flags
...
class TestMarketClosed:
    ...
    async def test_closed_market_off_path_runs_inc6_research_cycle(self):
        """OFF path (MARKET_CLOSED_REPORT_ONLY=False) — byte-identical to legacy INC-6.

        Given: Markt geschlossen (is_open=False), BYPASS off, report-only mode DISABLED
        When:  live_trading_loop einen Zyklus ausführt
        Then:  INC-6 — die Analyse/Research läuft TROTZDEM (Snapshot-Fetch wird gerufen,
               KEIN 300s-Skip vor der Research); Cadence modest (300s Sleep, nicht 60s).

        The DEFAULT (report-only ON) behaviour is pinned in
        ``test_closed_market_report_only_skips_trading_and_runs_one_pass``.
        """
        import config as _cfg

        mixin, data_api, shutdown_flags = _make_rlagent_cycle_mixin(market_open=False)

        sleep_calls = []

        # §5.2: AsyncMock with a sync side_effect — never a hand-rolled `async def` stub.
        def _sleep_side_effect(s):
            sleep_calls.append(s)
            if s >= 60:
                shutdown_flags[0] = True

        mock_sleep = AsyncMock(side_effect=_sleep_side_effect)

        mock_graph = MagicMock()
        mock_graph.ainvoke = AsyncMock(side_effect=lambda state, **kw: state)
        ...
        ), patch("core.engine.trading_loop.BYPASS_MARKET_HOURS", False), patch.object(
            _cfg.get_config(), "MARKET_CLOSED_REPORT_ONLY", False
        ):
            await mixin.live_trading_loop()

        # Research ran — the cycle was NOT sleep-skipped when the market was closed.
        data_api.get_stock_snapshot.assert_called()
        # Modest cadence: the closed-cycle end sleep is 300s (not the open-cycle 60s).
        assert 300 in sleep_calls, "closed cycle must sleep at a modest 300s cadence"

    @pytest.mark.anyio
    async def test_closed_market_report_only_skips_trading_and_runs_one_pass(self):
        """DEFAULT (MARKET_CLOSED_REPORT_ONLY=True) — trade only during market hours.

        Given: Markt geschlossen (is_open=False), BYPASS off, report-only mode ON (default)
        When:  live_trading_loop einen Zyklus ausführt
        Then:  EIN full-universe Panel/Report-Pass läuft (Snapshot-Fetch + strategy
               .update_lstm_rankings gerufen), aber der Round-Table-Deep-Eval wird
               ÜBERSPRUNGEN (build_symbol_eval_graph NICHT gerufen) → GPU ruht; danach
               idle (nicht die 300s-INC-6-Cadence).
        """
        import config as _cfg

        mixin, data_api, shutdown_flags = _make_rlagent_cycle_mixin(market_open=False)
        # The helper's clock.next_open is a MagicMock → the loop's float() guard falls back to
        # the bounded cap idle (never 300s), which is exactly the report-only cadence we assert.
        strategy = mixin.active_strategy
        ...
        with patch("core.engine.trading_loop.asyncio.sleep", mock_sleep), patch(
            ...
            "core.engine.trading_loop.BYPASS_MARKET_HOURS", False
        ), patch.object(
            _cfg.get_config(), "MARKET_CLOSED_REPORT_ONLY", True
        ):
            await mixin.live_trading_loop()

        # The ONE report/panel pass ran: snapshots fetched + the ranker was updated.
        data_api.get_stock_snapshot.assert_called()
        strategy.update_lstm_rankings.assert_awaited()
        # The trading deep-eval was SKIPPED — the round-table graph is never built (GPU rests).
        # This is THE behavioural distinction from the INC-6 OFF path (which builds it).
        mock_build_graph.assert_not_called()
        # The loop idled after the one pass (bounded heartbeat), then re-checks the clock.
        assert any(
            s >= 60 for s in sleep_calls
        ), "report-only cycle must idle after the pass"

    async def test_reconcile_entry_time_runs_even_when_market_closed(self):
        """Restart while the market is CLOSED must STILL reconcile entry-time from Alpaca
        fills — otherwise days_held stays 0 for EVERY held position ("0 days after restart").

        The once/session entry-time reconcile (#2046) sat AFTER the market-closed report-only
        `continue`, so on any restart while closed it never ran and _trade_history stayed empty.
        It must run BEFORE the closed branch (it is read-only vs the broker + once/session-guarded,
        so running it on the closed path too is safe + idempotent).
        """
        import config as _cfg
        ...
        mock_sleep = AsyncMock(side_effect=_sleep_side_effect)
        mixin._reconcile_active_strategy_entry_time = AsyncMock()
        ...
        mixin._reconcile_active_strategy_entry_time.assert_awaited()


@pytest.mark.vc0
@allure.story("Core Utilities")
class TestClosedReportPassRetry:
    """F1/F4 — the market-closed report pass reports success honestly (so the caller only
    latches once-per-close on SUCCESS and a transient failure retries) and mirrors the open
    path's FULL_UNIVERSE universe."""
    ...
    def _mixin(self):
        ...
        mixin.data_api.get_stock_snapshot.return_value = {"AAPL": MagicMock()}
        mixin.current_market_data = {}
        mixin._rank_panel_producer = None
        return mixin

    @pytest.mark.anyio
    async def test_empty_symbols_returns_false(self):
        mixin = self._mixin()
        assert await mixin._run_closed_report_pass(MagicMock(), []) is False

    @pytest.mark.anyio
    async def test_fetch_failure_returns_false_so_caller_retries(self):
        # F1: a snapshot-fetch RemoteDisconnect must NOT count as a completed pass.
        mixin = self._mixin()
        mixin.data_api.get_stock_snapshot.side_effect = ConnectionError(
            "RemoteDisconnected"
        )
        strat = MagicMock()
        strat.update_lstm_rankings = AsyncMock()
        assert await mixin._run_closed_report_pass(strat, ["AAPL", "MSFT"]) is False

    @pytest.mark.anyio
    async def test_successful_rank_returns_true(self):
        mixin = self._mixin()
        strat = MagicMock()
        strat.update_lstm_rankings = AsyncMock()
        assert await mixin._run_closed_report_pass(strat, ["AAPL", "MSFT"]) is True
        strat.update_lstm_rankings.assert_awaited_once()

    @pytest.mark.anyio
    async def test_rank_exception_returns_false(self):
        mixin = self._mixin()
        strat = MagicMock()
        strat.update_lstm_rankings = AsyncMock(side_effect=RuntimeError("model boom"))
        assert await mixin._run_closed_report_pass(strat, ["AAPL"]) is False

    @pytest.mark.anyio
    async def test_full_universe_off_truncates_to_200_like_open_path(self):
        # F4: closed pass ranks the SAME universe as the open path (200-cap when flag off).
        import config as _cfg
        ...
        with patch.object(_cfg.get_config(), "FULL_UNIVERSE_TRADING_ENABLED", False):
            assert await mixin._run_closed_report_pass(strat, symbols) is True
        fetched_symbols = [
            s
            for call in mixin.data_api.get_stock_snapshot.call_args_list
            for s in call[0][0].symbol_or_symbols
        ]
        assert len(fetched_symbols) == 200
...
class TestSIPFeedConfiguration:
    ...
    async def test_trading_loop_uses_sip_feed(self):
        ...
        with (
            patch("core.engine.trading_loop.config.ALPACA_DATA_FEED", "sip"),
            patch(
