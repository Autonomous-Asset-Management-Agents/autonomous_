...
class TestStartupHealthCheckKey:
    @pytest.mark.anyio
    async def test_llm_failure_is_reported_under_the_llm_key(self, engine):
        # B11: an unreachable LLM is a WAIT state now (tests/unit/test_llm_wait_state.py);
        # a shutdown during the wait ends it with the RuntimeError that names "llm".
        ...
        with patch.object(
            ...
            AsyncMock(side_effect=shutdown_during_wait),
        ):
