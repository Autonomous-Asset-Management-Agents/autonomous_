...
class TestDormancyFlagOff:
    ...
    def test_flag_off_vs_on_scoring_invariant_full_tuple(self):
        ...
        # ...and flag-ON fills the company_summary (still the raw parse; #2249 §4
        # keeps company_summary intentionally out of scope, byte-identical).
        assert on.company_summary != ""
        # #2249 FIX 1: bull/bear/thesis are now GATED behind grounding — with
        # SPECIALIST_PROMPT_V2 ON but SPECIALIST_GROUNDING_ENABLED OFF (this
        # _set_flag), the displayed prose stays "" (honest empty) instead of the
        # raw, possibly-fabricated parse. Grounding ON is covered in
        # test_card_prose_grounding_gate.py::test_grounding_on_fills_displayed_prose_and_sets_ok.
        assert on.bull_case == ""
        assert on.bear_case == ""
