"""#3135 â€” proportional slot-cash split.
...
sizer â€” empirically observed on the live install (8 buys, risk_size_scaler 0.50-1.089, all  # noqa: E501
...
get a bigger slice â€” but ONLY ever SHRINKS vs the equal split, so the no-leverage safety  # noqa: E501
...
def _force_legacy_sizing(monkeypatch):
    import config
...
def _rm():
    # client=None â†’ the optional broker-backed total-exposure cap (Â§5b) is skipped, so this  # noqa: E501
...
_COMMON = dict(
    ...
    market_data={"vix": 15.0},  # VIX <= 18 â†’ vix_risk_scaler = 1.0
    num_stocks_in_strategy=10,  # slots = 10 â†’ (cash-buffer)/slots â‰ˆ $999.9
    current_price=100.0,
    account_cash=10_000.0,
    allow_fractional=True,
    forecast_vol=None,  # vol_targeting no-op â†’ vol_scaler = 1.0
)
...
def test_flag_on_conviction_differentiates_slot_cash():
    """ON â†’ higher conviction gets a bigger cash slice (today: both clamp equal â†’ RED)."""  # noqa: E501
    rm = _rm()
...
def test_flag_off_is_equal_split_byte_identical():
    """OFF â†’ every concurrent BUY capped at the SAME (cash-buffer)/slots (today's behaviour)."""  # noqa: E501
    rm = _rm()
...
def test_on_never_exceeds_equal_split_no_leverage():
    """SAFETY: the proportional cap can only SHRINK vs the equal split â†’ no leverage."""  # noqa: E501
    rm = _rm()
    with patch("config.PROPORTIONAL_SLOT_CASH_ENABLED", False, create=True):
...
def test_iv_forecast_vol_differentiates_slot_cash():
    """ON + vol-targeting â†’ calmer (lower IV/forecast_vol) name gets a bigger slice than a  # noqa: E501
...
def test_dynamic_sizing_off_falls_back_to_equal():
    ...
    â†’ equal split (no proportional scaling to base it on)."""
    rm = _rm()
    with patch(
        "config.PROPORTIONAL_SLOT_CASH_ENABLED", True, create=True
    ), patch(  # noqa: E501
