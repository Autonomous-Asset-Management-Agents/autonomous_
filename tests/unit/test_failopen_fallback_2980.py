...
def patch_clean_weight_off(monkeypatch):
    import config

    monkeypatch.setattr(config, "CLEAN_WEIGHT_SIZING", "off")
...
class TestResolverDriftGuard:
    ...
    def test_p3_unconfirmed_iff_sentinel(self, raw):
        val, confirmed, _ = resolve_vix({"vix": raw})
        if confirmed and raw == VIX_FAILCLOSED_SENTINEL:
            pass  # Natively generated the sentinel, so it's confirmed but equals sentinel
        else:
