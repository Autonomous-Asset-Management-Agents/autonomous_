...
def test_watchdog_active_ping_timeout():
    ...
    with patch("core.latency_watchdog.kill_switch") as mock_ks, patch(
        ...
        # #2142: this test verifies the active-ping-timeout TRIP PATH + reason
        # string. The consecutive-failure DEBOUNCE is covered separately in
        # test_latency_watchdog_core.py; collapse it here (limit=1) so a single
        # timeout still exercises the trip.
        wd.consecutive_limit = 1
