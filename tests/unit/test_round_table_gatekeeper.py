...
class TestGatekeeperPDTVeto:
    ...
    def _pdt_legacy_strict(self, monkeypatch):
        # R2 flipped GATEKEEPER_PDT_FINRA_ACCURATE default ON; these tests pin the legacy
        # strict gate (>=3, equity-blind) → arm the flag OFF. FINRA-accurate (flag-on)
        # behaviour is covered by test_gatekeeper_pdt_finra_accurate.py.
        import config
...
class TestGatekeeperConcentrationVeto:
    ...
    @pytest.mark.anyio
    async def test_veto_reason_is_english_only(self):
        """#2132: user-facing gatekeeper reasons must be ENGLISH (regulated system). The
        concentration veto previously read 'Nur SELL-Signale zur Risikoreduzierung erlaubt.'
        """
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(symbol_weights={"TSLA": 0.30})
        result = await gk.check("TSLA", 0.8, ctx)
        assert result.approved is False
        assert "Only risk-reducing SELL signals are allowed" in result.reason
        assert not any(
            c in result.reason for c in "äöüÄÖÜß"
        ), f"German text leaked into a user-facing reason: {result.reason}"
...
@allure.feature("VC-2 Portfolio Construction")
@allure.story("Portfolio & Strategy")
class TestGatekeeperPDTCrossDayExemption:
    """#2037: a cross-day risk-reducing SELL (no fill today) is exempt from the PDT block.
    ``traded_today`` is populated by build_portfolio_context ONLY when the opt-in
    GATEKEEPER_PDT_CROSSDAY_EXEMPTION flag is on; empty otherwise ⇒ .get(sym, True) fail-safe.
    """

    @pytest.fixture(autouse=True)
    def _pdt_legacy_strict(self, monkeypatch):
        # R2 flipped GATEKEEPER_PDT_FINRA_ACCURATE default ON; these #2037 cross-day tests
        # assume the legacy >=3 threshold, so arm the flag OFF.
        import config
    ...
    @pytest.mark.anyio
    async def test_cross_day_sell_exempt(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(day_trades_last_5d=3, traded_today={"HOOD": False})
        result = await gk.check("HOOD", 0.2, ctx)  # SELL (0.2<0.35), no fill today
        assert result.approved is True

    @pytest.mark.anyio
    async def test_same_day_sell_blocked(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(day_trades_last_5d=3, traded_today={"HOOD": True})
        result = await gk.check("HOOD", 0.2, ctx)
        assert result.approved is False
        assert "PDTLimit" in result.reason

    @pytest.mark.anyio
    async def test_buy_never_exempt(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(day_trades_last_5d=3, traded_today={"HOOD": False})
        result = await gk.check("HOOD", 0.9, ctx)  # BUY
        assert result.approved is False
        assert "PDTLimit" in result.reason

    @pytest.mark.anyio
    async def test_unknown_symbol_failsafe_block(self):
        """Absent from traded_today (unknown, or flag off ⇒ empty map) → default True → block."""
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(day_trades_last_5d=3, traded_today={})
        result = await gk.check("HOOD", 0.2, ctx)
        assert result.approved is False
        assert "PDTLimit" in result.reason

    @pytest.mark.anyio
    async def test_hold_boundary_035_not_exempt(self):
        """score == SELL_THRESHOLD (0.35) is HOLD, not a risk-reducing SELL → no exemption."""
        from core.round_table.gatekeeper import ComplianceGatekeeper

        gk = ComplianceGatekeeper()
        ctx = make_context(day_trades_last_5d=3, traded_today={"HOOD": False})
        result = await gk.check("HOOD", 0.35, ctx)
        assert result.approved is False

    def test_sell_threshold_coupled_to_consensus(self):
        from core.round_table import consensus
        from core.round_table.gatekeeper import ComplianceGatekeeper

        assert (
            ComplianceGatekeeper.SELL_THRESHOLD
            == consensus.SIGNAL_SELL_THRESHOLD
            == 0.35
        )


@allure.feature("VC-2 Portfolio Construction")
@allure.story("Anti-Churn per-symbol entry cap (#2153 Part A)")
class TestSymbolChurnCap:
    """Per-symbol daily BUY cap. Only active behind symbol_churn_guard_active (the
    build_portfolio_context mirror of GATEKEEPER_SYMBOL_CHURN_LIMIT). SELL-exempt like
    every other BUY guard (gated on score > BUY_THRESHOLD). Fail-closed: an invalid
    fill-count query vetoes ALL BUYs; a valid query with the symbol absent = 0 = allow.
    """

    @staticmethod
    def _ctx(**over):
        base = dict(
            symbol_churn_guard_active=True,
            symbol_trades_today_valid=True,
            symbol_trades_today={},
    ...
    @pytest.mark.anyio
    async def test_buy_at_cap_is_vetoed(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(symbol_trades_today={"AAPL": 5})
        r = await ComplianceGatekeeper().check("AAPL", 0.7, ctx)
        assert r.approved is False
        assert "SymbolDailyLimit" in r.reason
        assert "5/5" in r.reason

    @pytest.mark.anyio
    async def test_buy_under_cap_passes(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(symbol_trades_today={"AAPL": 4})
        r = await ComplianceGatekeeper().check("AAPL", 0.7, ctx)
        assert r.approved is True

    @pytest.mark.anyio
    async def test_absent_symbol_with_valid_query_is_zero_and_passes(self):
        """A fresh/untraded symbol (absent from a HEALTHY query) is 0/5 → tradable."""
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(symbol_trades_today={})  # valid query, MSFT absent
        r = await ComplianceGatekeeper().check("MSFT", 0.7, ctx)
        assert r.approved is True

    @pytest.mark.anyio
    async def test_invalid_query_fails_closed_and_blocks_buys(self):
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(symbol_trades_today_valid=False)
        r = await ComplianceGatekeeper().check("MSFT", 0.7, ctx)
        assert r.approved is False
        assert "SymbolChurnGuard" in r.reason

    @pytest.mark.anyio
    async def test_sell_is_never_blocked_by_the_churn_cap(self):
        """A risk-reducing SELL passes even at the cap AND even on an invalid query."""
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(
            symbol_trades_today={"AAPL": 9}, symbol_trades_today_valid=False
        )
        r = await ComplianceGatekeeper().check("AAPL", 0.1, ctx)  # SELL
        assert r.approved is True

    @pytest.mark.anyio
    async def test_guard_inactive_never_vetoes(self):
        """Flag off (guard inactive) → the cap is provably inert, even far over it."""
        from core.round_table.gatekeeper import ComplianceGatekeeper

        ctx = self._ctx(
            symbol_churn_guard_active=False, symbol_trades_today={"AAPL": 99}
        )
        r = await ComplianceGatekeeper().check("AAPL", 0.7, ctx)
        assert r.approved is True
