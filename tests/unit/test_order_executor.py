...
def _make_decision_context(**kwargs):
    ...
    ctx.decision_id = kwargs.get("decision_id", "test-uuid")
    ctx.intent_kind = kwargs.get("intent_kind", "entry")
    ctx.stop_intent = kwargs.get("stop_intent", False)
    ctx.action_executed = kwargs.get("action_executed", False)
    ctx.intelligence_approved = kwargs.get("intelligence_approved", True)
    ctx.gatekeeper_approved = kwargs.get("gatekeeper_approved", True)
    ctx.current_price = kwargs.get("current_price", 150.0)
    ctx.conviction_score = kwargs.get("conviction_score", 0.8)
    ctx.gatekeeper_reason = kwargs.get("gatekeeper_reason", "")
    ctx.votes_json = kwargs.get("votes_json", "{}")
    ctx.voted_price = kwargs.get("voted_price", 150.0)
    ctx.cycle_open = kwargs.get("cycle_open", 150.0)
    ctx.cycle_high = kwargs.get("cycle_high", 155.0)
    ctx.cycle_low = kwargs.get("cycle_low", 145.0)
    ctx.cycle_close = kwargs.get("cycle_close", 150.0)
    ctx.cycle_volume = kwargs.get("cycle_volume", 1000)
    ctx.alpaca_order_id = ""
    return ctx
...
class TestBuyFallbackGlobalAccount:
    ...
    @pytest.mark.anyio
    async def test_fallback_buy_with_suggested_quantity_clamps_to_sizer(self):
        """INC-7 / #3528: A suggested_quantity > 0 must STILL route through the RiskManager
        sizer, and the final quantity must be clamped to max_allowed_qty."""
        from unittest.mock import AsyncMock, MagicMock, patch

        api = MagicMock()
        api.submit_order.return_value = MagicMock(id="order-123")
        api.get_account = MagicMock(
            return_value=MagicMock(cash=10000.0, equity=10000.0)
        )
        executor = _make_executor(api=api)

        executor.live_risk_manager = MagicMock()
        executor.live_risk_manager.calculate_position_size.return_value = 5.0

        event = _make_signal(action="BUY", qty=70.0, is_simulation=False)
        event.decision_context.atr_14d = 1.0
        event.decision_context.vix_level = 20.0
        event.decision_context.current_price = 150.0
        ...
        with (
            ...
            patch(
                "core.engine.order_executor._regime_throttled_size", return_value=5.0
            ),
        ):
            await executor._process_signal_event(event)
...
class TestComplianceBlocksTenantOrder:
    ...
    async def test_compliance_check_order_blocks_execution(self):
        ...
        tenant_client = MagicMock()
        tenant_client.get_account = MagicMock(
...
class TestOrderLifecyclePolling:
    ...
    async def test_order_polling_timeout_and_cancel(self):
        ...
        with (
            ...
            ks.check_halt = MagicMock()
            # #3447: Der Halt-Zustand wird seit dem Umhaengen hinters Tor an ZWEI Stellen
            # gelesen — `check_halt` im Halt-Tor und `is_halted` im OrderGateway. In
            # Produktion ist das eine Quelle (`check_halt` ruft woertlich `is_halted`). Die
            # Attrappe ersetzt aber den ganzen Kill-Switch, und ein unbesetztes
            # `is_halted` liefert einen wahrheitswertigen MagicMock — also "gehalten",
            # waehrend `check_halt` "frei" sagt: ein Zustand, den es real nicht gibt.
...
class TestOrderExecutorAntiChurn:
    ...
    async def test_sell_blocked_by_anti_churn(self):
        ...
        executor = _make_executor()
        tenant_client = MagicMock()
        tenant_client.get_open_position = MagicMock(return_value=MagicMock(qty="10"))
        tenant = {"user_id": "user-churn", "client": tenant_client, "equity": 10000.0}
    ...
    async def test_buy_swap_blocked_by_anti_churn_after_restart(self):
        ...
        with (
            ...
            ks.check_halt = MagicMock()
            tenant["client"].get_account = MagicMock(
                return_value=MagicMock(
                    cash="50000", equity="100000.0", buying_power="100000.0"
                )
            )
            mock_redis.get_redis = AsyncMock(
                return_value=MagicMock(
...
class TestDesktopFallbackRecordTrade:
    ...
    async def test_fallback_buy_records_trade_on_active_strategy_pm(self):
        ...
        with (
            ...
            ks.check_halt = MagicMock()
            # #3447: auch die Schatten-Order geht durchs Tor, und das fragt den Halt.
            ks.is_halted = MagicMock(return_value=False)
            await executor._process_signal_event(event)
