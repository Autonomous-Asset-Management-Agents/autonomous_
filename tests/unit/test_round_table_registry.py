...
class TestAgentRegistry:
    ...
    def test_load_plugins_from_directory(self, monkeypatch):
        # monkeypatch, not os.environ: the raw assignment below never removed
        # ALLOW_UNTRUSTED_PLUGINS, so every later test in the session ran with untrusted
        # plugin loading ENABLED. Found by the global-state canary (#2793).
...
"""
            ...
            monkeypatch.setenv("ALLOW_UNTRUSTED_PLUGINS", "false")
            ...
            monkeypatch.setenv("ALLOW_UNTRUSTED_PLUGINS", "true")
            registry.load_plugins_from_directory(tmpdir)
            agents = registry.get_active_agents()
