...
async def test_ratchet_up_and_evict_when_enabled():
    """The hwm map ratchets UP (max) per symbol and evicts symbols no longer held.

    Persist flag defaults OFF (getattr) → in-memory only, no store access (byte-identical).
    """
    ...
    with patch("core.engine.trading_loop.get_config", return_value=cfg):
        ...
        m2 = await eng._ratchet_high_water_marks(
        ...
        m3 = await eng._ratchet_high_water_marks([_sn_pos("DDOG", 110.0)])
        assert m3 == {"DDOG": 120.0}
        assert "AAPL" not in eng._position_high_water_marks


async def test_ratchet_disabled_returns_none_byte_identical():
