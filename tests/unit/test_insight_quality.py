...
class TestImportContract:
    ...
    def test_flag_read_nowhere_in_engine_path_yet(self):
        # PR-1 invariant: the flag is NOT consumed by stock_specialist.research()
        # / _build_report yet (that wiring is PR-2). Prove it stays unread there.
        import pathlib

        src = pathlib.Path("core/stock_specialist.py").read_text(encoding="utf-8")
        assert "INSIGHT_QUALITY_ENABLED" not in src
