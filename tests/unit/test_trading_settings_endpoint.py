...
def test_get_returns_engine_ist_bounds_defaults_and_agents(client, monkeypatch):
    ...
    # Agents-Vertrag für die S4-Karte (13 Voter inkl. #3250 Trend/VolumeConfirmation
    # + der #3199 Skew-Sizing-Input)
    assert len(body["agents"]) == 15  # +QualityAgent (#3275, dark)
    skew = next(a for a in body["agents"] if a["name"] == "SkewSizeTilt")
    assert skew["role"] == "size_input"  # Position-sizing stage, next to VIXAware
