...
def test_update_dynamic_agent_weights(redis_mock):
        ...
        "agent_trust_scores": json.dumps(
            {
                ...
                # #1951: PatternRecognitionAgent deleted — NewsSentimentAgent now
                # exercises the "slight positive shift + clamp" branch instead.
                "NewsSentimentAgent": 0.10,  # Slight positive trust shift
            }
        ),
    ...
    # NewsSentiment +0.10 -> 0.30 + 0.10 = 0.40 (<= 1.0) -> OK
    ...
    assert abs(new_weights["NewsSentimentAgent"] - 0.40) < 1e-5
