...
def test_the_panel_has_exactly_one_writer_and_it_is_lstm_dynamic():
    """Taucht ein weiterer Schreiber auf oder wandert einer, aendert sich die
    Voraussetzung des Flags und der Config-Kommentar wird falsch. Festnageln."""
    import pathlib
    ...
    assert not unexpected, (
        "die Schreiber des Panels haben sich verschoben — der Reachability-Kommentar "
        f"stimmt nicht mehr: {unexpected}"
    )
...
def test_the_prerequisite_is_documented_where_the_flag_is_defined():
    ...
    cfg = (pathlib.Path(config.__file__).parent / "settings.py").read_text(
        encoding="utf-8"
    )
