...
def _load_with_env(env: dict):
    ...
    with patch.dict(os.environ, clear=False):
        ...
        try:
            # Manually create the new state instead of reloading the module.
            settings._config_state = settings.RuntimeConfigState(_env_file=None, **env)
...
def test_live_boot_active_alias_is_live():
    with _load_with_env(_LIVE_ENV) as mod:
        assert (
...
def test_paper_boot_active_alias_is_paper():
    with _load_with_env(_PAPER_ENV) as mod:
...
def test_init_trading_clients_uses_active_live_alias():
    ...
    with patch.object(
        ...
        # _init_trading_clients setzt das modulglobale ``engine``. Ohne diese Klammer
        # blieb es als MagicMock stehen — und jeder spaetere Test im selben Worker, der
        # ``engine`` liest, sah eine Attrappe (so gemessen: test_g1b_console_routes,
        # sichtbar geworden, als sich die Worker-Verteilung verschob, #3449).
        ar,
        "engine",
        None,
    ):
