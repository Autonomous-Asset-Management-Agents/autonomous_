...
def test_restricted_symbol(guardian):
    guardian.restricted_list = [
        "SCAM_TOKEN"
    ]  # #3222: die Sperre gehoert in den Test, nicht in den Produktions-Default
    order = {
        "symbol": "SCAM_TOKEN",
        "side": "buy",
        "quantity": 10,
...
# --- #2065: Order-Value-Cap Exit-Exemption -------------------------------------
# The per-order value cap (ADR-C01) is an ENTRY/sizing limit; a risk-reducing SELL
# of an already-held position (quantity <= held_qty) must NOT be blocked by it —
# blocking an exit would prevent closing a position and thus increase risk.


@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_exit_exemption_full_sell_over_cap(guardian):
    guardian.max_order_value = 10000.0
    order = {
        "symbol": "AAPL",
        "side": "sell",
        "quantity": 200,
        "price": 100.0,  # value 20,000 > cap
        "strategy_id": "test_strat",
        "timestamp": time.time(),
        "held_qty": 200,  # full exit: quantity == held
    }
    assert guardian.check_order(order) is True
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_exit_exemption_partial_sell_over_cap(guardian):
    guardian.max_order_value = 10000.0
    order = {
        "symbol": "AAPL",
        "side": "sell",
        "quantity": 150,
        "price": 100.0,  # value 15,000 > cap
        "strategy_id": "test_strat",
        "timestamp": time.time(),
        "held_qty": 200,  # partial exit: quantity < held
    }
    assert guardian.check_order(order) is True
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_exit_no_exemption_sell_exceeds_held(guardian):
    # Selling MORE than held (would flip to short) stays blocked.
    guardian.max_order_value = 10000.0
    order = {
        "symbol": "AAPL",
        "side": "sell",
        "quantity": 250,
        "price": 100.0,  # value 25,000 > cap
        "strategy_id": "test_strat",
        "timestamp": time.time(),
        "held_qty": 200,
    }
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_exit_no_exemption_sell_without_held_qty(guardian):
    # A SELL over the cap without a known held position (short/unknown) stays blocked.
    guardian.max_order_value = 10000.0
    order = {
        "symbol": "AAPL",
        "side": "sell",
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_buy_over_cap_still_blocked_even_with_held_qty(guardian):
    # Exemption is SELL-only: a BUY over the cap stays blocked (held_qty ignored).
    guardian.max_order_value = 10000.0
    order = {
        "symbol": "AAPL",
        "side": "buy",
...
# --- #2069: compliance choke-point — every reject emits ONE scrubbed block span ---
# The finally-block of check_order carries decision + reason_code for EVERY path;
# a single emit there covers all reject codes. Call-site double-guarded so a broken
# emit can never alter the decision, the counters or the single-audit invariant.


@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_reject_emits_compliance_block_span(guardian, monkeypatch):
    """R10 — a max_order_value reject reaches emit_block_span with the
    'compliance:'-prefixed machine reason code + the symbol."""
    import core.round_table.execution_outcomes as eo

    calls = []
    monkeypatch.setattr(eo, "emit_block_span", lambda *a, **k: calls.append(a))
    guardian.max_order_value = 10000.0
    order = _valid_order("AAPL")
    order["quantity"] = 100
    order["price"] = 101.0  # 10,100 > cap
    assert guardian.check_order(order) is False
    assert calls == [("AAPL", "compliance:max_order_value")]


@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_approve_does_not_emit_block_span(guardian, monkeypatch):
    """R11 — an approved order emits nothing."""
    import core.round_table.execution_outcomes as eo
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_flag_off_reject_emits_no_span(guardian, monkeypatch):
    """R12 — default flag posture: the REAL emit helper never reaches the tracer
    (byte-identical behaviour, BORA)."""
    import core.telemetry as tele

    tracer_requests = []
    monkeypatch.setattr(tele, "get_tracer", lambda name: tracer_requests.append(name))

    import config as _cfg

    monkeypatch.setattr(_cfg, "EXECUTION_BLOCK_TELEMETRY_ENABLED", False, raising=False)
...
@allure.feature("VC-4 Risk Management & Compliance")
@allure.story("Risk & Compliance")
def test_raising_emit_never_alters_decision_audit_or_counters(guardian, monkeypatch):
    """R13 — defense-in-depth: even a wholly-broken emit cannot change the
    decision, the single-audit invariant or the GO/NO-GO counters."""
    import core.round_table.execution_outcomes as eo
    from core.compliance import get_compliance_counters, reset_compliance_counters
    ...
    assert guardian.check_order(order) is False
    assert guardian.cloud_logger.log_compliance_event.call_count == 1
    counters = get_compliance_counters()
    assert counters["nogo_count"] == 1
    assert counters["reject_reasons"].get("max_order_value") == 1
