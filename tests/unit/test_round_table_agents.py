# tests/unit/test_round_table_agents.py
# Epic 2.5 / Issue I-1 — TDD Red-Phase
# Round Table V2: VotingAgent Basisklasse + 9 spezialisierte Agents
#
# Gherkin-Kriterien (Architect Blueprint):
#   Given: SymbolEvalState mit validen OHLC-Skalaren
#   When:  agent.vote(state) aufgerufen
#   Then:  SignalCandidate mit score in [0.0, 1.0] + reasoning non-empty
#
#   Given: VIX-Proxy (Volume-Inverse) > Threshold
#   When:  VIXAwareRiskAgent.vote(state)
#   Then:  score < 0.3 (Strong Avoid)
#
# Policy Ref: docs/CODING_POLICY.md §11.5 TDD - Red → Green → Refactor

from __future__ import annotations

from unittest.mock import AsyncMock, MagicMock, patch

import allure
import pytest

try:
    import sb3_contrib  # noqa: F401

    HAS_SB3 = True
except ImportError:
    HAS_SB3 = False


@pytest.fixture(autouse=True)
def _pin_preactivation_round_table_defaults(monkeypatch):
    """This file is the PRE-ACTIVATION per-agent DIRECTIONAL suite: every test mocks the
    ``get_active()`` + per-symbol-prediction path (the OFF path). ADR-018 flipped the
    round-table-v2 flags ON by default; the ACTIVATED path — RegimeDetection→conditioner,
    Momentum→abstain, LSTM→cross-sectional rank via ``registry.get("LSTMDynamic")`` — is
    covered by the wave-1 files (test_regime_conditioner / test_momentum_fallback_abstain /
    test_round_table_ml_sources / test_round_table_lstm_rank_vote). Here we pin those flags
    back to their OFF default on the REAL config singleton (config.get_config() → the module
    global _config_state, config.py:1408) so the directional/OFF path stays covered — it is
    still reachable via env and is the env-reversible rollback path. Patching the real
    singleton (not a SimpleNamespace) keeps every OTHER config attr real, avoiding the
    partial-namespace trap. NB: RL_CONFIDENCE_WEIGHT is import-time captured on the agent
    class (agents.py:296) and is asserted at its activated 0.0 value directly — a runtime
    pin cannot change an already-bound class attribute."""
    import config

    cfg = config.get_config()
    for _flag in (
        "REGIME_CONDITIONER_ENABLED",
        "MOMENTUM_FALLBACK_ABSTAIN_ENABLED",
        "ROUND_TABLE_DISTINCT_ML_SOURCES",
        "LSTM_VOTE_USES_CROSS_SECTIONAL_RANK",
    ):
        monkeypatch.setattr(cfg, _flag, False, raising=False)


# ---------------------------------------------------------------------------
# TDD Test: Epic 3 — SuspectDataException importierbar + verhalten korrekt
# ---------------------------------------------------------------------------


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestSuspectDataGuard:
    def test_suspect_data_exception_importable(self):
        """Epic 3: SuspectDataException muss aus agents importierbar sein."""
        from core.round_table.agents import SuspectDataException

        assert issubclass(SuspectDataException, ValueError)

    def test_suspect_data_exception_message(self):
        """Epic 3: Exception sollte Symbol und Preis im Message haben."""
        from core.round_table.agents import SuspectDataException

        exc = SuspectDataException("[AAPL] Flat-Candle mit vol=1000")
        assert "AAPL" in str(exc)


# ---------------------------------------------------------------------------
# Helper: Standard SymbolEvalState Fixture
# ---------------------------------------------------------------------------


def make_state(
    symbol: str = "AAPL",
    open_: float = 150.0,
    high: float = 155.0,
    low: float = 148.0,
    close: float = 152.0,
    volume: float = 1_000_000.0,
    vix=None,
) -> dict:
    return {
        "symbol": symbol,
        "ohlc": {
            "open": open_,
            "high": high,
            "low": low,
            "close": close,
            "volume": volume,
        },
        "vix": vix,
        "market_data_keys": [],
        "current_time": "2026-03-10T06:00:00+00:00",
        "signal": None,
        "error": None,
        "round_table_scores": None,
        "consensus_ranking": None,
    }


# ---------------------------------------------------------------------------
# 1. Smoke: SignalCandidate + VotingAgent importierbar
# ---------------------------------------------------------------------------


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestImports:
    def test_signal_candidate_importable(self):
        """SignalCandidate muss aus core.round_table.base_agent importierbar sein."""
        from core.contracts.signal_candidate import SignalCandidate  # noqa: F401

        assert SignalCandidate is not None

    def test_voting_agent_importable(self):
        """VotingAgent Basisklasse muss importierbar sein."""
        from core.round_table.base_agent import VotingAgent  # noqa: F401

        assert VotingAgent is not None

    def test_signal_candidate_has_slots(self):
        """SignalCandidate muss __fields__ nutzen (Serialisierungs-Optimierung)."""
        from core.contracts.signal_candidate import SignalCandidate

        assert hasattr(
            SignalCandidate, "__fields__"
        ), "SignalCandidate muss @dataclass(pydantic) nutzen"

    def test_signal_candidate_fields(self):
        """SignalCandidate muss alle Pflichtfelder haben."""
        from core.contracts.signal_candidate import SignalCandidate

        vr = SignalCandidate(
            agent_name="TestAgent",
            symbol="AAPL",
            score=0.7,
            weight=0.5,
            reasoning="Test reason",
            vetoed=False,
        )
        assert vr.agent_name == "TestAgent"
        assert vr.symbol == "AAPL"
        assert vr.score == 0.7
        assert vr.weight == 0.5
        assert vr.reasoning == "Test reason"
        assert vr.vetoed is False

    def test_all_agents_importable(self):
        """Alle 8 Agents müssen aus core.round_table.agents importierbar sein."""
        from core.round_table.agents import (  # noqa: F401
            DrawdownGuardAgent,
            LSTMSignalAgent,
            MomentumAgent,
            NewsSentimentAgent,
            RegimeDetectionAgent,
            RLConfidenceAgent,
            SpecialistAlphaAgent,
            VIXAwareRiskAgent,
        )

    def test_pattern_recognition_agent_fully_removed(self):
        """#1951 — PatternRecognitionAgent (dormant, weight 0.0) ist vollständig
        gelöscht: Klasse, ALL_AGENTS-Roster, Paket-Exporte und Entitlement-Mirror."""
        import core.round_table as round_table_pkg
        import core.round_table.agents as agents_mod
        from core.entitlement.tier import _ALL_ROUND_TABLE_AGENTS

        assert not hasattr(agents_mod, "PatternRecognitionAgent")
        names = [a.__class__.__name__ for a in agents_mod.ALL_AGENTS]
        assert "PatternRecognitionAgent" not in names
        assert "PatternRecognitionAgent" not in round_table_pkg.__all__
        assert not hasattr(round_table_pkg, "PatternRecognitionAgent")
        assert "PatternRecognitionAgent" not in _ALL_ROUND_TABLE_AGENTS


# ---------------------------------------------------------------------------
# 2. Agent-Gewichte prüfen
# ---------------------------------------------------------------------------


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestAgentWeights:
    def test_agent_weights_correct(self):
        """Alle Agents müssen die korrekten Gewichte haben."""
        from core.round_table.agents import (
            DrawdownGuardAgent,
            LSTMSignalAgent,
            MomentumAgent,
            NewsSentimentAgent,
            RegimeDetectionAgent,
            RLConfidenceAgent,
            SpecialistAlphaAgent,
            VIXAwareRiskAgent,
        )

        expected = {
            DrawdownGuardAgent: 0.60,
            # #2839: 0.0→0.40 = shipped desktop launcher profile (desktop is master).
            SpecialistAlphaAgent: 0.40,
            RegimeDetectionAgent: 0.50,
            MomentumAgent: 0.45,
            VIXAwareRiskAgent: 0.45,
            LSTMSignalAgent: 0.40,
            # ADR-018: RL vote MUTED — RL_CONFIDENCE_WEIGHT default → 0.0 (direction-blind,
            # import-time captured on the class, agents.py:296). min/max clamp to 0.0 too.
            RLConfidenceAgent: 0.0,
            NewsSentimentAgent: 0.35,
        }
        for cls, expected_weight in expected.items():
            assert (
                cls.default_weight == expected_weight
            ), f"{cls.__name__}.default_weight = {cls.default_weight}, erwartet {expected_weight}"


# ---------------------------------------------------------------------------
# 3. Einzelne Agents: Happy Path (score in [0,1], reasoning non-empty)
# ---------------------------------------------------------------------------


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestDrawdownGuardAgent:
    @pytest.mark.anyio
    async def test_vote_returns_valid_result(self):
        from core.round_table.agents import DrawdownGuardAgent

        agent = DrawdownGuardAgent()
        state = make_state(
            high=151.0, low=148.0, close=150.0
        )  # drawdown = 3/151 ~ 1.98% <= 5% threshold
        result = await agent.vote(state)
        assert 0.0 <= result.score <= 1.0
        assert result.reasoning
        assert result.agent_name == "DrawdownGuardAgent"
        assert result.weight == 0.60
        assert result.vetoed is False

    @pytest.mark.anyio
    async def test_high_drawdown_lowers_score(self):
        from core.round_table.agents import DrawdownGuardAgent

        agent = DrawdownGuardAgent()
        state = make_state(
            high=200.0, low=100.0, close=105.0
        )  # extreme drawdown 50% > 5% threshold
        result = await agent.vote(state)
        assert result.score < 0.5, "Hoher Drawdown soll niedrigen Score erzeugen"
        assert result.vetoed is True


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestSpecialistAlphaAgent:
    @pytest.mark.anyio
    async def test_vote_returns_stub_score(self):
        from core.round_table.agents import (
            SpecialistAlphaAgent,
            set_specialist_registry,
        )

        # Reset global registry state — earlier tests may leave a mock that changes score
        set_specialist_registry(None)
        agent = SpecialistAlphaAgent()
        state = make_state()
        result = await agent.vote(state)
        assert result.agent_name == "SpecialistAlphaAgent"
        # Stub: None (Enthaltung) wenn keine Registry aktiv (Epic 3.3 nicht aktiv)
        assert result.score is None

    # -- RTR-3 (#1950): Aktivierungs-Pfad + Warmup-Diagnostik ----------------
    # Aktivierung ist import-zeitlich (agents._SPECIALIST_ALPHA_WEIGHT) — die
    # Tests spiegeln das echte Gate via importlib.reload mit gepatchter Config
    # (gleiches Idiom wie tests/unit/test_specialist_alpha_weight.py).

    @staticmethod
    def _fake_report_registry():
        report = MagicMock()
        report.sentiment_score = 75.0
        report.recommendation = "buy"
        report.escalate = False
        registry = MagicMock()
        registry.get_report.return_value = report
        return registry

    @pytest.mark.anyio
    async def test_activated_weight_with_report_enters_consensus(self, monkeypatch):
        """R4 (#1950): Gewicht 0.55 (Env-at-import) + injizierte Registry mit
        gecachtem Report → vote() trägt weight 0.55 und die ConsensusEngine
        nimmt den Vote in active_votes auf (aggregate == score, nicht 0.0)."""
        import importlib
        from types import SimpleNamespace

        import config
        import core.round_table.agents as agents_mod
        import core.round_table.base_agent as base_agent
        from core.round_table.consensus import ConsensusEngine

        monkeypatch.setattr(
            config,
            "get_config",
            lambda: SimpleNamespace(SPECIALIST_ALPHA_WEIGHT=0.55),
        )
        # Deterministic: no Redis in the weight property → default_weight rules.
        monkeypatch.setattr(base_agent, "RedisClient", None)
        importlib.reload(agents_mod)
        try:
            agents_mod.set_specialist_registry(self._fake_report_registry())
            agent = agents_mod.SpecialistAlphaAgent()
            result = await agent.vote(make_state())
            assert result.weight == 0.55
            assert "EXCLUDED" not in result.reasoning
            # Konsens-Eintritt: einziger Vote → weighted avg == score (≠ 0.0).
            consensus = ConsensusEngine().aggregate([result])
            assert consensus == pytest.approx(result.score)
            assert consensus > 0.0
        finally:
            agents_mod.set_specialist_registry(None)
            monkeypatch.undo()
            importlib.reload(agents_mod)  # restore dormant module state

    @pytest.mark.anyio
    async def test_active_weight_missing_report_warns_and_abstains(
        self, monkeypatch, caplog
    ):
        """R5 (#1950): Gewicht aktiv, aber KEIN Report gecacht (Warmup) →
        Abstain (weight 0.0, kein 0.5-Drag im Konsens) und logging.WARNING
        (§5.6, nie DEBUG), gedrosselt auf 1×/Symbol/Prozess."""
        import importlib
        import logging
        from types import SimpleNamespace

        import config
        import core.round_table.agents as agents_mod
        import core.round_table.base_agent as base_agent
        from core.round_table.consensus import ConsensusEngine

        monkeypatch.setattr(
            config,
            "get_config",
            lambda: SimpleNamespace(SPECIALIST_ALPHA_WEIGHT=0.55),
        )
        monkeypatch.setattr(base_agent, "RedisClient", None)
        importlib.reload(agents_mod)
        try:
            agents_mod.set_specialist_registry(None)  # Registry nicht injiziert
            agent = agents_mod.SpecialistAlphaAgent()
            with caplog.at_level(logging.DEBUG, logger="core.round_table.agents"):
                result = await agent.vote(make_state())
                await agent.vote(make_state())  # 2. Vote gleiches Symbol
            assert result.weight == 0.0  # Abstain — excluded, no neutral drag
            assert "EXCLUDED" in result.reasoning
            assert ConsensusEngine().aggregate([result]) == 0.0
            warnings = [
                r
                for r in caplog.records
                if r.name == "core.round_table.agents"
                and r.levelno == logging.WARNING
                and "SpecialistAlpha" in r.getMessage()
            ]
            assert len(warnings) == 1, (
                "aktives Gewicht + fehlender Report muss GENAU EINMAL pro Symbol "
                f"WARNING loggen (§5.6, gedrosselt) — got {len(warnings)}"
            )
        finally:
            agents_mod.set_specialist_registry(None)
            monkeypatch.undo()
            importlib.reload(agents_mod)  # restore dormant module state

    @pytest.mark.anyio
    async def test_dormant_with_report_stays_excluded_and_debug(
        self, caplog, monkeypatch
    ):
        """R6 (#1950): dormant (weight 0.0) + gecachter Report → weight 0.0,
        aus dem Konsens ausgeschlossen, Agent-Log bleibt DEBUG-frei von
        WARNINGs. #2839: Default ist jetzt 0.40 (Desktop-Master) — der
        Dormanz-Pfad wird explizit erzwungen statt über den Default."""
        import logging

        from core.round_table.agents import (
            SpecialistAlphaAgent,
            set_specialist_registry,
        )
        from core.round_table.consensus import ConsensusEngine

        monkeypatch.setattr(SpecialistAlphaAgent, "default_weight", 0.0)
        monkeypatch.setattr(SpecialistAlphaAgent, "max_weight", 0.0)
        assert SpecialistAlphaAgent.max_weight == 0.0  # forced dormant posture
        set_specialist_registry(self._fake_report_registry())
        try:
            agent = SpecialistAlphaAgent()
            with caplog.at_level(logging.DEBUG, logger="core.round_table.agents"):
                result = await agent.vote(make_state())
        finally:
            set_specialist_registry(None)
        assert result.weight == 0.0
        assert ConsensusEngine().aggregate([result]) == 0.0  # excluded
        agent_warnings = [
            r
            for r in caplog.records
            if r.name == "core.round_table.agents" and r.levelno >= logging.WARNING
        ]
        assert not agent_warnings, "dormant darf NIE warnen (byte-identisch)"


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestRegimeDetectionAgent:
    @pytest.mark.anyio
    async def test_bullish_regime_high_score(self):
        from core.round_table.agents import RegimeDetectionAgent

        agent = RegimeDetectionAgent()
        state = make_state(open_=100.0, close=115.0, vix=15.0)  # +15% (bullish)
        result = await agent.vote(state)
        assert 0.0 <= result.score <= 1.0
        assert result.score > 0.5, "Bullisches Regime soll hohen Score geben"

    @pytest.mark.anyio
    async def test_bearish_regime_low_score(self):
        from core.round_table.agents import RegimeDetectionAgent

        agent = RegimeDetectionAgent()
        state = make_state(open_=100.0, close=85.0, vix=35.0)  # -15% (bearish)
        result = await agent.vote(state)
        assert result.score < 0.5, "Bärisches Regime soll niedrigen Score geben"


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestMomentumAgent:
    @pytest.mark.anyio
    async def test_positive_momentum(self):
        from core.round_table.agents import MomentumAgent

        agent = MomentumAgent()
        state = make_state(open_=100.0, close=110.0, vix=20.0)  # +10%
        result = await agent.vote(state)
        assert 0.0 <= result.score <= 1.0
        assert result.score > 0.5


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestVIXAwareRiskAgent:
    @pytest.mark.anyio
    async def test_high_volume_proxy_low_score(self, monkeypatch):
        """
        Gherkin (Architect):
          Given: VIX sehr hoch (hohe Volatilität)
          When:  VIXAwareRiskAgent.vote(state)
          Then:  score < 0.3 (Strong Avoid)
        """
        import config

        monkeypatch.setattr(config.get_config(), "VIXAWARE_IMPLIED_VOL_ENABLED", False)
        from core.round_table.agents import VIXAwareRiskAgent

        agent = VIXAwareRiskAgent()
        # VIX = 45.0 (extreme stress) → score soll < 0.3
        state = make_state(vix=45.0)
        result = await agent.vote(state)
        assert (
            result.score < 0.3
        ), f"Hoher VIX soll score < 0.3 erzeugen, got {result.score}"

    @pytest.mark.anyio
    async def test_normal_volume_neutral_score(self, monkeypatch):
        import config

        monkeypatch.setattr(config.get_config(), "VIXAWARE_IMPLIED_VOL_ENABLED", False)
        from core.round_table.agents import VIXAwareRiskAgent

        agent = VIXAwareRiskAgent()
        state = make_state(vix=15.0)  # Normaler VIX
        result = await agent.vote(state)
        assert 0.0 <= result.score <= 1.0
        assert result.score > 0.5

    @pytest.mark.anyio
    async def test_vix_missing_abstains(self, monkeypatch):
        import config

        monkeypatch.setattr(config.get_config(), "VIXAWARE_IMPLIED_VOL_ENABLED", False)
        from core.round_table.agents import VIXAwareRiskAgent

        agent = VIXAwareRiskAgent()
        state = make_state(vix=None)  # Fehlender VIX
        result = await agent.vote(state)
        assert result.weight == 0.0
        assert result.score is None
        assert "EXCLUDED" in result.reasoning


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestLSTMSignalAgent:
    @pytest.mark.anyio
    async def test_vote_no_registry_triggers_watchdog_and_dropout(self):
        """TRD-7: Ohne aktive Registry wird der MLWatchdog getriggert und weight=0.0 (Dropout) zurückgegeben."""
        from core.round_table.agents import LSTMSignalAgent

        with (
            patch("core.round_table.agents.get_global_registry", return_value=None),
            patch("core.ml_watchdog.ml_watchdog.record_error") as mock_record,
        ):
            agent = LSTMSignalAgent()
            state = make_state()
            result = await agent.vote(state)

            assert (
                result.weight == 0.0
            ), "Bei Fehler muss weight auf 0 gesetzt werden (Dropout)"
            mock_record.assert_called_once()

    @pytest.mark.anyio
    async def test_vote_strategy_returns_none_abstains(self):
        """If active strategy returns None, agent must abstain with weight=0.0 and update reasoning."""
        from core.round_table.agents import LSTMSignalAgent

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_strategy.evaluate_for_symbol = AsyncMock(return_value=None)
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = LSTMSignalAgent()
            state = make_state()
            result = await agent.vote(state)

            assert result.score is None
            assert result.weight == 0.0
            assert "abstention" in result.reasoning.lower()

    @pytest.mark.anyio
    async def test_vote_continuous_monotone_not_three_bucket(self):
        """#1969 (RT-BUG-3): the vote score must be a MONOTONE CONTINUOUS function of
        the LSTM prediction — not one of three fixed buckets {0.25, 0.5, 0.75}.

        Before the fix the score was `{"BUY":0.75,"HOLD":0.5,"SELL":0.25}[action]`,
        which discretised the +0.067-IC continuous signal down to +0.011 IC with a
        negative consensus contribution (#1947 attribution).
        """
        from core.cloud_logger import DecisionContext
        from core.events import SignalEvent
        from core.round_table.agents import LSTMSignalAgent

        def _signal(pred, action="HOLD"):
            return SignalEvent(
                symbol="AAPL",
                action=action,
                decision_context=DecisionContext(
                    symbol="AAPL", action=action, lstm_prediction=pred
                ),
            )

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = LSTMSignalAgent()
            preds = [-4.0, -2.0, -0.5, 0.0, 0.5, 2.0, 4.0]
            scores = []
            for p in preds:
                mock_strategy.evaluate_for_symbol = AsyncMock(return_value=_signal(p))
                r = await agent.vote(make_state())
                assert 0.0 <= r.score <= 1.0, f"score out of bounds for pred={p}"
                scores.append(r.score)

            # Strictly monotone increasing in the prediction
            assert all(
                b > a for a, b in zip(scores, scores[1:])
            ), f"scores must strictly increase with pred, got {scores}"
            # NOT collapsed onto the 3 legacy buckets
            assert (
                len({round(s, 6) for s in scores}) >= 6
            ), f"score must be continuous, not 3-bucket, got {scores}"

    @pytest.mark.anyio
    async def test_vote_direction_and_neutral(self):
        """#1969: pred>0 → score>0.5 (bullish), pred<0 → score<0.5, pred==0 → 0.5."""
        from core.cloud_logger import DecisionContext
        from core.events import SignalEvent
        from core.round_table.agents import LSTMSignalAgent

        def _signal(pred):
            return SignalEvent(
                symbol="AAPL",
                action="HOLD",
                decision_context=DecisionContext(
                    symbol="AAPL", action="HOLD", lstm_prediction=pred
                ),
            )

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = LSTMSignalAgent()

            mock_strategy.evaluate_for_symbol = AsyncMock(return_value=_signal(3.0))
            assert (await agent.vote(make_state())).score > 0.5

            mock_strategy.evaluate_for_symbol = AsyncMock(return_value=_signal(-3.0))
            assert (await agent.vote(make_state())).score < 0.5

            mock_strategy.evaluate_for_symbol = AsyncMock(return_value=_signal(0.0))
            neutral = await agent.vote(make_state())
            assert abs(neutral.score - 0.5) < 1e-9
            # A present prediction (even neutral) is a real vote — full weight, not dropout
            assert neutral.weight > 0.0

    @pytest.mark.anyio
    async def test_vote_missing_prediction_abstains(self):
        """#1969: when the prediction is MISSING (None), the agent must ABSTAIN with
        weight=0.0 — NOT fake a neutral 0.5 with full weight."""
        from core.round_table.agents import LSTMSignalAgent

        sig = MagicMock()
        sig.decision_context.lstm_prediction = None  # prediction unavailable

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_strategy.evaluate_for_symbol = AsyncMock(return_value=sig)
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = LSTMSignalAgent()
            result = await agent.vote(make_state())

            assert result.weight == 0.0, "missing prediction must abstain (weight 0)"
            assert "abstention" in result.reasoning.lower()


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestRLConfidenceAgent:
    @pytest.mark.anyio
    async def test_vote_no_registry_triggers_watchdog_and_dropout(self):
        """TRD-7: Ohne aktive Registry wird der MLWatchdog getriggert und weight=0.0 (Dropout) zurückgegeben."""
        from core.round_table.agents import RLConfidenceAgent

        with (
            patch("core.round_table.agents.get_global_registry", return_value=None),
            patch("core.ml_watchdog.ml_watchdog.record_error") as mock_record,
        ):
            agent = RLConfidenceAgent()
            state = make_state()
            result = await agent.vote(state)

            assert (
                result.weight == 0.0
            ), "Bei Fehler muss weight auf 0 gesetzt werden (Dropout)"
            mock_record.assert_called_once()

    @pytest.mark.anyio
    async def test_vote_strategy_returns_none_abstains(self):
        """If active strategy returns None, agent must abstain with weight=0.0 and update reasoning."""
        from core.round_table.agents import RLConfidenceAgent

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_strategy.evaluate_for_symbol = AsyncMock(return_value=None)
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = RLConfidenceAgent()
            state = make_state()
            result = await agent.vote(state)

            assert result.score is None
            assert result.weight == 0.0
            assert "abstention" in result.reasoning.lower()

    @pytest.mark.anyio
    async def test_vote_reflects_prediction_regression_656(self):
        """Regression (reverses #656 / f5cc27da): the RL vote must read the ALWAYS-set
        `decision_context.lstm_prediction`, not the BUY-only `conviction_score`. Under #656
        every HOLD/SELL collapsed to a dead neutral 0.5 — the RL effectively stopped voting.
        A strong directional prediction must move the vote off 0.5 even when action==HOLD.
        """
        from core.cloud_logger import DecisionContext
        from core.events import SignalEvent
        from core.round_table.agents import RLConfidenceAgent

        def _signal(action, pred):
            return SignalEvent(
                symbol="AAPL",
                action=action,
                decision_context=DecisionContext(
                    symbol="AAPL", action=action, lstm_prediction=pred
                ),
            )

        mock_registry = MagicMock()
        mock_strategy = MagicMock()
        mock_registry.get_active.return_value = mock_strategy

        with patch(
            "core.round_table.agents.get_global_registry", return_value=mock_registry
        ):
            agent = RLConfidenceAgent()

            # Strong bullish lean while HOLDing → vote > 0.5 (was a dead 0.5 under #656)
            mock_strategy.evaluate_for_symbol = AsyncMock(
                return_value=_signal("HOLD", 2.0)
            )
            bull = await agent.vote(make_state())
            assert (
                bull.score > 0.5
            ), f"bullish HOLD must lift the vote, got {bull.score}"

            # Strong bearish lean while HOLDing → vote < 0.5
            mock_strategy.evaluate_for_symbol = AsyncMock(
                return_value=_signal("HOLD", -2.0)
            )
            bear = await agent.vote(make_state())
            assert (
                bear.score < 0.5
            ), f"bearish HOLD must lower the vote, got {bear.score}"

            # A BUY with strong conviction scores clearly bullish
            mock_strategy.evaluate_for_symbol = AsyncMock(
                return_value=_signal("BUY", 2.0)
            )
            buy = await agent.vote(make_state())
            assert (
                buy.score > 0.7
            ), f"BUY with strong pred must score high, got {buy.score}"

            # #2480: A BUY action paired with a BEARISH prediction (-2.24) MUST score < 0.5
            mock_strategy.evaluate_for_symbol = AsyncMock(
                return_value=_signal("BUY", -2.24)
            )
            bearish_buy = await agent.vote(make_state())
            assert (
                bearish_buy.score < 0.5
            ), f"#2480: negative prediction must produce score < 0.5 even on BUY action, got {bearish_buy.score}"


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestNewsSentimentAgent:
    @pytest.mark.anyio
    async def test_vote_gemini_unavailable_returns_fallback(self):
        """Wenn Gemini nicht erreichbar: Fallback 0.5 (kein Crash)."""
        from core.round_table.agents import NewsSentimentAgent

        agent = NewsSentimentAgent()
        state = make_state()
        # Gemini-Client wird nicht aufgerufen (kein API-Key in Test) → Fallback
        result = await agent.vote(state)
        assert result.score is None
        assert not result.vetoed


# NB (#1951): TestPatternRecognitionAgent wurde mitsamt dem Agenten gelöscht —
# der Agent war dormant (default_weight=0.0) und ohne Walk-Forward-Evidenz.


# ---------------------------------------------------------------------------
# Regression: Monitor Loop Strategy Switch Stability (Bug 2026-04-15)
# ---------------------------------------------------------------------------


@pytest.mark.skipif(not HAS_SB3, reason="Requires ML dependencies like sb3_contrib")
@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestMonitorLoopStrategySwitchStability:
    """
    Regression test for the endless strategy-switch bug.

    Root cause: monitor_loop compared active_strategy.strategy_name ("RLAgent")
    against TargetStrategyClass.__name__ ("RLStrategy") — these never match,
    causing a new RLStrategy to be instantiated every 30-min monitor cycle,
    wiping the AgentRegistry state and making LSTMSignalAgent + RLConfidenceAgent
    permanently blind (score=None). Result: Round Table consensus stuck below 0.65
    BUY threshold → zero trades executed.

    Fix: compare against target_strategy_name (config value) not __name__.
    This test ensures the comparison is stable after the first cycle.
    """

    def test_strategy_name_matches_config_active_strategy(self):
        """
        RLStrategy.strategy_name must equal config.ACTIVE_STRATEGY ('RLAgent').

        If this fails: monitor_loop will trigger STRATEGY SWITCH every cycle,
        wiping the AgentRegistry and blocking all trades.
        """
        from unittest.mock import MagicMock, patch

        mock_client = MagicMock()
        mock_client.__class__.__name__ = "TradingClient"

        with (
            patch("core.strategies.rl_strategy.os.path.exists", return_value=False),
            patch(
                "core.strategies.rl_strategy.HistoricalDataProvider",
                return_value=MagicMock(),
            ),
        ):
            from core.risk_manager import RiskManager
            from core.strategies.rl_strategy import RLStrategy

            mock_rm = MagicMock(spec=RiskManager)

            strategy = RLStrategy(
                client=mock_client,
                symbols=["AAPL"],
                running_event=MagicMock(),
                total_capital=100_000.0,
                risk_manager=mock_rm,
                data_provider=MagicMock(),
            )

        # The name set in __init__ must match what monitor_loop reads from config
        import config

        active_strategy_config = getattr(config, "ACTIVE_STRATEGY", "RLAgent")

        assert strategy.strategy_name == active_strategy_config, (
            f"RLStrategy.strategy_name='{strategy.strategy_name}' does not match "
            f"config.ACTIVE_STRATEGY='{active_strategy_config}'. "
            f"This causes monitor_loop to trigger STRATEGY SWITCH every 30 minutes, "
            f"wiping AgentRegistry and blocking all trades. "
            f"Fix: ensure strategy_name matches the config key used in STRATEGY_CLASSES."
        )

    def test_strategy_classes_key_matches_strategy_name(self):
        """
        STRATEGY_CLASSES key must equal the strategy_name set in __init__.

        Monitor loop does:
          target_name = config.ACTIVE_STRATEGY           # e.g. "RLAgent"
          cls = STRATEGY_CLASSES[target_name]            # → RLStrategy class
          current = active_strategy.strategy_name        # e.g. "RLAgent"
          if current != target_name: SWITCH              # ← must be stable

        If STRATEGY_CLASSES["RLAgent"] maps to a class whose strategy_name
        attribute is NOT "RLAgent", the switch will fire every cycle.
        """
        from unittest.mock import MagicMock, patch

        import config
        from core.strategies import STRATEGY_CLASSES

        active_strategy_name = getattr(config, "ACTIVE_STRATEGY", "RLAgent")

        assert active_strategy_name in STRATEGY_CLASSES, (
            f"config.ACTIVE_STRATEGY='{active_strategy_name}' not found in "
            f"STRATEGY_CLASSES keys: {list(STRATEGY_CLASSES.keys())}. "
            f"Add the mapping to core/strategies/__init__.py."
        )

        target_cls = STRATEGY_CLASSES[active_strategy_name]

        # The class must set strategy_name = active_strategy_name in __init__
        # We check via a lightweight mock (no ML models loaded)
        mock_client = MagicMock()
        with (
            patch("core.strategies.rl_strategy.os.path.exists", return_value=False),
            patch(
                "core.strategies.rl_strategy.HistoricalDataProvider",
                return_value=MagicMock(),
            ),
        ):
            instance = target_cls(
                client=mock_client,
                symbols=["AAPL"],
                running_event=MagicMock(),
                total_capital=100_000.0,
                risk_manager=MagicMock(),
                data_provider=MagicMock(),
            )

        assert instance.strategy_name == active_strategy_name, (
            f"STRATEGY_CLASSES['{active_strategy_name}'] instantiates class "
            f"'{target_cls.__name__}' but its strategy_name='{instance.strategy_name}' "
            f"!= '{active_strategy_name}'. monitor_loop will SWITCH every cycle. "
            f"Fix: set self.strategy_name = '{active_strategy_name}' in {target_cls.__name__}.__init__."
        )


# ---------------------------------------------------------------------------
# TestRedisWeightInjectionDetection (I-1 #942 — Rogue Agent Hardening)
# ---------------------------------------------------------------------------


@pytest.mark.vc2
@allure.story("Portfolio & Strategy")
class TestRedisWeightInjectionDetection:
    """
    TDD coverage for the Redis weight manipulation WARNING log added in I-1 #942.

    Gherkin:
        Given: Redis contains agent_weights_v2[AgentName] = 99999 (out-of-bounds)
        When:  agent.weight is accessed
        Then:  A SECURITY WARNING is emitted in the logs
        And:   The returned weight is clamped to max_weight (not 99999)
    """

    def test_out_of_bounds_redis_weight_triggers_security_warning(self, caplog):
        """
        Given: Redis returns 99999.0 for NewsSentimentAgent (max_weight=1.50)
        When:  agent.weight property is accessed
        Then:  SECURITY WARNING log is emitted with 'rogue' or 'manipulation' context
        And:   Returned weight == max_weight (1.50), NOT 99999.0
        """
        import logging
        from unittest.mock import MagicMock, patch

        from core.round_table.agents import NewsSentimentAgent

        agent = NewsSentimentAgent()
        mock_redis = MagicMock()
        mock_redis.hget.return_value = b"99999.0"  # Redis returns bytes

        with (
            patch("core.round_table.base_agent.RedisClient") as mock_client,
            caplog.at_level(logging.WARNING, logger="core.round_table.base_agent"),
        ):
            mock_client.get_sync_redis.return_value = mock_redis
            actual_weight = agent.weight

        # Weight must be clamped to max_weight
        assert actual_weight == agent.max_weight, (
            f"Out-of-bounds Redis weight 99999.0 should be clamped to max_weight={agent.max_weight}, "
            f"got {actual_weight}."
        )
        # SECURITY WARNING must be emitted
        assert any("SECURITY" in r.message for r in caplog.records), (
            "Expected a SECURITY WARNING when Redis weight is out of bounds. "
            "Rogue weight injection must be visible in production logs."
        )

    def test_out_of_bounds_low_redis_weight_triggers_security_warning(self, caplog):
        """
        Given: Redis returns -5.0 for LSTMSignalAgent (min_weight=0.15)
        When:  agent.weight property is accessed
        Then:  SECURITY WARNING log is emitted
        And:   Returned weight == min_weight (0.15), NOT -5.0
        """
        import logging
        from unittest.mock import MagicMock, patch

        from core.round_table.agents import LSTMSignalAgent

        agent = LSTMSignalAgent()
        mock_redis = MagicMock()
        mock_redis.hget.return_value = b"-5.0"

        with (
            patch("core.round_table.base_agent.RedisClient") as mock_client,
            caplog.at_level(logging.WARNING, logger="core.round_table.base_agent"),
        ):
            mock_client.get_sync_redis.return_value = mock_redis
            actual_weight = agent.weight

        assert actual_weight == agent.min_weight, (
            f"Sub-minimum Redis weight -5.0 should be clamped to min_weight={agent.min_weight}, "
            f"got {actual_weight}."
        )
        assert any(
            "SECURITY" in r.message for r in caplog.records
        ), "SECURITY WARNING must be emitted for sub-minimum weight injection."

    def test_in_bounds_redis_weight_no_warning(self, caplog):
        """
        Given: Redis returns 1.0 for LSTMSignalAgent (bounds: [0.15, 1.50])
        When:  agent.weight property is accessed
        Then:  No SECURITY WARNING emitted, returned weight == 1.0
        """
        import logging
        from unittest.mock import MagicMock, patch

        from core.round_table.agents import LSTMSignalAgent

        agent = LSTMSignalAgent()
        mock_redis = MagicMock()
        mock_redis.hget.return_value = b"1.0"

        with (
            patch("core.round_table.base_agent.RedisClient") as mock_client,
            caplog.at_level(logging.WARNING, logger="core.round_table.base_agent"),
        ):
            mock_client.get_sync_redis.return_value = mock_redis
            actual_weight = agent.weight

        assert (
            actual_weight == 1.0
        ), f"In-bounds weight 1.0 should pass through, got {actual_weight}"
        security_warnings = [r for r in caplog.records if "SECURITY" in r.message]
        assert (
            len(security_warnings) == 0
        ), f"No SECURITY WARNING expected for in-bounds weight, got: {security_warnings}"

    def test_redis_unavailable_falls_back_to_default_no_warning(self, caplog):
        """
        Given: Redis connection fails
        When:  agent.weight property is accessed
        Then:  Returned weight == default_weight, no SECURITY WARNING (only DEBUG)
        """
        import logging
        from unittest.mock import patch

        from core.round_table.agents import RegimeDetectionAgent

        agent = RegimeDetectionAgent()

        with (
            patch("core.round_table.base_agent.RedisClient") as mock_client,
            caplog.at_level(logging.DEBUG, logger="core.round_table.base_agent"),
        ):
            mock_client.get_sync_redis.side_effect = ConnectionError(
                "Redis unavailable"
            )
            actual_weight = agent.weight

        assert actual_weight == agent.default_weight, (
            f"Redis failure should fall back to default_weight={agent.default_weight}, "
            f"got {actual_weight}."
        )
        security_warnings = [r for r in caplog.records if "SECURITY" in r.message]
        assert len(security_warnings) == 0, (
            "Redis connection failure should NOT emit SECURITY WARNING (only DEBUG). "
            "Alert fatigue would mask real security events."
        )
