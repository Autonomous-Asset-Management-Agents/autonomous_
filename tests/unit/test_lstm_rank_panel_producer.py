...
def test_only_lstm_dynamic_writes_the_panel():
    """Die erlaubten Schreiber, festgenagelt. Taucht ein weiterer auf, aendert sich
    die Praemisse dieses Features — und der naechste Leser soll das laut erfahren,
    statt es zu entdecken."""
    import pathlib
    ...
    for py in (root / "core").rglob("*.py"):
        ...
        try:
            ...
            continue
        rel = py.relative_to(root / "core").as_posix()
        for i, line in enumerate(text.splitlines(), 1):
            if ".record_cycle(" in line:
                writers.append(f"{rel}:{i}")

    # #3228 Phase 2: der dritte Schreiber ist die Offline-Attribution. Sie ist KEIN
    # Live-Pfad: sie liegt unter core/analysis/attribution/, laeuft gegen Bar-Dateien
    # und stellt SIM_MODE, Sim-Uhr und Panel-Retention in `finally` wieder her. Die
    # Praemisse, die dieser Test schuetzt — wer das Panel in einer LAUFENDEN Engine
    # liest, kann seiner Herkunft trauen — bleibt damit unberuehrt.
    #
    # Verglichen wird der Pfad relativ zu core/, nicht der Dateiname: es gibt zwei
    # runner.py (core/sim und core/round_table), ein Namens-Match wuerde also auch
    # einen Schreiber durchlassen, der hier nie gemeint war.
    ALLOWED = {
        "strategies/lstm_strategy.py",  # LSTMDynamic — der Live-Produzent
        "sim/runner.py",  # SimRunner
        "analysis/attribution/replay.py",  # RTR-0-Replay, offline (#3228)
    }
    unexpected = [w for w in writers if w.rsplit(":", 1)[0] not in ALLOWED]
    assert not unexpected, (
        "das Panel hat einen Schreiber ausserhalb von LSTMDynamic/SimRunner/Replay "
        f"bekommen — die Praemisse des Produzenten gehoert neu geprueft: {unexpected}"
    )
