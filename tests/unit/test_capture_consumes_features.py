...
class TestFailOpenWithDefaultFeatures:
    def test_row_written_and_warning_logged(self, flag_on, monkeypatch, caplog):
        ...
        # #2839: the flag defaults ON now (desktop master) — the fail-open path
        # under snapshot-OFF is forced explicitly to stay tested.
        monkeypatch.setattr(
            config.get_config(), "DESKTOP_FEATURE_SNAPSHOT_ENABLED", False
        )
