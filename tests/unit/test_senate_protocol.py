...
@allure.feature("VC-6 Reporting & Client Servicing")
@allure.story("Reporting & Auditing")
class TestLocalJSONAuditLoggerEventLoopLock:
    def test_multi_loop_restart_safety(self, tmp_path, monkeypatch):
        """
        Gherkin:
          Given: A LocalJSONAuditLogger instance
          When:  It logs across multiple different event loops (like strategy restarts)
          Then:  It doesn't crash with "got Future <Future pending> attached to a different loop"
        """
        import asyncio
        ...
        def run_in_fresh_loop():
            ...
            try:
                loop.run_until_complete(logger._async_log_to_jsonl(session))
            finally:
                loop.close()

        # First run creates the lock for loop 1
        run_in_fresh_loop()

        # Second run should create a new lock for loop 2, not crash with the old lock
        run_in_fresh_loop()

        # Verify JSONL was written twice
        import glob
        import os

        jsonl_files = glob.glob(os.path.join(str(tmp_path), "audit_log_*.jsonl"))
        assert len(jsonl_files) == 1
        with open(jsonl_files[0], "r") as f:
            lines = f.readlines()
        assert len(lines) == 2
