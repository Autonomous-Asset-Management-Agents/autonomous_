...
def test_target_window_realized_vol_shift():
    """Assert that the forward realized vol target is shifted by exactly 1.
    A shock at index 200 must affect the forward volatility target of today t
    for t < 200 (specifically up to t=199, where the forward window is [200, 219]),
    but must not affect t=200 (where the forward window is [201, 220]).
    """
    horizon = 20
    lr = np.zeros(400)
    lr[200] = 0.5  # shock

    # Replicate target calculation from vol_model.py
    fwd = pd.Series(lr[::-1]).rolling(horizon).std().to_numpy()[::-1]
    fwd_shifted = np.concatenate([fwd[1:], [np.nan] * 1])

    # t=199 forward window contains index 200
    assert fwd_shifted[199] > 0.0
    # t=200 forward window starts at 201, shock is in the past
    assert fwd_shifted[200] == 0.0 or np.isnan(fwd_shifted[200])
