...
def test_c_rank_collapse_within_min_hold_holds_hard_gate():
    """#2711 (Archon Option A): inside min-hold a rank collapse alone may NOT exit —
    the old early-SELL flushed 60-second-old positions on rank noise (HAS forensics).
    Legacy OR-behavior stays restorable via the flag (see companion test)."""
...
def test_c_legacy_early_collapse_sell_via_flag_off():
    d = should_sell_smart(
        **_kwargs(hours_held=1 * DAY, lstm_rank=35), min_hold_hard_gate=False
    )
    assert d.action == "SELL"
    assert "rebalanc" in d.reason.lower()
