...
class TestScalerNeverBypassesCaps:
    def test_t9_max_position_cap_still_binds(self, monkeypatch):
        ...
        _flag(monkeypatch, True)
        # #3284: this asserts the CONVICTION sizer's MAX cap (conviction=1.0 -> 30% target,
        # capped at 25%). Default sizing is now clean-weight "b" (1/N-based), so pin the
        # legacy conviction path explicitly for this legacy-behaviour test.
