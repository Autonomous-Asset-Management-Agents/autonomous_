...
def test_verify_sig_default_is_fail_closed():
    """Default REQUIRE_SIG must be true under Cloud Run (fail-closed)."""
    secret = "my-shared-secret"
    user_id = "test_user@example.com"
    result = _call_verify(
        x_user_id=user_id,
        ...
        env_vars={
            "PROXY_ENGINE_SHARED_SECRET": secret,
            "K_SERVICE": "aaa-api",
        },  # Cloud Run context — no REQUIRE_SIG set!
    )
    assert result is not None
    assert result.status_code == 403


def test_verify_sig_default_is_bypassed_off_cloud():
    """Default REQUIRE_SIG is false off-cloud (loopback/desktop/OSS)."""
    secret = "my-shared-secret"
    user_id = "test_user@example.com"
    result = _call_verify(
        x_user_id=user_id,
        ...
        env_vars={
            "PROXY_ENGINE_SHARED_SECRET": secret,
        },  # No K_SERVICE and no REQUIRE_SIG set!
    )
    assert result is None
