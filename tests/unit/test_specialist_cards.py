...
def test_build_summary_omits_outlook_and_sentiment_verdict():
    # #2772: the summary must NOT append "Overall outlook: <rec> (sentiment N/100)" — that per-agent
    # verdict could contradict the authoritative Round-Table decision on the same card.
    out = build_summary(
        news_summary="PACCAR reported Q2 earnings; insider selling observed.",
        alt_signals="",
        recommendation="hold",
        sentiment_score=50.0,
        existing_summary="",
    )
    assert out == "PACCAR reported Q2 earnings; insider selling observed."
    assert "Overall outlook" not in out
    assert "sentiment" not in out.lower()
