...
pytestmark = pytest.mark.mutates_global_state
...
def _load_config_fresh(name: str, datei: str):
    ...
    sys.modules.pop("ai_trading_bot.settings", None)
...
def test_oss_falls_back_to_off_on_garbage(monkeypatch):
    """Mit der Pydantic-Migration (PR 662) hat auch die OSS-Edition die Validierungsschicht.
    Daher fällt sie nun ebenfalls fail-closed mit einer Exception.
    """
    monkeypatch.setenv(_FLAG, "vielleicht")
    with pytest.raises(Exception):
        _read(monkeypatch, "config.oss.py", "vielleicht")
