...
def test_position_exit_hwm_persist_flag_edition_parity():
    """#2557: the HWM-persistence flag (persist the trailing-stop peak across a restart) must be
    identical across editions — same env var, same default — so desktop and cloud can never drift.
    """
    import config as ent

    oss = _load_oss_config()
    ent_val = ent.get_config().POSITION_EXIT_HWM_PERSIST_ENABLED
    assert oss.POSITION_EXIT_HWM_PERSIST_ENABLED == ent_val, (
        "POSITION_EXIT_HWM_PERSIST_ENABLED must match across editions: "
        f"oss={oss.POSITION_EXIT_HWM_PERSIST_ENABLED!r} vs enterprise={ent_val!r}"
    )


def test_position_exit_hwm_persist_default_off_both_editions():
    """#2557: default OFF on both editions (opt-in; a prove-it/OOS gate re-enables it)."""
    import os

    if os.getenv("POSITION_EXIT_HWM_PERSIST_ENABLED") is None:
        import config as ent

        oss = _load_oss_config()
        assert oss.POSITION_EXIT_HWM_PERSIST_ENABLED is False
        assert ent.get_config().POSITION_EXIT_HWM_PERSIST_ENABLED is False
