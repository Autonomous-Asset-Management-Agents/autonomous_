...
# TDD: Tests written FIRST (RED phase) — implementation in core/telemetry.py and core/otel_middleware.py  # noqa: E501
#
...
class TestInitTelemetry:
    ...
    def test_init_telemetry_does_not_raise(self):
        """init_telemetry() must never raise, even if Cloud Trace is unavailable."""  # noqa: E501
        from core.telemetry import init_telemetry
    ...
    def test_init_telemetry_is_idempotent(self):
        """Calling init_telemetry() multiple times must be safe (no duplicate providers)."""  # noqa: E501
        from core.telemetry import init_telemetry
...
class TestServiceVersion:
    ...
    def test_service_version_fallback_when_env_missing(self):
        """When GIT_COMMIT is not set, service.version must fall back to 'unknown'."""  # noqa: E501
    ...
    def test_app_version_takes_precedence_over_git_commit(self):
        """#1940: AAA_APP_VERSION (the desktop shell's real app version) wins over
        GIT_COMMIT (the cloud build SHA)."""
        with patch.dict(
            os.environ,
            {"AAA_APP_VERSION": "0.1.11", "GIT_COMMIT": "abc1234"},
            clear=False,
        ):
            if "core.telemetry" in sys.modules:
                del sys.modules["core.telemetry"]
            ...
            assert get_service_version() == "0.1.11"

    def test_app_version_used_when_git_commit_absent(self):
        """#1940: on the desktop GIT_COMMIT is never set, so AAA_APP_VERSION is reported."""
        env = {k: v for k, v in os.environ.items() if k != "GIT_COMMIT"}
        env["AAA_APP_VERSION"] = "0.1.11"
    ...
    def test_empty_app_version_falls_back_to_git_commit(self):
        """#1940: an empty AAA_APP_VERSION must be treated as unset (no shadowing)."""
        with patch.dict(
            os.environ,
            {"AAA_APP_VERSION": "", "GIT_COMMIT": "abc1234"},
            clear=False,
        ):
            if "core.telemetry" in sys.modules:
                del sys.modules["core.telemetry"]
...
class TestOtelSpanMiddleware:
    ...
    def test_middleware_attaches_to_fastapi_app(self):
        """Adding OtelSpanMiddleware to a FastAPI app must not crash at startup."""  # noqa: E501
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
