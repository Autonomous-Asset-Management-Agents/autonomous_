...
def test_debias_default_factor_is_the_calibrated_085():
    """ADR-a01: the default is the SWEEP-CALIBRATED exposure-neutral factor.

    Measured on corpus 7cf4c4b03bf0 (15,544 point-in-time symbol/day pairs,
    2026-05-08..2026-08-04): the mean vol scaler under HAR-RV is 0.8472; the
    factor reproducing that exposure with IV is 0.8513 -> 0.85. The previous
    0.80 was an UNCALIBRATED placeholder that over-sized by ~5.4%.
    """
    assert debiased_daily_vol_from_iv(0.30) == pytest.approx(
        round(0.30 / SQRT252 * 0.85, 6), abs=1e-9
    )


def test_config_default_matches_vol_model_default():
    """The config default and the function default must never drift apart."""
    import config as _config

    assert float(_config.get_config().IV_VRP_DEBIAS_FACTOR) == pytest.approx(0.85)
