...
class TestTrailingStop:
    ...
    def _mock_legacy_config(self, monkeypatch):
...
# ---------------------------------------------------------------------------
# #3180 — RISK vs OPINION tier (consensus-retention gate may only veto OPINION)
# ---------------------------------------------------------------------------
...
@allure.story("Risk & Compliance")
class TestExitTier:
    def test_default_is_fail_safe_risk(self):
        """An untagged ExitAnalysis defaults to 'risk' → the gate never vetoes it."""
        from core.intelligent_exit import ExitAnalysis

        assert ExitAnalysis(symbol="X", pnl_pct=0.0, hours_held=5.0).tier == "risk"

    def test_hard_stop_is_risk(self):
        result = analyze_exit(_ctx(entry=100.0, current=91.5, hwm=100.0, hours=0.5))
        assert result.should_sell is True and "HARD STOP" in result.reason
        assert result.tier == "risk"

    def test_loss_cut_is_risk(self):
        result = analyze_exit(_ctx(entry=100.0, current=90.0, hwm=100.0, hours=30.0))
        assert result.should_sell is True and "LOSS CUT" in result.reason
        assert result.tier == "risk"

    def test_trailing_stop_is_risk(self, monkeypatch):
        # Isolate the tier logic from the (config/profile-dependent) trailing score: force the
        # trailing direct-trigger and assert it tags 'risk'.
        import core.intelligent_exit as ie

        monkeypatch.setattr(ie, "_calculate_trailing_stop_score", lambda *a: 90.0)
        result = analyze_exit(_ctx(entry=100.0, current=110.0, hwm=125.0, hours=30.0))
        assert result.should_sell is True and "TRAILING" in result.reason
        assert result.tier == "risk"

    def test_momentum_fade_winner_is_opinion(self):
        # +10% winner, bearish LSTM + falling momentum → profit-taking (a judgment call).
        result = analyze_exit(
            _ctx(
                entry=100.0,
                current=110.0,
        ...
        assert result.should_sell is True and "MOMENTUM FADE" in result.reason
        assert result.tier == "opinion"

    def test_weighted_composite_sell_is_opinion(self, monkeypatch):
        # Force a weighted-composite SELL with NO direct trigger (loss<90, trailing<85, momentum<80):
        # the soft judgment exit must tag 'opinion' so the gate may veto it.
        import core.intelligent_exit as ie

        monkeypatch.setattr(ie, "_calculate_loss_pressure", lambda *a: 75.0)
        monkeypatch.setattr(ie, "_calculate_trailing_stop_score", lambda *a: 0.0)
        monkeypatch.setattr(ie, "_calculate_momentum_fade", lambda *a: 60.0)
        monkeypatch.setattr(ie, "_calculate_news_pressure", lambda *a: 90.0)
        monkeypatch.setattr(ie, "_calculate_time_pressure", lambda *a: 90.0)
        result = analyze_exit(_ctx(entry=100.0, current=97.0, hwm=100.0, hours=30.0))
        assert result.should_sell is True
        assert "HARD STOP" not in result.reason and "LOSS CUT" not in result.reason
        assert result.tier == "opinion"
