...
class _Order:
    """Stands in for a broker order.

    #2632: carries ``status`` because every real one does — ``SimOrder.__init__`` and the
    seeded opening fills both set it, and the fill engine overwrites it with "filled". The
    status used to come from the recorder EVENT ("executed" → filled); that indirection is
    gone, because matching events to broker orders by symbol reported a rotation SELL as the
    seeded opening BUY. Fill facts now come from the broker, which is the only component
    that knows them.
    """
