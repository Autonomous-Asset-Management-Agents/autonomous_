...
class TestConfigFlags:
    ...
    def test_use_limit_exits_exists(self):
        import config

        assert hasattr(config, "USE_LIMIT_EXITS")
        assert isinstance(config.USE_LIMIT_EXITS, bool)

    def test_use_limit_exits_default_false(self):
        cfg = _reload_config_with_env()
        assert cfg.USE_LIMIT_EXITS is False

    def test_use_limit_exits_env_override(self):
        cfg = _reload_config_with_env(USE_LIMIT_EXITS="True")
        assert cfg.USE_LIMIT_EXITS is True

    def test_use_limit_exits_independent_of_use_limit_orders(self):
        # #2558: the exit-selective flag must NOT flip the global buy-side flag.
        cfg = _reload_config_with_env(USE_LIMIT_EXITS="True")
        assert cfg.USE_LIMIT_EXITS is True
        assert cfg.USE_LIMIT_ORDERS is False
...
class TestReloadLeakContainment:
    ...
    def test_config_is_clean_here_although_an_earlier_test_overrode_it(self):
        ...
        assert config.USE_LIMIT_EXITS is False
