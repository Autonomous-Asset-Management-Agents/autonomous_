...
def test_t1_default_profile_is_midterm():
    # #2839: default legacy->midterm = shipped desktop launcher profile (desktop is
    # master). The legacy identity is still pinned explicitly in test_t1b below.
    from core.intelligent_exit import TRAIL_PROFIT_TIERS_MIDTERM, _active_trail_tiers

    assert _active_trail_tiers() == TRAIL_PROFIT_TIERS_MIDTERM
...
def test_t7_enterprise_config_defines_profile_default_legacy():
    ...
    assert (
        cfg.EXIT_TRAIL_PROFILE == "midterm"
    ), "default = desktop launcher profile (#2839)"
...
def test_t7b_oss_config_defines_profile_default_legacy():
    ...
    assert module.EXIT_TRAIL_PROFILE == "midterm"
