...
def test_config_flag_exists_and_defaults_on():
    """RQ-1 B5 (#1525): the guard is now ENABLED by default on both editions (B1/B2/B3 made
    the inputs trustworthy). Decision-neutral -- display data_quality/degraded + skip_llm.
    """
    assert get_config().DATA_INTEGRITY_GUARD_ENABLED is True
...
async def test_research_guard_no_decision_delta():
    ...
    # Under RQ-1 B4, confidence differs when guard is on (tracks data quality = 0.8) vs off (score extremity = 0.51)
    assert report_off.confidence == 0.51
    assert report_on.confidence == 0.80
