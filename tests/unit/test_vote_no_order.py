...
@pytest.mark.asyncio
async def test_lstm_agent_vote_uses_evaluate(monkeypatch):
    ...
    """
    # ADR-018 flipped ROUND_TABLE_DISTINCT_ML_SOURCES ON by default → the LSTM voice now
    # resolves registry.get("LSTMDynamic") instead of get_active() (agents.py:916). This
    # regression pins the get_active().evaluate_for_symbol wiring, so pin the flag back OFF
    # on the real config singleton; the distinct-source routing has its own coverage
    # (test_round_table_ml_sources).
    import config

    monkeypatch.setattr(
        config.get_config(), "ROUND_TABLE_DISTINCT_ML_SOURCES", False, raising=False
    )
    monkeypatch.setattr(
        config.get_config(), "LSTM_VOTE_USES_CROSS_SECTIONAL_RANK", False, raising=False
    )
...
@pytest.mark.asyncio
async def test_rl_agent_vote_uses_evaluate(monkeypatch):
    ...
    Gherkin:
      ...
      And    active.run_for_symbol is NOT called
    """
    # ADR-018 flipped ROUND_TABLE_DISTINCT_ML_SOURCES ON by default → the RL voice resolves
    # registry.get(...) instead of get_active() (agents.py:1134). This regression pins the
    # get_active().evaluate_for_symbol wiring, so pin the flag back OFF on the real config
    # singleton; the distinct-source routing has its own coverage (test_round_table_ml_sources).
    import config
