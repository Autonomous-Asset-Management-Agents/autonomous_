...
class _FakeSnapshotAPI:
    ...
    def get_stock_snapshot(self, request):
        ...
        if chunk_idx in self._hang:
            time.sleep(0.5)  # exceed the small per-chunk timeout used in the test
...
class TestChunking:
    ...
    async def test_one_timeout_chunk_keeps_the_rest(self):
        ...
        m.data_api = api
        with _patch_size(100):
            res = await m._fetch_snapshots_chunked(_syms(300), per_chunk_timeout=0.15)
    ...
    async def test_concurrent_a_hung_chunk_does_not_serialize_the_rest(self):
        # CONCURRENCY (Archon WARNING-1): with 3 chunks each hung 0.5s, a SEQUENTIAL
        # implementation would take ~1.5s; concurrent gather resolves in ~one timeout.
        api = _FakeSnapshotAPI(hang=(0, 1, 2), chunk_size=100)
        m = _mixin()
        m.data_api = api
        t0 = time.perf_counter()
        with _patch_size(100):
            res = await m._fetch_snapshots_chunked(_syms(300), per_chunk_timeout=0.2)
        elapsed = time.perf_counter() - t0
        assert res == {}  # all three hung past the timeout
        # bounded by ~ONE timeout, not 3x. Generous ceiling to stay non-flaky.
        assert elapsed < 0.45, f"chunks did not run concurrently (took {elapsed:.2f}s)"
