...
class TestAlpacaSymbol:
    ...
    # -- Class-share symbology (INC — 0-decisions outage) --------------------
    # Alpaca's Market Data API requires DOT notation for class shares
    # (BRK.B, BF.B). Our internal universe carries the yfinance-era HYPHEN form
    # (BRK-B, BF-B) — see get_sp500_symbols() `.replace(".", "-")`. A single
    # hyphenated symbol makes Alpaca reject the ENTIRE snapshot batch atomically
    # (HTTP 400 "invalid symbol: BRK-B") -> snapshots={} -> every symbol skipped
    # -> the round table never runs -> 0 decisions. _alpaca_symbol is the
    # canonical external->Alpaca mapper and MUST heal the class-share form.
    def test_class_share_hyphen_to_dot_brk(self):
        from core.data_provider import _alpaca_symbol

        assert _alpaca_symbol("BRK-B") == "BRK.B"

    def test_class_share_hyphen_to_dot_bf(self):
        from core.data_provider import _alpaca_symbol

        assert _alpaca_symbol("BF-B") == "BF.B"

    def test_class_share_lowercase_normalised(self):
        from core.data_provider import _alpaca_symbol

        assert _alpaca_symbol("brk-b") == "BRK.B"

    def test_dot_form_is_idempotent(self):
        # Already-Alpaca dot form must pass through unchanged (re-normalisation
        # of live_universe must be safe to apply more than once).
        from core.data_provider import _alpaca_symbol
...
# ---------------------------------------------------------------------------
# 1b. to_alpaca_symbols() — batch external->Alpaca universe normaliser
# ---------------------------------------------------------------------------


@allure.feature("VC-1 Research & Analysis")
@allure.story("Research & Modeling")
class TestToAlpacaSymbols:
    """The live/Alpaca boundary normaliser: maps every symbol through
    _alpaca_symbol, drops the ones Alpaca cannot serve (index symbols -> None),
    and preserves order + de-dups. This is what base.py applies to live_universe
    so the engine's symbol identity (snapshot key == lookup key == order/position
    key) is Alpaca-native end-to-end."""

    def test_converts_class_shares_and_passes_plain_symbols(self):
        from core.data_provider import to_alpaca_symbols

        assert to_alpaca_symbols(["AAPL", "BRK-B", "BF-B", "MSFT"]) == [
            "AAPL",
            "BRK.B",
            "BF.B",
            "MSFT",
    ...
    def test_drops_index_symbols_alpaca_cannot_serve(self):
        from core.data_provider import to_alpaca_symbols

        # ^SPX -> None (not in Alpaca stock data) must be filtered, not kept raw.
        assert to_alpaca_symbols(["AAPL", "^SPX", "MSFT"]) == ["AAPL", "MSFT"]

    def test_preserves_order_and_dedups(self):
        from core.data_provider import to_alpaca_symbols

        assert to_alpaca_symbols(["BRK-B", "AAPL", "BRK.B", "AAPL"]) == [
            "BRK.B",
            "AAPL",
        ]
    ...
    def test_empty_input(self):
        ...
        assert to_alpaca_symbols([]) == []

    def test_regression_engine_universe_carries_no_hyphenated_class_shares(self):
        # The exact outage trigger, pinned end-to-end and OFFLINE: the Wikipedia
        # scrape is mocked to return the class shares in their real dot form
        # (BRK.B, BF.B). get_sp500_symbols' `.replace(".", "-")` turns them into
        # the hyphen form the engine actually carried (BRK-B, BF-B); once passed
        # through to_alpaca_symbols they must be Alpaca-native dot form again and
        # never carry a hyphen into the snapshot batch.
        from core.data_provider import HistoricalDataProvider, to_alpaca_symbols

        wiki_df = pd.DataFrame({"Symbol": ["AAPL", "BRK.B", "BF.B", "MSFT"]})
        resp = MagicMock()
        resp.text = "<html></html>"
        resp.raise_for_status = MagicMock()
        with patch("core.data_provider.requests.get", return_value=resp), patch(
            "core.data_provider.pd.read_html", return_value=[wiki_df]
        ):
            raw = HistoricalDataProvider().get_sp500_symbols()

        # Sanity: the engine really did carry the hyphen form that 400s Alpaca.
        assert "BRK-B" in raw and "BF-B" in raw

        alpaca_ready = to_alpaca_symbols(raw)
        assert not any(
            "-" in s for s in alpaca_ready
        ), "hyphenated class-share tickers must never reach the Alpaca snapshot batch"
        assert "BRK.B" in alpaca_ready
        assert "BF.B" in alpaca_ready
...
class TestGetBatchData:
    ...
    def test_batch_cache_file_loaded_when_exists(self, tmp_path):
        ...
        symbols = ["AAPL", "TSLA"]
        cache_name = (
            f"batch_{end_date.strftime('%Y-%m-%d')}_365d_{len(symbols)}s.parquet"
        )
...
# ---------------------------------------------------------------------------
# 7.1 get_nasdaq_symbols — success + fallback
# ---------------------------------------------------------------------------


@allure.feature("VC-1 Research & Analysis")
@allure.story("Research & Modeling")
class TestGetNasdaqSymbols:

    def test_wikipedia_success(self, dp):
        """Mock requests.get + pd.read_html to return a fake NASDAQ 100 table."""
        fake_table = pd.DataFrame({"Ticker": ["AAPL", "MSFT", "GOOGL"]})
        with patch("core.data_provider.requests.get") as mock_get, patch(
            "core.data_provider.pd.read_html", return_value=[fake_table]
        ):
            mock_response = MagicMock()
    ...
    def test_wikipedia_failure_returns_fallback(self, dp):
        ...
        assert isinstance(result, list)
        assert len(result) >= 10
        assert "AAPL" in result  # Default fallback contains AAPL

    def test_no_symbol_column_returns_fallback(self, dp):
        fake_table = pd.DataFrame({"CompanyName": ["Apple"]})
        with patch("core.data_provider.requests.get") as mock_get, patch(
            "core.data_provider.pd.read_html", return_value=[fake_table]
        ):
            mock_response = MagicMock()
    ...
    def test_wikipedia_uses_custom_user_agent(self, dp):
        """Ensures that the Wikipedia scraper uses the required custom User-Agent."""
        fake_table = pd.DataFrame({"Ticker": ["AAPL"]})
        with patch("core.data_provider.requests.get") as mock_get, patch(
            "core.data_provider.pd.read_html", return_value=[fake_table]
        ):
            mock_response = MagicMock()
            ...
            mock_response.raise_for_status.return_value = None
            mock_get.return_value = mock_response
            dp.get_nasdaq_symbols()

            # Verify requests.get was called with the correct headers
            mock_get.assert_called_once()
            args, kwargs = mock_get.call_args
            assert "headers" in kwargs
            assert (
                kwargs["headers"].get("User-Agent")
                == "AI-Trading-Bot/1.0 (contact@aaagents.de)"
            )
...
class TestLoggingCompliance:
    """Verify exception/fallback logs in HistoricalDataProvider.get_data are WARNING level (BORA Rule 5)."""

    def test_get_data_logging_levels(self, tmp_path, caplog):
        import logging
        from unittest.mock import MagicMock
        ...
        mock_databento = MagicMock()
        mock_databento.get_bars.side_effect = Exception("Databento failed")

        # Mock Alpaca client to raise an exception
        mock_alpaca = MagicMock()
        mock_alpaca.get_stock_bars.side_effect = APIError("Alpaca API failed")
        ...
        # Verify that WARNING logs were emitted
        warning_logs = [
            r.message for r in caplog.records if r.levelno == logging.WARNING
        ]
        assert (
            len(warning_logs) > 0
        ), "Exceptions in get_data MUST be logged as WARNING or higher, not hidden as DEBUG. ADR Rule 5."
        assert any(
            "Alpaca" in msg or "fallback" in msg or "Bento" in msg
            for msg in warning_logs
        ), "Should log fallback warning"


# ---------------------------------------------------------------------------
# 11. warm_universe_cache() — LSTM Panel Starvation Fix (Part 2)
# ---------------------------------------------------------------------------


@allure.feature("VC-1 Research & Analysis")
@allure.story("Bar-Store Warm-Up")
class TestWarmUniverseCacheCoverage:
    def test_warm_universe_cache_empty_symbols_returns_zero(self):
        from core.data_provider import HistoricalDataProvider

        dp = HistoricalDataProvider(api=MagicMock())
        assert dp.warm_universe_cache([], datetime.now(), 250) == 0

    def test_warm_universe_cache_fetches_and_stores_in_memory(self):
        from core.data_provider import HistoricalDataProvider

        dates = pd.date_range("2026-07-01", periods=10)
        idx = pd.MultiIndex.from_arrays(
            [["AAPL"] * 10, dates], names=["symbol", "timestamp"]
        )
        df = pd.DataFrame(
            {
                "open": [150.0] * 10,
        ...
        warmed = dp.warm_universe_cache(["AAPL"], now, 250)
        assert warmed == 1
        key = f"AAPL_{now.strftime('%Y-%m-%d')}_250"
        assert key in dp.data_cache
