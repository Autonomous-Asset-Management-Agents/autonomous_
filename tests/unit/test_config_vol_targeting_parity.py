...
The 4 new keys must exist in BOTH editions with identical env names and
identical safe defaults (False / 0.015 / 0.5 / 1.5), mirrored on the
TFT_DEFLATED_SHARPE_GATE_ENABLED idiom (config.py:587 <-> config.oss.py:729).
Master flag OFF by default => the shipped increment is byte-identical
(dark-ship; activation is gated on the OOS net-of-cost proof, plan §11 Inc 2).
"""
...
def test_enterprise_defaults():
    ...
    assert cfg.VOL_TARGETING_SIZING_ENABLED is False
...
def test_enterprise_module_getattr_resolves_keys():
    ...
    assert config.VOL_TARGETING_SIZING_ENABLED is False
