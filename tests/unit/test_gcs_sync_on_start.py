...
class TestGcsSyncOnStart:
    ...
    def test_correct_bucket_name_used(self, monkeypatch, tmp_path):
        ...
        with patch("google.cloud.storage.Client") as mock_client:
            ...
            # sync() pulls both data/ and tft/ (cloud TFT auto-sync) → bucket() is
            # called once per sync; EVERY call must use the gs://-stripped name.
            bucket_calls = mock_client.return_value.bucket.call_args_list
            assert bucket_calls, "bucket() was never called"
            assert all(c.args == ("aaa-trading-bot-models",) for c in bucket_calls)
