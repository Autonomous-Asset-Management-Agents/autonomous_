...
class TestGuardianSurfacesDivergence:
    ...
    def test_r12_guardian_clamps_at_boot(self):
        """INC-8 / #3529: The Guardian must clamp values read from the environment
        at BOOT, not just during /start-live."""
        from unittest.mock import MagicMock, patch

        import config
        from core.compliance import ComplianceGuardian

        with patch("config.get_config") as mock_conf:
            cfg = MagicMock()
            cfg.COMPLIANCE_MAX_ORDER_VALUE = 900_000.0
            cfg.COMPLIANCE_MAX_DAILY_TRADES = 500
            mock_conf.return_value = cfg

            guardian = ComplianceGuardian()

            assert guardian.max_order_value <= 10000.0
            assert guardian.max_daily_trades <= 50
