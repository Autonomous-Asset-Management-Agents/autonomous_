...
try:
    ...
    from models.torch_model import EnsembleLSTMModel, get_lstm_paths
...
# ======================================================================
# TC-10: Exit authority (#3632: core.intelligent_exit is the ONE exit rule set)
# ======================================================================
section("TC-10: Exit authority")

try:
    from core.intelligent_exit import PositionContext, analyze_exit

    def _exit(entry, curr, hwm, hours):
        return analyze_exit(
            PositionContext(
                symbol="AAPL",
                entry_price=entry,
    ...
    # TC-10-01: No exit trigger (small gain, no drawdown)
    r = _exit(150, 152, 153, 30.0)
    record(
        "TC-10-01",
        f"No trigger: sell={r.should_sell}, '{r.reason}'",
        "PASS" if not r.should_sell else "FAIL",
    )

    # TC-10-02: Loss cut (-8 % after the panic window)
    r = _exit(150, 138, 150, 2.0)
    record(
        "TC-10-02",
        f"Loss cut: sell={r.should_sell}, '{r.reason}'",
        "PASS" if r.should_sell and "LOSS CUT" in r.reason else "FAIL",
    )

    # TC-10-03: Hard stop inside the panic window
    r = _exit(150, 136, 150, 0.5)
    record(
        "TC-10-03",
        f"Hard stop: sell={r.should_sell}, '{r.reason}'",
        "PASS" if r.should_sell and "HARD STOP" in r.reason else "FAIL",
    )

    # TC-10-04: Panic protection (-4 % in the first two hours -> hold)
    r = _exit(150, 144, 150, 0.5)
    record(
        "TC-10-04",
        f"Panic protection: sell={r.should_sell}, '{r.reason}'",
        "PASS" if not r.should_sell else "FAIL",
    )

    # TC-10-05: The loss cut needs 24 h at the cut level (-4.5 %): 5 h hold, 30 h sell
    early = _exit(100, 95.5, 100, 5.0)
    late = _exit(100, 95.5, 100, 30.0)
    record(
        "TC-10-05",
        f"Loss cut needs 24h: 5h sell={early.should_sell}, 30h sell={late.should_sell}",
        "PASS" if (not early.should_sell and late.should_sell) else "FAIL",
    )

    # TC-10-06: A winner without drawdown is held (no take-profit rule)
    r = _exit(100, 140, 140, 48.0)
    record(
        "TC-10-06",
        f"Winner held: sell={r.should_sell}, '{r.reason}'",
        "PASS" if not r.should_sell else "FAIL",
    )
...
try:
    ...
    clock_mock.now.side_effect = datetime.now
    ti = TradeIntelligence(clock=clock_mock, data_file=tmp_path)
    ...
    if os.path.exists(prod_file):
        ti_prod = TradeIntelligence(clock=clock_mock, data_file=prod_file)
