...
def main():
    ...
    for pf in file_candidates:
        ...
        # Fallback to ai_trading_bot prefix if relative path in logs omitted it
        if not os.path.exists(filepath):
            alt_path = os.path.join(workspace, "ai_trading_bot", pf)
...
"""
    ...
    for match in matches:
        ...
        if not os.path.exists(filepath):
            alt_path = os.path.join(workspace, "ai_trading_bot", filename)
            if os.path.exists(alt_path):
                filepath = alt_path
