...
from core.analysis.attribution.source_capture import (  # noqa: E402, E501
    load_capture_votes,
)
...
# #3272: the attribution horizon defaults to the HOLDING PERIOD (SMART_EXIT_MIN_HOLD_DAYS  # noqa: E501
# ~ 20-21 trading days), NOT 5. A slow direction signal (12-1M momentum, LSTM rank,  # noqa: E501
# options skew) must be scored over the window the position is actually held; a 5-bar IC  # noqa: E501
# structurally mis-measures every direction agent (RCA 2026-09-08). The HAC/Newey-West  # noqa: E501
# lag already tracks the horizon (``hac_lag_days = ic_horizon`` below), so the overlap of  # noqa: E501
# the longer forward window is handled — only the default was wrong.
IC_HORIZON_DEFAULT_BARS = 21


def build_parser() -> argparse.ArgumentParser:
    """RTR-0 CLI parser — extracted from ``main()`` so the horizon default is unit-testable (#3272)."""  # noqa: E501
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--data-dir",
        required=True,
        help="dir of {SYMBOL}.parquet|.pkl daily bars",  # noqa: E501
    )
    parser.add_argument(
        "--symbols",
        default=None,
        help="comma-separated subset (default: all files)",  # noqa: E501
    )
    parser.add_argument(
        "--start", required=True, help="replay start (YYYY-MM-DD)"
    )  # noqa: E501
    ...
    parser.add_argument(
        "--ic-horizon",
        type=int,
        default=IC_HORIZON_DEFAULT_BARS,
        help="forward MtM horizon (bars) for IC/hit-rate; default = holding period "  # noqa: E501
        "(SMART_EXIT_MIN_HOLD_DAYS ~ 21 bars, #3272), NOT 5",
    )
    parser.add_argument(
        "--fdr-alpha",
        type=float,
        default=0.10,
        help="Benjamini-Hochberg FDR level",  # noqa: E501
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="seed for the shuffled-target sanity IC",  # noqa: E501
    )
    parser.add_argument(
        "--vix-file",
        default=None,
        help="optional VIX close series (parquet/pkl/csv)",  # noqa: E501
    )
    parser.add_argument(
        "--iv-file",
        default=None,
        help="#3228: per-symbol ATM-30 IV series (parquet: sym,tag,iv) for "
        "VIXAware offline attribution; default auto-detects "
        "<data-dir>/iv/atm30_implied_vol.parquet",
    )
    parser.add_argument(
        "--rr-file",
        default=None,
        help="#3248: per-symbol 25Δ risk-reversal series (parquet: sym,tag,rr) for "  # noqa: E501
        "UpsideSkew offline attribution; default auto-detects "
        "<data-dir>/iv/risk_reversal.parquet",
    )
    parser.add_argument(
        "--source", choices=("replay", "capture"), default="replay"
    )  # noqa: E501
    parser.add_argument(
        "--lstm-artifacts",
        default=None,
...
def main() -> int:
    ...
    # Replay must stay hermetic: never talk to a real Redis from an offline run.  # noqa: E501
    ...
    if not provider.symbols:  # noqa: E501
        print(
            f"ERROR: no readable {{SYMBOL}}.parquet|.pkl bars "
            f"in {args.data_dir}"  # noqa: E501
        )  # noqa: E501
        return 2
    ...
    # #3228 Phase 1: per-symbol IV series -> VIXAware offline attribution. Default  # noqa: E501
    # auto-detects <data-dir>/iv/atm30_implied_vol.parquet; empty/absent -> no-op  # noqa: E501
    # (VIXAware abstains honestly, byte-identical to the pre-#3228 run).
    iv_path = args.iv_file or os.path.join(
        args.data_dir, "iv", "atm30_implied_vol.parquet"  # noqa: E501
    )
    _iv_by_date, _iv_dates = load_iv_table(iv_path)
    iv_table = (_iv_by_date, _iv_dates) if _iv_dates else None
    if iv_table is not None:
        print(f"IV series: {len(_iv_dates)} dates from {iv_path}")

    # #3248 Phase 2: per-symbol RR series -> UpsideSkew offline attribution. Default  # noqa: E501
    # auto-detects <data-dir>/iv/risk_reversal.parquet; empty/absent -> no-op
    # (UpsideSkew abstains honestly, byte-identical to the pre-#3248 run).
    rr_path = args.rr_file or os.path.join(
        args.data_dir, "iv", "risk_reversal.parquet"
    )  # noqa: E501
    _rr_by_date, _rr_dates = load_rr_table(rr_path)
    rr_table = (_rr_by_date, _rr_dates) if _rr_dates else None
    if rr_table is not None:
        print(f"RR series: {len(_rr_dates)} dates from {rr_path}")
    ...
    if lstm_meta.get("enabled"):
        from models.torch_model import (  # lazy, like the scorer  # noqa: E501
            FEATURE_WARMUP_ROWS,
        )
    ...
    md_path, json_path = write_gate_artifact(
        args.out, markdown=md, payload=payload
    )  # noqa: E501
            ...
            json.dumps(
                {
                    "book_quality": {
                        ...
                        "rankings": {
                            src: data["verdict"]
                            for src, data in payload["book_quality"][
                                "rankings"
                            ].items()  # noqa: E501
                        },
                    }
