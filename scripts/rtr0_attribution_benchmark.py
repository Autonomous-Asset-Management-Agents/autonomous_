...
def main() -> int:
    ...
    parser.add_argument(
        "--rr-file",
        default=None,
        help="#3248: per-symbol 25Δ risk-reversal series (parquet: sym,tag,rr) for "
    ...
    # #3228 Phase 1: per-symbol IV series -> VIXAware offline attribution. Default
    # auto-detects <data-dir>/iv/atm30_implied_vol.parquet; empty/absent -> no-op
    # (VIXAware abstains honestly, byte-identical to the pre-#3228 run).
    iv_path = args.iv_file or os.path.join(
        args.data_dir, "iv", "atm30_implied_vol.parquet"
    )
    ...
    # #3248 Phase 2: per-symbol RR series -> UpsideSkew offline attribution. Default
    ...
    rr_path = args.rr_file or os.path.join(args.data_dir, "iv", "risk_reversal.parquet")
