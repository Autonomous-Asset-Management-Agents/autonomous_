...
def run_watchdog(
    ...
    # ``_perf`` is a test seam: the timing clock is injected rather than patching the
    # process-wide ``time.perf_counter`` (which the coverage tracer / requests also
    # call, making the old stack-walking mock flaky). Prod default is the real
    # ``time.perf_counter`` — behaviour is unchanged.
    setup_logging()
    ...
    while True:
        try:
            ...
            latency_ms = (_perf() - start_time) * 1000
