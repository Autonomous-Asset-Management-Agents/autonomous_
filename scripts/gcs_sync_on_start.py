...
def _is_safe_filename(name: str) -> bool:
    ...
    Rejects: empty, contains ``/`` or ``\\``, contains ``..``, starts with ``.``.
    Accepts: simple filenames like ``lstm_model_v2.pth``.
    """
...
def _pull_from_github_release(manifest_path: str, data_dir: str) -> int:
    ...
    for entry in entries:
        ...
        if not (filename and url and expected_sha):
            log.warning(
                "⚠️ Skipping malformed manifest entry (need filename/url/sha256): %r",
                entry,
            )
            continue
