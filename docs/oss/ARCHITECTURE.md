...
- **Public API Proxy:** Binds to `127.0.0.1:8081` (der Aufruf steht am Ende von [serve_public_api.py](../../ai_trading_bot/serve_public_api.py): `uvicorn.run(app, host=host, port=port)`), though for pure local desktop operation proxy signatures are disabled (`REQUIRE_SIG=false`).
