...
> The **canonical, code-verified capability matrix** is the generated block at the top of this file (source: §0 of the master [`docs/0_strategy_and_roadmap/EDITIONS.md`](../0_strategy_and_roadmap/EDITIONS.md)). The table below is a short operational summary; where they differ, the master wins.

| Feature | Community Edition (this repository) | Enterprise Edition ([aaagents.de](https://aaagents.de)) |
|---|---|---|
| Deployment | Docker Compose (local-first) or Native Desktop | Google Cloud Run / AWS (BYOC / Self-hosted Cloud) |
...
| Setup | Behavior |
|---|---|
| `.env.oss` only (no Alpaca keys) | **Offline Mode** — engine boots, all 14 agents vote, no orders execute. Useful for code exploration. |
...
| Alpaca + `GEMINI_API_KEY` | **Full Sentiment Mode** — activates the `NewsSentimentAgent` (`agents.py::NewsSentimentAgent`). Without it, the bot runs in **Degraded Sentiment Mode** (13/14 agents active — `NewsSentimentAgent` votes with weight 0). |
...
> 🍏 **On macOS?** Follow the verified step-by-step **[macOS Quickstart](./macos-quickstart.md)** — it covers the two must-do steps (`LLM_PROVIDER=ollama` + Alpaca paper keys) without which the engine will not boot.
