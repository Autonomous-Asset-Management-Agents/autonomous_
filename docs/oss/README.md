...
| 4. Reporting | **BORA Control Center** — React/TypeScript dashboard with live portfolio view, agent voting log, and kill-switch | Docker image (nginx) |
...
The entire stack — backend engine, public API proxy, frontend dashboard, PostgreSQL, and Redis — is delivered as a pre-configured multi-container Compose stack. The only local dependency is **Docker Desktop**.
