...
pip install pandas-ta-classic==0.3.78   # indicator library (rsi/adx) — verified published wheel
```
...
### ⑤ Run
Start the engine and the web UI as two separate processes (the engine log stays readable in Terminal 1):
```bash
npm install                      # first run only
python -m core.engine            # Terminal 1 → API on http://localhost:8001
npm run dev                      # Terminal 2 → vite UI on http://localhost:8082 (proxies to the engine)
```
> The one-command `bash mac-quickstart.sh` (top of this guide) does both for you. Do **not** use
> `npm run desktop:dev` on the public repo — it `cd`s into `ai_trading_bot/`, which the flat public
> layout does not have (see Troubleshooting).

### ⑤b ML models (LSTM / RL)
The LSTM + RL agents need model weights that are **not** in the git checkout — they ship from GitHub
Releases and are SHA-256-verified against `data/models_manifest.json` (~7 MB total). The one-command
`bash mac-quickstart.sh` now pulls them **before boot**; the manual path must do it once:
```bash
python scripts/gcs_sync_on_start.py     # downloads data/lstm_model.pth, rl_agent_v3_dsr.zip, …
```
Without them the engine still boots, but the ML agents vote a neutral `0.5` and `/health/deep` reports
**`degraded`**. Re-run the command any time to repair a partial download.

### ⑥ Open + verify
- Dashboard: **`http://localhost:8082`**  *(Vite serves on 8082)*
- Engine alive: `curl http://localhost:8001/health` → expect `HTTP 200`.

> **Configuration is via `.env.oss`, not an in-browser wizard.** In the browser build the setup wizard
> (model choice / key entry) is **not** shown — those controls need the Electron desktop shell. Configure
> the model + keys by editing `.env.oss` (steps ④A/④B above). A native in-browser config flow is tracked
> in [#2648](https://github.com/Autonomous-Asset-Management-Agents/Dev-Enviroment/issues/2648).
...
The engine is a **long-running service — it does not "finish"**. After boot it repeats an analysis cycle
on a cadence (you'll see periodic log lines); leaving it running for an hour is normal and expected. You
**use it through the dashboard** (`http://localhost:8082`), not by waiting for the terminal to return.
Press **Ctrl+C** in the terminal (or the one-command wrapper) to stop the engine and the web UI.

## Use on iPhone / iPad (view-mostly)
```bash
npm run dev -- --host        # exposes vite on your Mac's IP
```
Open `http://<MAC-IP>:8082` (IP via `ipconfig getifaddr en0`), then **Share → Add to Home Screen**.
...
- **`ModuleNotFound` / import errors** → wrong Python; must be **3.12** with the venv activated. Also
  confirm `pandas-ta-classic==0.3.78` is installed (`pip show pandas-ta-classic`).
...
- **Port 8082 or 8001 already in use** → another server/instance is running.
- **`ComplianceGatekeeper … Missing core ML votes` right after boot** → not a crash. It is the
  strict-ML **safety default** while the LSTM panel warms up (it populates over the first analysis
  cycle). In **Mode A** the engine is recommendation-only anyway, so this is expected on a fresh boot.
  If it persists across many cycles, check the indicator library is healthy
  (`pip show pandas-ta-classic` → `0.3.78`; the one-command script verifies `ta.rsi`/`ta.adx` at start).
- **Local model / memory** → a small local model is plenty for the dashboard. On a 16 GB Mac prefer
  `qwen2.5:3b-instruct` (set `LOCAL_LLM_MODEL`); 24 GB+ comfortably runs `mistral:7b-instruct-v0.3-q4_K_M`.
  A larger model does not change the trading logic, only the sentiment agent's prose.
- **Engine runs but never trades / `/health/deep` says `degraded`** → two usual causes: (1) **ML models
  missing** — run `python scripts/gcs_sync_on_start.py` (step ⑤b) so `data/lstm_model.pth` exists;
  (2) a **stray `.env`** in the repo root. `config.py` loads `.env` via `load_dotenv()` (no `override`),
  so it fills any variable your shell has **not** already set — a leftover `.env` can silently set
  `PAPER_TRADING=false` or inject live keys. `bash mac-quickstart.sh doctor` warns when it finds one;
  review or remove it. Note the app's status pill may still read "running" even when the trading core
  is degraded — trust `/health/deep`, not `/health`.
- **`pip install` fails / times out (proxy or air-gapped Mac)** → the from-source path fetches **all**
  engine dependencies (torch, pandas, `pandas-ta-classic`, …) from **PyPI**; it is not an offline
  install. `bash mac-quickstart.sh doctor` probes PyPI up front and flags this. Air-gapped setups are
  out of scope here — use the [Docker path](#intel-macs) with a pre-pulled image, or a local PyPI mirror.
